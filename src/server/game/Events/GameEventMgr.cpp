...
{
    {
        ...
        do
        {
            ...
            if (event_id == 0)
            {
                TC_LOG_DEBUG("sql.sql", "`game_event`: game event entry 0 is reserved and can't be used.");
                continue;
            }
            ...
            {
                TC_LOG_DEBUG("sql.sql", "`game_event`: game event id (%i) is not a world event and has length = 0, thus cannot be used.", event_id);
                continue;
            }
            ...
            if (pGameEvent.holiday_id != HOLIDAY_NONE)
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event`: game event id (%i) contains nonexisting holiday id %u.", event_id, pGameEvent.holiday_id);
                    pGameEvent.holiday_id = HOLIDAY_NONE;
                    continue;
                }
                if (pGameEvent.holidayStage > MAX_HOLIDAY_DURATIONS)
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event` game event id (%i) has out of range holidayStage %u.", event_id, pGameEvent.holidayStage);
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Saves Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_save`: game event entry (%i) is out of range compared to max event entry in `game_event`.", event_id);
                    continue;
                }
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "game_event_save includes event save for non-worldevent id %u.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Prerequisite Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_prerequisite`: game event id (%i) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
                ...
                if (mGameEvent[event_id].state != GAMEEVENT_NORMAL && mGameEvent[event_id].state != GAMEEVENT_INTERNAL)
                {
                    ...
                    {
                        TC_LOG_DEBUG("sql.sql", "`game_event_prerequisite`: game event prerequisite id (%i) is out of range compared to max event id in `game_event`.", prerequisite_event);
                        continue;
                    }
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "game_event_prerequisiste includes event entry for non-worldevent id %u.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Creature Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_creature` contains creature (GUID: " UI64FMTD ") not found in `creature` table.", guid);
                    continue;
                }

                if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_creature`: game event id (%i) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }

                // Log error for pooled object, but still spawn it
                if (data->poolId)
                    TC_LOG_DEBUG("sql.sql", "`game_event_creature`: game event id (%i) contains creature (" UI64FMTD ") which is part of a pool (%u). This should be spawned in game_event_pool", event_id, guid, data->poolId);
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event GO Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_gameobject` contains gameobject (GUID: " UI64FMTD ") not found in `gameobject` table.", guid);
                    continue;
                }

                if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_gameobject`: game event id (%i) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }

                // Log error for pooled object, but still spawn it
                if (data->poolId)
                    TC_LOG_DEBUG("sql.sql", "`game_event_gameobject`: game event id (%i) contains game object (" UI64FMTD ") which is part of a pool (%u). This should be spawned in game_event_pool", event_id, guid, data->poolId);
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Model/Equipment Change Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_model_equip`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
                ...
                if (newModelEquipSet.equipment_id > 0)
                {
                    ...
                    {
                        TC_LOG_DEBUG("sql.sql", "Table `game_event_model_equip` contains creature (Guid: " UI64FMTD ", entry: %u) with equipment_id %u not found in table `creature_equip_template`. Setting entry to no equipment.",
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Quest Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_creature_quest`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event GO Quest Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_gameobject_quest`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Quest Condition Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_quest_condition`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Condition Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_condition`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Condition Save Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_condition_save`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "game_event_condition_save contains not present condition event id %u condition id %u.", event_id, condition);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event NPCflag Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_npcflag`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Seasonal Quest Relations...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!questTemplate)
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_seasonal_questrelation`: quest id (%u) does not exist in `quest_template`.", questId);
                    continue;
                }

                if (eventEntry >= mGameEvent.size())
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_seasonal_questrelation`: event id (%u) is out of range compared to max event in `game_event`.", eventEntry);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Vendor Additions Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_npc_vendor`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Battleground Holiday Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_battleground_holiday`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Pool Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "`game_event_pool`: game event id (%i) is out of range compared to max event id in `game_event`.", event_id);
                    continue;
                }

                if (!sPoolMgr->CheckPool(entry))
                {
                    TC_LOG_DEBUG("sql.sql", "Pool Id (%u) has all creatures or gameobjects with explicit chance sum <> 100 and no equal chance defined. The pool system cannot pick one to spawn.", entry);
                    continue;
                }
...
void GameEventMgr::StartArenaSeason()
{
    ...
    if (!result)
    {
        TC_LOG_DEBUG("gameevent", "ArenaSeason (%u) must be an existing Arena Season.", season);
        return;
    }
    ...
    {
        TC_LOG_DEBUG("gameevent", "EventEntry %u for ArenaSeason (%u) does not exist.", eventId, season);
        return;
    }
...
uint32 GameEventMgr::Update()                               // return the next event delay in ms
{
    ...
    for (uint16 itr = 1; itr < mGameEvent.size(); ++itr)
    {
        ...
        // so first queue it
        //TC_LOG_DEBUG("sql.sql", "Checking event %u", itr);
...
void GameEventMgr::GameEventSpawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
    {
        TC_LOG_DEBUG("gameevent", "GameEventMgr::GameEventSpawn attempted access to out of range mGameEventCreatureGuids element %i (size: %zu).",
    ...
    {
        TC_LOG_DEBUG("gameevent", "GameEventMgr::GameEventSpawn attempted access to out of range mGameEventGameobjectGuids element %i (size: %zu).",
    ...
    {
        TC_LOG_DEBUG("gameevent", "GameEventMgr::GameEventSpawn attempted access to out of range mGameEventPoolIds element %u (size: %zu).",
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
    {
        TC_LOG_DEBUG("gameevent", "GameEventMgr::GameEventUnspawn attempted access to out of range mGameEventCreatureGuids element %i (size: %zu).",
            internal_event_id, mGameEventCreatureGuids.size());
        return;
    }
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
    {
        TC_LOG_DEBUG("gameevent", "GameEventMgr::GameEventUnspawn attempted access to out of range mGameEventGameobjectGuids element %i (size: %zu).",
            internal_event_id, mGameEventGameobjectGuids.size());
        return;
    }
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventPoolIds.size()))
    {
        TC_LOG_DEBUG("gameevent", "GameEventMgr::GameEventUnspawn attempted access to out of range mGameEventPoolIds element %u (size: %zu).", internal_event_id, mGameEventPoolIds.size());
        return;
    }
...
void GameEventMgr::SetHolidayEventTime(GameEventData& event)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "Missing date or duration for holiday %u.", event.holiday_id);
        return;
    }
