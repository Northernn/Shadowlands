...
#define CHECK(exp, verbose) \
    ...
    {\
        if (unit)\
            TC_LOG_DEBUG("misc.movesplineinitargs", "MoveSplineInitArgs::Validate: expression '%s' failed for %s", #exp, (verbose ? unit->GetDebugInfo() : unit->GetGUID().ToString()).c_str());\
        else\
            TC_LOG_DEBUG("misc.movesplineinitargs", "MoveSplineInitArgs::Validate: expression '%s' failed for cyclic spline continuation", #exp); \
        return false;\
    }
