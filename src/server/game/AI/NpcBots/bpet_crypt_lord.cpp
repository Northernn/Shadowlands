    ...
    struct locust_swarm_botpetAI : public bot_pet_ai
    {
        ...
        void DoLocustActions(uint32 diff)
        {
            ...
            if (Unit* u = me->GetVictim())
            {
                ...
                {
                    me->resetAttackTimer();
                    SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_SOUL_BITE, DIFFICULTY_NONE);
            ...
            else
            {
                ...
                if (me->GetExactDist(petOwner) < (1.5f + 5u * uint32(expired)))
                {
                    ...
                    {
                        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_LOCUST_SWARM, DIFFICULTY_NONE);
