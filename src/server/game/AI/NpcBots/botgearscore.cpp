...
{
    auto smcit = ItemSlotMods.find(proto->GetInventoryType());
    if (smcit == ItemSlotMods.cend())
        return;

    uint32 quality = proto->GetQuality();
    float itemlvl = proto->GetBaseItemLevel();
...
float CalculateItemGearScore(uint32 botentry, uint8 botlevel, uint8 botclass, uint8 botspec, uint8 slot, ItemTemplate const* proto)
{
    ...
    if (slot > BOT_SLOT_RANGED /*|| einfo->Items[slot] != proto->GetId()*/)
    {
        CalculateRawItemScore(proto, itemscore);

        if (slot == BOT_SLOT_MAINHAND || slot == BOT_SLOT_OFFHAND)
        {
            if (botspec == BOT_SPEC_WARRIOR_FURY && botlevel >= 60 && proto->GetInventoryType() == INVTYPE_2HWEAPON)
                itemscore *= 0.5f;
