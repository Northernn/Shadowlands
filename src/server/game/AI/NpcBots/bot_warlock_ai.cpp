    ...
    struct warlock_botAI : public bot_ai
    {
        ...
        void DoNonCombatActions(uint32 diff)
        {
            ...
            /*    else
                ...
                if (!targets.empty())
                {
                    ...
                    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(CREATE_SOULSTONE_1, DIFFICULTY_NONE);
        ...
        void UpdateAI(uint32 diff) override
        {
                ...
                /*GetSpell(CREATE_HEALTHSTONE_1) && */!IsCasting() && GetHealthPCT(me) < 65)
            {
                ...
                SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(healthStone, DIFFICULTY_NONE);
                //ASSERT(spellInfo);
                uint32 rank = spellInfo->GetRank();
                //ASSERT(rank >= 1 && rank <= 8);
                spellInfo = sSpellMgr->GetSpellInfo(_healthStoneSpells[rank - 1], DIFFICULTY_NONE);
                ASSERT(spellInfo);
        ...
        void DoNormalAttack(uint32 diff)
        {
            ...
            if (IsSpellReady(SOUL_FIRE_1, diff) && can_do_fire && Rand() < 150 && dist < CalcSpellMaxRange(SOUL_FIRE_1) &&
                (mytar->IsPolymorphed() || me->HasAuraTypeWithAffectMask(SPELL_AURA_NO_REAGENT_USE, sSpellMgr->GetSpellInfo(SOUL_FIRE_1, DIFFICULTY_NONE))))
            {
    ...
    /*        if ((GetSpec() == BOT_SPEC_WARLOCK_DESTRUCTION) &&
        ...
        void DamageDealt(Unit* victim, uint32& damage, DamageEffectType damageType) override
        {
            ...
            if (damage && botPet && me->GetLevel() >= 10 && (damageType == SPELL_DIRECT_DAMAGE || damageType == DOT))
            {
                ...
                if (healVal)
                {
                    SpellInfo const* synhealInfo = sSpellMgr->GetSpellInfo(FEL_SYNERGY_HEAL, DIFFICULTY_NONE);
