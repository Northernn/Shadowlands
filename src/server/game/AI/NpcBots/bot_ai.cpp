///*
//NpcBot System by Trickerer (https://github.com/trickerer/Trinity-Bots)
// Rewrited By Thordekk
//Version 5.2.77a
//TODO:
...
bool bot_ai::SetBotOwner(Player* newowner)
{
    ...
    (const_cast<CreatureTemplate*>(me->GetCreatureTemplate()))->unit_flags2 &= ~(UNIT_FLAG2_INTERACT_WHILE_HOSTILE);
...
void bot_ai::CheckOwnerExpiry()
{
    ...
    ObjectGuid ownerGuid = ObjectGuid::Create<HighGuid::Player>(npcBotData->owner);
    ...
    if (timeNow >= lastLoginTime + expireTime)
    {
        ...
        //"SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomBonusListId, durability, playedTime, text, guid, itemEntry, owner_guid "
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, TriggerCastFlags flags)
{
    ...
    m_botSpellInfo = sSpellMgr->GetSpellInfo(spellId, DIFFICULTY_NONE);
    if (!m_botSpellInfo)
        return false;
    ...
    if (victim->isType(TYPEMASK_UNIT) && victim->isDead() &&
        ...
        !m_botSpellInfo->HasEffect(SPELL_EFFECT_RESURRECT_WITH_AURA) &&
...
void bot_ai::ResurrectGroup(uint32 spell_id)
{
   ...
   // if (int32(me->GetPower(spellInfo->PowerType)) < spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask()))
    ...
    if (!group)
    {
        ...
        {
            target = player->HasPlayerFlag(PLAYER_FLAGS_GHOST) ? player->ToUnit() : (Unit*)player->GetCorpse();
...
bool bot_ai::_canCureTarget(Unit const* target, uint32 cureSpell) const
{
    ...
    SpellInfo const* info = sSpellMgr->GetSpellInfo(cureSpell, DIFFICULTY_NONE);
    if (!info)
        return false;
...
void bot_ai::_getBotDispellableAuraList(Unit const* target, uint32 dispelMask, std::list<Aura const*> &dispelList) const
{
    ...
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if (aura->GetSpellInfo()->GetDispelMask() & dispelMask)
        {
            ...
            if (HasRole(BOT_ROLE_HEAL) && aura->GetSpellInfo()->IsRankOf(sSpellMgr->GetSpellInfo(34914, DIFFICULTY_NONE)))
                continue;
        }
    }
}
...
void bot_ai::_listAuras(Player const* player, Unit const* unit) const
{
    ...
    for (Unit::AuraMap::const_iterator itr = vAuras.begin(); itr != vAuras.end(); ++itr)
    {
        ...
        SpellInfo const* learnSpellInfo = sSpellMgr->GetSpellInfo(spellInfo->_effects[0].TriggerSpell, DIFFICULTY_NONE);
        const LocalizedString name = spellInfo->SpellName[locale];
       // botstring << "\n" << id << " - |cffffffff|Hspell:" << id << "|h[" << name;
        botstring << ' ' << localeNames[locale] << "]|h|r";
       // uint32 talentcost = GetTalentSpellCost(id);
        uint32 rank = 0;
       // if ((spellInfo->GetNextRankSpell() || spellInfo->GetPrevRankSpell()))
      //      rank = talentcost;
...
void bot_ai::SetStats(bool force)
{
    ...
    if (IsWanderer())
    {
        ...
        {
            mylevel = urand(me->GetCreatureDifficulty()->DeltaLevelMin, me->GetCreatureDifficulty()->DeltaLevelMax);
    ...
    if (force)
    {
        ...
        sObjectMgr->GetPlayerClassLevelInfo(GetPlayerClass(), std::min<uint8>(mylevel, DEFAULT_MAX_LEVEL), basemana);
    ...
    if (_botclass == BOT_CLASS_DRUID && RespectEquipsAttackTime())
    {
        ...
        if (!ssEntry || !ssEntry->CombatRoundTime)
            delay = _equips[BOT_SLOT_MAINHAND] ? _equips[BOT_SLOT_MAINHAND]->GetTemplate()->GetDelay() : me->GetCreatureTemplate()->BaseAttackTime;
        else
    ...
    Unit::AuraEffectList const& mAPbyStat = me->GetAuraEffectsByType(SPELL_AURA_MELEE_ATTACK_POWER_ATTACKER_BONUS);
    ...
    atpower += me->GetTotalAuraModifier(SPELL_AURA_MELEE_ATTACK_POWER_ATTACKER_BONUS);
    ...
    if (mylevel >= 10)
    {
        ...
        if (_botclass == BOT_CLASS_DRUID)
        {
            ...
            if (myclass == DRUID_BEAR_FORM && GetSpec() == BOT_SPEC_DRUID_FERAL)
                armor_mod += 0.33f + (me->GetShapeshiftForm() == FORM_BEAR_FORM ? 1.8f : 3.7f);
            //Moonkin Form innate
    ...
    value += tempval * std::max<float>(_getRatingMultiplier(CR_CRIT_MELEE), std::max<float>(_getRatingMultiplier(CR_CRIT_RANGED), _getRatingMultiplier(CR_CRIT_SPELL)));
    ...
    if (_botclass == BOT_CLASS_WARLOCK)
    {
        ...
        uint8 ratingBonus;
        if      (mylevel >= 68) ratingBonus = 60;
        else if (mylevel >= 62) ratingBonus = 50;
        else if (mylevel >= 56) ratingBonus = 40;
        else if (mylevel >= 50) ratingBonus = 30;
    ...
    if (CanCrit())
    {
        ...
        if (_botclass == BOT_CLASS_MAGE && me->HasAuraTypeWithFamilyFlags(SPELL_AURA_MOD_RATING, SPELLFAMILY_MAGE, 0x40000))
        ...
        if (_botclass == BOT_CLASS_ROGUE)
        {
            ...
            if (mylevel >= 20)
            {
                ...
                {
                    if (mainhand->GetTemplate()->GetClass() == ITEM_CLASS_WEAPON &&
                        (mainhand->GetTemplate()->GetSubClass() == ITEM_SUBCLASS_WEAPON_DAGGER))
                        value += 5.f;
                }
            }
        }
...
void bot_ai::CalculateAoeSpots(Unit const* unit, AoeSpotsVec& spots)
{
    ...
    for (std::list<WorldObject*>::const_iterator ci = doList.begin(); ci != doList.end(); ++ci)
    {
        ...
        spellInfo = sSpellMgr->GetSpellInfo(dObj->GetSpellId(), DIFFICULTY_NONE);
    ...
    if (unit->GetMapId() == 616 && unit->GetVehicle())
    {
        ...
        spellInfo = sSpellMgr->GetSpellInfo(57429, DIFFICULTY_NONE); //Static Field damage
    ...
    else if (unit->GetMapId() == 568)
    {
        ...
        spellInfo = sSpellMgr->GetSpellInfo(42630, DIFFICULTY_NONE); //Fire Bomb
    ...
    else if (unit->GetMapId() == 574)
    {
        ...
        if (creature)
        {
            spellInfo = sSpellMgr->GetSpellInfo(42751, DIFFICULTY_NONE); //Shadow Axe
...
uint32 bot_ai::_selectMountSpell() const
{
    ...
    bool map_allows_mount = (!me->GetMap()->IsDungeon() || me->GetMap()->IsBattlegroundOrArena()) && (!instt);
    ...
    if (!myMountSpellId)
    {
        ...
        else //if (can_fly)
        {
            ...
            Optional<MountArray> myMounts;
            if (me->GetRaceMask() & TEAM_ALLIANCE)
               myMounts = useSlowMount ? MOUNTS_150_ALLIANCE : MOUNTS_280_ALLIANCE;
            else if (me->GetRaceMask() & TEAM_HORDE)
...
{
    SpellInfo const* info = sSpellMgr->GetSpellInfo(spell, DIFFICULTY_NONE);
    if (!info)
    {
...
{
    SpellInfo const* info = sSpellMgr->GetSpellInfo(basespell, DIFFICULTY_NONE);
    if (!info)
    {
    ...
    if (Utf8toWStr(spell_name, wname))
    {
        ...
        for (BotSpellMap::const_iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
        {
            ...
            if (itr->second->spellId == 0) //not init'ed
                continue;
          //  spell_name = sSpellMgr->GetSpellInfo(itr->first, DIFFICULTY_NONE)->SpellName[locale];
            std::wstring wcname;
...
void bot_ai::SetSpellCategoryCooldown(SpellInfo const* spellInfo, uint32 msCooldown)
{
    ...
    for (BotSpellMap::const_iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        info = sSpellMgr->GetSpellInfo(itr->second->spellId, DIFFICULTY_NONE);
...
{
    SpellInfo const* baseInfo = sSpellMgr->GetSpellInfo(basespell, DIFFICULTY_NONE);
...
void bot_ai::_UpdateWMOArea()
{
    ...
    me->GetMap()->GetAreaInfo(me->GetPhaseShift(), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(),
...
void bot_ai::_OnAreaUpdate(uint32 areaId)
{
    ...
    if (!IAmFree())
    {
        ...
        for (Unit::AuraMap::const_iterator iter = ownerAuras.cbegin(); iter != ownerAuras.cend(); ++iter)
        {
            ...
            if (iter->second->GetSpellInfo()->CheckLocation(me->GetMapId(), _lastZoneId, areaId, master) != SPELL_CAST_OK)
            {
...
void bot_ai::OnSpellHit(Unit* caster, SpellInfo const* spell)
{
    ...
    for (uint8 i = 0; i != MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (auraname == SPELL_AURA_MOD_STAT || auraname == SPELL_AURA_MOD_PERCENT_STAT ||
            ...
            auraname == SPELL_AURA_MOD_RATING)
            shouldUpdateStats = true;
        ...
        if (effect == SPELL_EFFECT_RESURRECT || effect == SPELL_EFFECT_RESURRECT_WITH_AURA || effect == SPELL_EFFECT_SELF_RESURRECT)
        {
            ...
            if (!me->IsAlive())
            {
                ...
                if (effect == SPELL_EFFECT_RESURRECT_WITH_AURA)
                {
                    //Glyph of Rebirth: resurrect with 100% health
                    if (spell->IsRankOf(sSpellMgr->GetSpellInfo(20484, DIFFICULTY_NONE)))
        ...
        if (effect == SPELL_EFFECT_ENCHANT_HELD_ITEM)
        {
            ...
            if (!weap || weap->GetTemplate()->GetClass() != ITEM_CLASS_WEAPON || weap->GetEnchantmentId(slot))
                continue;
...
bool bot_ai::OnGossipHello(Player* player, uint32 /*option*/)
{
    ...
    {
        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_DEBUG), GOSSIP_SENDER_DEBUG, GOSSIP_ACTION_INFO_DEF + 1);
        menus = true;
    }
    ...
    if (player->GetGUID().GetCounter() != _ownerGuid)
    {
        if (IAmFree() && !IsWanderer())
        {
            ...
            if (!reason)
            {
              //  player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GossipOptionIcon::GOSSIP_ICON_TAXI, message2.str().c_str(),
                  //  GOSSIP_SENDER_HIRE, GOSSIP_ACTION_INFO_DEF + 0, message1.str().c_str(), cost, false);
            }
            else
                AddGossipItemFor(player, GossipOptionNpc::None, message2.str().c_str(), GOSSIP_SENDER_HIRE, GOSSIP_ACTION_INFO_DEF + reason);
    ...
    if (_ownerGuid)
    {
        ...
        if (player == master)
        {
            ...
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_MANAGE_EQUIPMENT), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_MANAGE_ROLES), GOSSIP_SENDER_ROLES_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_MANAGE_FORMATION), GOSSIP_SENDER_FORMATION, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_MANAGE_ABILITIES), GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 1);
            if (_botclass < BOT_CLASS_EX_START)
            {
                if (me->GetLevel() >= 10)
                    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_MANAGE_TALENTS), GOSSIP_SENDER_SPEC, GOSSIP_ACTION_INFO_DEF + 1);
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_GIVE_CONSUMABLE), GOSSIP_SENDER_USEITEM, GOSSIP_ACTION_INFO_DEF + 1);
            }

            if (!gr)
            {
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_CREATE_GROUP), GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 1);
                if (player->GetNpcBotsCount() > 1)
                    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_CREATE_GROUP_ALL), GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 2);
            }
            else if (!gr->IsMember(me->GetGUID()))
            {
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ADD_TO_GROUP), GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 1);
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ADD_TO_GROUP_ALL), GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 2);
            }
            else
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_REMOVE_FROM_GROUP), GOSSIP_SENDER_LEAVE_GROUP, GOSSIP_ACTION_INFO_DEF + 1);

            //movement toggle
            if (HasBotCommandState(BOT_COMMAND_MASK_UNMOVING))
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_FOLLOW_ME), GOSSIP_SENDER_FOLLOWME, GOSSIP_ACTION_INFO_DEF + 1);
            if (!HasBotCommandState(BOT_COMMAND_STAY))
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_HOLD_POSITION), GOSSIP_SENDER_HOLDPOSITION, GOSSIP_ACTION_INFO_DEF + 1);
            if (!HasBotCommandState(BOT_COMMAND_FULLSTOP))
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_STAY_HERE), GOSSIP_SENDER_DONOTHING, GOSSIP_ACTION_INFO_DEF + 1);
        }
        if (player == master || (gr && gr->IsMember(master->GetGUID())))
        {
            ...
            switch (_botclass)
            {
                case BOT_CLASS_MAGE:
                {
                    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_MAGE_FOOD), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_MAGE_DRINK), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                    if (me->GetLevel() >= 60)
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_MAGE_TABLE), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                    menus = true;
                    break;
                }
                case BOT_CLASS_ROGUE:
                {
                    ...
                    if (me->GetLevel() >= 16/* && !player->HasSkill(SKILL_LOCKPICKING)*/)
                    {
                        ...
                        AddGossipItemFor(player, GossipOptionNpc::None, msg.str().c_str(), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                        menus = true;
                    }
                    break;
                }
                case BOT_CLASS_WARLOCK:
                {
                    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_WARLOCK_HEALTHSTONE), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                    if (me->GetLevel() >= 58)
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_WARLOCK_SOULWELL), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                    menus = true;
                    break;
                }
                default:
        ...
        if (player == master)
        {
            ...
            switch (_botclass)
            {
                case BOT_CLASS_ROGUE:
                {
                    if (me->GetLevel() >= 20)
                    {
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ROGUE_POISON_REFRESH), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ROGUE_POISON_MH), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                        Item const* oweap = _equips[BOT_SLOT_OFFHAND];
                        if (oweap && oweap->GetTemplate()->GetClass() == ITEM_CLASS_WEAPON)
                            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ROGUE_POISON_OH), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 4);
                    }
                    break;
                }
                case BOT_CLASS_SHAMAN:
                {
                    if (me->GetLevel() >= 10)
                    {
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SHAMAN_ENCH_REFRESH), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SHAMAN_ENCH_MH), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                        Item const* oweap = _equips[BOT_SLOT_OFFHAND];
                        if (oweap && oweap->GetTemplate()->GetClass() == ITEM_CLASS_WEAPON)
                            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SHAMAN_ENCH_OH), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 4);
                    }
                    if (me->GetShapeshiftForm() != FORM_NONE)
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_REMOVE_SHAPESHIFT), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 5);
                    break;
                }
                case BOT_CLASS_DRUID:
                {
                    if (me->GetShapeshiftForm() != FORM_NONE)
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_REMOVE_SHAPESHIFT), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                    break;
                }
                case BOT_CLASS_HUNTER:
                {
                    if (me->GetLevel() >= 10)
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_CHOOSE_PET_TYPE), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
          ...
          //  player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_UR_DISMISSED),
              //  GOSSIP_SENDER_DISMISS, GOSSIP_ACTION_INFO_DEF + 1, astr.str().c_str(), 0, false);

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_PULL_URSELF), GOSSIP_SENDER_TROUBLESHOOTING, GOSSIP_ACTION_INFO_DEF + 1);
        }
    }
    ...
    {
        menus = true;
        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_STUDY_CREATURE), GOSSIP_SENDER_SCAN, GOSSIP_ACTION_INFO_DEF + 1);
    }
    ...
    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NEVERMIND), 0, GOSSIP_ACTION_INFO_DEF + 1);
...
bool bot_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
    ...
    switch (sender)
    {
            ...
            switch (_botclass)
            {
                case BOT_CLASS_MAGE:
                {
                    ...
                    if (option == 1 || option == 2) //food, water
                    {
                        ...
                        SpellInfo const* Info = sSpellMgr->GetSpellInfo(food, DIFFICULTY_NONE);
                ...
                case BOT_CLASS_ROGUE:
                {
                    ...
                    if (action == 1)
                    {
                        ...
                        if (obj)
                        {
                            ...
                            AddGossipItemFor(player, GossipOptionNpc::None, msg.str().c_str(), GOSSIP_SENDER_CLASS_ACTION1, GOSSIP_ACTION_INFO_DEF + ++count);
                        }
                        ...
                        for (uint8 i = INVENTORY_SLOT_ITEM_START; i != INVENTORY_SLOT_ITEM_END; ++i)
                        {
                            ...
                            if (item && item->IsLocked() && item->GetTemplate()->GetLockID())
                            {
                                lockInfo = sLockStore.LookupEntry(item->GetTemplate()->GetLockID());
                                if (!lockInfo)
                                    continue;
                                ...
                                for (uint8 j = 0; j != MAX_LOCK_CASE; ++j)
                                {
                                        ...
                                        lockInfo->Skill[j] <= uint32(15 + creature->GetLevel() * 5))
                                    {
                                        ...
                                        AddGossipItemFor(player, GossipOptionNpc::None, name.str().c_str(), GOSSIP_SENDER_CLASS_ACTION1, GOSSIP_ACTION_INFO_DEF + item->GetGUID().GetCounter());
                                        break;
                                    }
                                }
                            }
                            ...
                            if (Bag const* bag = player->GetBagByPos(i))
                            {
                                for (uint32 j = 0; j != bag->GetBagSize() && count <= maxcounter; ++j)
                                {
                                    ...
                                    if (item && item->IsLocked() && item->GetTemplate()->GetLockID())
                                    {
                                        ...
                                        {
                                            if (lockInfo->Type[k] == LOCK_KEY_SKILL && lockInfo->Index[k] == LOCKTYPE_LOCKPICKING &&
                        ...
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + ++count);
                    }
                    else if (action == 2)
                    {
                    ...
                    else if (action == 3)
                    {
                        ...
                        for (uint32 i = BOTAI_MISC_ENCHANT_AVAILABLE_1; i <= BOTAI_MISC_ENCHANT_AVAILABLE_6; ++i)
                        {
                            ...
                            {
                                SpellInfo const* availableInfo = sSpellMgr->GetSpellInfo(possiblePoisonMaxRank, DIFFICULTY_NONE);
                                ...
                                AddGossipItemFor(player, same ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, spellName, GOSSIP_SENDER_CLASS_ACTION2, GOSSIP_ACTION_INFO_DEF + possiblePoison);
                            }
                        }
                        AddGossipItemFor(player, isauto ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_AUTO), GOSSIP_SENDER_CLASS_ACTION2, GOSSIP_ACTION_INFO_DEF + 0);
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 1);
                    }
                    else if (action == 4)
                    {
                        subMenu = true;
                        ...
                        for (uint32 i = BOTAI_MISC_ENCHANT_AVAILABLE_1; i <= BOTAI_MISC_ENCHANT_AVAILABLE_6; ++i)
                        {
                            ...
                            if (uint32 possiblePoisonMaxRank = GetSpell(possiblePoison))
                            {
                                ...
                                std::string spellName;
                                _LocalizeSpell(player, spellName, availableInfo->Id);
                                AddGossipItemFor(player, same ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, spellName, GOSSIP_SENDER_CLASS_ACTION3, GOSSIP_ACTION_INFO_DEF + possiblePoison);
                            }
                        }
                        AddGossipItemFor(player, isauto ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_AUTO), GOSSIP_SENDER_CLASS_ACTION3, GOSSIP_ACTION_INFO_DEF + 0);
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 1);
                    }
                ...
                case BOT_CLASS_SHAMAN:
                {
                    ...
                    else if (action == 3)
                    {
                        ...
                        for (uint32 i = BOTAI_MISC_ENCHANT_AVAILABLE_1; i <= BOTAI_MISC_ENCHANT_AVAILABLE_5; ++i)
                        {
                            ...
                            {
                                SpellInfo const* availableInfo = sSpellMgr->GetSpellInfo(possibleEcnhantMaxRank, DIFFICULTY_NONE);
                                uint32 curMHId = GetAIMiscValue(BOTAI_MISC_ENCHANT_CURRENT_MH);
                                bool same = possibleEnchant == curMHId;
                                std::string spellName;
                                _LocalizeSpell(player, spellName, availableInfo->Id);
                                AddGossipItemFor(player, same ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, spellName, GOSSIP_SENDER_CLASS_ACTION2, GOSSIP_ACTION_INFO_DEF + possibleEnchant);
                            }
                        }
                        AddGossipItemFor(player, isauto ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_AUTO), GOSSIP_SENDER_CLASS_ACTION2, GOSSIP_ACTION_INFO_DEF + 0);
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 1);
                    ...
                    else if (action == 4)
                    {
                        ...
                        for (uint32 i = BOTAI_MISC_ENCHANT_AVAILABLE_1; i <= BOTAI_MISC_ENCHANT_AVAILABLE_5; ++i)
                        {
                            ...
                            if (uint32 possibleEcnhantMaxRank = GetSpell(possibleEnchant))
                            {
                                ...
                                std::string spellName;
                                _LocalizeSpell(player, spellName, availableInfo->Id);
                                AddGossipItemFor(player, same ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, spellName, GOSSIP_SENDER_CLASS_ACTION3, GOSSIP_ACTION_INFO_DEF + possibleEnchant);
                            }
                        }
                        AddGossipItemFor(player, isauto ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_AUTO), GOSSIP_SENDER_CLASS_ACTION3, GOSSIP_ACTION_INFO_DEF + 0);
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 1);
                    ...
                    if (action == 2)
                    {
                        ...
                        for (uint32 i = BOTAI_MISC_PET_AVAILABLE_1; i <= BOTAI_MISC_PET_AVAILABLE_11; ++i)
                        {
                            if (uint32 possibleType = GetAIMiscValue(i))
                            {
                                ...
                                AddGossipItemFor(player, same ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, name.c_str(), GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + possibleType);
                            }
                        }
                        bool noPet = curType == BOT_PET_INVALID;
                        AddGossipItemFor(player, noPet ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NONE2), GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + uint32(BOT_PET_INVALID));
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_AUTO), GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + 0);
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 1);
                    }
                    break;
                }
                case BOT_CLASS_WARLOCK:
                {
                    ...
                    if (action == 2)
                    {
                        ...
                        for (uint32 i = BOTAI_MISC_PET_AVAILABLE_1; i <= BOTAI_MISC_PET_AVAILABLE_5; ++i)
                        {
                            if (uint32 possibleType = GetAIMiscValue(i))
                            {
                                ...
                                ASSERT(cinfo);
                                bool same = possibleType == curType;
                                AddGossipItemFor(player, same ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, cinfo->Name.c_str(), GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + possibleType);
                            }
                        }
                        bool noPet = curType == BOT_PET_INVALID;
                        AddGossipItemFor(player, noPet ? GossipOptionNpc::Battlemaster  : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NONE2), GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + uint32(BOT_PET_INVALID));
                    ...
                    else if (action == 1)
                    {
                        ...
                        SpellInfo const* spellInfo = spellId ? sSpellMgr->GetSpellInfo(spellId, DIFFICULTY_NONE) : nullptr;
                        if (!spellInfo)
                        {
                        ...
                        for (; i != 0; --i)
                            if (ItemTemplate const* stone = sObjectMgr->GetItemTemplate(HealthStones[i]))
                                if (stone->GetBaseRequiredLevel() <= checker->GetLevel())
                                    break;
            ...
            switch (_botclass)
            {
                case BOT_CLASS_ROGUE:
                {
                    if (!IsCasting())
                    {
                        ...
                        SpellInfo const* Info = sSpellMgr->GetSpellInfo(picklock, DIFFICULTY_NONE);
        ...
        case GOSSIP_SENDER_EQUIPMENT: //equips change s1: send what slots we can use
        {
            ...
            //general
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SHOW_INVENTORY), GOSSIP_SENDER_EQUIPMENT_LIST, GOSSIP_ACTION_INFO_DEF + 1);
            if (BotMgr::IsGearBankEnabled())
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BOT_GEAR_BANK), GOSSIP_SENDER_EQUIPMENT_BANK_MENU, GOSSIP_ACTION_INFO_DEF + 1);

           // auto-equip
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_AUTOEQUIP) + "...", GOSSIP_SENDER_EQUIP_AUTOEQUIP, GOSSIP_ACTION_INFO_DEF + 1);

           // weapons
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_MH) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_MAINHAND));
            if (_canUseOffHand())
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_OH) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_OFFHAND));
            if (_canUseRanged())
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_RH) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_RANGED));
            if (_canUseRelic())
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_RELIC) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_RANGED));

            //armor
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_HEAD) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_HEAD));
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_SHOULDERS) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_SHOULDERS));
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_CHEST) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_CHEST));
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_WAIST) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_WAIST));
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_LEGS) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_LEGS));
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_FEET) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_FEET));
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_WRIST) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_WRIST));
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_HANDS) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_HANDS));

            if (IsHumanoidClass(_botclass))
            {
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_BACK) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_BACK));
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_SHIRT) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_BODY));
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_FINGER1) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_FINGER1));
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_FINGER2) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_FINGER2));
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_TRINKET1) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_TRINKET1));
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_TRINKET2) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_TRINKET2));
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SLOT_NECK) + "...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + uint32(BOT_SLOT_NECK));
            }

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_UNEQUIP_ALL), GOSSIP_SENDER_UNEQUIP_ALL, GOSSIP_ACTION_INFO_DEF + 1, LocalizedNpcText(player, BOT_TEXT_UNEQUIP_ALL) + "?", 0, false);
            if (creature->GetCreatureTemplate()->unit_flags2 & UNIT_FLAG2_MIRROR_IMAGE)
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_UPDATE_VISUAL), GOSSIP_SENDER_MODEL_UPDATE, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        case GOSSIP_SENDER_EQUIP_TRANSMOGS:
        {
            ...
            if (tramsmogData && tramsmogData->transmogs[slot].first)
            {
                ...
                if (item_id >= 0)
                {
                    ...
                    AddGossipItemFor(player, GossipOptionNpc::Battlemaster, msg.str(), GOSSIP_SENDER_EQUIP_TRANSMOG_INFO, GOSSIP_ACTION_INFO_DEF + slot);

                    //s5.2.1.2a: reset
                    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NONE), GOSSIP_SENDER_EQUIP_TRANSMOGRIFY + slot, std::numeric_limits<uint32>::max());
                }
                else
                {
                    //s5.2.1.2b: None
                    AddGossipItemFor(player, GossipOptionNpc::Battlemaster, LocalizedNpcText(player, BOT_TEXT_NONE), GOSSIP_SENDER_EQUIP_TRANSMOGS, action);
                }
            }
            ...
            if (slot > BOT_SLOT_RANGED &&
                ...
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_HIDDEN), GOSSIP_SENDER_EQUIP_TRANSMOGRIFY + slot, 0);
                ...
                for (std::set<uint32>::const_iterator itr = itemList.begin(); itr != itemList.end() && counter < maxcounter; ++itr)
                {
                    ...
                    for (uint8 i = INVENTORY_SLOT_ITEM_START; i != INVENTORY_SLOT_ITEM_END; ++i)
                    {
                        ...
                        if (item && item->GetGUID().GetCounter() == (*itr))
                        {
                            ...
                            AddGossipItemFor(player, GossipOptionNpc::None, name.str(), GOSSIP_SENDER_EQUIP_TRANSMOGRIFY + slot, item->GetEntry());
                            ++counter;
                            found = true;
                            break;
                        }
            ...
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 2);
        ...
        case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
        {
            ...
            //    AddGossipItemFor(player, GossipOptionNpc::None, str.str().c_str(), GOSSIP_SENDER_EQUIPMENT_INFO, action);
            ...
            //        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_TRANSMOGRIFICATION), GOSSIP_SENDER_EQUIP_TRANSMOGS, action);
            //}
            //else
            //{
            //    str << LocalizedNpcText(player, BOT_TEXT_NOTHING);
            //    AddGossipItemFor(player, GossipOptionNpc::None, str.str().c_str(), GOSSIP_SENDER_EQUIPMENT_SHOW, action);
            //}
            ...
            //            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_USE_OLD_EQUIPMENT), GOSSIP_SENDER_EQUIP_RESET, action);
            //        else
            //            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_UNEQUIP), GOSSIP_SENDER_UNEQUIP, action);
            //    }
            ...
            //if (itemList.empty())
            //{
            //    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NOTHING_TO_GIVE), 0, GOSSIP_ACTION_INFO_DEF + 1);
            //}
            //else
            //{
            //    uint32 counter = 0;
            ...
            //                AddGossipItemFor(player, GossipOptionNpc::None, name.str().c_str(), GOSSIP_SENDER_EQUIP + slot, GOSSIP_ACTION_INFO_DEF + item->GetGUID().GetCounter());
            //                ++counter;
            //                found = true;
            //                break;
            //            }
        ...
        case GOSSIP_SENDER_EQUIP_AUTOEQUIP:
        {
            ...
            //                if (BotMgr::SendEquipListItems())
            //                    BotWhisper(name.str(), player);
            //                AddGossipItemFor(player, GossipOptionNpc::None, name.str().c_str(), GOSSIP_SENDER_EQUIP_AUTOEQUIP_EQUIP + k, GOSSIP_ACTION_INFO_DEF + item->GetGUID().GetCounter());
            //                ++counter;
            //                found = true;
            //                break;
            //            }
        ...
        case GOSSIP_SENDER_EQUIPMENT_BANK_DEPOSIT:
        {
            ...
            for (uint8 i = INVENTORY_SLOT_ITEM_START; i != INVENTORY_SLOT_ITEM_END && can_add_count <= items_per_page; ++i)
            {
                if (Item const* pItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
                {
                    if (is_bot_equippable_item(pItem->GetTemplate()))
                    {
                        ...
                        AddGossipItemFor(player, GossipOptionNpc::None, name.str(), GOSSIP_SENDER_EQUIPMENT_BANK_DEPOSIT_ITEM, GOSSIP_ACTION_INFO_DEF + pItem->GetGUID().GetCounter());
                    }
                }
            }
            ...
            if (page > 0)
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_PREVIOUS_PAGE), GOSSIP_SENDER_EQUIPMENT_BANK_DEPOSIT, action - 1);
            if (can_add_count > items_per_page)
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NEXT_PAGE), GOSSIP_SENDER_EQUIPMENT_BANK_DEPOSIT, action + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), GOSSIP_SENDER_EQUIPMENT_BANK_MENU, GOSSIP_ACTION_INFO_DEF + 1);
            break;
        }
        ...
        case GOSSIP_SENDER_EQUIPMENT_BANK_WITHDRAW:
        {
          ...
          /*  BotBankItemContainer const* botBankItems = BotDataMgr::GetBotBankItems(player->GetGUID());
            ...
            //    std::ostringstream name;
            //    _AddItemLink(player, item, name);
            //    //AddGossipItemFor(player, GossipOptionNpc::None, name.str(), GOSSIP_SENDER_EQUIPMENT_BANK_WITHDRAW_ITEM, GOSSIP_ACTION_INFO_DEF + item->GetGUID().GetCounter());
            //}

           // if (page > 0)
              //  AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_PREVIOUS_PAGE), GOSSIP_SENDER_EQUIPMENT_BANK_WITHDRAW, action - 1);
          //  if (botBankItems->size() > (page + 1) * items_per_page)
            //    AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NEXT_PAGE), GOSSIP_SENDER_EQUIPMENT_BANK_WITHDRAW, action + 1);
          //  AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), GOSSIP_SENDER_EQUIPMENT_BANK_MENU, GOSSIP_ACTION_INFO_DEF + 1);
            break;
        }
        case GOSSIP_SENDER_EQUIPMENT_BANK_MENU:
        {
            ...
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_DEPOSIT_ITEMS), GOSSIP_SENDER_EQUIPMENT_BANK_DEPOSIT, GOSSIP_ACTION_INFO_DEF + 0);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_WITHDRAW_ITEMS), GOSSIP_SENDER_EQUIPMENT_BANK_WITHDRAW, GOSSIP_ACTION_INFO_DEF + 0);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        case GOSSIP_SENDER_ROLES_MAIN: //ROLES 1: list
        {
            ...
            if (IsHumanoidClass(_botclass))
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_GATHERING) + "...", GOSSIP_SENDER_ROLES_GATHERING, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_LOOTING) + "...", GOSSIP_SENDER_ROLES_LOOTING, GOSSIP_ACTION_INFO_DEF + 2);
            ...
            for (; role != BOT_MAX_ROLE; role <<= 1)
            {
                ...
                AddGossipItemFor(player,GossipOptionNpc::None, LocalizedNpcText(player, GetRoleString(role)), GOSSIP_SENDER_ROLES_MAIN_TOGGLE, GOSSIP_ACTION_INFO_DEF + role);
            }
        ...
        case GOSSIP_SENDER_ROLES_GATHERING:
        {
            ...
            for (; role != BOT_MAX_ROLE; role <<= 1)
            {
                ...
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, GetRoleString(role)), GOSSIP_SENDER_ROLES_GATHERING_TOGGLE, GOSSIP_ACTION_INFO_DEF + role);
            }

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), GOSSIP_SENDER_ROLES_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        case GOSSIP_SENDER_ROLES_LOOTING:
        {
            ...
            for (; role != BOT_MAX_ROLE; role <<= 1)
            {
                ...
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, GetRoleString(role)), GOSSIP_SENDER_ROLES_LOOTING_TOGGLE, GOSSIP_ACTION_INFO_DEF + role);
            }
        ...
        case GOSSIP_SENDER_ABILITIES:
        {
            ...
            if (HasAbilitiesSpecifics())
               AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ABILITIES_STATUS) + "...", GOSSIP_SENDER_ABILITIES_SPECIFICS_LIST, GOSSIP_ACTION_INFO_DEF + 1);
               AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ALLOWED_ABILITIES) + "...", GOSSIP_SENDER_ABILITIES_USAGE_LIST, GOSSIP_ACTION_INFO_DEF + 2);
            ...
            for (BotSpellMap::const_iterator itr = myspells.begin(); itr != myspells.end(); ++itr)
            {
                ...
                spellInfo = sSpellMgr->GetSpellInfo(basespell, DIFFICULTY_NONE); //always valid

                std::ostringstream name;
                name << LocalizedNpcText(player, BOT_TEXT_USE_);
                _AddSpellLink(player, spellInfo, name);
                AddGossipItemFor(player, GossipOptionNpc::Trainer, name.str().c_str(), GOSSIP_SENDER_ABILITIES_USE, GOSSIP_ACTION_INFO_DEF + basespell);
            }

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_UPDATE), sender, action);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 2);
        ...
        case GOSSIP_SENDER_ABILITIES_SPECIFICS_LIST:
        {
            ...
            for (std::list<std::string>::const_iterator itr = specList.begin(); itr != specList.end(); ++itr)
                AddGossipItemFor(player, GossipOptionNpc::None, *itr, GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF);

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        case GOSSIP_SENDER_ABILITIES_USAGE_LIST_SUPPORT:
        {
            ...
            for (std::vector<uint32>::const_iterator itr = myspells->begin(); itr != myspells->end(); ++itr)
            {
                ...
                spellInfo = sSpellMgr->GetSpellInfo(*itr, DIFFICULTY_NONE); //always valid
                std::ostringstream name;
                _AddSpellLink(player, spellInfo, name);

                GossipOptionIcon icon = (GetSpell(*itr) != 0) ? BOT_ICON_ON : BOT_ICON_OFF;
                AddGossipItemFor(player, GossipOptionNpc::None, name.str().c_str(), toggleSender, GOSSIP_ACTION_INFO_DEF + *itr);
            ...
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), GOSSIP_SENDER_ABILITIES_USAGE_LIST, GOSSIP_ACTION_INFO_DEF + 2);
            break;
        }
        case GOSSIP_SENDER_ABILITIES_USAGE_LIST:
        {
            ...
            if (GetDamagingSpellsList())
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_DAMAGE) + "...", GOSSIP_SENDER_ABILITIES_USAGE_LIST_DAMAGE, GOSSIP_ACTION_INFO_DEF + 1);
            if (GetCCSpellsList())
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_CONTROL) + "...", GOSSIP_SENDER_ABILITIES_USAGE_LIST_CC, GOSSIP_ACTION_INFO_DEF + 2);
            if (GetHealingSpellsList())
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_HEAL) + "...", GOSSIP_SENDER_ABILITIES_USAGE_LIST_HEAL, GOSSIP_ACTION_INFO_DEF + 3);
            if (GetSupportSpellsList())
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_OTHER) + "...", GOSSIP_SENDER_ABILITIES_USAGE_LIST_SUPPORT, GOSSIP_ACTION_INFO_DEF + 4);

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 5);
            break;
        }
        ...
        case GOSSIP_SENDER_SPEC:
        {
            ...
            for (uint8 i = specIndex; i < specIndex + 3; ++i)
            {
                ...
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, TextForSpec(i)), GOSSIP_SENDER_SPEC_SET, GOSSIP_ACTION_INFO_DEF + i);
            }
        ...
        case GOSSIP_SENDER_USEITEM:
        {
            ...
            static const auto is_consumable_item = [](Item const* item, Creature const* bot) {
                if (ItemTemplate const* proto = item ? item->GetTemplate() : nullptr)
                {
                    if (!(proto->GetClass() != ITEM_CLASS_WEAPON && proto->GetClass() != ITEM_CLASS_ARMOR &&
                        (proto->GetAllowableClass() == 0 || (proto->GetAllowableClass() & (1 << (bot->GetBotClass() - 1)))) &&
                        proto->GetRequiredSkill() == 0 && proto->GetRequiredSpell() == 0 && bot->GetLevel() >= proto->GetBaseRequiredLevel()))
                        return false;
            ...
            for (uint8 i = INVENTORY_SLOT_ITEM_START; i != INVENTORY_SLOT_ITEM_END; ++i)
            {
                ...
                if (is_consumable_item(item, me))
                {
                    std::ostringstream name;
                    _AddItemLink(player, item, name);
                    AddGossipItemFor(player, GossipOptionNpc::None, name.str().c_str(), GOSSIP_SENDER_USEITEM_USE, GOSSIP_ACTION_INFO_DEF + item->GetGUID().GetCounter());
        ...
        case GOSSIP_SENDER_FORMATION:
        {
           ...
           // player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GossipOptionNpc::None, diststr.str(),
              //  GOSSIP_SENDER_FORMATION_FOLLOW_DISTANCE_SET, GOSSIP_ACTION_INFO_DEF + 1, "", 0, true);

            if (HasRole(BOT_ROLE_RANGED))
            {
                AddGossipItemFor(player, !player->GetBotMgr()->GetBotAllowCombatPositioning() ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None,
                ...
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ATTACK_DISTANCE) + "...", GOSSIP_SENDER_FORMATION_ATTACK_DISTANCE, GOSSIP_ACTION_INFO_DEF + 3);
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ATTACK_ANGLE) + "...", GOSSIP_SENDER_FORMATION_ATTACK_ANGLE, GOSSIP_ACTION_INFO_DEF + 4);
            }

            if (!HasRole(BOT_ROLE_TANK) && HasRole(BOT_ROLE_DPS | BOT_ROLE_HEAL))
                AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_ENGAGE_BEHAVIOR) + "...", GOSSIP_SENDER_ENGAGE_BEHAVIOR, GOSSIP_ACTION_INFO_DEF + 5);
            ...
            if (player->GetGroup())
            {
                for (uint32 role = BOT_ROLE_TANK; !!(role & BOT_ROLE_MASK_MAIN); role <<= 1)
                {
                    if (role & (BOT_ROLE_TANK | BOT_ROLE_DPS) && HasRole(role))
                        AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_PRIORITY_TARGET) + " (" + LocalizedNpcText(player, GetRoleString(role)) + ")...", GOSSIP_SENDER_PRIORITY_TARGET, uint32(GOSSIP_ACTION_INFO_DEF) + role);
                }
            }

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 6);
            break;
        }
        ...
        case GOSSIP_SENDER_FORMATION_ATTACK_DISTANCE:
        {
            ...
            AddGossipItemFor(player, mode == BOT_ATTACK_RANGE_SHORT ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_SHORT_RANGE_ATTACKS), GOSSIP_SENDER_FORMATION_ATTACK_DISTANCE_SET, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, mode == BOT_ATTACK_RANGE_LONG ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_LONG_RANGE_ATTACKS), GOSSIP_SENDER_FORMATION_ATTACK_DISTANCE_SET, GOSSIP_ACTION_INFO_DEF + 2);
            ...
            //player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, mode == BOT_ATTACK_RANGE_EXACT ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None,
              //  diststr.str(), GOSSIP_SENDER_FORMATION_ATTACK_DISTANCE_SET, GOSSIP_ACTION_INFO_DEF + 3, "", 0, true);

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 4);
            break;
        }
        ...
        case GOSSIP_SENDER_FORMATION_ATTACK_ANGLE:
        {
            ...
            AddGossipItemFor(player, mode == BOT_ATTACK_ANGLE_NORMAL ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NORMAL), GOSSIP_SENDER_FORMATION_ATTACK_ANGLE_SET, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, mode == BOT_ATTACK_ANGLE_AVOID_FRONTAL_AOE ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_AVOID_FRONTAL_AOE), GOSSIP_SENDER_FORMATION_ATTACK_ANGLE_SET, GOSSIP_ACTION_INFO_DEF + 2);

            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 3);
            break;
        }
        case GOSSIP_SENDER_ENGAGE_BEHAVIOR:
        {
            ...
            if (HasRole(BOT_ROLE_DPS))
            {
               ...
               // player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GossipOptionNpc::None, delaystr.str(),
                ...
                case BOT_ROLE_TANK:
                    for (int8 i = TARGET_ICONS_COUNT - 1; i >= 0; --i)
                    {
                      ...
                      //  if (guid && BotMgr::GetTankTargetIconFlags() & GroupIconsFlags[i])
                            AddGossipItemFor(player, prio ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, player->GetBotMgr()->GetTargetIconString(uint8(i + 1)), GOSSIP_SENDER_PRIORITY_TARGET_SET_TANK, uint32(GOSSIP_ACTION_INFO_DEF) + uint32(i));
                    }
                    AddGossipItemFor(player, (_primaryIconTank == -1) ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NONE2), GOSSIP_SENDER_PRIORITY_TARGET_SET_TANK, uint32(GOSSIP_ACTION_INFO_DEF - 1));
                    break;
                case BOT_ROLE_DPS:
                    for (int8 i = TARGET_ICONS_COUNT - 1; i >= 0; --i)
                    {
                       ...
                       // if (guid && iconMask & GroupIconsFlags[i])
                            AddGossipItemFor(player, prio ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, player->GetBotMgr()->GetTargetIconString(uint8(i + 1)), GOSSIP_SENDER_PRIORITY_TARGET_SET_DPS, uint32(GOSSIP_ACTION_INFO_DEF) + uint32(i));
                    }
                    AddGossipItemFor(player, (_primaryIconDamage == -1) ? GossipOptionNpc::Battlemaster : GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_NONE2), GOSSIP_SENDER_PRIORITY_TARGET_SET_DPS, uint32(GOSSIP_ACTION_INFO_DEF - 1));
                    break;
                default:
                ...
                case 2: //Remove a visible buff
                {
                    ...
                    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                    {
                       ...
                       // if ((info->Attributes & (SPELL_ATTR0_CANT_CANCEL | SPELL_ATTR0_HIDDEN_CLIENTSIDE)) ||
                        ...
                        AddGossipItemFor(player, GossipOptionNpc::None, msg.str().c_str(), GOSSIP_SENDER_TROUBLESHOOTING_AURA, GOSSIP_ACTION_INFO_DEF + itr->first);
        ...
        {
            subMenu = true;
            AddGossipItemFor(player, GossipOptionNpc::None, "Fix not mounting/following", GOSSIP_SENDER_TROUBLESHOOTING_FIX, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_REMOVE_BUFF) + "...", GOSSIP_SENDER_TROUBLESHOOTING_FIX, GOSSIP_ACTION_INFO_DEF + 2);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_FIX_POWER), GOSSIP_SENDER_TROUBLESHOOTING_FIX, GOSSIP_ACTION_INFO_DEF + 3);
            AddGossipItemFor(player, GossipOptionNpc::None, LocalizedNpcText(player, BOT_TEXT_BACK), 1, GOSSIP_ACTION_INFO_DEF + 4);
            break;
        }
            ...
            switch (action - GOSSIP_ACTION_INFO_DEF)
            {
                ...
                case 5: //list spells
                {
                    ...
                    for (BotSpellMap::const_iterator itr = myspells.begin(); itr != myspells.end(); ++itr)
                    {
                        ...
                        spellInfo = sSpellMgr->GetSpellInfo(itr->first, DIFFICULTY_NONE); //always valid
        ...
        case GOSSIP_SENDER_DEBUG:
        {
           ...
           // else
            ...
            AddGossipItemFor(player, GossipOptionNpc::None, ostr.str().c_str(), GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 0);

            AddGossipItemFor(player, GossipOptionNpc::None, "<Reset Owner>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, "<Reset Stats>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 2);
            AddGossipItemFor(player, GossipOptionNpc::None, "<List Stats>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 3);
            AddGossipItemFor(player, GossipOptionNpc::None, "<List Roles>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 4);
            AddGossipItemFor(player, GossipOptionNpc::None, "<List Spells>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 5);
            AddGossipItemFor(player, GossipOptionNpc::None, "<List Items>", GOSSIP_SENDER_EQUIPMENT_LIST, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GossipOptionNpc::None, "<Reload Config>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 6);
...
void bot_ai::_autoLootCreature(Creature* creature)
{
  ...
  /*  Player* receiver = pLooters.size() == 1 ? *pLooters.begin() :
   ...
   // _prevRRobin = receiver;
    Player* receiver = nullptr; //need remove later when rewrite
...
bool bot_ai::_canUseOffHand() const
{
    ...
    if (protoMH->GetClass() == ITEM_CLASS_WEAPON &&
        (protoMH->GetInventoryType() == INVTYPE_WEAPON || protoMH->GetInventoryType() == INVTYPE_WEAPONMAINHAND) &&
        (protoMH->GetSubClass() == ITEM_SUBCLASS_WEAPON_AXE || protoMH->GetSubClass() == ITEM_SUBCLASS_WEAPON_DAGGER ||
         protoMH->GetSubClass() == ITEM_SUBCLASS_WEAPON_MACE ||
        protoMH->GetSubClass() == ITEM_SUBCLASS_WEAPON_SWORD))
        return true;
...
bool bot_ai::_canEquip(ItemTemplate const* newProto, uint8 slot, bool ignoreItemLevel, Item const* newItem) const
{
    ...
    if (Item const* oldItem = _equips[slot])
    {
        ...
        if (newProto->GetId() == oldProto->GetId() && (!newItem || !newItem->GetItemRandomBonusListId()))
            return false;
        //prevent equipping worse items (only standard or not)
        if (!ignoreItemLevel)
            if (slot > BOT_SLOT_RANGED/* || einfo->ItemEntry[slot] != oldProto->GetId()*/)
    ...
    ////level requirements
    if (me->GetLevel() < newProto->GetBaseRequiredLevel())
        return false;

    ////class requirements
    if (_botclass < BOT_CLASS_EX_START && !(newProto->GetAllowableClass() & (1 << (_botclass - 1))))
        return false;

    //skip race requirements
    //inventory related conditions
    if (newProto->GetClass() == ITEM_CLASS_WEAPON)
    {
        switch (slot)
        {
            case BOT_SLOT_MAINHAND:
                ...
                {
                    case BOT_CLASS_BM:
                        if (newProto->GetSubClass() != ITEM_SUBCLASS_WEAPON_POLEARM &&
                            newProto->GetSubClass() != ITEM_SUBCLASS_WEAPON_AXE2 &&
                            newProto->GetSubClass() != ITEM_SUBCLASS_WEAPON_SWORD2)
                            return false;
                        break;
                    case BOT_CLASS_ARCHMAGE:
                    case BOT_CLASS_NECROMANCER:
                        if (newProto->GetSubClass() != ITEM_SUBCLASS_WEAPON_STAFF)
                            return false;
                        break;
                    default:
                        break;
            ...
            case BOT_SLOT_OFFHAND:
                if (newProto->GetSubClass() == ITEM_SUBCLASS_WEAPON_POLEARM || newProto->GetSubClass() == ITEM_SUBCLASS_WEAPON_STAFF)
                    return false;
                switch (_botclass)
                {
                    case BOT_CLASS_DEATH_KNIGHT:
        ...
        switch (newProto->GetInventoryType())
        {
    ...
    else if (newProto->GetClass() == ITEM_CLASS_ARMOR || newProto->GetClass() == ITEM_CLASS_QUEST)
    {
        switch (newProto->GetInventoryType())
        {
            case INVTYPE_HEAD:
        ...
        switch (newProto->GetSubClass())
        {
            ...
            case ITEM_SUBCLASS_ARMOR_PLATE:
                switch (_botclass)
                {
                    ...
                    case BOT_CLASS_PALADIN:
                    case BOT_CLASS_DEATH_KNIGHT:
                        if (me->GetLevel() >= 40 || newProto->GetQuality() == ITEM_QUALITY_HEIRLOOM)
                            break;
                        return false;
                    default:
                        return false;
            ...
            case ITEM_SUBCLASS_ARMOR_CLOTH:
                switch (_botclass)
                {
                    ...
                    case BOT_CLASS_SPELLBREAKER:
                    case BOT_CLASS_CRYPT_LORD:
                        if (newProto->GetInventoryType() != INVTYPE_CLOAK)
                            return false;
                        break;
                    default:
                        break;
...
bool bot_ai::_equip(uint8 slot, Item* newItem, ObjectGuid receiver)
{
    ...
    if (newItem->GetState() == ITEM_REMOVED)
    {
     //   TC_LOG_ERROR("entities.player",
          //  "minion_ai::_equip(): player {} ({}) is trying to make bot {} (id: {}) equip item: {} (id: {}, {}) which has state ITEM_REMOVED!",
           // master->GetName(), master->GetGUID().ToString(), me->GetName(), me->GetEntry(), proto->GetName(), proto->ItemId, newItem->GetGUID().ToString());
        return false;
    }
    ...
    //        TC_LOG_ERROR("entities.player",
    //            "minion_ai::_equip(): player %s (%s) is trying to make bot %s (id: {}) equip item: %s (id: {}, %s) but either does not have this item or does not own it",
...
void bot_ai::ApplyItemSetBonuses(Item* item, bool apply)
{
    if (item) //(un)equip, NOT from loops
    {
        ...
        for (uint8 i = BOT_SLOT_MAINHAND; i != BOT_INVENTORY_SIZE; ++i)
            if (_equips[i] && _equips[i]->GetTemplate()->GetItemSet() == setId)
    ...
    for (std::list<uint32>::const_iterator itr = itemSets.begin(); itr != itemSets.end(); ++itr)
    {
        ...
        uint8 setItemCount = 0;
        for (uint8 k = BOT_SLOT_MAINHAND; k != BOT_INVENTORY_SIZE; ++k)
            if (_equips[k] && _equips[k]->GetTemplate()->GetItemSet() == *itr)
                ++setItemCount;

        for (uint8 j = 0; j != MAX_ITEM_SPELLS; ++j)
        {
...
void bot_ai::_castBotItemUseSpell(Item const* item, SpellCastTargets const& targets/*, uint8 cast_count, uint32 glyphIndex*/)
{
    ...
    for (uint8 e_slot = 0; e_slot != MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s != MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            spellInfo = sSpellMgr->GetSpellInfo(pEnchant->EffectArg[s], DIFFICULTY_MYTHIC_RAID);
            if (!spellInfo)
                continue;
...
uint32 bot_ai::GetEquipDisplayId(uint8 slot) const
{
    ...
    if (_equips[slot])
    {
        ...
        if (transmogData && BotMgr::IsTransmogEnabled() &&
            (_equips[slot]->GetTemplate()->GetId() == transmogData->transmogs[slot].first || BotMgr::TransmogUseEquipmentSlots()))
        {
...
float bot_ai::GetAverageItemLevel() const
{
    ...
    for (uint8 i = BOT_SLOT_MAINHAND; i != BOT_INVENTORY_SIZE; ++i)
    {
        if (_equips[i] && !(/*i == BOT_SLOT_TABARD || */i == BOT_SLOT_OFFHAND || i == BOT_SLOT_RANGED || i == BOT_SLOT_BODY))
        {
            ...
            {
                ++count;
                sum += itemProto->GetBaseItemLevel();
            }
        }
    }
...
{
    return target ? target->HasUnitState(UNIT_STATE_CONFUSED | UNIT_STATE_STUNNED | UNIT_STATE_FLEEING | UNIT_STATE_DISTRACTED | UNIT_STATE_CONFUSED_MOVE | UNIT_STATE_FLEEING_MOVE) || (root && (target->HasUnitState(UNIT_STATE_ROOT) || target->IsFrozen())) : true;
}
    ...
    if (IsWanderer())
    {
        ...
        for (uint8 i = BOT_SLOT_MAINHAND; i < BOT_INVENTORY_SIZE; ++i)
        {
            ...
            Item* item = BotDataMgr::GenerateWanderingBotItem(i, _botclass, lvl, [this, lslot = i](ItemTemplate const* proto) {
                ...
                {
                    if (Rand() < 20 && proto->GetBaseItemLevel() < 145)
                        return false;
                    if (Rand() < 10 && proto->GetBaseItemLevel() < 264)
                        return false;
                ...
                switch (GetSpec())
                {
                    case BOT_SPEC_WARRIOR_ARMS:
                        ...
                        {
                            case BOT_SLOT_MAINHAND:
                                return proto->GetInventoryType() == INVTYPE_2HWEAPON;
                            default:
                                break;
                        }
                        break;
                    case BOT_SPEC_WARRIOR_FURY:
                        ...
                        {
                            case BOT_SLOT_MAINHAND:
                                return (me->GetLevel() < 60) ? (proto->GetInventoryType() == INVTYPE_WEAPON || proto->GetInventoryType() == INVTYPE_WEAPONMAINHAND) :
                                    (proto->GetInventoryType() == INVTYPE_2HWEAPON);
                            case BOT_SLOT_OFFHAND:
                                return (me->GetLevel() < 60) ? (proto->GetInventoryType() == INVTYPE_WEAPON || proto->GetInventoryType() == INVTYPE_WEAPONOFFHAND) :
                                    (proto->GetInventoryType() == INVTYPE_2HWEAPON);
                            default:
                                break;
                        }
                    ...
                    case BOT_SPEC_WARRIOR_PROTECTION:
                        ...
                        {
                            case BOT_SLOT_MAINHAND:
                                return proto->GetInventoryType() == INVTYPE_WEAPON || proto->GetInventoryType() == INVTYPE_WEAPONMAINHAND;
                            case BOT_SLOT_OFFHAND:
                                return proto->GetInventoryType() == INVTYPE_SHIELD;
                            default:
                                break;
                        }
                        break;
                    case BOT_SPEC_PALADIN_PROTECTION:
                        ...
                        {
                            case BOT_SLOT_MAINHAND:
                                if (!(proto->GetInventoryType() == INVTYPE_WEAPON || proto->GetInventoryType() == INVTYPE_WEAPONMAINHAND))
                                    return false;
                                if (me->GetLevel() < 30)
                                    break;
                             //   return !std::any_of(proto->ItemStat.cbegin(), proto->ItemStat.cend(), [](_ItemStat const& stat) {
                               ...
                               // });
                            case BOT_SLOT_OFFHAND:
                                if (!(proto->GetInventoryType() == INVTYPE_SHIELD))
                                    return false;
                                if (me->GetLevel() < 30)
                                    break;
                             //   return !std::any_of(proto->ItemStat.cbegin(), proto->ItemStat.cend(), [](_ItemStat const& stat) {
                    ...
                    case BOT_SPEC_ROGUE_COMBAT:
                        ...
                        {
                            case BOT_SLOT_MAINHAND: case BOT_SLOT_OFFHAND:
                                return proto->GetSubClass() == ITEM_SUBCLASS_WEAPON_SWORD || proto->GetSubClass() == ITEM_SUBCLASS_WEAPON_AXE;
                            case BOT_SLOT_RANGED:
                                return me->GetLevel() < 64 || proto->GetSubClass() == ITEM_SUBCLASS_WEAPON_THROWN;
                            default:
                                break;
                        }
                        break;
                    case BOT_SPEC_ROGUE_SUBTLETY:
                        ...
                        {
                            case BOT_SLOT_MAINHAND: case BOT_SLOT_OFFHAND:
                                return proto->GetSubClass() == ITEM_SUBCLASS_WEAPON_MACE;
                            case BOT_SLOT_RANGED:
                                return me->GetLevel() < 54 || proto->GetSubClass() == ITEM_SUBCLASS_WEAPON_THROWN;
                            default:
                                break;
                        }
                        break;
                    case BOT_SPEC_DK_FROST:
                        ...
                        {
                            case BOT_SLOT_MAINHAND:
                                return me->GetLevel() < 61 || proto->GetInventoryType() == INVTYPE_2HWEAPON;
                            default:
                                break;
                        }
                        break;
                    case BOT_SPEC_SHAMAN_ENHANCEMENT:
                        ...
                        {
                            case BOT_SLOT_OFFHAND:
                                return proto->GetInventoryType() == INVTYPE_WEAPON || proto->GetInventoryType() == INVTYPE_WEAPONOFFHAND;
                            case BOT_SLOT_TRINKET1: case BOT_SLOT_TRINKET2:
                                break;
                            default:
                                if (me->GetLevel() < 40)
            ...
            else
            {
                ...
                gss << " [" << uint32(i) << "] " << _equips[i]->GetTemplate()->GetName(LOCALE_enUS) << " (" << _equips[i]->GetEntry() << ')';
            }
        }
...
void bot_ai::_AddItemTemplateLink(Player const* forPlayer, ItemTemplate const* item, std::ostringstream &str) const
{
    ...
    str << "|Hitem:" << uint32(item->GetId()) << ':';

    ////permanent enchantment, 3 gems, 4 unknowns, reporter_level (9)
    str << "0:0:0:0:0:0:0:0:0";

    ////name
    std::string name = item->GetName(LOCALE_enUS);
    _LocalizeItem(forPlayer, name, item->GetId());
    str << "|h[" << name << "]|h|r";

    ////max in stack
    if (item->GetBuyCount() > 1)
        str<< "|cff009900x" << item->GetBuyCount() << ".|r";
    else
        str << "|cff009900.|r";
}
...
void bot_ai::_AddItemLink(Player const* forPlayer, Item const* item, std::ostringstream &str, bool addIcon) const
{
    ...
    std::string name = proto->GetName(LOCALE_enUS);
    std::string suffix = "";
    ...
    str << 0 << ':';
    //random property
    str << item->GetItemRandomBonusListId() << ':';
...
void bot_ai::_AddQuestLink(Player const* forPlayer, Quest const* quest, std::ostringstream &str) const
{
    ...
    str << "|cFFEFFD00|Hquest:" << quest->GetQuestId() << ':' << quest->GetMaxLevel() << "|h[" << questTitle << "]|h|r";
}
...
void bot_ai::_LocalizeItem(Player const* forPlayer, std::string &itemName, std::string &suffix, Item const* item) const
{
    ...
    //ItemTemplate const* itemInfo = sObjectMgr->GetItemLocale(item->GetEntry());
    ...
    int32 randomPropId = item->GetItemRandomBonusListId();
    if (!randomPropId)
        return;
...
void bot_ai::_LocalizeQuest(Player const* forPlayer, std::string &questTitle, uint32 entry) const
{
    ...
    QuestTemplateLocale const* questInfo = sObjectMgr->GetQuestLocale(entry);
    if (!questInfo)
        return;
...
void bot_ai::_LocalizeSpell(Player const* forPlayer, std::string &spellName, uint32 entry) const
{
    ...
    //LocalizedString title = spellInfo->SpellName[loc];
    //if (Utf8FitTo(title, wnamepart))
    //    spellName = title;
    //else
            ...
            veh->GetBase()->ToCreature()->HasSpell(spellInfo->Id))
        {
            ...
            if (spell->m_targets.HasDst())
            {
                ...
                if (veh->GetBase()->GetTransport())
                    destangle = Position::NormalizeOrientation(destangle - veh->GetBase()->GetTransport()->GetTransportOrientation());
        ...
        for (uint8 s = 0; s != MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (entry)
            {
                if (entry->ProcsPerMinute)
                {
                    ...
                    chance = me->GetPPMProcChance(proto->GetDelay(), chance, spellInfo);
                }
...
bool bot_ai::GlobalUpdate(uint32 diff)
{
        ...
        for (uint8 i = CURRENT_FIRST_NON_MELEE_SPELL; i != CURRENT_MAX_SPELL; ++i)
        {
            ...
            if (!interrupt && !info->IsPositive())
            {
               ...
               // else if (target->HasAuraType(SPELL_AURA_PERIODIC_DAMAGE) && !IsCasting(target) &&
                  //  (info->AuraInterruptFlags) && _botclass != BOT_CLASS_MAGE)
                   // interrupt = true; //useless control breaks immediately (skip glyphed poly)
                else if (info->DmgClass == SPELL_DAMAGE_CLASS_MAGIC && !(info->Attributes) &&
                    !(info->AttributesEx) &&
                    !(info->Attributes) &&
    ...
    if (_updateTimerEx1 <= diff)
    {
        ...
        */
        ...
        //Phase
        if (me->GetDBPhase() != master->GetDBPhase())
        {
            BotWhisper("Somehow we are not is same phase! Fixing that...");
            me->SetDBPhase(master->GetDBPhase());
        }
        //Transport state
            ...
            !HasBotCommandState(BOT_COMMAND_MASK_UNMOVING))
        {
            ...
            if (!crList.empty())
            {
                ...
                else
                {
                    ...
                    Position pos;
                    un->GetNearPoint(me, pos.m_positionX, pos.m_positionY, pos.m_positionZ, CONTACT_DISTANCE, un->GetAbsoluteAngle(me));
    ...
    if (checkAurasTimer <= lastdiff)
    {
        ...
        {
            if (Unit* victim = CanBotAttackOnVehicle() ? me->GetVictim() : true ? ObjectAccessor::GetUnit(*mover, mover->GetTarget()) : nullptr)
            {
                _aoeSpots.clear();
                if (IAmFree())
        ...
        if (shouldUpdateStats/* && me->GetPhaseShift() == master->GetPhaseShift()*/)
            SetStats(false);
        ...
        (!me->GetVehicle() || (!CCed(me->GetVehicleBase(), true) && !me->GetVehicleBase()->GetTarget())))
    {
        ...
        if (!master->IsAlive())
        {
            ...
            if (master->HasPlayerFlag(PLAYER_FLAGS_GHOST))
            {
...
void bot_ai::GetNextEvadeMovePoint(Position& pos, bool& use_path) const
{
    ...
    ZLiquidStatus lstatus = me->GetMap()->GetLiquidStatus(
        me->GetPhaseShift(), mypos.m_positionX, mypos.m_positionY, mypos.m_positionZ, map_liquidHeaderTypeFlags::AllLiquids, &ldata, me->GetCollisionHeight());
...
Unit* bot_ai::SpawnVehicle(uint32 creEntry, uint32 vehEntry)
{
    ...
    if (!me->GetTransport())
    {
     ...
     //   ASSERT(vc->Create(map->GenerateLowGuid<HighGuid::Unit>(), map, master->GetPhaseMask(), creEntry, Position(x,y,z,o), nullptr, vehEntry, true));
        ...
        vc->InitSummon(vc); //not needed really
    }
    else
    {
        if (master->GetVehicle())
        ...
        vc = new TempSummon(nullptr, me, false);
       // ASSERT(vc->Create(map->GenerateLowGuid<HighGuid::Unit>(), map, master->GetPhaseShift(), creEntry, Position(x, y, z, o), nullptr, vehEntry, true));
...
void bot_ai::ChooseVehicleForEncounter(uint32 &creEntry, uint32 &vehEntry) const
{
    ...
    switch (mVeh->GetBase()->GetEntry())
    {
        ...
        case CREATURE_OCULUS_DRAKE_EMERALD:
        case CREATURE_OCULUS_DRAKE_AMBER:
            creEntry = (HasRole(BOT_ROLE_TANK)) ? CREATURE_OCULUS_DRAKE_RUBY :
                (HasRole(BOT_ROLE_HEAL)) ? CREATURE_OCULUS_DRAKE_EMERALD :
...
{
    return me->IsInWorld() &&
        (me->GetMap()->GetLiquidStatus(me->GetPhaseShift(), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(),
            map_liquidHeaderTypeFlags::Water | map_liquidHeaderTypeFlags::Ocean) & MAP_LIQUID_STATUS_IN_CONTACT);
    return 0;
}
...
{
    if (me->CanUseAttackType(BASE_ATTACK))
    {
        switch (_botclass)
        {
            case BOT_CLASS_SPHYNX:
...
bool bot_ai::CanUseAmmo() const
{
        ...
        _equips[BOT_SLOT_RANGED])
    {
        ...
        if (ranged->GetClass() == ITEM_CLASS_WEAPON &&
            (ranged->GetSubClass() == ITEM_SUBCLASS_WEAPON_BOW ||
            ranged->GetSubClass() == ITEM_SUBCLASS_WEAPON_CROSSBOW ||
            ranged->GetSubClass() == ITEM_SUBCLASS_WEAPON_GUN))
            return true;
    }
    return false;
}
...
bool bot_ai::IsValidTransmog(uint8 slot, ItemTemplate const* source) const
{
    ...
    switch (target->GetInventoryType())
    {
        case INVTYPE_RELIC:
        case INVTYPE_NECK:
        case INVTYPE_FINGER:
    ...
    switch (source->GetInventoryType())
    {
        case INVTYPE_RELIC:
        case INVTYPE_NECK:
        case INVTYPE_FINGER:
    ...
    if (target->GetSubClass() != source->GetSubClass())
    {
        if (target->GetClass() == ITEM_CLASS_WEAPON && !BotMgr::MixWeaponClasses())
            return false;
        if (target->GetClass() == ITEM_CLASS_ARMOR && !BotMgr::MixArmorClasses())
            return false;
    }

    if (target->GetInventoryType() != source->GetInventoryType())
    {
        if (target->GetClass() == ITEM_CLASS_ARMOR)
        {
            if (!((target->GetInventoryType() == INVTYPE_ROBE || target->GetInventoryType() == INVTYPE_CHEST) &&
                (source->GetInventoryType() == INVTYPE_ROBE || source->GetInventoryType() == INVTYPE_CHEST)))
                return false;
        }
        if (target->GetClass() == ITEM_CLASS_WEAPON && !BotMgr::MixWeaponInventoryTypes())
            return false;
    }

    NpcBotTransmogData const* transmogData = BotDataMgr::SelectNpcBotTransmogs(me->GetEntry());
    if (transmogData && transmogData->transmogs[slot].second == int32(source->GetId()))
        return false;
