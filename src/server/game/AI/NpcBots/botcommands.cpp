...
#define FILL_VISUALS_REPORT2(s,r) s \
    <<  DB2Manager::GetRaceName(r, loc) << " Male:" \
    ...
    << "\n" <<  DB2Manager::GetRaceName(r, loc) << " Female:" \
...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        {
            { "flags",      HandleNpcBotToggleFlagsCommand,         rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
        };

        static ChatCommandTable npcbotWPCommandTable =
        {
            { "generate",   HandleNpcBotWPGenerateCommand,          rbac::RBAC_PERM_COMMAND_GM,              Console::Yes },
            { "spawnall",   HandleNpcBotWPSpawnAllCommand,          rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "move",       HandleNpcBotWPMoveCommand,              rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "add",        HandleNpcBotWPAddCommand,               rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "del",        HandleNpcBotWPDeleteCommand,            rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "list",       HandleNpcBotWPListCommand,              rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "list all",   HandleNpcBotWPListAllCommand,           rbac::RBAC_PERM_COMMAND_GM,              Console::Yes },
            { "go",         HandleNpcBotWPGoCommand,                rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "setlevels",  HandleNpcBotWPSetLevelsCommand,         rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "setlevels z",HandleNpcBotWPSetLevelsZoneCommand,     rbac::RBAC_PERM_COMMAND_GM,              Console::Yes },
            { "setflags",   HandleNpcBotWPSetFlagsCommand,          rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "setflags z", HandleNpcBotWPSetFlagsZoneCommand,      rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "setname",    HandleNpcBotWPSetNameCommand,           rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "setlinks",   HandleNpcBotWPLinksSetCommand,          rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "info",       HandleNpcBotWPCommand,                  rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "links",      HandleNpcBotWPLinksCommand,             rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
        };

        static ChatCommandTable npcbotDebugCommandTable =
        {
            { "raid",       HandleNpcBotDebugRaidCommand,           rbac::RBAC_PERM_COMMAND_GM,         Console::No  },
            { "mount",      HandleNpcBotDebugMountCommand,          rbac::RBAC_PERM_COMMAND_GM,        Console::No  },
            { "model",      HandleNpcBotDebugModelCommand,          rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "spellvisual",HandleNpcBotDebugSpellVisualCommand,    rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "states",     HandleNpcBotDebugStatesCommand,         rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "names",      HandleNpcBotDebugNamesCommand,          rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "spells",     HandleNpcBotDebugSpellsCommand,         rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "guids",      HandleNpcBotDebugGuidsCommand,          rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "wbequips",   HandleNpcBotDebugWBEquipsCommand,       rbac::RBAC_PERM_COMMAND_GM,       Console::Yes },
        };

        static ChatCommandTable npcbotSetCommandTable =
        {
            { "faction",    HandleNpcBotSetFactionCommand,          rbac::RBAC_PERM_COMMAND_GM,        Console::No  },
            { "owner",      HandleNpcBotSetOwnerCommand,            rbac::RBAC_PERM_COMMAND_GM,          Console::No  },
            { "spec",       HandleNpcBotSetSpecCommand,             rbac::RBAC_PERM_COMMAND_GM,           Console::No  },
        };

        static ChatCommandTable npcbotCommandFollowCommandTable =
        {
            { "",           HandleNpcBotCommandFollowCommand,       rbac::RBAC_PERM_COMMAND_GM,     Console::No  },
            { "only",       HandleNpcBotCommandFollowOnlyCommand,   rbac::RBAC_PERM_COMMAND_GM,     Console::No  },
        };

        static ChatCommandTable npcbotCommandCommandTable =
        {
            { "standstill", HandleNpcBotCommandStandstillCommand,   rbac::RBAC_PERM_COMMAND_GM, Console::No  },
            { "stopfully",  HandleNpcBotCommandStopfullyCommand,    rbac::RBAC_PERM_COMMAND_GM,  Console::No  },
            { "follow",     npcbotCommandFollowCommandTable                                                                         },
            { "walk",       HandleNpcBotCommandWalkCommand,         rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "nogossip",   HandleNpcBotCommandNoGossipCommand,     rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "unbind",     HandleNpcBotCommandUnBindCommand,       rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "rebind",     HandleNpcBotCommandReBindCommand,       rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "nocast",     HandleNpcBotCommandNoCastCommand,       rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "nolongcast", HandleNpcBotCommandNoLongCastCommand,   rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
        };

        static ChatCommandTable npcbotAttackDistanceCommandTable =
        {
            { "short",      HandleNpcBotAttackDistanceShortCommand, rbac::RBAC_PERM_COMMAND_GM,  Console::No  },
            { "long",       HandleNpcBotAttackDistanceLongCommand,  rbac::RBAC_PERM_COMMAND_GM,   Console::No  },
            { "",           HandleNpcBotAttackDistanceExactCommand, rbac::RBAC_PERM_COMMAND_GM,  Console::No  },
        };

        static ChatCommandTable npcbotDistanceCommandTable =
        {
            { "attack",     npcbotAttackDistanceCommandTable                                                                        },
            { "",           HandleNpcBotFollowDistanceCommand,      rbac::RBAC_PERM_COMMAND_GM,  Console::No  },
        };

        static ChatCommandTable npcbotOrderCommandTable =
        {
            { "cast",       HandleNpcBotOrderCastCommand,           rbac::RBAC_PERM_COMMAND_GM,         Console::No  },
        };

        static ChatCommandTable npcbotVehicleCommandTable =
        {
            { "eject",      HandleNpcBotVehicleEjectCommand,        rbac::RBAC_PERM_COMMAND_GM,      Console::No  },
        };

        static ChatCommandTable npcbotDumpCommandTable =
        {
            { "load",       HandleNpcBotDumpLoadCommand,            rbac::RBAC_PERM_COMMAND_GM,          Console::Yes },
            { "write",      HandleNpcBotDumpWriteCommand,           rbac::RBAC_PERM_COMMAND_GM,         Console::Yes },
        };

        static ChatCommandTable npcbotRecallCommandTable =
        {
            { "",           HandleNpcBotRecallCommand,              rbac::RBAC_PERM_COMMAND_GM,             Console::No  },
            { "spawns",     HandleNpcBotRecallSpawnsCommand,        rbac::RBAC_PERM_COMMAND_GM,             Console::No  },
            { "teleport",   HandleNpcBotRecallTeleportCommand,      rbac::RBAC_PERM_COMMAND_GM,             Console::No  },
        };

        static ChatCommandTable npcbotListSpawnedCommandTable =
        {
            { "",           HandleNpcBotSpawnedCommand,             rbac::RBAC_PERM_COMMAND_GM,            Console::Yes },
            { "free",       HandleNpcBotSpawnedFreeCommand,         rbac::RBAC_PERM_COMMAND_GM,            Console::Yes },
        };
        ...
        {
            { "",           HandleNpcBotDeleteCommand,              rbac::RBAC_PERM_COMMAND_GM,             Console::No  },
            { "id",         HandleNpcBotDeleteByIdCommand,          rbac::RBAC_PERM_COMMAND_GM,             Console::Yes },
            { "free",       HandleNpcBotDeleteFreeCommand,          rbac::RBAC_PERM_COMMAND_GM,             Console::Yes },
        };

        static ChatCommandTable npcbotSendToPointCommandTable =
        {
            { "",           HandleNpcBotSendToPointCommand,         rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
            { "set",        HandleNpcBotSendToPointSetCommand,      rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
        };

        static ChatCommandTable npcbotSendToCommandTable =
        {
            { "",           HandleNpcBotSendToCommand,              rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
            { "last",       HandleNpcBotSendToLastCommand,          rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
            { "point",      npcbotSendToPointCommandTable                                                                           },
        };

        static ChatCommandTable npcbotUseOnBotCommandTable =
        {
            { "spell",      HandleNpcBotUseOnBotSpellCommand,       rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
            { "item",       HandleNpcBotUseOnBotItemCommand,        rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
        };
        ...
        static ChatCommandTable npcbotCommandTable =
        {
            ...
            { "add",        HandleNpcBotAddCommand,                 rbac::RBAC_PERM_COMMAND_GM,                Console::No  },
            { "remove",     HandleNpcBotRemoveCommand,              rbac::RBAC_PERM_COMMAND_GM,             Console::No  },
            { "createnew",  HandleNpcBotCreateNewCommand,           rbac::RBAC_PERM_COMMAND_GM,          Console::Yes },
            { "spawn",      HandleNpcBotSpawnCommand,               rbac::RBAC_PERM_COMMAND_GM,              Console::No  },
            { "move",       HandleNpcBotMoveCommand,                rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
            { "delete",     npcbotDeleteCommandTable                                                                                },
            { "lookup",     HandleNpcBotLookupCommand,              rbac::RBAC_PERM_COMMAND_GM,             Console::Yes },
            { "list",       npcbotListCommandTable                                                                                  },
            { "revive",     HandleNpcBotReviveCommand,              rbac::RBAC_PERM_COMMAND_GM,             Console::No  },
            { "reloadconfig",HandleNpcBotReloadConfigCommand,       rbac::RBAC_PERM_COMMAND_GM,       Console::Yes },
            { "useonbot",   npcbotUseOnBotCommandTable                                                                              },
            { "command",    npcbotCommandCommandTable                                                                               },
            { "info",       HandleNpcBotInfoCommand,                rbac::RBAC_PERM_COMMAND_GM,               Console::Yes },
            { "hide",       HandleNpcBotHideCommand,                rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
            { "unhide",     HandleNpcBotUnhideCommand,              rbac::RBAC_PERM_COMMAND_GM,             Console::No  },
            { "show",       HandleNpcBotUnhideCommand,              rbac::RBAC_PERM_COMMAND_GM,             Console::No  },
            { "recall",     npcbotRecallCommandTable                                                                                },
            { "kill",       HandleNpcBotKillCommand,                rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
            { "suicide",    HandleNpcBotKillCommand,                rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
            { "go",         HandleNpcBotGoCommand,                  rbac::RBAC_PERM_COMMAND_GM,               Console::No  },
            { "gs",         HandleNpcBotGearScoreCommand,           rbac::RBAC_PERM_COMMAND_GM,       Console::No  },
    ...
    static bool HandleNpcBotWPGenerateCommand(ChatHandler* handler, Optional<bool> save)
    {
        ...
        for (AreaPOIEntry const* aProto : sAreaPOIStore)
        {
            ...
            LocalizedString poiName = aProto->Name;
            if (LocalizedString(aProto->Description), false)
            {
              //  poiName += " - ";
                    ...
                    for (uint32 itemId : vec)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                ss << "|Hitem:" << uint32(proto->GetId()) << ":0:0:0:0:0:0:0:0:0|h[";
                            }
                        }
                    }
    ...
    static bool HandleNpcBotDebugNamesCommand(ChatHandler* handler, Optional<std::string_view> name)
    {
        ...
        std::string locName(*name), locTitle = ci->TitleAlt;
    ...
    static bool HandleNpcBotOrderCastCommand(ChatHandler* handler, Optional<std::string> bot_name, Optional<std::string> spell_name, Optional<std::string_view> target_token)
    {
        ...
        /*    if (!canBotUseSpell(bot, base_spell))
            {
                handler->PSendSysMessage("%s's %s is not ready yet!", bot->GetName().c_str(), sSpellMgr->GetSpellInfo(base_spell, DIFFICULTY_NONE)->SpellName[handler->GetSessionDbcLocale()]);
                return true;
            }*/
        }
        else
    ...
    static bool HandleNpcBotCreateNewCommand(ChatHandler* handler, Optional<std::string> name, Optional<uint8> bclass, Optional<uint8> race, Optional<uint8> gender, Optional<uint8> skin, Optional<uint8> face, Optional<uint8> hairstyle, Optional<uint8> haircolor, Optional<uint8> features, Optional<uint8> soundset)
    {
        ...
        if ((*bclass < BOT_CLASS_EX_START && !sObjectMgr->GetPlayerInfo(*race, *bclass)) ||
            ...
            return ret_err_invalid_args_for(handler, "class", DB2Manager::GetClassName(*bclass, handler->GetSessionDbcLocale()));

        if (can_change_appearance && !IsValidVisual(*race, *gender, *skin, *face, *hairstyle, *haircolor, *features))
            return ret_err_invalid_args_for(handler, "race", DB2Manager::GetRaceName(*race, handler->GetSessionDbcLocale()));
    ...
    /*    if (!creInfo->IsNPCBot())
        ...
        Creature* creature = chr->SummonCreature(id, chr->GetPositionX(), chr->GetPositionY(), chr->GetPositionZ(), chr->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN);
        if (!creature)
        {
            delete creature;
  ...
  /*         for (auto const& kv : player->GetSpellMap())
        ...
        SpellInfo const* spellInfo = spellId ? sSpellMgr->AssertSpellInfo(spellId, DIFFICULTY_NONE) : nullptr;
        if (!spellInfo)
        {
    ...
    static bool HandleNpcBotUseOnBotItemCommand(ChatHandler* handler, Optional<Variant<ItemTemplate const*, std::vector<std::string>>> item_name_parts_or_template)
    {
        ...
        if (item_name_parts_or_template->holds_alternative<ItemTemplate const*>())
            item = player->GetItemByEntry(item_name_parts_or_template->get<ItemTemplate const*>()->GetId());
        else
        {
            ...
            for (uint8 i = EQUIPMENT_SLOT_START; i < INVENTORY_SLOT_ITEM_END && !item; ++i)
            {
                ...
                std::string pItemName = pItemTemplate->GetDefaultLocaleName();
     ...
     /*   for (auto const& itemspell : itemtemplate->Spells)
        ...
        if (itemtemplate->GetClass() == ITEM_CLASS_CONSUMABLE && !itemtemplate->HasFlag(ITEM_FLAG_IGNORE_DEFAULT_ARENA_RESTRICTIONS) && player->InArena())
        {
    ...
    static bool HandleNpcBotInfoCommand(ChatHandler* handler, Optional<Variant<uint32, std::string>> player_lg_name)
    {
        ...
        ObjectGuid master_guid = true ? cached_guid :
