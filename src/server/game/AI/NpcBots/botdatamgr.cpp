/*
*  DekkCore Dev Team dapted by thordekk
*
*
*
*/
...
bool BotBankItemCompare::operator()(Item const* item1, Item const* item2) const
{
    ...
    return  proto1->GetName(LOCALE_enUS);
}
        ...
        if (!result)
        {
            ...
            for (uint32 bot_cid : BotDataMgr::GetExistingNPCBotIds())
                if (bot_cid > next_bot_id)
                    next_bot_id = bot_cid;
            CharacterDatabase.DirectPExecute("INSERT INTO worldstates (entry, value, comment) VALUES ({}, {}, '%s')",
...
void BotDataMgr::LoadNpcBotGearStorage()
{
    ...
    do
    {
        ...
        ASSERT(item->LoadFromDB(item_guidlow, player_guid, fields, item_id), "LoadNpcBotGearStorage(): unable to load item %u id %u! Owner: %s", item_guidlow, item_id, player_guid.ToString().c_str()); //%u
...
void BotDataMgr::LoadWanderMap(bool reload)
{
    ...
    //    }
    //}
 //   if (!spawn_node_minclasslvl_exists_all) crash comment 
        //ABORT();
...
void BotDataMgr::CreateWanderingBotsSortedGear()
{
    ...
    auto push_gear_to_classes = [](ItemTemplate const& itt, uint8 slot, uint8 lstep, std::initializer_list<BotClasses> const& cs) {
        for (BotClasses c : cs)
        {
            if (c == BOT_CLASS_SPHYNX &&
                (itt.GetInventoryType() == INVTYPE_FINGER || itt.GetInventoryType() == INVTYPE_TRINKET || itt.GetInventoryType() == INVTYPE_CLOAK || itt.GetInventoryType() == INVTYPE_NECK || itt.GetInventoryType() == INVTYPE_SHIELD))
                continue;
            if (!itt.GetAllowableClass() || itt.GetAllowableClass() >= ((1u << MAX_CLASSES) - 1) || !!(itt.GetAllowableClass() & (1 << (c - 1))))
                _botsWanderCreaturesSortedGear[c][slot][lstep].push_back(itt.GetId());
        }
    };
    ...
    for (auto const& kv : all_item_templates)
    {
        ...
        if (!proto.GetBaseItemLevel() || proto.GetRequiredSpell())
            continue;
        ...
        switch (proto.GetQuality())
        {
            case ITEM_QUALITY_NORMAL:
               // if (std::any_of(proto.Spells.cbegin(), proto.Spells.cend(), [](_Spell const& spell) { return !!spell.SpellId; }))
                    skip = true;
                if (proto.GetBaseRequiredLevel() > 14)
                    skip = true;
                break;
            case ITEM_QUALITY_UNCOMMON:
                if (proto.GetBaseRequiredLevel() > 55)
                    skip = true;
            [[fallthrough]];
            case ITEM_QUALITY_RARE:
                if (proto.GetBaseRequiredLevel() > 65 && proto.GetBaseRequiredLevel() < 70)
                    skip = true;
            [[fallthrough]];
            case ITEM_QUALITY_EPIC:
                if (!(proto.GetBaseRequiredLevel() >= 2 && proto.GetBaseRequiredLevel() <= DEFAULT_MAX_LEVEL))
                    skip = true;
                if (!proto.RandomBonusListTemplateId &&
                    !(proto.GetClass() == ITEM_CLASS_WEAPON && proto.GetSubClass() == ITEM_SUBCLASS_WEAPON_WAND) &&
                    !(proto.GetClass() == ITEM_CLASS_WEAPON && proto.GetSubClass() == ITEM_SUBCLASS_WEAPON_BOW && proto.GetBaseRequiredLevel() < 45))
                    skip = true;
                break;
            default:
                skip = true;
        ...
        if (!!disabled_item_ids.count(proto.GetId()))
        {
            TC_LOG_INFO("server.loading", "Item {} is disabled...", proto.GetId());
            continue;
        }
    ...
    //        continue;
       ...
       /* uint8 reqLstep = (((proto.GetBaseRequiredLevel() == 1) ? 0 : proto.GetBaseRequiredLevel()) + ITEM_SORTING_LEVEL_STEP - 1) / ITEM_SORTING_LEVEL_STEP;
...
Item* BotDataMgr::GenerateWanderingBotItem(uint8 slot, uint8 botclass, uint8 level, std::function<bool(ItemTemplate const*)>&& check)
{
    ...
    if (!itemIdVec->empty())
    {
        ...
        if (!validVec.empty())
        {
            ...
            if (Item* newItem = Item::CreateItem(itemId, 1, ItemContext::NONE))
            {
                if (uint32 randomPropertyId = GenerateItemRandomBonusListId(itemId))
                    newItem->SetItemRandomBonusList(randomPropertyId);
...
bool BotDataMgr::GenerateWanderingBotItemEnchants(Item* item, uint8 slot, uint8 spec)
{
    ...
    if (proto->GetBaseRequiredLevel() < 60)
        return result;

    static const auto is_enchantable = [](ItemTemplate const* p, SpellInfo const* s) {
        SpellEffectInfo const& e = s->GetEffect(EFFECT_0);
        return e.Effect == SPELL_EFFECT_ENCHANT_ITEM && s->EquippedItemClass == int32(p->GetClass()) && s->BaseLevel <= p->GetBaseRequiredLevel() && e.MiscValue > 0 &&
            (s->EquippedItemClass == ITEM_CLASS_WEAPON ? !!(s->EquippedItemSubClassMask & (1 << p->GetSubClass())) : !!(s->EquippedItemInventoryTypeMask & (1 << p->GetInventoryType()))) &&
        ...
        case BOT_SPEC_DRUID_RESTORATION:
            switch (proto->GetClass())
            {
                case ITEM_CLASS_WEAPON:
                    ...
                    {
                        sInfo = sSpellMgr->AssertSpellInfo(spellId, DIFFICULTY_NONE);
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_SOCKETS; ++i)
    {
        ...
        switch (spec)
        {
            ...
            case BOT_SPEC_DRUID_RESTORATION:
                ...
                {
                    GemPropertiesEntry const* gprops = sGemPropertiesStore.LookupEntry(sObjectMgr->GetItemTemplate(gId)->GetGemProperties());
                    if (gprops->Type)
            ...
            for (k = BOT_SLOT_MAINHAND; k != BOT_INVENTORY_SIZE; ++k)
            {
                ...
                if (Item const* botitem = items[k])
                {
                   ...
                   /* for (uint8 i = 0; i != MAX_EQUIPMENT_ITEMS; ++i)
                    {
                        if (einfo->Items[i] == botitem->GetEntry())
                        {
              ...
              //      stmt->setUInt32(++index, botitem->GetGuidValue(ITEM_FIELD_CREATOR).GetCounter());
                  ...
                  //  stmt->setUInt32(++index, botitem->GetUInt32Value(ITEM_FIELD_FLAGS));
                    ...
                    stmt->setInt16 (++index, botitem->GetItemRandomBonusListId());
