    ...
    struct rogue_botAI : public bot_ai
    {
        ...
        /*    if (IsSpellReady(DISMANTLE_1, diff) && !stealthed && !shadowdance &&
         ...
         /*   if (!hasnormalstun)
            ...
            DiminishingLevels const stunDivider = mytar->GetDiminishing(DIMINISHING_INCAPACITATE);
         ...
         //   bool hasHunger = me->GetAuraEffect(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, SPELLFAMILY_ROGUE, 0x0, 0x1000000, 0x0);
            ...
            if (comboPoints > 0)
            {
                ...
                if (GetSpell(KIDNEY_SHOT_1) && !stealthed && stunDivider < DIMINISHING_LEVEL_4 &&
                    Rand() < 80 && !CCed(mytar) &&
                    !IsImmunedToMySpellEffect(mytar, sSpellMgr->GetSpellInfo(KIDNEY_SHOT_1, DIFFICULTY_NONE), EFFECT_0) &&
            ...
            if (stealthed || shadowdance)
            {
                uint32 opener =
                    ...
                    !IsImmunedToMySpellEffect(mytar, sSpellMgr->GetSpellInfo(GARROTE_1, DIFFICULTY_NONE), EFFECT_0) &&
        ...
        void DoNonCombatActions(uint32 diff)
        {
         ...
         /*   if (ohEnchantExpireTimer > 0 && ohEnchantExpireTimer <= diff)
            ...
            SpellInfo const* MhPoisonInfo = mhReady && MhPoison ? sSpellMgr->GetSpellInfo(MhPoison, DIFFICULTY_NONE) : nullptr;
            SpellInfo const* OhPoisonInfo = ohReady && OhPoison ? sSpellMgr->GetSpellInfo(OhPoison, DIFFICULTY_NONE) : nullptr;
