    ...
    struct crypt_lord_botAI : public bot_ai
    {
        ...
        void Attack(uint32 diff)
        {
                ...
                me->isAttackReady() && Rand() < 75)
            {
                ...
                SpellInfo const* impaleSpellInfo = sSpellMgr->AssertSpellInfo(GetSpell(IMPALE_1), DIFFICULTY_NONE);
        ...
        void DamageTaken(Unit* u, uint32& damage, DamageEffectType damageType, SpellInfo const* spellInfo) override
        {
            ...
            {
                SpellInfo const* damageSpellInfo = sSpellMgr->AssertSpellInfo(SPIKED_CARAPACE_DAMAGE, DIFFICULTY_NONE);
                if (u->IsImmunedToDamage(damageSpellInfo))
                {
                    me->SendSpellDamageImmune(u, SPIKED_CARAPACE_DAMAGE, DIFFICULTY_NONE);
                }
                else
                {
