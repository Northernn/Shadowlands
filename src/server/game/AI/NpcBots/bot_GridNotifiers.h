    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
                ...
                (ai->IsInBotParty(u->GetVictim()) || me->GetVictim() == u))
            {
                ...
                if (checksteal)
                {
                    if (u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(30449, DIFFICULTY_NONE), me))
                        return false; //immune to steal
                }
                else if (ai->GetBotClass() == BOT_CLASS_SHAMAN)
                {
                    if (u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(370, DIFFICULTY_NONE), me))
...
class PolyUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_MAGE && !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(118, DIFFICULTY_NONE), me))//Polymorph
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_SHAMAN && !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(51514, DIFFICULTY_NONE), me))//Hex
                return true;
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_WARLOCK &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(5782, DIFFICULTY_NONE), me))//fear rank1
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(1513, DIFFICULTY_NONE), me))//scare beast rank1
                return true;
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_PALADIN && me->GetDistance(u) < 10)//prevent break due to AOE damage
                return false;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_PALADIN &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(20066, DIFFICULTY_NONE), me))//repentance
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(60210, DIFFICULTY_NONE), me))//freezing arrow effect
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(19386, DIFFICULTY_NONE), me))//wyvern sting rank 1
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(1991, DIFFICULTY_NONE), me))//scatter shot
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_ROGUE &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(2094, DIFFICULTY_NONE), me))//blind
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_WARLOCK &&
                u->GetCreatureType() == CREATURE_TYPE_HUMANOID &&
                (u->GetVictim() || u->IsControlledByPlayer()) &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(6358, DIFFICULTY_NONE), me))//seduction
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_DREADLORD &&
                u->GetCreatureType() != CREATURE_TYPE_UNDEAD &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(SPELL_SLEEP, DIFFICULTY_NONE), me))
                return true;
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (m_spell)
            {
                ...
                SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(m_spell, DIFFICULTY_NONE)/*->TryGetSpellInfoOverride(me)*/;
            ...
            if (spellInfo->HasEffect(SPELL_EFFECT_INTERRUPT_CAST) && spellInfo->GetFirstRankSpell()->Id != 853) //hammer of justice
            {
                ...
                for (uint8 i = CURRENT_FIRST_NON_MELEE_SPELL; i != CURRENT_AUTOREPEAT_SPELL; ++i)
                {
                    ...
                    if (!((curSpell->getState() == SPELL_STATE_CASTING ||
                        ...
                        ((i == CURRENT_GENERIC_SPELL/* && curSpell->GetSpellInfo()->InterruptFlags*/) ||
                        (i == CURRENT_CHANNELED_SPELL/* && curSpell->GetSpellInfo()->ChannelInterruptFlags*/))))
                        return false;
                }
            }
...
class TranquilTargetCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            {
                if (u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(19801, DIFFICULTY_NONE), me))
...
class FarTauntUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (!ai->IsInBotParty(u->GetVictim()))
                return false;
            if (ai->GetBotClass() == BOT_CLASS_WARRIOR && u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(355, DIFFICULTY_NONE), me))
                return false; //taunt
            else if (ai->GetBotClass() == BOT_CLASS_PALADIN && u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(62124, DIFFICULTY_NONE), me))
                return false; //HoR
            else if (ai->GetBotClass() == BOT_CLASS_DRUID && u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(6795, DIFFICULTY_NONE), me))
                return false; //Growl
