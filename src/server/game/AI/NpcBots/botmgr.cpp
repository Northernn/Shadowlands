...
void BotMgr::LoadConfig(bool reload)
{
    ...
    uint32 total_pct = 0;
    for (decltype(toks2)::size_type i = 0; i != toks2.size() && i < BracketsCount; ++i)
    {
...
void BotMgr::CleanupsBeforeBotDelete(Creature* bot)
{
    ...
    bot->RemoveUnitFlag(UNIT_FLAG_PVP_ENABLING);
...
void BotMgr::BuildBotPartyMemberStatsPacket(ObjectGuid bot_guid, WorldPacket* data)
{
    ...
    Creature const* pet = bot->GetBotAI()->GetBotsPet();
...
{
    return 0; //_dpstracker->GetDPSTaken(u->GetGUID().GetRawValue());
}
...
void BotMgr::OnBotSpellInterrupt(Unit const* caster, CurrentSpellTypes spellType)
{
    if (spellType == CURRENT_AUTOREPEAT_SPELL)
    {
        ...
        cancelAutoRepeat.Guid = caster->GetGUID();
