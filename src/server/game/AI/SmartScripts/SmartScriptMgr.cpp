        ...
        if (temp.entryOrGuid >= 0)
        {
            switch (source_type)
            {
                ...
                case SMART_SCRIPT_TYPE_QUEST:
                {
                    ...
                    {
                        TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Quest entry (%u) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                        continue;
                    }
                    break;
                }
                ...
                case SMART_SCRIPT_TYPE_AREATRIGGER_ENTITY:
                {
                    ...
                    {
                        TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: AreaTrigger entry (%u IsServerSide false) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                        continue;
                    }
                    break;
                }
                case SMART_SCRIPT_TYPE_AREATRIGGER_ENTITY_SERVERSIDE:
                {
                    ...
                    {
                        TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: AreaTrigger entry (%u IsServerSide true) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                        continue;
                    }
                    break;
                }
...
bool SmartAIMgr::IsTargetValid(SmartScriptHolder const& e)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_INVOKER_PARTY:
            ...
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry " SI64FMTD " SourceType %u Event %u Action %u has invoker target, but event does not provide any invoker!", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
                return false;
            }
            break;
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_MOUNT_TO_ENTRY_OR_MODEL:
            if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry " SI64FMTD " SourceType %u Event %u Action %u uses non-existent Creature entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.morphOrMount.creature);
                    return false;
                }
        ...
        case SMART_ACTION_RESPAWN_BY_SPAWNID:
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Entry " SI64FMTD " SourceType %u Event %u Action %u specifies invalid spawn data (%u,%u)", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.respawnData.spawnType, e.action.respawnData.spawnId);
                return false;
            }
            break;
        }
        ...
        case SMART_ACTION_COVENANT_PREVIEW:
        ...
        case SMART_ACTION_REMOVE_ALL_GAMEOBJECTS:
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Unused action_type(%u), event_type(%u), Entry " SI64FMTD " SourceType %u Event %u, skipped.", e.GetActionType(), e.GetEventType(), e.entryOrGuid, e.GetScriptType(), e.event_id);
            return false;
        default:
