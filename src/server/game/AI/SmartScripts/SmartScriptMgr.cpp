...
{ \
    if (value > 1) \
    { \
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses param {} of type Boolean with value {}, valid values are 0 or 1, skipped.", \
...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    do
    {
        ...
        if (!temp.entryOrGuid)
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: invalid entryorguid (0), skipped loading.");
            continue;
        }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: invalid source_type ({}), skipped loading.", uint32(source_type));
            continue;
        }
        if (temp.entryOrGuid >= 0)
        {
            ...
            case SMART_SCRIPT_TYPE_CREATURE:
            {
                ...
                if (!creatureInfo)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Creature entry ({}) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
            ...
            case SMART_SCRIPT_TYPE_GAMEOBJECT:
            {
                ...
                if (!gameObjectInfo)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GameObject entry ({}) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
            ...
            case SMART_SCRIPT_TYPE_AREATRIGGER:
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: AreaTrigger entry ({}) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
                break;
            }
            ...
            case SMART_SCRIPT_TYPE_QUEST:
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Quest entry ({}) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
                break;
            }
            case SMART_SCRIPT_TYPE_SCENE:
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Scene id ({}) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
                break;
            }
            ...
            case SMART_SCRIPT_TYPE_AREATRIGGER_ENTITY:
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: AreaTrigger entry ({} IsServerSide false) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
                break;
            }
            case SMART_SCRIPT_TYPE_AREATRIGGER_ENTITY_SERVERSIDE:
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: AreaTrigger entry ({} IsServerSide true) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
                break;
            }
            default:
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: not yet implemented source_type {}", (uint32)source_type);
                continue;
            }
        }
        else
        {
            ...
            case SMART_SCRIPT_TYPE_CREATURE:
            {
                ...
                if (!creature)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Creature guid ({}) does not exist, skipped loading.", -temp.entryOrGuid);
                    continue;
                }
                ...
                if (!creatureInfo)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Creature entry ({}) guid ({}) does not exist, skipped loading.", creature->id, -temp.entryOrGuid);
                    continue;
                }
            ...
            case SMART_SCRIPT_TYPE_GAMEOBJECT:
            {
                ...
                if (!gameObject)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GameObject guid ({}) does not exist, skipped loading.", -temp.entryOrGuid);
                    continue;
                }
                ...
                if (!gameObjectInfo)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GameObject entry ({}) guid ({}) does not exist, skipped loading.", gameObject->id, -temp.entryOrGuid);
                    continue;
                }
            ...
            default:
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GUID-specific scripting not yet implemented for source_type {}", (uint32)source_type);
                continue;
            }
        }
        ...
        case SMART_EVENT_TARGET_BUFFED:
            if (temp.event.minMaxRepeat.repeatMin == 0 && temp.event.minMaxRepeat.repeatMax == 0 && !(temp.event.event_flags & SMART_EVENT_FLAG_NOT_REPEATABLE) && temp.source_type != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST)
            {
                ...
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Entry {} SourceType {}, Event {}, Missing Repeat flag.",
        ...
        for (std::pair<int64 const, SmartAIEventList>& eventlistpair : eventmap)
        {
            for (SmartScriptHolder const& e : eventlistpair.second)
            {
                if (e.link)
                {
                    ...
                    {
                        TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Entry {} SourceType {}, Event {}, Link Event {} not found or invalid.",
                ...
                if (e.GetEventType() == SMART_EVENT_LINK)
                {
                    ...
                    {
                        TC_LOG_DEBUG("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Entry {} SourceType {}, Event {}, Link Source Event not found or invalid. Event will never trigger.",
...
bool SmartAIMgr::IsTargetValid(SmartScriptHolder const& e)
{
    if (std::abs(e.target.o) > 2 * float(M_PI))
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has abs(`target.o` = {}) > 2*PI (orientation is expressed in radians)",
    ...
    case SMART_TARGET_CREATURE_RANGE:
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Creature entry {} as target_param1, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.target.unitDistance.creature);
            return false;
        }
        break;
    }
    ...
    case SMART_TARGET_CREATURE_GUID:
    {
        ...
        if (!data)
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} using invalid creature guid {} as target_param1, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), guid);
            return false;
        }
        else if (e.target.unitGUID.entry && e.target.unitGUID.entry != data->id)
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} using invalid creature entry {} (expected {}) for guid {} as target_param1, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.target.unitGUID.entry, data->id, guid);
            return false;
        }
        break;
    }
    case SMART_TARGET_GAMEOBJECT_GUID:
    {
        ...
        if (!data)
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} using invalid gameobject guid {} as target_param1, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), guid);
            return false;
        }
        else if (e.target.goGUID.entry && e.target.goGUID.entry != data->id)
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} using invalid gameobject entry {} (expected {}) for guid {} as target_param1, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.target.goGUID.entry, data->id, guid);
            return false;
        }
        break;
    }
    ...
    case SMART_TARGET_INVOKER_PARTY:
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invoker target, but event does not provide any invoker!", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
            return false;
        }
        break;
    ...
    case SMART_TARGET_CLOSEST_UNSPAWNED_GAMEOBJECT:
        break;
    default:
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Not handled target_type({}), Entry {} SourceType {} Event {} Action {}, skipped.", e.GetTargetType(), e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
        return false;
    }
...
bool SmartAIMgr::IsMinMaxValid(SmartScriptHolder const& e, uint32 min, uint32 max)
{
    if (max < min)
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses min/max params wrong ({}/{}), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), min, max);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::NotNULL(SmartScriptHolder const& e, uint32 data)
{
    if (!data)
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} Parameter can not be NULL, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsCreatureValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Creature entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsQuestValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Quest entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsGameObjectValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent GameObject entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsSpellValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Spell entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsItemValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Item entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsTextEmoteValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Text Emote entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsEmoteValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Emote entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsAreaTriggerValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent AreaTrigger entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsSoundValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Sound entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsAnimKitValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent AnimKit entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsSpellVisualKitValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent SpellVisualKit entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has invalid event type ({}), skipped.", e.entryOrGuid, e.event_id, e.GetEventType());
        return false;
    }
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: EntryOrGuid {}, event type {} can not be used for Script type {}", e.entryOrGuid, e.GetEventType(), e.GetScriptType());
        return false;
    }

    if (e.action.type <= 0 || e.action.type >= SMART_ACTION_END)
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has invalid action type ({}), skipped.", e.entryOrGuid, e.event_id, e.GetActionType());
        return false;
    }

    if (e.event.event_phase_mask > SMART_EVENT_PHASE_ALL)
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has invalid phase mask ({}), skipped.", e.entryOrGuid, e.event_id, e.event.event_phase_mask);
        return false;
    }

    if (e.event.event_flags > SMART_EVENT_FLAGS_ALL)
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has invalid event flags ({}), skipped.", e.entryOrGuid, e.event_id, e.event.event_flags);
        return false;
    }

    if (e.link && e.link == e.event_id)
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: EntryOrGuid {} SourceType {}, Event {}, Event is linking self (infinite loop), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id);
        return false;
    }
    ...
    else
    {
        ...
        case SMART_EVENT_SPELLHIT_TARGET:
            if (e.event.spellHit.spell)
            {
                ...
                if (!spellInfo)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Spell entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.spellHit.spell);
                    return false;
                }
                if (e.event.spellHit.school && (e.event.spellHit.school & spellInfo->SchoolMask) != spellInfo->SchoolMask)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses Spell entry {} with invalid school mask, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.spellHit.spell);
                    return false;
                }
            }
        ...
        case SMART_EVENT_DISTANCE_CREATURE:
            ...
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using invalid creature guid {}, skipped.", e.event.distance.guid);
                return false;
            }

            if (e.event.distance.entry != 0 && !sObjectMgr->GetCreatureTemplate(e.event.distance.entry))
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using invalid creature entry {}, skipped.", e.event.distance.entry);
                return false;
            }
            break;
        case SMART_EVENT_DISTANCE_GAMEOBJECT:
            ...
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT using invalid gameobject guid {}, skipped.", e.event.distance.guid);
                return false;
            }

            if (e.event.distance.entry != 0 && !sObjectMgr->GetGameObjectTemplate(e.event.distance.entry))
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT using invalid gameobject entry {}, skipped.", e.event.distance.entry);
                return false;
            }
            break;
        ...
        case SMART_EVENT_IS_BEHIND_TARGET:
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Unused event_type({}), Entry {} SourceType {} Event {}, skipped.", e.GetEventType(), e.entryOrGuid, e.GetScriptType(), e.event_id);
            return false;
        default:
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Not handled event_type({}), Entry {} SourceType {} Event {} Action {}, skipped.", e.GetEventType(), e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
    }
    ...
    case SMART_ACTION_MOUNT_TO_ENTRY_OR_MODEL:
        if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Creature entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.morphOrMount.creature);
                return false;
            }
    ...
    case SMART_ACTION_ACTIVATE_TAXI:
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid Taxi path ID {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.taxi.id);
            return false;
        }
        break;
    }
    case SMART_ACTION_RANDOM_EMOTE:
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} does not have any non-zero emote",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
        ...
        for (SpellEffectInfo const& spellEffectInfo : sSpellMgr->AssertSpellInfo(e.action.cast.spell, DIFFICULTY_NONE)->GetEffects())
        {
            if (spellEffectInfo.IsEffect(SPELL_EFFECT_KILL_CREDIT) || spellEffectInfo.IsEffect(SPELL_EFFECT_KILL_CREDIT2))
            {
                if (spellEffectInfo.TargetA.GetTarget() == TARGET_UNIT_CASTER)
                    TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} Effect: SPELL_EFFECT_KILL_CREDIT: (SpellId: {} targetA: {} - targetB: {}) has invalid target for this Action",
    ...
    case SMART_ACTION_CROSS_CAST:
    {
        ...
        if (targetType == SMART_TARGET_CREATURE_GUID || targetType == SMART_TARGET_GAMEOBJECT_GUID)
        {
            ...
            if (!data)
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} specifies invalid CasterTargetType guid ({},{})", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), AsUnderlyingType(spawnType), guid);
                return false;
            }
            else if (e.action.crossCast.targetParam2 && e.action.crossCast.targetParam2 != data->id)
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} specifies invalid entry {} (expected {}) for CasterTargetType guid ({},{})", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.crossCast.targetParam2, data->id, AsUnderlyingType(spawnType), guid);
                return false;
            }
        }
        break;
    ...
    case SMART_ACTION_INVOKER_CAST:
        if (e.GetScriptType() != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST && e.GetEventType() != SMART_EVENT_LINK && !EventHasInvoker(e.event.type))
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invoker cast action, but event does not provide any invoker!", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
            return false;
        }
        [[fallthrough]];
    ...
    case SMART_ACTION_CALL_GROUPEVENTHAPPENS:
        if (Quest const* qid = sObjectMgr->GetQuestTemplate(e.action.quest.quest))
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} Flags for Quest entry {} does not include QUEST_FLAGS_COMPLETION_EVENT or QUEST_FLAGS_COMPLETION_AREA_TRIGGER, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.quest.quest);
                return false;
            }
        }
        else
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Quest entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.quest.quest);
            return false;
        }
        break;
    case SMART_ACTION_SET_EVENT_PHASE:
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} attempts to set phase {}. Phase mask cannot be used past phase {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.setEventPhase.phase, SMART_EVENT_PHASE_MAX - 1);
            return false;
        }
        break;
    case SMART_ACTION_INC_EVENT_PHASE:
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} is incrementing phase by 0, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
        else if (e.action.incEventPhase.inc > SMART_EVENT_PHASE_MAX || e.action.incEventPhase.dec > SMART_EVENT_PHASE_MAX)
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} attempts to increment phase by too large value, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
        break;
    ...
    case SMART_ACTION_RANDOM_PHASE:
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} does not have any non-zero phase",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
    ...
    case SMART_ACTION_SUMMON_CREATURE:
    {
        ...
        for (CacheSpellContainer::const_iterator itr = sBounds.first; itr != sBounds.second; ++itr)
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} creature summon: There is a summon spell for creature entry {} (SpellId: {}, effect: {})",
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses incorrect TempSummonType {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.summonCreature.type);
            return false;
        }
    ...
    case SMART_ACTION_CALL_KILLEDMONSTER:
    {
        ...
        for (CacheSpellContainer::const_iterator itr = sBounds.first; itr != sBounds.second; ++itr)
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} Kill Credit: There is a killcredit spell for creatureEntry {} (SpellId: {} effect: {})",
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses incorrect TargetType {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
            return false;
        }
        break;
    }
    ...
    case SMART_ACTION_TELEPORT:
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Map entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.teleport.mapID);
            return false;
        }
        break;
    ...
    case SMART_ACTION_WP_START:
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Creature {} Event {} Action {} uses non-existent WaypointPath id {}, skipped.", e.entryOrGuid, e.event_id, e.GetActionType(), e.action.wpStart.pathID);
            return false;
        }
    ...
    case SMART_ACTION_EQUIP:
    {
        if (e.GetScriptType() == SMART_SCRIPT_TYPE_CREATURE)
        {
            if (int8 equipId = static_cast<int8>(e.action.equip.entry))
            {
                ...
                if (!eInfo)
                {
                    TC_LOG_DEBUG("sql.sql", "SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info id {} for creature {}, skipped.", equipId, e.entryOrGuid);
                    return false;
                }
            }
        }
    ...
    case SMART_ACTION_SET_INGAME_PHASE_ID:
    {
        ...
        if (apply != 0 && apply != 1)
        {
            TC_LOG_DEBUG("sql.sql", "SmartScript: SMART_ACTION_SET_INGAME_PHASE_ID uses invalid apply value {} (Should be 0 or 1) for creature {}, skipped", apply, e.entryOrGuid);
            return false;
        }

        if (!sPhaseStore.LookupEntry(phaseId))
        {
            TC_LOG_DEBUG("sql.sql", "SmartScript: SMART_ACTION_SET_INGAME_PHASE_ID uses invalid phaseid {} for creature {}, skipped", phaseId, e.entryOrGuid);
            return false;
        }
        break;
    }
    ...
    case SMART_ACTION_RESPAWN_BY_SPAWNID:
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Entry {} SourceType {} Event {} Action {} specifies invalid spawn data ({},{})", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.respawnData.spawnType, e.action.respawnData.spawnId);
            return false;
        }
        break;
    }
    ...
    case SMART_ACTION_CREATE_CONVERSATION:
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "SmartAIMgr: SMART_ACTION_CREATE_CONVERSATION Entry {} SourceType {} Event {} Action {} uses invalid entry {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.conversation.id);
            return false;
        }
    ...
    case SMART_ACTION_REMOVE_ALL_GAMEOBJECTS:
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Unused action_type({}), event_type({}), Entry {} SourceType {} Event {}, skipped.", e.GetActionType(), e.GetEventType(), e.entryOrGuid, e.GetScriptType(), e.event_id);
        return false;
    default:
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Not handled action_type({}), event_type({}), Entry {} SourceType {} Event {}, skipped.", e.GetActionType(), e.GetEventType(), e.entryOrGuid, e.GetScriptType(), e.event_id);
        return false;
    }
...
bool SmartAIMgr::IsTextValid(SmartScriptHolder const& e, uint32 id)
{
    ...
    {
        TC_LOG_DEBUG("sql.sql", "SmartAIMgr: Entry {}SourceType {} Event {} Action {} using non-existent Text id {}, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), id);
        return false;
    }
