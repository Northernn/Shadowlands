...
void ScenarioMgr::LoadScenarioPOI()
{
    ...
    do
    {
        ...
        if (!sCriteriaMgr->GetCriteriaTree(criteriaTreeID))
            TC_LOG_DEBUG("sql.sql", "`scenario_poi` CriteriaTreeID (%u) Idx1 (%u) does not correspond to a valid criteria tree", criteriaTreeID, idx1);
        ...
        TC_LOG_DEBUG("server.loading", "Table scenario_poi references unknown scenario poi points for criteria tree id %i POI id %i", criteriaTreeID, blobIndex);
...
//DekkCore
InstanceScenario* ScenarioMgr::CreateInstanceScenarioByID(Map* map, uint32 scenarioID)
{
    auto itr = _scenarioData.find(scenarioID);
    if (itr == _scenarioData.end())
    {
