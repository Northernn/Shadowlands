...
enum MonkSummons
{
    MONK_SUMMON_YULON = 165374,
    MONK_SUMMON_BLACK_OX_STATUE = 61146,
    MONK_SUMMON_JADE_SERPENT_STATUE = 60849,
};
...
public:
    ...
    static Position GetPetSummonPosition(Player* player)
    {
        Position pos = player->GetPositionWithDistInOrientation(3.f, player->GetOrientation() / M_PI);
        pos.m_positionZ = player->GetPositionZ() + 1.f; // for the portal visual we add more z
        return pos;
    };

    /// Unit Functions >
    static void PetSizeCorrection(Guardian* guardian);
    static bool IsYulon(Unit* unit) { return unit->GetEntry() == MONK_SUMMON_YULON; }
    static bool IsBlackOxStatue(Unit* unit) { return unit->GetEntry() == MONK_SUMMON_BLACK_OX_STATUE; }
    static bool IsJadeSerpentStatue(Unit* unit) { return unit->GetEntry() == MONK_SUMMON_JADE_SERPENT_STATUE; }
    static bool IsMonkStatue(Unit* unit) { return IsBlackOxStatue(unit) || IsJadeSerpentStatue(unit); }
    static bool IsMonkSummon(Unit* unit) { return IsYulon(unit) || IsBlackOxStatue(unit) || IsJadeSerpentStatue(unit); }
    ...
    static void PlayWarlockDemonSummoningVisual(Spell* spell);

    /// Session Functions >
    static bool AccountHasAchievement(uint32 accountID, uint32 achievementID);
};
