...
enum SpellScriptHookType
{
    ...
    SPELL_SCRIPT_HOOK_ON_JUMP_CHARGE, // < DekkCore
};
...
public:
    ...
    {
    public:
        union SpellOnTakePowerFnType
        {
            void(SpellScript::* Member)(SpellPowerCost& powerCost);
            void(*Static)(SpellPowerCost& powerCost);
        };
        ...
        explicit OnTakePowerHandler(ScriptFunc handler)
        {
            ...
            if constexpr (!std::is_void_v<ScriptClass>)
            {
                static_assert(std::is_invocable_r_v<void, ScriptFunc, ScriptClass, SpellPowerCost&>,
                    "OnTakePowerHandler signature must be \"void OnTakePower(SpellPowerCost& powerCost)\"");

                _callImpl = { .Member = reinterpret_cast<decltype(SpellOnTakePowerFnType::Member)>(handler) };
                _safeWrapper = [](SpellScript* spellScript, SpellPowerCost& powerCost, SpellOnTakePowerFnType callImpl) -> void
                {
                    return (static_cast<ScriptClass*>(spellScript)->*reinterpret_cast<ScriptFunc>(callImpl.Member))(powerCost);
                };
            }
            else
            {
                static_assert(std::is_invocable_r_v<void, ScriptFunc, SpellPowerCost&>,
                    "OnTakePowerHandler signature must be \"static void OnTakePower(SpellPowerCost& powerCost)\"");

                _callImpl = { .Static = reinterpret_cast<decltype(SpellOnTakePowerFnType::Static)>(handler) };
                _safeWrapper = [](SpellScript* /*spellScript*/, SpellPowerCost& powerCost, SpellOnTakePowerFnType callImpl) -> void
                {
                    return reinterpret_cast<ScriptFunc>(callImpl.Static)(powerCost);
                };
            }
        }
    ...
    class OnEffectSummonHandler final
    {
    public:
        union SpellOnEffectSummonFnType
        {
            void(SpellScript::* Member)(Creature* creature);
            void(*Static)(Creature* creature);
        };

        using SafeWrapperType = void(*)(SpellScript* spellScript, Creature* creature, SpellOnEffectSummonFnType callImpl);

        template<typename ScriptFunc>
        explicit OnEffectSummonHandler(ScriptFunc handler)
        {
            using ScriptClass = GetScriptClass_t<ScriptFunc>;

            static_assert(sizeof(SpellOnEffectSummonFnType) >= sizeof(ScriptFunc));
            static_assert(alignof(SpellOnEffectSummonFnType) >= alignof(ScriptFunc));

            if constexpr (!std::is_void_v<ScriptClass>)
            {
                static_assert(std::is_invocable_r_v<void, ScriptFunc, ScriptClass, Creature*>,
                    "OnEffectSummonHandler signature must be \"void OnEffectSummon(Creature* creature)\"");

                _callImpl = { .Member = reinterpret_cast<decltype(SpellOnEffectSummonFnType::Member)>(handler) };
                _safeWrapper = [](SpellScript* spellScript, Creature* creature, SpellOnEffectSummonFnType callImpl) -> void
                {
                    return (static_cast<ScriptClass*>(spellScript)->*reinterpret_cast<ScriptFunc>(callImpl.Member))(creature);
                };
            }
            else
            {
                ...
                _callImpl = { .Static = reinterpret_cast<decltype(SpellOnEffectSummonFnType::Static)>(handler) };
                _safeWrapper = [](SpellScript* /*spellScript*/, Creature* creature, SpellOnEffectSummonFnType callImpl) -> void
                {
                    return reinterpret_cast<ScriptFunc>(callImpl.Static)(creature);
                };
            }
        }
        ...
        {
            return _safeWrapper(spellScript, creature, _callImpl);
        }
    private:
        SpellOnEffectSummonFnType _callImpl;
        SafeWrapperType _safeWrapper;
    };
    ...
    {
    public:
        union SpellOnPrepareFnType
        {
            void(SpellScript::* Member)();
            void(*Static)();
        };
        ...
        explicit OnPrepareHandler(ScriptFunc handler)
        {
            ...
            {
                static_assert(std::is_invocable_r_v<void, ScriptFunc, ScriptClass>,
                    "OnPrepareHandler signature must be \"void OnPrepare()\"");

                _callImpl = { .Member = reinterpret_cast<decltype(SpellOnPrepareFnType::Member)>(handler) };
            ...
            else
            {
                ...
                _callImpl = { .Static = reinterpret_cast<decltype(SpellOnPrepareFnType::Static)>(handler) };
                _safeWrapper = [](SpellScript* /*spellScript*/, SpellOnPrepareFnType callImpl) -> void
                {
                    return reinterpret_cast<ScriptFunc>(callImpl.Static)();
                };
    ...
    class OnJumpChargeHandler final
    {
    public:
        union SpellOnJumpChargeFnType
        {
            void(SpellScript::* Member)(Optional<JumpArrivalCastArgs>& arrivalCast);
            void(*Static)(Optional<JumpArrivalCastArgs>& arrivalCast);
        };

        using SafeWrapperType = void(*)(SpellScript* spellScript, Optional<JumpArrivalCastArgs>& arrivalCast, SpellOnJumpChargeFnType callImpl);

        template<typename ScriptFunc>
        explicit OnJumpChargeHandler(ScriptFunc handler)
        {
            using ScriptClass = GetScriptClass_t<ScriptFunc>;

            static_assert(sizeof(SpellOnJumpChargeFnType) >= sizeof(ScriptFunc));
            static_assert(alignof(SpellOnJumpChargeFnType) >= alignof(ScriptFunc));

            if constexpr (!std::is_void_v<ScriptClass>)
            {
                static_assert(std::is_invocable_r_v<void, ScriptFunc, ScriptClass, Optional<JumpArrivalCastArgs>&>,
                    "OnJumpChargeHandler signature must be \"void OnTakePower(Optional<JumpArrivalCastArgs>& arrivalCast)\"");

                _callImpl = { .Member = reinterpret_cast<decltype(SpellOnJumpChargeFnType::Member)>(handler) };
                _safeWrapper = [](SpellScript* spellScript, Optional<JumpArrivalCastArgs>& arrivalCast, SpellOnJumpChargeFnType callImpl) -> void
                    {
                        return (static_cast<ScriptClass*>(spellScript)->*reinterpret_cast<ScriptFunc>(callImpl.Member))(arrivalCast);
                    };
            }
            else
            {
                static_assert(std::is_invocable_r_v<void, ScriptFunc, Optional<JumpArrivalCastArgs>&>,
                    "OnJumpChargeHandler signature must be \"void OnTakePower(Optional<JumpArrivalCastArgs>& arrivalCast)\"");

                _callImpl = { .Static = reinterpret_cast<decltype(SpellOnJumpChargeFnType::Static)>(handler) };
                _safeWrapper = [](SpellScript* /*spellScript*/, Optional<JumpArrivalCastArgs>& arrivalCast, SpellOnJumpChargeFnType callImpl) -> void
                    {
                        return reinterpret_cast<ScriptFunc>(callImpl.Static)(arrivalCast);
                    };
            }
        }

        void Call(SpellScript* spellScript, Optional<JumpArrivalCastArgs>& arrivalCast) const
        {
            return _safeWrapper(spellScript, arrivalCast, _callImpl);
        }
    private:
        SpellOnJumpChargeFnType _callImpl;
        SafeWrapperType _safeWrapper;
    };
    // < DekkCore
...
public:
    ...
    // DekkCore >
        ...
        // example: OnEmpowerEnd += SpellCastFn(class::function);
        HookList<CastHandler> OnEmpowerEnd;
        ...
        HookList<OnEffectSummonHandler> OnEffectSummon;
        #define SpellOnEffectSummonFn(F) OnEffectSummonHandler(&F)
        ...
        // example: OnJumpCharge += SpellOnJumpChargeFn(class::function);
        HookList<OnJumpChargeHandler> OnJumpCharge;
        #define SpellOnJumpChargeFn(F) OnJumpChargeHandler(&F)
...
enum AuraScriptHookType
{
    ...
    AURA_SCRIPT_HOOK_EFFECT_ATTEMPT_APPLY,
...
public:
    ...
    class TC_GAME_API EffectApplyHandler final : public EffectBase
    {
    public:
    ...
    public:
        ...
        template<typename ScriptFunc>
        static AuraUpdateHandler Create(ScriptFunc handler)
        {
            using ScriptClass = GetScriptClass_t<ScriptFunc>;

            static_assert(sizeof(AuraUpdateFnType) >= sizeof(ScriptFunc));
            static_assert(alignof(AuraUpdateFnType) >= alignof(ScriptFunc));
            static_assert(std::is_invocable_v<ScriptFunc, ScriptClass, uint32>
                && std::is_same_v<std::invoke_result_t<ScriptFunc, ScriptClass, uint32>, void>,
                "AuraUpdateHandler signature must be \"void AuraUpdate(uint32 diff)\"");

            return { reinterpret_cast<AuraUpdateFnType>(handler) };
        }

        AuraUpdateHandler(AuraUpdateFnType callImpl);
        AuraUpdateHandler(AuraUpdateHandler const& right) = delete;
        AuraUpdateHandler(AuraUpdateHandler&& right) noexcept;
        AuraUpdateHandler& operator=(AuraUpdateHandler const& right) = delete;
        AuraUpdateHandler& operator=(AuraUpdateHandler&& right) noexcept;
        ~AuraUpdateHandler();
...
public:
    ...
    #define AuraUpdateFn(F) AuraUpdateHandler::Create(&F)
