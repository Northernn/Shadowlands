...
bool AuraScript::_IsDefaultActionPrevented() const
{
    ...
    case AURA_SCRIPT_HOOK_EFFECT_ATTEMPT_APPLY:
...
// DekkCore >
AuraScript::AuraUpdateHandler::AuraUpdateHandler(AuraUpdateFnType callImpl) : _callImpl(callImpl)
{
}

AuraScript::AuraUpdateHandler::AuraUpdateHandler(AuraUpdateHandler&& right) noexcept = default;
AuraScript::AuraUpdateHandler& AuraScript::AuraUpdateHandler::operator=(AuraUpdateHandler&& right) noexcept = default;
AuraScript::AuraUpdateHandler::~AuraUpdateHandler() = default;

void AuraScript::AuraUpdateHandler::Call(AuraScript* auraScript, uint32 diff) const
{
    (auraScript->*_callImpl)(diff);
}
// < DekkCore
