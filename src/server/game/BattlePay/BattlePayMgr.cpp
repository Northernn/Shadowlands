...
void BattlepayManager::ProcessDelivery(Purchase * purchase)
{
    ...
    for (auto productId : productInfo.ProductIds)
    {
        ...
        case Battlepay::Item_: // 0
            ...
            if (item && player)
                if (player->GetFreeInventorySpace() > product->Unk1)
                    player->AddItemWithToast(product->Flags, product->Unk1, 0);
                else
                    player->SendABunchOfItemsInMail({product->Flags}, "Ingame Shop item delivery");
            else
            ...
            for (BattlePayData::ProductItem _item : *sBattlePayDataStore->GetItemsOfProduct(product->ProductId))
            {
                if (sObjectMgr->GetItemTemplate(_item.ItemID))
                    if (player->GetFreeInventorySpace() > _item.Quantity)
                        player->AddItemWithToast(_item.ItemID, _item.Quantity, 0);
                    else
        ...
        case Battlepay::WoWToken: // 4
            if (item && player)
                if (player->GetFreeInventorySpace() > product->Unk1)
                    ...
                    player->SendABunchOfItemsInMail({ product->Flags }, "Ingame Shop - WoW Token Delivery");
            else
                _session->SendStartPurchaseResponse(_session, *GetPurchase(), Battlepay::Error::PurchaseDenied);
            break;
        ...
        break;
                ...
                if (player)
                    player->ModifyCurrency(product->Flags, product->Unk1, 0); // implement currencyID in DB
                break;
/*
...
void BattlepayManager::SendProductList()
{
    ...
    /*
    // debug
    TC_LOG_INFO("server.BattlePay", "SendProductList with %u ProductInfos, %u Products, %u Shops. CurrencyID: %u.", response.ProductInfos.size(), response.Products.size(), response.Shops.size(), response.CurrencyID);
    for (int i = 0; i != response.ProductInfos.size(); i++)
    {
        TC_LOG_INFO("server.BattlePay", "(%u) ProductInfo: ProductId [%zu], First SubProductId [%zu], CurrentPriceFixedPoint [%lu]", i, response.ProductInfos[i].ProductId, response.ProductInfos[i].ProductIds[0], response.ProductInfos[i].CurrentPriceFixedPoint);
        TC_LOG_INFO("server.BattlePay", "(%u) Products: ProductId [%zu], UnkString [%s]", i, response.Products[i].ProductId, response.Products[i].UnkString.c_str());
        TC_LOG_INFO("server.BattlePay", "(%u) Shops: ProductId [%zu]", i, response.Shops[i].ProductID);
    }
    */
...
std::tuple<bool, WorldPackets::BattlePay::DisplayInfo> BattlepayManager::WriteDisplayInfo(uint32 displayInfoEntry, uint32 productId /*= 0*/)
{
...
void BattlepayManager::SendBattlePayBattlePetDelivered(ObjectGuid petguid, uint32 creatureID) const
{
    ...
    _session->SendPacket(response.Write());
    TC_LOG_ERROR("", "Send BattlePayBattlePetDelivered guid: %lu && creatureID: %u", petguid.GetCounter(), creatureID);
}
