...
struct TC_GAME_API CreatureTemplate
{
    ...
    {
        return creatureDifficulty && (creatureDifficulty->TypeFlags & CREATURE_TYPE_FLAG_TAMEABLE_EXOTIC) != 0;
    }

    bool IsTameable(bool canTameExotic, CreatureDifficulty const* creatureDifficulty) const
    {
        if (type != CREATURE_TYPE_BEAST || family == CREATURE_FAMILY_NONE || (creatureDifficulty && (creatureDifficulty->TypeFlags & CREATURE_TYPE_FLAG_TAMEABLE) == 0))
            return false;
...
//npcbot
    ...
    {
        return (CreatureTemplate::flags_extra & CREATURE_FLAG_EXTRA_NPCBOT) == CREATURE_FLAG_EXTRA_NPCBOT;
    }
