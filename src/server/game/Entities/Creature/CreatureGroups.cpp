...
void FormationMgr::LoadCreatureFormations()
{
    ...
    do
    {
        ...
        // check data correctness
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "creature_formations table leader guid " UI64FMTD " incorrect (not exist)", member.LeaderSpawnId);
                continue;
            }

            if (!sObjectMgr->GetCreatureData(memberSpawnId))
            {
                TC_LOG_DEBUG("sql.sql", "creature_formations table member guid " UI64FMTD " incorrect (not exist)", memberSpawnId);
                continue;
            }
    ...
    for (ObjectGuid::LowType leaderSpawnId : leaderSpawnIds)
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "creature_formation contains leader spawn " UI64FMTD " which is not included on its formation, removing", leaderSpawnId);
...
FormationInfo* FormationMgr::CreateCustomFormation(Creature* c)
{
   ...
   // group_member->leaderGUID = c->GetGUIDLow();
    ...
    group_member->FollowDist = 1.0f;
    group_member->FollowAngle = 45 * float(M_PI / 180);
