...
Unit::Unit(bool isWorldObject) :
    ...
    _spellHistory(std::make_unique<SpellHistory>(this)), _scheduler(this)
{
...
void Unit::Update(uint32 p_time)
{
    ...
    UpdateDelayedEventOperations(p_time);
...
void Unit::CalculateMeleeDamage(Unit* victim, CalcDamageInfo* damageInfo, WeaponAttackType attackType /*= BASE_ATTACK*/)
{
    ...
    //if (GetTypeId() == TYPEID_UNIT && ToCreature()->IsNPCBotOrPet())
    //{
    //    damageInfo->Damages[i].Damage = damage; TODO THORDEKK 
...
void Unit::RemoveAppliedAuras(std::function<bool(AuraApplication const*)> const& check, AuraRemoveMode removeMode /*= AURA_REMOVE_BY_DEFAULT*/)
{
    ...
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if (m_appliedAuras.size() != size)
        {
            size = m_appliedAuras.size();
...
void Unit::RemoveOwnedAuras(std::function<bool(Aura const*)> const& check, AuraRemoveMode removeMode /*= AURA_REMOVE_BY_DEFAULT*/)
{
    ...
    for (AuraMap::iterator iter = m_ownedAuras.begin(); iter != m_ownedAuras.end();)
    {
        ...
        if (m_ownedAuras.size() != size)
        {
            size = m_ownedAuras.size();
...
void Unit::RemoveAllAuras()
{
    ...
    for (int counter = 0; !m_appliedAuras.empty() || !m_ownedAuras.empty(); counter++)
    {
        ...
        for (aurAppIter = m_appliedAuras.begin(); aurAppIter != m_appliedAuras.end();)
        {
            ...
            aurAppIter = m_appliedAuras.begin();
        }
    ...
    default:
    {
        ...
        else
        {
            ...
            else if (Pet const* pet = ToPet())
            {
                ...
                else if (pet->IsPetGhoul() || pet->IsPetAbomination() || pet->IsWarlockPet()) // DK pets & Warlock pets have energy // < Fluxurion
                    displayPower = POWER_ENERGY;
            }
        }
        break;
...
void Unit::SetMinion(Minion* minion, bool apply)
{
    ...
    if (apply)
    {
        ...
        if (minion->m_Properties && SummonTitle(minion->m_Properties->Title) == SummonTitle::Companion)
        {
            ...
            if (Player* thisPlayer = ToPlayer())
            {
                ObjectGuid SummonedGUID = thisPlayer->GetSummonedBattlePetGUID();
                minion->SetBattlePetCompanionGUID(SummonedGUID);
            }
        }
        ...
        // Fluxurion >
        int8 minionCount = minion->GetOwner()->m_Controlled.size();
        switch ((minionCount - 1) % 5)
        {
        case 0:
            minion->m_followAngle = static_cast<float>(M_PI_4); // left side
            break;
        case 1:
            minion->m_followAngle = -static_cast<float>(M_PI_4); // right side
            break;
        case 2:
            minion->m_followAngle = static_cast<float>(M_PI); // back center
            break;
        case 3:
            minion->m_followAngle = static_cast<float>(M_PI_2); // back left
            break;
        case 4:
            minion->m_followAngle = -static_cast<float>(M_PI_2); // back right
            break;
        }

        if (minionCount > 5)
        {
            minion->m_followDist = PET_FOLLOW_DIST * 2;

            if (Player* thisPlayer = ToPlayer())
                minion->GetMotionMaster()->MoveFollow(thisPlayer, minion->GetFollowDist(), minion->GetFollowAngle());
        }
        // < Fluxurion
...
Unit* Unit::GetNextRandomRaidMemberOrPet(float radius)
{
    ...
    nearMembers.reserve(static_cast<std::vector<Unit*, std::allocator<Unit*>>::size_type>(group->GetMembersCount()) * 2);
...
int32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, int32 pdamage, DamageEffectType damagetype, SpellEffectInfo const* spellEffectInfo /*= nullptr*/) const
{
...
float Unit::SpellCritChanceTaken(Unit const* caster, Spell* spell, AuraEffect const* aurEff, SpellSchoolMask /*schoolMask*/, float doneChance, WeaponAttackType attackType /*= BASE_ATTACK*/) const
{
  ...
  //  if (caster && caster->GetTypeId() == TYPEID_UNIT && caster->ToCreature()->IsNPCBot())
      //  caster->ToCreature()->ApplyBotCritMultiplierAll(this, crit_chance, spellInfo, attackType);
    //end npcbot
    ...
    if (Player* player = ToPlayer())
    {
        ...
        if (VehicleId)
        {
            if (CreateVehicleKit(VehicleId, creatureEntry))
            {
                ...
                // DekkCore >
                    // Grand Expedition Yak & Mighty Caravan Brutosaur needs some delay for the passengers
                    if (creatureEntry == 62809 || creatureEntry == 144742) 
                        player->GetScheduler().Schedule(Milliseconds(300), [this](TaskContext /*context*/)
                            {
...
void Unit::CancelAllShapeshifts()
{
    std::list<AuraEffect*> const& shapeshiftAuras = GetAuraEffectsByType(SPELL_AURA_MOD_SHAPESHIFT);
    ...
    for (Aura* aura : removableShapeshifts)
        RemoveOwnedAura(aura, AURA_REMOVE_BY_CANCEL);
}
...
void Unit::SetPower(Powers power, int32 val, bool withPowerUpdate /*= true*/)
{
    ...
    if (UnitAI* ai = GetAI())
        if (oldPower != val)
            ai->OnPowerChange(power, oldPower, val);
...
void Unit::NearTeleportTo(Position const& pos, bool casting /*= false*/, bool forceNear /*= false*/)
{
    DisableSpline();
    if (GetTypeId() == TYPEID_PLAYER && !forceNear)
    {
...
void Unit::SetFacingToObject(WorldObject const* object, bool force)
{
    // FIXME PUTO
    if (!object) {
        TC_LOG_ERROR("GetAbsoluteAngle(object)", "object null PUTO");
        return;
    }
    // do not face when already moving
    if (!force && (!IsStopped() || !movespline->Finalized()))
...
float Unit::GetCollisionHeight() const
{
    ...
    float const modelHeight = modelData ? modelData->CollisionHeight * modelData->ModelScale : 1.f;
    ...
    if (IsMounted())
    {
        if (CreatureDisplayInfoEntry const* mountDisplayInfo = sCreatureDisplayInfoStore.LookupEntry(GetMountDisplayId()))
        {
            ...
            {
                float const collisionHeight = scaleMod * ((mountModelData->MountHeight * mountDisplayInfo->CreatureModelScale) + (modelHeight * displayInfo->CreatureModelScale * 0.5f));
                return collisionHeight == 0.0f ? DEFAULT_COLLISION_HEIGHT : collisionHeight;
            }
        }
    }

    float const collisionHeight = scaleMod * modelHeight * displayInfo->CreatureModelScale;
    return collisionHeight == 0.0f ? DEFAULT_COLLISION_HEIGHT : collisionHeight;
}
...
void Unit::UpdateDelayedEventOperations(uint32 const diff)
{
...
void Unit::CancelPendingCastRequest()
{
    ...
    if (IsPlayer())
    {
        ...
        data << ObjectGuid(_pendingSpellCastRequest->CastRequest.CastID);
