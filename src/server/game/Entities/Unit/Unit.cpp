...
void Unit::RemoveGameObjectByEntry(uint32 entry, bool del /*= true*/) //dekkcore
{
    if (m_gameObj.empty())
        return;
    GameObjectList::iterator i, next;
      ...
      /*  if (properties && SummonTitle(properties->Title) == SummonTitle::Companion)
        {
            ...
            if (Player const* thisPlayer = ToPlayer()) TODO 
            {
    ...
    if (Player* player = ToPlayer())
    {
        ...
        if (VehicleId)
        {
            if (CreateVehicleKit(VehicleId, creatureEntry))
            {
                ...
                // DekkCore >
                if (creatureEntry == 62809 || creatureEntry == 144742) // If Grand Expedition Yak / Mighty Caravan Brutosaur the passengers will have a seat after some delay
                    player->GetScheduler().Schedule(500ms, [this](TaskContext context)
                        {
...
void Unit::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        ...
        CancelPendingCastRequest(); //DekkCore
...
void Unit::EnterVehicle(Unit* base, int8 seatId /*= -1*/)
{
    ...
    if (base->IsPlayer())
        sScriptMgr->OnPlayerEnterVehicle(base->ToPlayer());
    // < DekkCore
}
...
void Unit::ExitVehicle(Position const* /*exitPosition*/)
{
    ...
    if (this->IsPlayer())
        sScriptMgr->OnPlayerExitVehicle(this->ToPlayer());
    // < DekkCore
}
