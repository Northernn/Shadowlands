...
uint32 LootStore::LoadLootTable()
{
    ...
    do
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table '%s' Entry %d Item %d: GroupId (%u) must be less %u - skipped", GetName(), entry, item, groupid, 1 << 7);
            return 0;
        }
...
void LootStore::ReportUnusedIds(LootIdSet const& lootIdSet) const
{
    ...
    for (LootIdSet::const_iterator itr = lootIdSet.begin(); itr != lootIdSet.end(); ++itr)
        TC_LOG_DEBUG("sql.sql", "Table '%s' Entry %d isn't %s and not referenced from loot, and thus useless.", GetName(), *itr, GetEntryName());
}
...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    if (reference == 0)                                      // item (quest or non-quest) entry, maybe grouped
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table '%s' Entry %d Item %d: equal-chanced grouped entry, but group not defined - skipped", store.GetName(), entry, itemid);
            return false;
        }
...
void LootTemplate::LootGroup::Verify(LootStore const& lootstore, uint32 id, uint8 group_id) const
{
    ...
    if (chance > 101.0f)                                    /// @todo replace with 100% when DBs will be ready
        TC_LOG_DEBUG("sql.sql", "Table '%s' entry %u group %d has total chance > 100%% (%f)", lootstore.GetName(), id, group_id, chance);

    if (chance >= 100.0f && !EqualChanced.empty())
        TC_LOG_DEBUG("sql.sql", "Table '%s' entry %u group %d has items with chance=0%% but group total chance >= 100%% (%f)", lootstore.GetName(), id, group_id, chance);
}
