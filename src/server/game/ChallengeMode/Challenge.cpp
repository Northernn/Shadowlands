...
Challenge::Challenge(Map* map, Player* player, uint32 instanceID, Scenario* scenario) : InstanceScript(instance), _instanceScript(nullptr), _challengeEntry(nullptr), _isKeyDepleted(false), _scenario(scenario)
{
    if (!player)
    {
        _canRun = false;
    ...
    else
    {
        ...
        if (_challengeLevel > 15)
            _affixes[2] = player->m_challengeKeyInfo.Affix4;
...
void Challenge::Complete()
{
  ...
  //  challengeData->ID = ObjectGuid::GetGenerator<HighGuid::Scenario>()->Generate();
...
std::array<uint32, 5> Challenge::GetAffixes() const
{
    return _affixes;
}
...
void Challenge::SendChallengeModeMapStatsUpdate(Player* player)
{
    ...
    WorldPackets::Misc::ChallengeModeMap update;
    update.MapId = _mapID;
    update.BestCompletionMilliseconds = best->RecordTime;
    update.LastCompletionMilliseconds = _challengeTimer;
    update.ChallengeID = _challengeEntry->ID;
    update.MedalDate = best->Date;
