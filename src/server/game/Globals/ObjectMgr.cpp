...
void ObjectMgr::LoadCreatureTemplateResistances()
{
    ...
    do
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "creature_template_resistance has resistance definitions for creature %u but this school %u doesn't exist", creatureID, school);
            continue;
        }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "creature_template_resistance has resistance definitions for creature %u but this creature doesn't exist", creatureID);
            continue;
        }
...
void ObjectMgr::LoadCreatureTemplateModels()
{
    ...
    do
    {
        ...
        if (!cInfo)
        {
            TC_LOG_DEBUG("sql.sql", "Creature template (Entry: %u) does not exist but has a record in `creature_template_model`", creatureId);
            continue;
        }
...
void ObjectMgr::LoadCreatureTemplateAddons()
{
    ...
    do
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Creature template (Entry: %u) does not exist but has a record in `creature_template_addon`", entry);
            continue;
        }
        ...
        for (std::string_view aura : Trinity::Tokenize(fields[10].GetStringView(), ' ', false))
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u) has wrong spell '%s' defined in `auras` field in `creature_template_addon`.", entry, std::string(aura).c_str());
                continue;
            }

            if (spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE))
                TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u) has SPELL_AURA_CONTROL_VEHICLE aura %u defined in `auras` field in `creature_template_addon`.", entry, spellInfo->Id);
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u) has temporary aura (spell %u) in `auras` field in `creature_template_addon`.", entry, spellInfo->Id);
                continue;
            }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u) has invalid emote (%u) defined in `creature_template_addon`.", entry, creatureAddon.emote);
            creatureAddon.emote = 0;
        }

        if (creatureAddon.aiAnimKit && !sAnimKitStore.LookupEntry(creatureAddon.aiAnimKit))
        {
            TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u) has invalid aiAnimKit (%u) defined in `creature_template_addon`.", entry, creatureAddon.aiAnimKit);
            creatureAddon.aiAnimKit = 0;
        }

        if (creatureAddon.movementAnimKit && !sAnimKitStore.LookupEntry(creatureAddon.movementAnimKit))
        {
            TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u) has invalid movementAnimKit (%u) defined in `creature_template_addon`.", entry, creatureAddon.movementAnimKit);
            creatureAddon.movementAnimKit = 0;
        }

        if (creatureAddon.meleeAnimKit && !sAnimKitStore.LookupEntry(creatureAddon.meleeAnimKit))
        {
            TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u) has invalid meleeAnimKit (%u) defined in `creature_template_addon`.", entry, creatureAddon.meleeAnimKit);
...
void ObjectMgr::LoadCreatureScalingData()
{
    ...
    do
    {
        ...
        if (itr == _creatureTemplateStore.end())
        {
            TC_LOG_DEBUG("sql.sql", "Creature template (Entry: %u) does not exist but has a record in `creature_template_scaling`", entry);
            continue;
        }
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    for (uint32 diff = 0; diff < MAX_CREATURE_DIFFICULTIES && ok; ++diff)
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Creature (ID: %u, Expansion: %u) has different `HealthScalingExpansion` in difficulty %u mode (ID: %u, Expansion: %u).",
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u, minlevel: %u) has lower `minlevel` in difficulty %u mode (Entry: %u, minlevel: %u).",
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u, maxlevel: %u) has lower `maxlevel` in difficulty %u mode (Entry: %u, maxlevel: %u).",
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u, family: %u) has different `family` in difficulty %u mode (Entry: %u, family: %u).",
    ...
    {
        TC_LOG_DEBUG("sql.sql", "Creature (Entry: %u) has invalid spell school value (%u) in `dmgschool`.", cInfo->Entry, cInfo->dmgschool);
    ...
    {
        TC_LOG_DEBUG("sql.sql", "Table `creature_template` lists creature (ID: %u) with invalid `HealthScalingExpansion` %i. Ignored and set to 0.", cInfo->Entry, cInfo->HealthScalingExpansion);
    ...
    {
        TC_LOG_DEBUG("sql.sql", "Table `creature_template` lists creature (Entry: %u) with disallowed `flags_extra` %u, removing incorrect flag.", cInfo->Entry, badFlags);
        const_cast<CreatureTemplate*>(cInfo)->flags_extra &= CREATURE_FLAG_EXTRA_DB_ALLOWED;
    }

    if (uint32 disallowedUnitFlags = (cInfo->unit_flags & ~UNIT_FLAG_ALLOWED))
    {
        TC_LOG_DEBUG("sql.sql", "Table `creature_template` lists creature (Entry: %u) with disallowed `unit_flags` %u, removing incorrect flag.", cInfo->Entry, disallowedUnitFlags);
    ...
    {
        TC_LOG_DEBUG("sql.sql", "Table `creature_template` lists creature (Entry: %u) with disallowed `unit_flags2` %u, removing incorrect flag.", cInfo->Entry, disallowedUnitFlags2);
    ...
    {
        TC_LOG_DEBUG("sql.sql", "Table `creature_template` lists creature (Entry: %u) with disallowed `unit_flags2` %u, removing incorrect flag.", cInfo->Entry, disallowedUnitFlags3);
...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    do
    {
        ...
        if (!creData)
        {
            TC_LOG_DEBUG("sql.sql", "Creature (GUID: " UI64FMTD ") does not exist but has a record in `creature_addon`", guid);
            continue;
        }
        ...
        for (std::string_view aura : Trinity::Tokenize(fields[10].GetStringView(), ' ', false))
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_DEBUG("sql.sql", "Creature (GUID: " UI64FMTD ") has wrong spell '%s' defined in `auras` field in `creature_addon`.", guid, std::string(aura).c_str());
                continue;
            }

            if (spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE))
                TC_LOG_DEBUG("sql.sql", "Creature (GUID: " UI64FMTD ") has SPELL_AURA_CONTROL_VEHICLE aura %u defined in `auras` field in `creature_addon`.", guid, spellInfo->Id);
            ...
            if (spellInfo->GetDuration() > 0)
            {
                TC_LOG_DEBUG("sql.sql", "Creature (GUID: " UI64FMTD ") has temporary aura (spell %u) in `auras` field in `creature_addon`.", guid, spellInfo->Id);
                continue;
            }
...
void ObjectMgr::LoadEquipmentTemplates()
{
    ...
    do
    {
        ...
        for (uint8 i = 0; i < MAX_EQUIPMENT_ITEMS; ++i)
        {
            ...
            if (!dbcItem)
            {
                TC_LOG_DEBUG("sql.sql", "Unknown item (ID=%u) in creature_equip_template.ItemID%u for CreatureID = %u and ID=%u, forced to 0.",
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Unknown item appearance for (ID=%u, AppearanceModID=%u) pair in creature_equip_template.ItemID%u creature_equip_template.AppearanceModID%u "
...
void ObjectMgr::LoadCreatureModelInfo()
{
    ...
    do
    {
        ...
        if (!creatureDisplay)
        {
            TC_LOG_DEBUG("sql.sql", "Table `creature_model_info` has a non-existent DisplayID (ID: %u). Skipped.", displayId);
            continue;
        }
    ...
    do
    {
        ...
        switch (linkType)
        {
            case LINKED_RESPAWN_CREATURE_TO_CREATURE:
            {
                ...
                if (!master)
                {
                    TC_LOG_DEBUG("sql.sql", "LinkedRespawn: Creature (linkedGuid) '" UI64FMTD "' not found in creature table", linkedGuidLow);
                    error = true;
                    break;
                }
...
void ObjectMgr::LoadTempSummons()
{
    ...
    do
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `creature_summon_groups` has creature in group [Summoner ID: %u, Summoner Type: %u, Group ID: %u] with non existing creature entry %u, skipped.", summonerId, summonerType, group, data.entry);
            continue;
        }
    ...
    std::set<Difficulty> const& mapDifficulties)
{
    ...
    for (std::string_view token : Trinity::Tokenize(difficultyString, ',', false))
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `%s` has %s (GUID: " UI64FMTD ") has unsupported difficulty %u for map (Id: %u).",
...
void ObjectMgr::LoadCreatures()
{
    ...
    do
    {
        ...
        if (!cInfo)
        {
            TC_LOG_DEBUG("sql.sql", "Table `creature` has creature (GUID: " UI64FMTD ") with non existing creature entry %u, skipped.", guid, entry);
            continue;
        }
        ...
        if (data.equipmentId != 0)
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Table `creature` has creature (Entry: %u) with equipment_id %u not found in table `creature_equip_template`, set to no equipment.", data.id, data.equipmentId);
                data.equipmentId = 0;
            }
        }
        ...
        else if (data.movementType == RANDOM_MOTION_TYPE)
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Table `creature` has creature (GUID: " UI64FMTD " Entry: %u) with `MovementType`=1 (random movement) but with `wander_distance`=0, replace by idle movement type (0).", guid, data.id);
                data.movementType = IDLE_MOTION_TYPE;
            }
        }
        else if (data.movementType == IDLE_MOTION_TYPE)
        {
            if (data.wander_distance != 0.0f)
            {
                TC_LOG_DEBUG("sql.sql", "Table `creature` has creature (GUID: " UI64FMTD " Entry: %u) with `MovementType`=0 (idle) have `wander_distance`<>0, set to 0.", guid, data.id);
                data.wander_distance = 0.0f;
            }
        }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with both `phaseid` and `phasegroup` set, `phasegroup` set to 0", guid, data.id);
            data.phaseGroup = 0;
        }
        ...
        if (data.phaseId)
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with `phaseid` %u does not exist, set to 0", guid, data.id, data.phaseId);
                data.phaseId = 0;
            }
        }
        ...
        if (data.phaseGroup)
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with `phasegroup` %u does not exist, set to 0", guid, data.id, data.phaseGroup);
                data.phaseGroup = 0;
            }
        }
        ...
        if (data.terrainSwapMap != -1)
        {
            ...
            if (!terrainSwapEntry)
            {
                TC_LOG_DEBUG("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with `terrainSwapMap` %u does not exist, set to -1", guid, data.id, data.terrainSwapMap);
                data.terrainSwapMap = -1;
            }
            else if (terrainSwapEntry->ParentMapID != int16(data.mapId))
            {
                TC_LOG_DEBUG("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with `terrainSwapMap` %u which cannot be used on spawn map, set to -1", guid, data.id, data.terrainSwapMap);
                data.terrainSwapMap = -1;
            }
        }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `creature` has creature (GUID: " UI64FMTD " Entry: %u) with disallowed `unit_flags` %u, removing incorrect flag.", guid, data.id, disallowedUnitFlags);
            data.unit_flags &= UNIT_FLAG_ALLOWED;
        }

        if (uint32 disallowedUnitFlags2 = (data.unit_flags2 & ~UNIT_FLAG2_ALLOWED))
        {
            TC_LOG_DEBUG("sql.sql", "Table `creature` has creature (GUID: " UI64FMTD " Entry: %u) with disallowed `unit_flags2` %u, removing incorrect flag.", guid, data.id, disallowedUnitFlags2);
            data.unit_flags2 &= UNIT_FLAG2_ALLOWED;
        }

        if (uint32 disallowedUnitFlags3 = (data.unit_flags3 & ~UNIT_FLAG3_ALLOWED))
        {
            TC_LOG_DEBUG("sql.sql", "Table `creature` has creature (GUID: " UI64FMTD " Entry: %u) with disallowed `unit_flags2` %u, removing incorrect flag.", guid, data.id, disallowedUnitFlags3);
...
void ObjectMgr::LoadGameObjects()
{
    ...
    do
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `gameobject` has gameobject (GUID: " UI64FMTD " Entry: %u) with `spawntimesecs` (0) value, but the gameobejct is marked as despawnable at action.", guid, data.id);
        }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `gameobject` has gameobject (GUID: " UI64FMTD " Entry: %u) with invalid coordinates, skip", guid, data.id);
            continue;
        }

        if (!data.rotation.isUnit())
        {
            TC_LOG_DEBUG("sql.sql", "Table `gameobject` has gameobject (GUID: " UI64FMTD " Entry: %u) with invalid rotation quaternion (non-unit), defaulting to orientation on Z axis only", guid, data.id);
    ...
    if (result)
    {
        do
        {
            ...
            if (flags & ~SPAWNGROUP_FLAGS_ALL)
            {
                ...
                TC_LOG_DEBUG("sql.sql", "Invalid spawn group flag %u on group ID %u (%s), reduced to valid flag %u.", flags, groupId, group.name.c_str(), uint32(group.flags));
            }
            if (flags & SPAWNGROUP_FLAG_SYSTEM && flags & SPAWNGROUP_FLAG_MANUAL_SPAWN)
            {
                flags &= ~SPAWNGROUP_FLAG_MANUAL_SPAWN;
                TC_LOG_DEBUG("sql.sql", "System spawn group %u (%s) has invalid manual spawn flag. Ignored.", groupId, group.name.c_str());
            }
...
void ObjectMgr::LoadSpawnGroups()
{
    ...
    do
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Spawn data with invalid type %u listed for spawn group %u. Skipped.", uint32(spawnType), groupId);
            continue;
        }
        ...
        if (!data)
        {
            TC_LOG_DEBUG("sql.sql", "Spawn data with ID (%u," UI64FMTD ") not found, but is listed as a member of spawn group %u!", uint32(spawnType), spawnId, groupId);
            continue;
        }
        else if (data->spawnGroupData->groupId)
        {
            TC_LOG_DEBUG("sql.sql", "Spawn with ID (%u," UI64FMTD ") is listed as a member of spawn group %u, but is already a member of spawn group %u. Skipping.", uint32(spawnType), spawnId, groupId, data->spawnGroupData->groupId);
            continue;
        }
        auto it = _spawnGroupDataStore.find(groupId);
        if (it == _spawnGroupDataStore.end())
        {
            TC_LOG_DEBUG("sql.sql", "Spawn group %u assigned to spawn ID (%u," UI64FMTD "), but group is found!", groupId, uint32(spawnType), spawnId);
            continue;
        }
        else
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Spawn group %u has map ID %u, but spawn (%u," UI64FMTD ") has map id %u - spawn NOT added to group!", groupId, groupTemplate.mapId, uint32(spawnType), spawnId, data->mapId);
                continue;
            }
...
void ObjectMgr::LoadInstanceSpawnGroups()
{
    ...
    do
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Invalid spawn group %u specified for instance %u. Skipped.", spawnGroupId, fields[0].GetUInt16());
            continue;
        }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Instance spawn group %u specified for instance %u has spawns on a different map %u. Skipped.",
        ...
        if (states & ~ALL_STATES)
        {
            ...
            TC_LOG_DEBUG("sql.sql", "Instance spawn group (%u,%u) had invalid boss state mask %u - truncated to %u.", instanceMapId, spawnGroupId, states, info.BossStates);
        }
        else
        ...
        if (flags & ~InstanceSpawnGroupInfo::FLAG_ALL)
        {
            ...
            TC_LOG_DEBUG("sql.sql", "Instance spawn group (%u,%u) had invalid flags %u - truncated to %u.", instanceMapId, spawnGroupId, flags, info.Flags);
        }
        else
            info.Flags = flags;
...
void ObjectMgr::LoadVehicleTemplateAccessories()
{
    ...
    do
    {
        ...
        if (!GetCreatureTemplate(entry))
        {
            TC_LOG_DEBUG("sql.sql", "Table `vehicle_template_accessory`: creature template entry %u does not exist.", entry);
            continue;
        }
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `vehicle_template_accessory`: creature template entry %u has no data in npc_spellclick_spells", entry);
            continue;
        }
...
void ObjectMgr::PlayerCreateInfoAddItemHelper(uint32 race_, uint32 class_, uint32 itemId, int32 count)
{
    ...
    else
    {
        ...
        if (erased == items.end())
        {
            TC_LOG_DEBUG("sql.sql", "Item %u specified to be removed from original create info not found in db2!", itemId);
            return;
        }
...
void ObjectMgr::LoadQuests()
{
    ...
    for (SpellNameEntry const* spellNameEntry : sSpellNameStore)
    {
        ...
        for (SpellEffectInfo const& spellEffectInfo : spellInfo->GetEffects())
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE for quest %u, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT. Quest flags must be fixed, quest modified to enable objective.", spellInfo->Id, quest_id);
    ...
    do
    {
        ...
        for (uint8 i = 0; i < MAX_NPC_TEXT_OPTIONS; i++)
        {
            if (npcText.Data[i].BroadcastTextID)
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "NPCText (ID: %u) has a non-existing BroadcastText (ID: %u, Index: %u)", textID, npcText.Data[i].BroadcastTextID, i);
        ...
        {
            TC_LOG_DEBUG("sql.sql", "NPCText (ID: %u) has a probability sum 0, no text can be selected from it, skipped.", textID);
            continue;
        }
...
void ObjectMgr::LoadQuestAreaTriggers()
{
    ...
    do
    {
        ...
        if (!atEntry)
        {
            TC_LOG_DEBUG("sql.sql", "Area trigger (ID:%u) does not exist in `AreaTrigger.dbc`.", trigger_ID);
            continue;
        }
        ...
        if (!quest)
        {
            TC_LOG_DEBUG("sql.sql", "Table `areatrigger_involvedrelation` has record (id: %u) for not existing quest %u", trigger_ID, quest_ID);
            continue;
        }
...
void ObjectMgr::LoadAreaTriggerScripts()
{
    ...
    do
    {
        ...
        if (!atEntry)
        {
            TC_LOG_DEBUG("sql.sql", "AreaTrigger (ID: %u) does not exist in `AreaTrigger.dbc`.", triggerId);
            continue;
        }
...
void ObjectMgr::LoadWorldSafeLocs()
{
    ...
    if (QueryResult result = WorldDatabase.Query("SELECT ID, MapID, LocX, LocY, LocZ, Facing FROM world_safe_locs"))
    {
        do
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "World location (ID: %u) has a invalid position MapID: %u %s, skipped", id, loc.GetMapId(), loc.ToString().c_str());
                continue;
            }
...
inline void CheckGOLockId(GameObjectTemplate const* goInfo, uint32 dataN, uint32 N)
{
    ...
    TC_LOG_DEBUG("sql.sql", "Gameobject (Entry: %u GoType: %u) have data%d=%u but lock (Id: %u) not found.",
...
inline void CheckGOSpellId(GameObjectTemplate const* goInfo, uint32 dataN, uint32 N)
{
    ...
    TC_LOG_DEBUG("sql.sql", "Gameobject (Entry: %u GoType: %u) have data%d=%u but Spell (Entry %u) not exist.",
            ...
            case GAMEOBJECT_TYPE_SPELL_FOCUS:               //8
            {
                if (got.spellFocus.spellFocusType)
                {
                    if (!sSpellFocusObjectStore.LookupEntry(got.spellFocus.spellFocusType))
                        TC_LOG_DEBUG("sql.sql", "GameObject (Entry: %u GoType: %u) have data0=%u but SpellFocus (Id: %u) not exist.",
            ...
            case GAMEOBJECT_TYPE_GOOBER:                    //10
            {
                ...
                if (got.goober.pageID)                  // pageId
                {
                    if (!GetPageText(got.goober.pageID))
                        TC_LOG_DEBUG("sql.sql", "GameObject (Entry: %u GoType: %u) have data7=%u but PageText (Entry %u) not exist.",
...
void ObjectMgr::LoadQuestPOI()
{
    ...
    do
    {
        ...
        if (!GetQuestTemplate(questID))
            TC_LOG_DEBUG("sql.sql", "`quest_poi` quest id (%u) Idx1 (%u) does not exist in `quest_template`", questID, idx1);
        ...
        TC_LOG_DEBUG("sql.sql", "Table quest_poi references unknown quest points for quest %i POI id %i", questID, blobIndex);
...
void ObjectMgr::LoadNPCSpellClickSpells()
{
    ...
    do
    {
        ...
        if (!spellinfo)
        {
            TC_LOG_DEBUG("sql.sql", "Table npc_spellclick_spells creature: %u references unknown spellid %u. Skipping entry.", npc_entry, spellid);
            continue;
        }
    ...
    for (auto& creatureTemplatePair : _creatureTemplateStore)
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "npc_spellclick_spells: Creature template %u has UNIT_NPC_FLAG_SPELLCLICK but no data in spellclick table! Removing flag", creatureTemplatePair.first);
    ...
    for (auto const& gameObjectTemplatePair : _gameObjectTemplateStore)
    {
        switch (gameObjectTemplatePair.second.type)
        {
            case GAMEOBJECT_TYPE_QUESTGIVER:
                _gameObjectForQuestStore.insert(gameObjectTemplatePair.second.entry);
                ++count;
                break;
            case GAMEOBJECT_TYPE_CHEST:
            {
...
void ObjectMgr::LoadTrainers()
{
    ...
    if (QueryResult trainerSpellsResult = WorldDatabase.Query("SELECT TrainerId, SpellId, MoneyCost, ReqSkillLine, ReqSkillRank, ReqAbility1, ReqAbility2, ReqAbility3, ReqLevel FROM trainer_spell"))
    {
        do
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_DEBUG("sql.sql", "Table `trainer_spell` references non-existing spell (SpellId: %u) for TrainerId %u, ignoring", spell.SpellId, trainerId);
                continue;
            }

            if (spell.ReqSkillLine && !sSkillLineStore.LookupEntry(spell.ReqSkillLine))
            {
                TC_LOG_DEBUG("sql.sql", "Table `trainer_spell` references non-existing skill (ReqSkillLine: %u) for TrainerId %u and SpellId %u, ignoring",
            ...
            for (std::size_t i = 0; i < spell.ReqAbility.size(); ++i)
            {
                ...
                {
                    TC_LOG_DEBUG("sql.sql", "Table `trainer_spell` references non-existing spell (ReqAbility" SZFMTD ": %u) for TrainerId %u and SpellId %u, ignoring",
    ...
    for (auto const& unusedSpells : spellsByTrainer)
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table `trainer_spell` references non-existing trainer (TrainerId: %u) for SpellId %u, ignoring", unusedSpells.first, unusedSpell.SpellId);
        }
    }
...
void ObjectMgr::LoadGossipMenu()
{
    ...
    do
    {
        ...
        {
            TC_LOG_DEBUG("sql.sql", "Table gossip_menu: ID %u is using non-existing TextID %u", gMenu.MenuID, gMenu.TextID);
            continue;
        }
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, VendorItem const& vItem, Player* player, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
    ...
    if (!cInfo)
    {
        if (player)
            ...
            TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` has data for nonexistent creature template (Entry: %u), ignore", vendor_entry);
        return false;
    }
    ...
    if (!((cInfo->npcflag | ORnpcflag) & UNIT_NPC_FLAG_VENDOR))
    {
        if (!skip_vendors || skip_vendors->count(vendor_entry) == 0)
        {
            if (player)
                ...
                TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` has data for creature template (Entry: %u) without vendor flag, ignore", vendor_entry);
        ...
        (vItem.Type == ITEM_VENDOR_TYPE_CURRENCY && !sCurrencyTypesStore.LookupEntry(vItem.item)))
    {
        if (player)
            ...
            TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` for Vendor (Entry: %u) have in item list non-existed item (%u, type %u), ignore", vendor_entry, vItem.item, vItem.Type);
        return false;
    }

    if (vItem.PlayerConditionId && !sPlayerConditionStore.LookupEntry(vItem.PlayerConditionId))
    {
        TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` has Item (Entry: %u) with invalid PlayerConditionId (%u) for vendor (%u), ignore", vItem.item, vItem.PlayerConditionId, vendor_entry);
        return false;
    }
    ...
    if (vItem.ExtendedCost && !sItemExtendedCostStore.LookupEntry(vItem.ExtendedCost))
    {
        if (player)
            ...
            TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` has Item (Entry: %u) with wrong ExtendedCost (%u) for vendor (%u), ignore", vItem.item, vItem.ExtendedCost, vendor_entry);
        return false;
    }
    ...
    if (vItem.Type == ITEM_VENDOR_TYPE_ITEM) // not applicable to currencies
    {
        if (vItem.maxcount > 0 && vItem.incrtime == 0)
        {
            if (player)
                ...
                TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` has `maxcount` (%u) for item %u of vendor (Entry: %u) but `incrtime`=0, ignore", vItem.maxcount, vItem.item, vendor_entry);
            return false;
        }
        else if (vItem.maxcount == 0 && vItem.incrtime > 0)
        {
            if (player)
                ...
                TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` has `maxcount`=0 for item %u of vendor (Entry: %u) but `incrtime`<>0, ignore", vItem.item, vendor_entry);
            return false;
        }
        ...
        for (int32 bonusList : vItem.BonusListIDs)
        {
            ...
            {
                TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` have Item (Entry: %u) with invalid bonus %u for vendor (%u), ignore", vItem.item, bonusList, vendor_entry);
                return false;
            }
        }
    }
    ...
    {
        TC_LOG_DEBUG("sql.sql", "Table `(game_event_)npc_vendor` have Item (Entry: %u, type: %u) with missing maxcount for vendor (%u), ignore", vItem.item, vItem.Type, vendor_entry);
        return false;
    }
...
void ObjectMgr::LoadGameObjectQuestItems()
{
    ...
    do
    {
        ...
        if (!db2Data)
        {
            TC_LOG_DEBUG("sql.sql", "Table `gameobject_questitem` has nonexistent item (ID: %u) in gameobject (entry: %u, idx: %u), skipped", item, entry, idx);
            continue;
        };
...
void ObjectMgr::LoadQuestTasks()
{
    ...
    {
        const Quest quest_template = it.second;

        // Removing this comentary fix the client crash for overflow but create a new crash for acces violation
        /* Do we need this filter? It removes 2100 worldquest of 2500 xD
        CriteriaEntry const* criteria = sCriteriaStore.LookupEntry(quest_template._id);
        if (!criteria || criteria->Type !=  27) // 27 = CriteriaType::CompleteQuest
            continue;
        */
        ...
        if (!quest_template.GetQuestId() || quest_template.GetQuestType() != QUEST_TYPE_TASK)
            continue;
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u QuestTasks (WorldQuests & BonusQuests) in %u ms", uint32(BonusQuestsRects.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadFluxPhases()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u flux phasing in %u ms", uint32(_fluxphaseStore.size()), GetMSTimeDiffToNow(oldMSTime));
}

ObjectMgr::CurrencyLoot::CurrencyLoot(uint32 _entry, uint8 _type, uint32 _CurrencyId, uint32 _CurrencyAmount, uint32 _CurrencyMaxAmount, uint8 _lootmode, float _chance)
{
}
