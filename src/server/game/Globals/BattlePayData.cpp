...
void BattlePayDataStoreMgr::LoadProductAddons()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %lu Battlepay product addons in %u ms", uint64(_productAddons.size()), GetMSTimeDiffToNow(oldMsTime));
}
...
void BattlePayDataStoreMgr::LoadDisplayInfos()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %lu Battlepay display info with %u visual in %u ms.", uint64(_displayInfos.size()), visualCounter, GetMSTimeDiffToNow(oldMsTime));
}
...
void BattlePayDataStoreMgr::LoadProductGroups()
{
    ...
    auto result = WorldDatabase.PQuery("SELECT Entry, GroupId, IconFileDataID, DisplayType, Ordering, Unk, Name, Description FROM battlepay_group");
    if (!result)
        return;
    ...
    do
    {
        ...
        productGroup.Name = fields[6].GetString();
        productGroup.Description = fields[7].GetString();
...
void BattlePayDataStoreMgr::LoadProduct()
{
    ...
    do
    {
        ...
        product.Name = fields[16].GetUInt32(); // unused in packets but useful in other ways
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %lu Battlepay product infos and %lu Battlepay products in %u ms", uint64(_productInfos.size()), uint64(_products.size()), GetMSTimeDiffToNow(oldMsTime));
}
...
bool BattlePayDataStoreMgr::ProductExist(uint32 productID)
{
    ...
    TC_LOG_INFO("server.BattlePay", "ProductExist failed for productID %u", productID);
    return false;
}
...
bool BattlePayDataStoreMgr::DisplayInfoExist(uint32 displayInfoEntry)
{
    ...
    TC_LOG_INFO("server.BattlePay", "DisplayInfoExist failed for displayInfoEntry %u", displayInfoEntry);
    return false;
}
...
std::vector<BattlePayData::Product> const* BattlePayDataStoreMgr::GetProductsOfProductInfo(uint32 productInfoEntry)
{
    ...
    for (auto productInfo : _productInfos)
        if (productInfo.second.Entry == productInfoEntry)
            ...
            {
                //TC_LOG_INFO("server.BattlePay", "GetProductsOfProductInfo: found product [%u] at productInfo [%u]", productid, productInfoEntry);
    ...
    TC_LOG_INFO("server.BattlePay", "GetProductsOfProductInfo failed for productInfoEntry %u", productInfoEntry);
    return nullptr;
}
...
BattlePayData::ProductInfo const* BattlePayDataStoreMgr::GetProductInfoForProduct(uint32 productID)
{
    ...
    if (_productInfos.find(productID) == _productInfos.end())
    {
        ...
        TC_LOG_INFO("server.BattlePay", "GetProductInfoForProduct failed for productID %u", productID);
        return nullptr;
    }
...
std::vector<BattlePayData::ProductItem> const* BattlePayDataStoreMgr::GetItemsOfProduct(uint32 productID)
{
    ...
    TC_LOG_INFO("server.BattlePay", "GetItemsOfProduct failed for productid %u", productID);
    return nullptr;
}
