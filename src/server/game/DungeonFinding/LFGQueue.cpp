...
LfgCompatibility LFGQueue::CheckCompatibility(GuidList check)
{
    ...
    if (!sLFGMgr->IsSoloLFG() && !forceMinPlayers && check.size() == 1 && numPlayers != MAX_GROUP_SIZE) //solo lfg
    {
    ...
    // Enough players?
    if (!forceMinPlayers && !sLFGMgr->IsSoloLFG() && numPlayers != MAX_GROUP_SIZE) //solo  lfg
    {
