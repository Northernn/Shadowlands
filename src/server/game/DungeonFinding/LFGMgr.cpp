...
LFGDungeonData::LFGDungeonData() : id(0), name(), map(0), type(0), expansion(0), group(0), contentTuningId(0),
    difficulty(DIFFICULTY_NONE), seasonal(false), x(0.0f), y(0.0f), z(0.0f), o(0.0f), requiredItemLevel(0), requiredChromieTime(0)
{
}
...
LFGDungeonData::LFGDungeonData(LFGDungeonsEntry const* dbc) : id(dbc->ID), name(dbc->Name[sWorld->GetDefaultDbcLocale()]), map(dbc->MapID),
    ...
    requiredItemLevel(0), requiredChromieTime(0)
{
}
...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT dungeonId, position_x, position_y, position_z, orientation, requiredItemLevel, requiredChromieTime FROM lfg_dungeon_template");
    if (!result)
    {
    ...
    do
    {
        ...
        data.requiredChromieTime = fields[6].GetUInt16();
    ...
    for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
    {
        ...
        if (dungeon.type != LFG_TYPE_RANDOM && dungeon.x == 0.0f && dungeon.y == 0.0f && dungeon.z == 0.0f)
        {
            ...
            if (!at)
            {
                TC_LOG_DEBUG("sql.sql", "Failed to load dungeon %s (Id: %u), cant find areatrigger for map %u", dungeon.name.c_str(), dungeon.id, dungeon.map);
                continue;
            }
...
LfgLockMap LFGMgr::GetLockedDungeons(ObjectGuid guid)
{
    ...
    for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
    {
        ...
        uint32 lockStatus = [&]() -> uint32
        {
            ...
            if (dungeon->requiredChromieTime != player->GetChromieTime())
                return LFG_LOCKSTATUS_NOT_IN_CHROMIETIME;
...
bool LFGMgr::IsSeasonActive(uint32 dungeonId)
{
    switch (dungeonId)
    {
        ...
        case 1971: // Random Timewalking Dungeon (Warlords Of Draenor)
            return IsHolidayActive(HOLIDAY_TIMEWALKING_DUNGEON_EVENT_WOD_DEFAULT);
        default:
            break;
    }
    return false;
