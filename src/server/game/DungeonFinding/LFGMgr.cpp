...
namespace lfg
{
    ...
    LFGDungeonData::LFGDungeonData() : id(0), name(), map(0), type(0), expansion(0), group(0), contentTuningId(0),
        difficulty(DIFFICULTY_NONE), seasonal(false), x(0.0f), y(0.0f), z(0.0f), o(0.0f), requiredItemLevel(0), requiredChromieTime(0), finalDungeonEncounterId(0)
    {
    }
    ...
    void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
    {
        ...
        for (uint32 i = 0; i < sLFGDungeonsStore.GetNumRows(); ++i)
        {
            ...
            {
            case FLUX_LFG_TYPE_INSTANCE: // < Fluxurion
            case FLUX_LFG_TYPE_CATEGORY: // < Fluxurion
        ...
        for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
        {
            ...
            if (dungeon.type != LFG_TYPE_RANDOM && dungeon.x == 0.0f && dungeon.y == 0.0f && dungeon.z == 0.0f)
            {
                ...
                if (!at)
                {
                    TC_LOG_DEBUG("sql.sql", "Failed to load dungeon {} (Id: {}), cant find areatrigger for map {}", dungeon.name, dungeon.id, dungeon.map);
                    continue;
                }
    ...
    */
    void LFGMgr::JoinLfg(Player* player, uint8 roles, LfgDungeonSet& dungeons)
    {
        ...
        if (joinData.result == LFG_JOIN_OK)
        {
            ...
            {
// Fluxurion >
                LFGDungeonsEntry dungeon = Fluxurion::GetDungeonEntry(*it);
                FluxLFGType type = FluxLFGType(dungeon.TypeID);
                FluxLFGSubType subtype = FluxLFGSubType(dungeon.Subtype);

                //if (type == FLUX_LFG_TYPE_PVP && subtype == FLUX_LFG_SUBTYPE_NONE ) /// < 10v10 Rated Battleground >
                if (type == FLUX_LFG_TYPE_INSTANCE && subtype == FLUX_LFG_SUBTYPE_DUNGEON) ///  < Dungeons (Wailing Caverns, The Everbloom, Ruby Life Pools etc..) >
                {
                    if (isRaid)
                        joinData.result = LFG_JOIN_MISMATCHED_SLOTS;
                    isDungeon = true;
                }
                //if (type == FLUX_LFG_TYPE_INSTANCE && subtype == FLUX_LFG_SUBTYPE_RAID ) /// /// < Raid Finder Categories (Fall of Deathwing, Pinnacle of Storms, Caverns of Infusion etc..) >
                //if (type == FLUX_LFG_TYPE_INSTANCE && subtype == FLUX_LFG_SUBTYPE_SCENARIO ) /// < Scenarios (Unga Ingoo, Proving Grounds, Torghast etc..) >
                if (type == FLUX_LFG_TYPE_INSTANCE && subtype == FLUX_LFG_SUBTYPE_TIMEWALKING_RAID) /// <  ( Timewalking Raid: Black Temple, Timewalking Raid: Ulduar, Timewalking Raid: Firelands) >
                {
                    // Only allow 1 raid
                    if (dungeons.size() > 1)
                        joinData.result = LFG_JOIN_MISMATCHED_SLOTS;
                ...
                //if (type == FLUX_LFG_TYPE_INSTANCE && subtype == FLUX_LFG_SUBTYPE_OPENWORLD_PVP ) /// <  ( Ashran ) >
                //if (type == FLUX_LFG_TYPE_INSTANCE && subtype == FLUX_LFG_SUBTYPE_EXPEDITION ) /// < Adventures ( Shado-Pan Showdown, Arathi Basin Comp Stomp, Havenswood, Jorundall) >
                if (type == FLUX_LFG_TYPE_RAID && subtype == FLUX_LFG_SUBTYPE_RAID) /// < ( RAIDS ) >
                {
                    if (isDungeon)
                        joinData.result = LFG_JOIN_MISMATCHED_SLOTS;
                    isRaid = true;
                }
                if (type == FLUX_LFG_TYPE_CATEGORY && subtype == FLUX_LFG_SUBTYPE_DUNGEON) /// < Dungeon Categories ( Random Classic Dungeon, Random Timewalking Dungeon (Burning Crusade), Random Heroic (Dragonflight) etc.. ) >
                {
                    rDungeonId = *dungeons.begin(); // rDungeonId will be the category if we joined to random
                }
                //if (type == FLUX_LFG_TYPE_CATEGORY && subtype == FLUX_LFG_SUBTYPE_SCENARIO) /// < ( Island Expeditions, Random Mists of Pandaria Scenario ) >
                //if (type == FLUX_LFG_TYPE_CATEGORY && subtype == FLUX_LFG_SUBTYPE_EXPEDITION) /// < ( Teeming Island Expeditions ) >
// < Fluxurion
            }
            ...
            if (joinData.result == LFG_JOIN_OK)
            {
                ...
                for (uint32 dung : dungeons)
                    msg << dung << ", ";
                TC_LOG_INFO("server.debug", "LFG_JOIN_OK - rDungeonId {} - dungeons: {}", rDungeonId, msg.str().c_str());
                // < Fluxurion
            }
        ...
        else                                                   // Add player to queue
        {
            ...
            if (!isContinue)
            {
                /* Fluxurion > - This isn't relevant anymore!
                if (rDungeonId)
                {
                    dungeons.clear();
                    dungeons.insert(rDungeonId);
    ...
    */
    void LFGMgr::GetCompatibleDungeons(LfgDungeonSet* dungeons, GuidSet const& players, LfgLockPartyMap* lockMap, std::vector<std::string const*>* playersMissingRequirement, bool isContinue)
    {
        ...
        {
            ObjectGuid guid = (*it);
            LfgLockMap cachedLockMap = Fluxurion::GetLockedDungeons(guid);
...
*/
    ...
    */
    ...
    // Fluxurion > - This is the TrinityCore version (not working). We use Fluxurion version!
    ...
    LfgDungeonSet LFGMgr::GetRandomAndSeasonalDungeons(uint8 level, uint8 expansion, uint32 contentTuningReplacementConditionMask)
    {
        ...
        for (lfg::LFGDungeonContainer::const_iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
        {
            ...
            // Fluxurion >
            if (!(dungeon.type == FLUX_LFG_TYPE_INSTANCE
                || dungeon.type == FLUX_LFG_TYPE_CATEGORY
    ...
    void LFGMgr::SendUpdateStatus(ObjectGuid guid, LfgUpdateData const& updateData, bool Suspended)
    {
        ...
        if (!updateData.dungeons.empty())
            queueId = GetQueueId(guid);

        WorldPackets::LFG::LFGUpdateStatus update;
        if (auto ticket = GetTicket(player->GetGUID()))
    ...
    void LFGMgr::StopAllOtherQueue(ObjectGuid guid, uint32 queueId)
    {
        ...
        for (auto& queue : it->second)
        {
            ...
            SetState(guid, LFG_STATE_WAITE);
            SendLfgUpdatePlayer(guid, LfgUpdateData(LFG_UPDATETYPE_PAUSE, GetSelectedDungeons(guid)));
        }
    }
    ...
    void LFGMgr::SendLfgUpdateQueue(ObjectGuid guid)
    {
        ...
        sLFGMgr->SendLfgUpdatePlayer(guid, sLFGMgr->GetLfgStatus(guid), true);
    }
