...
namespace LuaPlayer
{
   ...
   /* int CanUninviteFromGroup(lua_State* L, Player* player)
    ...
    int GetSpellCooldownDelay(lua_State* L, Player* player)
    {
        ...
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId, Difficulty(difficulty)))
    ...
    int GetXPRestBonus(lua_State* L, Player* player)
    {
        ...
        Eluna::Push(L, restMgr->GetRestBonusFor(RestTypes::REST_TYPE_XP, xp));
        return 1;
    }
    ...
    int GetDifficulty(lua_State* L, Player* player)
    {
        ...
        Eluna::Push(L, (Difficulty)isRaid);
        return 1;
    }
     ...
     */
    int GetPower(lua_State* L, Player* player)
    {
    ...
    {
        RestMgr* restMgr = &player->GetRestMgr();
        Eluna::Push(L, restMgr->GetRestBonus(RestTypes::REST_TYPE_XP));
        return 1;
    }
    ...
    {
        Eluna::Push(L, player->GetChatFlags());
        return 1;
    }
    ...
    {
        Eluna::Push(L, player->m_activePlayerData->LifetimeHonorableKills);
        return 1;
    }
    ...
    int SendTrainerList(lua_State* L, Player* player)
    {
        ...
        uint32 trainerId = Eluna::CHECKVAL<uint32>(L, 3, 0);
