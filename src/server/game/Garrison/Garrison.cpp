...
void Garrison::AddMission(uint32 garrMissionId)
{
    ...
    mission.PacketInfo.OfferDuration = Seconds(missionEntry->OfferDuration);
    mission.PacketInfo.MissionState = GarrisonMission::State::Available;
    ...
    garrisonAddMissionResult.GarrTypeID = GetType();
    ...
    garrisonAddMissionResult.MissionData = mission.PacketInfo;
...
void Garrison::RewardMission(Mission* mission, bool withOvermaxReward)
{
    ...
    if (withOvermaxReward)
        rewardLists = { mission->Rewards, mission->Rewards };
