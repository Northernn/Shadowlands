...
Garrison::Garrison(Player* owner) : _garrisonType(), _owner(owner), _siteLevel(nullptr), _followerActivationsRemainingToday(1), _numMissionsStartedToday(8)
{
...
void Garrison::OnGossipSelect(WorldObject* source)
{
   // if (!source->HasFlag(UNIT_FIELD_NPC_FLAGS2, UNIT_NPC_FLAG2_AI_OBSTACLE | UNIT_NPC_FLAG2_RECRUITER))
    ...
    WorldPackets::Garrison::OpenShipmentNPCFromGossip openShipments;
    openShipments.NpcGUID = source->GetGUID();
    openShipments.CharShipmentContainerID = data->ContainerID;
    _owner->SendDirectMessage(openShipments.Write());
}
...
void Garrison::StartClassHallUpgrade(uint32 tallentID)
{
    ...
    // cost taken from old site, not from new
     ...
     _owner->ModifyCurrency(entry->ResearchCostSource, -entry->ResearchCostSource, false, true);
...
void Garrison::AddTalentToStore(uint32 talentID, uint32 _time, uint32 flags, ObjectDBState db_state/* = DB_STATE_UNCHANGED*/)
{
    auto talentEntry = sGarrTalentStore.LookupEntry(talentID);
    if (!talentEntry)
        return;
