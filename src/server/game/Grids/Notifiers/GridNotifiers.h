...
namespace Trinity
{
    ...
    {
        public:
            AnyUnitInObjectRangeCheck(WorldObject const* obj, float range, bool check3D = true, bool useBoundingRadius = false) :
                i_obj(obj), i_range(range), i_check3D(check3D), i_useBoundingRadius(useBoundingRadius) { }

            bool operator()(Unit* u) const
            {
                if (u->IsAlive() && i_obj->IsWithinDist(u, i_range, i_check3D, true, true, i_useBoundingRadius))
                    return true;
