...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundQueueTypeId queueId, PVPDifficultyEntry const* bracketEntry)
{
    ...
    switch (bgTypeId)
    {
        ...
        case BATTLEGROUND_BRAWL_WS:
            bg = new BattlegroundWS(*(BattlegroundWS*)bg_template);
            break;
        case BATTLEGROUND_AB:
        case BATTLEGROUND_DOM_AB:
        case BATTLEGROUND_BRAWL_ABW:
        case BATTLEGROUND_BRAWL_AB:
        case BATTLEGROUND_BRAWL_AB2:
            bg = new BattlegroundAB(*(BattlegroundAB*)bg_template);
            break;
        case BATTLEGROUND_NA:
        case BATTLEGROUND_NA2:
        case BATTLEGROUND_NA3:
            bg = new BattlegroundNA(*(BattlegroundNA*)bg_template);
            break;
        case BATTLEGROUND_BE:
        case BATTLEGROUND_BEA2:
        case BATTLEGROUND_BEA3:
        case BATTLEGROUND_BE2:
            bg = new BattlegroundBE(*(BattlegroundBE*)bg_template);
            break;
        case BATTLEGROUND_EY:
        case BATTLEGROUND_EY_RATED:
        case BATTLEGROUND_BRAWL_ES:
            bg = new BattlegroundEY(*(BattlegroundEY*)bg_template);
            break;
        case BATTLEGROUND_RL:
        case BATTLEGROUND_RL2:
        ...
        case BATTLEGROUND_DS2:
        case BATTLEGROUND_DS3:
    ...
    //DEkkCore
    bool isWarGame;
    bool IsBrawl; // later need difference brawls and wargames
    bg->SetWargame(isWarGame);
    bg->SetIsBrawl(IsBrawl);
   //DEkkCore
    return bg;
}
...
bool BattlegroundMgr::IsArenaType(BattlegroundTypeId bgTypeId)
{
    return bgTypeId == BATTLEGROUND_AA
        ...
        || bgTypeId == BATTLEGROUND_ANAG
        || bgTypeId == BATTLEGROUND_BE_L
        || bgTypeId == BATTLEGROUND_BRAWL_AA
        || bgTypeId == BATTLEGROUND_TVA2
        || bgTypeId == BATTLEGROUND_BRHA2
        || bgTypeId == BATTLEGROUND_NA2
        || bgTypeId == BATTLEGROUND_BEA2
        || bgTypeId == BATTLEGROUND_BRAWL_AA2
        || bgTypeId == BATTLEGROUND_BRAWL_AASH
        || bgTypeId == BATTLEGROUND_BEA3
        || bgTypeId == BATTLEGROUND_NA3
        || bgTypeId == BATTLEGROUND_TVA3
        || bgTypeId == BATTLEGROUND_BRHA3
        || bgTypeId == BATTLEGROUND_TR
        || bgTypeId == BATTLEGROUND_TOLV_A
        || bgTypeId == BATTLEGROUND_NA_AR_2
        || bgTypeId == BATTLEGROUND_BEDGE_2
        || bgTypeId == BATTLEGROUND_BLACKR_HOLD_2
        || bgTypeId == BATTLEGROUND_ALL_AR_BRAWL
        || bgTypeId == BATTLEGROUND_ENIGMA_ARENA
        || bgTypeId == BATTLEGROUND_ENIGMA_A_BRAWL
        || bgTypeId == BATTLEGROUND_MALDRAXXUS_COL
        || bgTypeId == BATTLEGROUND_MAL_COL_2
        || bgTypeId == BATTLEGROUND_MAL_COL_3
...
{
    _brawlTemplatesContainer[0] = BrawlData(BattlegroundTypeId::BATTLEGROUND_BRAWL_WS, 257, 322, "PvP Brawl: Warsong scramle", "?????? ??????", "- ?????? ??????? ????? ????????? ?????? - ?????????? ????????? ???? ?? ???- ?????????? ?????? ?? ?????????? ??????????", BrawlTypes::Battleground); // 239
    _brawlTemplatesContainer[1] = BrawlData(BattlegroundTypeId::BATTLEGROUND_BRAWL_SPS, 268, 329, "PvP Brawl: Shado-Pan Showdown", "????? 5 ?? 5", " ? ????? 5 ?? 5 ? ?????? ????? - ???? ??????? ?????????, ????? ???????? ?????? ? ???????? ?????????? ??????, ????? ????????? ????????",BrawlTypes::Lfg); // 240

    // temp
    _brawlTemplatesContainer[2] = BrawlData(BattlegroundTypeId::BATTLEGROUND_BRAWL_ABW, 262, 321, "PvP Brawl: Arathi Blizzard", "??????? ????", "- ????????? ??????? - ????????? ?????????? - ?????????? ??????????", BrawlTypes::Battleground); // 240
    _brawlTemplatesContainer[3] = BrawlData(BattlegroundTypeId::BATTLEGROUND_BRAWL_EH, 255, 324, "PvP Brawl: Gravity Lapse", "?????????? ????", "- ?????????? ????????? ????????- ???????? ????? ?????????? ??????? - ??? ????????? ?????? ???????? ? ??????", BrawlTypes::Battleground); // 239
    //

    _brawlTemplatesContainer[4] = BrawlData(BattlegroundTypeId::BATTLEGROUND_BRAWL_AA, 265, 327, "PvP Brawl: Packed House", "??? 15?15", "- ????????? ??????? - 15 ??????? ?? ??????? - ????????? ???????, ??????? ??????????? ??????", BrawlTypes::Arena); // 239
    _brawlTemplatesContainer[5] = BrawlData(BATTLEGROUND_BRAWL_TH, 264, 323, "PvP Brawl: Temple of Hotmogu", "", "", BrawlTypes::Battleground); // 240
    _brawlTemplatesContainer[6] = BrawlData(BattlegroundTypeId::BATTLEGROUND_BRAWL_DS, 267, 348, "Brawl: the magnificent six", "???? ??? 6 ?? 6", "? 6 ??????? ?? ??????? ? ????? ?????? ????", BrawlTypes::Battleground); // 240
    _brawlTemplatesContainer[7] = BrawlData(BattlegroundTypeId::BATTLEGROUND_BRAWL_ABW, 262, 321, "PvP Brawl: Arathi Blizzard", "??????? ????", "- ????????? ??????? - ????????? ?????????? - ?????????? ??????????", BrawlTypes::Battleground); // 240
    _brawlTemplatesContainer[8] = BrawlData(BattlegroundTypeId::BATTLEGROUND_BRAWL_GL, 255, 324, "PvP Brawl: Gravity Lapse", "?????????? ????", "- ?????????? ????????? ????????- ???????? ????? ?????????? ??????? - ??? ????????? ?????? ???????? ? ??????", BrawlTypes::Battleground); // 239
}
