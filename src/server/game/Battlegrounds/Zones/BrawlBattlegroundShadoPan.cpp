...
BrawlBattlegroundShadoPan::BrawlBattlegroundShadoPan(BattlegroundTemplate const* battlegroundTemplate) : Battleground(battlegroundTemplate)
{
...
void BrawlBattlegroundShadoPan::StartingEventOpenDoors()
{
    ...
    UpdateWorldState(8524, 1);
    UpdateWorldState(8529, int32(time(nullptr) + std::chrono::duration_cast<Seconds>(Minutes(15)).count()));
}
...
bool BrawlBattlegroundShadoPan::SetupBattleground()
{
    ...
    WorldSafeLocsEntry const* sg = sObjectMgr->GetWorldSafeLoc(5850);
    if (!sg || !AddSpiritGuide(SP_SPIRIT_MAIN_ALLIANCE, sg->Loc, TEAM_ALLIANCE))
        return false;

    sg = sObjectMgr->GetWorldSafeLoc(5851);
...
void BrawlBattlegroundShadoPan::AddPlayer(Player * player)
{
    ...
    PlayerScores[playerGuid] = new BrawlBattlegroundShadoPanScore(player->GetGUID(), player->GetTeam());

    if (player->GetTeam() == TEAM_ALLIANCE) // gold
    {
...
WorldSafeLocsEntry const * BrawlBattlegroundShadoPan::GetClosestGraveYard(Player * player)
{
    ...
    {
            return sObjectMgr->GetWorldSafeLoc(5850);
    }
    else
    {
            return sObjectMgr->GetWorldSafeLoc(5851);
    }
}
...
void BrawlBattlegroundShadoPan::EventPlayerUsedGO(Player * player, GameObject * go)
{
    ...
    for (auto itr : GetPlayers())
        if (Player* tempPlayer = ObjectAccessor::FindPlayer(GetBgMap(), itr.first))
            if (tempPlayer->GetTeamId() == player->GetTeamId())
...
void BrawlBattlegroundShadoPan::UpdateBossesAndController(uint32 diff)
{
    ...
    for (uint8 i = SP_BOSS_FOR_GOLD; i <= SP_BOSS_FOR_PURPLE; ++i)
        ...
        {
            UpdateWorldState(13198 + i % 2, boss->GetHealthPct(), false);
...
struct npc_bg_shado_pan_boss : ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.ScheduleEvent(EVENT_HEAL, 20s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_BRUTAL_SLASH:
                ...
                events.ScheduleEvent(EVENT_BRUTAL_SLASH, 14s);
                break;
            case EVENT_STORM:
                DoCast(me->GetEntry() == SP_BOSS_FOR_PURPLE_ENTRY ? SPELL_FIRESTORM : SPELL_THUNDERSTORM);
                events.ScheduleEvent(EVENT_STORM, 11s);
                break;
            case EVENT_HEAL:
