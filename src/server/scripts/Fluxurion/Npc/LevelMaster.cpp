    ...
    bool OnGossipSelect(Player* player, uint32 sender, uint32 action) override
    {
        ...
        case 1:
            if (player->GetLevel() <= 50)
            {
                ...
                me->CastSpell(player, 178570); // Force Beam [DNT] - channeling effect on player
...
public:
    ...
    static bool HandleLevelMasterCommand(ChatHandler* handler, const char* /*args*/)
    {
        if (sConfigMgr->GetBoolDefault("LevelMaster.Enable", true))
        {
            ...
            pos.RelocateOffset({0.f, 0.f, 3.f, 0.f});
            Creature* npc = player->SummonCreature(8000001, pos, TEMPSUMMON_TIMED_DESPAWN, 5 * 60s);
        }
