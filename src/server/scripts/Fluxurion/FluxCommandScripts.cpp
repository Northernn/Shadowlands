...
class phasechange_commandscript : public CommandScript
{
public:
    phasechange_commandscript() : CommandScript("phasechange_commandscript") { }

    static bool HandlePhaseChangeCommand(ChatHandler* handler, uint32 phaseId)
    {
        Creature* target = handler->getSelectedCreature();
        ...
        ObjectGuid::LowType spawnId = target->GetSpawnId();
        ...
        if (!phaseId)
        {
            handler->PSendSysMessage("[SetPhase Command]:");
        ...
        else
        {
            ...
            WorldDatabase.PExecute("UPDATE creature SET PhaseId={} WHERE guid={}", phaseId, spawnId);

            target->AddObjectToRemoveList();

            player->CastSpell(player, 301493);

            sObjectMgr->LoadCreatures();

            // need a little delay after loading otherwise it loads back the old data when creating the creature
            player->AddDelayedEvent(3, [player, spawnId, map, handler, cInfo, entry, phaseId]() -> void
                {
                    Creature const* newCreature = Creature::CreateCreatureFromDB(spawnId, map, true, true);
                    if (newCreature)
                    {
                    sObjectMgr->AddCreatureToGrid(sObjectMgr->GetCreatureData(spawnId));

                    player->CastStop();
                    player->SendPlaySpellVisual(player, 121003, 0, 0, 60, false);
                    handler->PSendSysMessage("[SetPhase Command]: Targeted creature %s (ID: %u)[Guid: %s] is now on phase %u.", cInfo->Name.c_str(), entry, std::to_string(spawnId).c_str(), phaseId);
                    }
                });
        }
    ...
    Trinity::ChatCommands::ChatCommandTable GetCommands() const override
    {
        static Trinity::ChatCommands::ChatCommandTable phaseChange_CommandTable =
        {
             { "phasechange",          HandlePhaseChangeCommand,          rbac::RBAC_PERM_COMMAND_GM,          Trinity::ChatCommands::Console::No },
        };

        return phaseChange_CommandTable;
    }

};

class unstackmobs_commandscript : public CommandScript
{
public:
    unstackmobs_commandscript() : CommandScript("unstackmobs_commandscript") { }

    static bool HandleUnstackMobsCommand(ChatHandler* handler)
    {
        Player* player = handler->GetSession()->GetPlayer();

        std::vector<Creature*> nearbyMobs;
        GetCreatureListWithOptionsInGrid(nearbyMobs, player, 20.0f, FindCreatureOptions().SetIgnorePhases(true));
        for (Creature* mob : nearbyMobs)
        {
            if (!mob->IsAlive() || mob->IsInCombat())
                continue;

            mob->GetMotionMaster()->MovePoint(0, mob->GetRandomNearPosition(3.f));
        }
    ...
    Trinity::ChatCommands::ChatCommandTable GetCommands() const override
    {
        static Trinity::ChatCommands::ChatCommandTable unstackMobs_CommandTable =
        {
             { "unstackmobs",          HandleUnstackMobsCommand,          rbac::RBAC_PERM_COMMAND_GM,          Trinity::ChatCommands::Console::No },
        };

        return unstackMobs_CommandTable;
    }
...
void AddSC_FluxCommands()
{
    ...
    new phasechange_commandscript();
    new unstackmobs_commandscript();
}
