...
// 366962 - Ash'adar, Harbinger of Dawn (Mount)
class spell_ashadar_mount : public SpellScriptLoader
{
public:
    spell_ashadar_mount() : SpellScriptLoader("spell_ashadar_mount") { }

    class spell_ashadar_mount_AuraScript : public AuraScript
    {
        void AfterAuraApplied(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            uint32 Ashadar_DisplayIDs[2] = { 104779, 104786 };
            Trinity::Containers::RandomShuffle(Ashadar_DisplayIDs);

            GetTarget()->Mount(Ashadar_DisplayIDs[0]);
        }

        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(spell_ashadar_mount_AuraScript::AfterAuraApplied, EFFECT_0, SPELL_AURA_MOUNTED, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_ashadar_mount_AuraScript();
    }
};
