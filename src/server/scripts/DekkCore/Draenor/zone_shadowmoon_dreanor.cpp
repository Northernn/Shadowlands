...
#include <Texts/CreatureTextMgr.h>
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_FINDING_A_FOOTHOLD)
        {
            ...
            if (TempSummon* waypointVelen = player->SummonCreature(creature->GetEntry(), creature->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0s, 0))
            {
            ...
            {
                if (TempSummon* waypointMaraad = player->SummonCreature(NPC_VELEN_FOLLOWER_MARAAD, normalMaraad->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0s, 0))
                {
            ...
            {
                if (TempSummon* waypointYrel = player->SummonCreature(NPC_VELEN_FOLLOWER_YREL, normalYrel->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0s, 0))
                {
...
public:
    ...
    struct npc_aqualirAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            me->SummonCreature(81542, -870.97f, -1109.27f, 83.38f, 4.824920f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 2s);
            me->SummonCreature(81542, -861.96f, -1108.41f, 83.72f, 4.475420f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 2s);
...
struct areatrigger_aqualir_submerge : AreaTriggerAI
{
    ...
    {
        if (unit->IsPlayer())
            unit->ApplyMovementForce(at->GetGUID(), bottom, 7.0f, MovementForceType::SingleDirectional);
    }
...
// 85645 - Gara
struct npc_gara : public ScriptedAI
{
    npc_gara(Creature* creature) : ScriptedAI(creature) {  }
    ...
    bool OnGossipHello(Player* player) override
    {
        if (player->GetQuestStatus(37423) != QUEST_STATUS_INCOMPLETE && player->GetQuestStatus(37423) != QUEST_STATUS_COMPLETE && player->GetQuestStatus(37423) != QUEST_STATUS_REWARDED)
            AddGossipItemFor(player, GossipOptionNpc::GlyphMaster, "(Beast Mastery) Lean down and scratch the wolf behind its ears.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        if (player->GetQuestStatus(37423) == QUEST_STATUS_REWARDED && player->GetQuestStatus(37424) != QUEST_STATUS_REWARDED && player->HasItemCount(ITEM_SHADOWBERRY, 1))
            AddGossipItemFor(player, GossipOptionNpc::GlyphMaster, "(Beast Mastery) Show Gara the Shadowberries.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);

        player->PlayerTalkClass->SendGossipMenu(85645, me->GetGUID());
    ...
    bool OnGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
    {
        ...
        {
            SendGossipMenuFor(player, TEXT_ID_GARA_GRAVE, me->GetGUID());
            player->CastSpell(player, SPELL_TRACKING_QUEST_1);
        }
        if (action == GOSSIP_ACTION_INFO_DEF + 2)
        {
            SendGossipMenuFor(player, TEXT_ID_GARA_BERRY, me->GetGUID());
...
public:
    ...
    struct go_spirit_effigyAI : public GameObjectAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_SOUL_EFFIGY_01, 0s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SOUL_EFFIGY_01:
                {
                    ...
                    events.ScheduleEvent(EVENT_SOUL_EFFIGY_02, 4s);
                ...
                case EVENT_SOUL_EFFIGY_02:
                {
                    ...
                    events.ScheduleEvent(EVENT_SOUL_EFFIGY_03, 4s);
                ...
                case EVENT_SOUL_EFFIGY_03:
                {
                    ...
                    events.ScheduleEvent(EVENT_SOUL_EFFIGY_04, 8s);
                ...
                case EVENT_SOUL_EFFIGY_04:
                {
                    ...
                    events.ScheduleEvent(EVENT_SOUL_EFFIGY_05, 11s);
                ...
                case EVENT_SOUL_EFFIGY_05:
                {
                    ...
                    events.ScheduleEvent(EVENT_SOUL_EFFIGY_06, 7s);
                ...
                case EVENT_SOUL_EFFIGY_06:
                {
                    ...
                    events.ScheduleEvent(EVENT_SOUL_EFFIGY_07, 2s);
                    if (TempSummon* Xan = me->SummonCreature(NPC_XAN, BurialEventPos[3], TEMPSUMMON_MANUAL_DESPAWN))
                    {
                        xanGuid = Xan->GetGUID();
                        Xan->SetReactState(REACT_PASSIVE);
                        Xan->SetUnitFlag(UnitFlags(UNIT_FLAG_UNINTERACTIBLE | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_PC));
                    }
                ...
                case EVENT_SOUL_EFFIGY_07:
                {
                    ...
                    events.ScheduleEvent(EVENT_SOUL_EFFIGY_08, 6s);
                ...
                case EVENT_SOUL_EFFIGY_08:
                {
                    ...
                    events.ScheduleEvent(EVENT_SOUL_EFFIGY_END, 4s);
...
struct go_shadowmoon_voidblade : public GameObjectAI
{
    go_shadowmoon_voidblade(GameObject* go) : GameObjectAI(go) { }
...
public:
    ...
    class spell_aura_void_realm_AuraScript : public AuraScript
    {
        ...
        void HandlePhasing(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            PhasingHandler::AddPhase(GetCaster(), 8, true);
        ...
        void HandlePhasingRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (!GetCaster()->IsPlayer())
                return;
            PhasingHandler::RemovePhase(GetCaster(), 8, true);
        }
...
public:
    ...
    struct npc_void_garaAI : public ScriptedAI
    {
        npc_void_garaAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            if (owner != nullptr)
                events.ScheduleEvent(EVENT_CHECK_PLAYER, 0s);
            else
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_SUMMON_XAN)
                events.ScheduleEvent(EVENT_GARA_01, 0s);
            else if (action == ACTION_XAN_DEATH)
                events.ScheduleEvent(EVENT_GARA_02, 0s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_GARA_01:
                {
                    if (TempSummon* Xan = me->SummonCreature(NPC_XAN, VoidRealmEventPos[0], TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60s))
                    {
                        Xan->SetFaction(14);
                ...
                case EVENT_GARA_02:
                {
                    ...
                    events.ScheduleEvent(EVENT_GARA_03, 10s);
                }
                break;
                case EVENT_GARA_03:
                {
                    ...
                    me->RemoveAllAuras();
                    me->SetVisible(false);
                    gara_teamable = me->SummonCreature(NPC_GARA_TAMABLE_PET, me->GetPosition(), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 30s);

                    if (gara_teamable)
                        gara_teamable->SetHealth(gara_teamable->GetHealth() * 60000);

                    events.ScheduleEvent(EVENT_GARA_04, 0s);
                }
                break;
                case EVENT_GARA_04:
                {
                    ...
                    if (gara_teamable->IsInCombat())
                        events.ScheduleEvent(EVENT_GARA_05, 2s);
                    else events.ScheduleEvent(EVENT_GARA_04, 3s);
                }
                break;
                case EVENT_GARA_05:
                {
                    ...
                    events.ScheduleEvent(EVENT_GARA_06, 11s);
                }
                break;
                case EVENT_GARA_06:
                {
                    ...
                    events.ScheduleEvent(EVENT_GARA_END, 1s);
                }
                break;
                case EVENT_GARA_END:
                {
    ...
    class spell_garrison_cache_loot_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            if (Unit* caster = GetCaster())
            {
                ...
                Garrison* garr = plr->GetGarrison(GARRISON_TYPE_GARRISON);
                if (!garr)
                    return;
...
void AddSC_shadowmoon_draenor()
{
    ...
    RegisterCreatureAI(npc_gara);
    new spell_use_effigy();
    new go_spirit_effigy();
    RegisterGameObjectAI(go_shadowmoon_voidblade);
