                    ...
                    if (Creature* l_Borka = m_InstanceScript->instance->GetCreature(m_InstanceScript->GetGuidData(GrimrailDepotData::DataBorka)))
                    {
                        if (l_RocketSpark->IsAIEnabled() && l_Borka->IsAIEnabled())
                        {
                            ...
                            case 0:
                            {
                                ...
                                l_RocketSpark->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(l_RocketSpark, 1, m_InstanceScript), l_RocketSpark->m_Events.CalculateTime(7s));
                                break;
                            }
                            case 1:
                            {
                                l_Borka->AI()->Talk(eRocketsparkAndBorkaTalks::TalkBorkaIntro02);
                                l_RocketSpark->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(l_RocketSpark, 2, m_InstanceScript), l_RocketSpark->m_Events.CalculateTime(7s));
                                break;
                            }
                            case 2:
                            {
                                l_RocketSpark->AI()->Talk(eRocketsparkAndBorkaTalks::TalkRocketsparkEvent03);
                                l_RocketSpark->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(l_RocketSpark, 3, m_InstanceScript), l_RocketSpark->m_Events.CalculateTime(7s));
                                break;
                            }
                            case 3:
                            {
                                l_RocketSpark->AI()->Talk(eRocketsparkAndBorkaTalks::TalkRocketsparkEvent04);
                                l_RocketSpark->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(l_RocketSpark, 4, m_InstanceScript), l_RocketSpark->m_Events.CalculateTime(7s));
                                break;
                            }
                            case 4:
                            {
                                l_RocketSpark->AI()->Talk(eRocketsparkAndBorkaTalks::TalkRocketsparkEvent05);
                                l_RocketSpark->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(l_RocketSpark, 5, m_InstanceScript), l_RocketSpark->m_Events.CalculateTime(7s));
                                break;
                            }
                            case 5:
                            {
                                l_RocketSpark->AI()->Talk(eRocketsparkAndBorkaTalks::TalkRocketsparkEvent06);
                                l_RocketSpark->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(l_RocketSpark, 6, m_InstanceScript), l_RocketSpark->m_Events.CalculateTime(7s));
                                break;
                            }
                            case 6:
                            {
                                l_RocketSpark->AI()->Talk(eRocketsparkAndBorkaTalks::TalkRocketsparkEvent07);
                                l_RocketSpark->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(l_RocketSpark, 7, m_InstanceScript), l_RocketSpark->m_Events.CalculateTime(7s));
                                break;
                            }
                            case 7:
                            {
                                l_RocketSpark->AI()->Talk(eRocketsparkAndBorkaTalks::TalkRocketsparkEvent08);
                                l_RocketSpark->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(l_RocketSpark, 8, m_InstanceScript), l_RocketSpark->m_Events.CalculateTime(7s));
                                break;
                            }
                            case 8:
                            {
    ...
    if (Creature* l_Borka = p_Instance->instance->GetCreature(p_Instance->GetGuidData(GrimrailDepotData::DataBorka)))
    {
        if (Creature* l_Rocketspark = p_Instance->instance->GetCreature(p_Instance->GetGuidData(GrimrailDepotData::DataRocketspark)))
        {
            if (l_Borka->isDead() || l_Rocketspark->isDead())
            {
                ...
                l_Rocketspark->SetTappedBy(l_Killer);
            }
        }
    }
}
    ...
    struct boss_rocketsparkAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            if (!m_First)
            {
                ...
                {
                    m_First = true;
                    me->m_Events.AddEvent(new EventRocketsparkAndBorkaConversation(me, 8, m_Instance), me->m_Events.CalculateTime(4s));
                }
            }
        ...
        void JustEngagedWith(Unit* p_Who) override
        {
            ...
            _JustEngagedWith(p_Who);
            HandleEncounterStartDoor();
            events.ScheduleEvent(eRocketsparkEvents::EventRandomMovement, 3s);
            events.ScheduleEvent(eRocketsparkEvents::EventX2101a, 30s);
        }
        ...
        void DoAction(int32 const p_Action) override
        {
            ...
            case eRocksparkAndBorkaActions::ActionRocketSparkNewPlan:
                if (!m_NewPlan)
                {
                    ...
                    events.ScheduleEvent(eRocketsparkEvents::EventNewPlan, 5s);
            ...
            case eRocksparkAndBorkaActions::ActionMadDashContact:
                ...
                events.ScheduleEvent(eRocketsparkEvents::EventX2101a, 30s);
                events.ScheduleEvent(eRocketsparkEvents::EventRandomMovement, 5s);
        ...
        void UpdateAI(uint32 const p_Diff) override
        {
            ...
            if (m_x210IsActivated)
            {
                ...
                {
                    me->CastSpell(me, eRocketsparkSpells::SpellX2101aMissileBarrage);
                    events.ScheduleEvent(eRocketsparkEvents::Eventx2101aRenew, 10s);
                }
            }
            ...
            case eRocketsparkEvents::EventX2101a:
            {
                ...
                events.ScheduleEvent(eRocketsparkEvents::EventX2101a, 40s);
                events.ScheduleEvent(eRocketsparkEvents::Eventx2101aRenew, 10s);
                events.ScheduleEvent(eRocketsparkEvents::Eventx2101aCancelation, 20s);
                break;
            }
            ...
            case eRocketsparkEvents::EventRandomMovement:
            {
                ...
                events.ScheduleEvent(eRocketsparkEvents::EventRandomMovement, 12s);
                events.ScheduleEvent(eRocketsparkEvents::EventVx18BTargetEliminator, 2s);
                break;
            }
            default:
                break;
...
public:
    ...
    struct grimrail_depot_rocketspark_mob_borkaAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* p_Who) override
        {
            ...
            events.ScheduleEvent(eBorkaEvents::EventSlam, 8s);
            events.ScheduleEvent(eBorkaEvents::EventMadDash, 20s);
        }
        ...
        void UpdateAI(uint32 const p_Diff) override
        {
                        ...
                        if (Creature* l_RocketSpark = m_Instance->instance->GetCreature(m_Instance->GetGuidData(GrimrailDepotData::DataRocketspark)))
                        {
                            if (Creature* l_Target = ObjectAccessor::GetCreature(*me, m_MadDashTargetGUID))
                            {
                                if (me->IsWithinDistInMap(l_Target, 2.0f))
                                {
                                    ...
                                    if (Unit* l_Victim = SelectTarget(SelectTargetMethod::MaxThreat, 0, 100.0f, true))
            ...
            case eBorkaEvents::EventMadDash: /// 01
            {
                ...
                events.ScheduleEvent(eBorkaEvents::EventMadDashActionDelayed, 3s);
                events.ScheduleEvent(eBorkaEvents::EventMadDash, 20s);
                break;
            }
            ...
            //    if (Creature* l_Creature = ObjectAccessor::GetCreature(*me, m_MadDashTargetGUID))
                ...
                events.ScheduleEvent(eBorkaEvents::EventRemoveDisableTurn, 4s);
                break;
            }
            ...
            if (GetCaster())
            {
                ...
                if (!l_ListPlayers.empty())
                {
                    for (Player* l_Itr : l_ListPlayers)
                    {
                        ...
                        if (Creature* l_Trigger = GetCaster()->SummonCreature(eRocksparkAndBorkaCreatures::CreatureX21Triggers, l_Position, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 3s))
