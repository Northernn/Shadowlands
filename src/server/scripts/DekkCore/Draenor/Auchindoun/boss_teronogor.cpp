...
struct boss_teronogor : public BossAI
{
    ...
    void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        if (me->HealthBelowPctDamaged(75, damage) && !m_SecondPhase)
        {
            ...
            me->SetUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_NON_ATTACKABLE));
        }
    }
    ...
    void JustDied(Unit* /*p_Killer*/) override
    {
        ...
        me->SummonGameObject(eAuchindounObjects::GameobjectChestAucheni, 1891.84f, 2973.80f, 16.844f, 5.664811f, QuaternionData(0, 0, 0, 0), 0s);
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case eTerongorEvents::EventShadowBolt:
        {
            ...
            events.ScheduleEvent(eTerongorEvents::EventShadowBolt, 6s);
            break;
        }
        case eTerongorEvents::EventCorruption:
        {
            if (Unit* l_Random = SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true, -eTerongorSpells::SpellCorruptionDmg))
                me->CastSpell(l_Random, eTerongorSpells::SpellCorruptionDmg);

            events.ScheduleEvent(eTerongorEvents::EventCorruption, 9s);
            break;
        }
        ...
        {
            if (Unit* l_Random = SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true, -eTerongorSpells::SpellCurseOfExhaustionDebuff))
        ...
        {
            if (Unit* l_Random = SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true, -eTerongorSpells::SpellUnstableAffliction))
        ...
        case eTerongorEvents::EventImmolate:
        {
            ...
            events.ScheduleEvent(eTerongorEvents::EventImmolate, 12s);
            break;
        }
        ...
        case eTerongorEvents::EventIncinrate:
        {
            ...
            events.ScheduleEvent(eTerongorEvents::EventIncinrate, 6s);
            break;
        }
        case eTerongorEvents::EventRainOfFire:
            ...
            events.ScheduleEvent(eTerongorEvents::EventRainOfFire, 10s);
            break;
    ...
    void DoCastTransformation(uint32 spell)
    {
        ...
        if (me->IsAIEnabled())
        {
            if (Unit* l_Victim = me->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
...
public:
    ...
    struct auchindoun_teronogor_mob_duragAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* p_Attacker)
        {
            ...
            events.ScheduleEvent(eTeronogorDuragEvents::EventShadowBolt, 6s);
            events.ScheduleEvent(eTeronogorDuragEvents::EventCorruption, 9s);
            events.ScheduleEvent(eTeronogorDuragEvents::EventChaosWave, 12s);
        }
        ...
        void UpdateAI(uint32 p_Diff) override
        {
            ...
            case eTeronogorDuragEvents::EventChaosWave:
                ...
                events.ScheduleEvent(eTeronogorDuragEvents::EventChaosWave, 20s);
                break;
            default:
                break;
            }
...
public:
    ...
    struct auchindoun_teronogor_mob_gulkoshAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* p_Attacker)
        {
            me->CastStop();
            me->RemoveAllAuras();
            events.ScheduleEvent(eTeronogorGulkoshEvents::EventShadowBolt, 6s);
            events.ScheduleEvent(eTeronogorGulkoshEvents::EventUnstableAffliction, 8s);
            events.ScheduleEvent(eTeronogorGulkoshEvents::EventDrainLife, 10s);
        }
        ...
        void Reset()
        {
           // m_Target = 0;
            ...
            if (m_First)
            {
                ...
                me->SetReactState(ReactStates::REACT_AGGRESSIVE);
                me->RemoveUnitFlag(UnitFlags::UNIT_FLAG_IMMUNE_TO_PC | UnitFlags::UNIT_FLAG_IMMUNE_TO_NPC);
                });
                ...
                me->SetUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL | UnitFlags::UNIT_FLAG_IMMUNE_TO_PC | UnitFlags::UNIT_FLAG_IMMUNE_TO_NPC));
            }
        }
...
struct auchindoun_teronogor_gameobject_soul_transporter_01 : public GameObjectScript
{
public:
    ...
    bool OnGossipHello(Player* p_Player, GameObject* p_Gobject)
    {
        ...
        p_Player->SetUnitFlag(UnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL | UnitFlags::UNIT_FLAG_IMMUNE_TO_NPC);
