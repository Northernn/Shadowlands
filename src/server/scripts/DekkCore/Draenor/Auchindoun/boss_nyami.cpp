...
struct boss_nyami : public BossAI
{
    ...
    void ScheduleTasks() override
    {
        ...
        events.ScheduleEvent(eNyamiEvents::EventMindSpikeNyami, 12s);
        events.ScheduleEvent(eNyamiEvents::EventShadowWordPain, 15s);
        events.ScheduleEvent(eNyamiEvents::EventSoulVessel, 20s);
        events.ScheduleEvent(eNyamiEvents::EventTornSpirit, 35s);
    }
    ...
    void JustDied(Unit* /*p_Killer*/) override
    {
        ...
        events.ScheduleEvent(eNyamiEvents::EventOnDeath1, 5s);
    }
    ...
    void HandleNonCombatVisuals(uint32 const p_Diff)
    {
        ...
        if (!UpdateVictim())
        {
            if (m_DiffVisual <= p_Diff)
            {
                ...
                {
                    auto l_Itr = l_ListCorpsesTriggers.begin();
                    std::advance(l_Itr, (0, l_ListCorpsesTriggers.size() - 1));
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case eNyamiEvents::EventMindSpikeNyami:
            ...
            events.ScheduleEvent(eNyamiEvents::EventMindSpikeNyami, 7s);
            break;
        ...
        case eNyamiEvents::EventSoulVessel:
            ...
            events.ScheduleEvent(eNyamiEvents::EventSoulVessel, 25s);
            break;
        case eNyamiEvents::EventTornSpirit:
        {
            ...
            events.ScheduleEvent(eNyamiEvents::EventTornSpiritsDummy, 3s);
            events.ScheduleEvent(eNyamiEvents::EventTornSpirit, 35s);
            break;
        }
        ...
        case EventOnDeath1:
        {
            if (Creature* l_Warden = me->GetSummonedCreatureByEntry(CreatureWardenAzzakael))
            {
                ...
                events.ScheduleEvent(eNyamiEvents::EventOnDeath2, 6s);
            }
            break;
        }
        case EventOnDeath2:
        {
            if (Creature* l_Warden = me->GetSummonedCreatureByEntry(CreatureWardenAzzakael))
            {
                ...
                events.ScheduleEvent(eNyamiEvents::EventOnDeath3, 10s);
            }
            break;
        }
        case EventOnDeath3:
        {
            if (instance)
            {
                if (GameObject* l_Window = instance->GetGameObject(eAuchindounDatas::DataAuchindounWindow))
                {
                    ...
                    l_Window->UseDoorOrButton(10);
                }
                if (Creature* l_Warden = me->GetSummonedCreatureByEntry(CreatureWardenAzzakael))
                {
                    l_Warden->SetReactState(ReactStates::REACT_PASSIVE);
                    l_Warden->AddUnitMovementFlag(MovementFlags::MOVEMENTFLAG_ROOT);
                    l_Warden->SetUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL));
                    l_Warden->SetUnitFlag3(UNIT_FLAG3_FAKE_DEAD);
                    l_Warden->SetUnitFlag(UnitFlags(UnitFlags2::UNIT_FLAG2_FEIGN_DEATH));
...
struct auchindoun_nyami_mob_malefic_defender : public ScriptedAI
{
    ...
    {
        events.ScheduleEvent(eMaleficDefenderEvents::EventCrusaderStirke, 5s);
    }
    ...
    void UpdateAI(uint32 p_Diff) override
    {
        ...
        case eMaleficDefenderEvents::EventCrusaderStirke:
            ...
            events.ScheduleEvent(eMaleficDefenderEvents::EventCrusaderStirke, 9s);
            break;
        default:
            break;
        }
...
struct auchindoun_nyami_mob_spiteful_arbitrer : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* p_Attacker)
    {
        events.ScheduleEvent(eNyamiEvents::EventRadiantFury, 8s);
        events.ScheduleEvent(eNyamiEvents::EventArbitrerHammer, 14s);
    }
    ...
    void UpdateAI(uint32 p_Diff) override
    {
        ...
        case eNyamiEvents::EventRadiantFury:
        {
            ...
            events.ScheduleEvent(eNyamiEvents::EventRadiantFuryStop, 6s);
            break;
        }
        case eNyamiEvents::EventRadiantFuryStop:
        {
            ...
            events.ScheduleEvent(eNyamiEvents::EventRadiantFury, 17s);
            break;
        }
        default:
            break;
...
struct auchindoun_nyami_mob_twisted_magus : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* p_Attacker)
    {
        events.ScheduleEvent(eTwistedMagusEvents::EventArcaneBolt, 4s);
        events.ScheduleEvent(eTwistedMagusEvents::EventArcaneBomb, 12s);
    }
    ...
    void UpdateAI(uint32 p_Diff) override
    {
        ...
        case eTwistedMagusEvents::EventArcaneBolt:
            ...
            events.ScheduleEvent(eTwistedMagusEvents::EventArcaneBolt, 15s);
            break;
        case eTwistedMagusEvents::EventArcaneBomb:
            me->CastSpell(me, eTwistedMagusSpells::SpellArcaneBombDummy);
            events.ScheduleEvent(eTwistedMagusEvents::EventArcaneBomb, 25s);
            break;
        default:
            break;
        }
