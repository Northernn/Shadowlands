...
struct boss_azaakel : public BossAI
{
    ...
    void DoAction(int32 p_Action)
    {
        ...
        case eAzzakelActions::ActionFelSpark:
            events.ScheduleEvent(eAzzakelEvents::EventFelSpark, 1s);
            break;
        case eAzzakelActions::ActionRenewEvents:
        {
            ...
            events.ScheduleEvent(eAzzakelEvents::EventFelLash, 8s);
            events.ScheduleEvent(eAzzakelEvents::EventClawsOfArgus, 45s);
            events.ScheduleEvent(eAzzakelEvents::EventCurtainOfFlame, 14s);
            events.ScheduleEvent(eAzzakelEvents::EventMalevolentCrush01, 20s);
            if (Unit* l_Target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                AttackStart(l_Target);
            break;
        }
        case eAzzakelActions::ActionMalevolentCrash:
            events.ScheduleEvent(eAzzakelEvents::EventMalevolentCrush01, 1s);
            break;
        default:
            break;
        }
    ...
    void ScheduleTasks() override
    {
        ...
        me->SummonGameObject(eAuchindounObjects::GameobjectFelBarrier, 1911.01f, 2722.89f, 30.799f, g_PositionAzzakel_Blackgate.GetOrientation(), QuaternionData(0, 0, 0, 0), 0s);
        events.ScheduleEvent(eAzzakelEvents::EventFelLash, 8s);
        events.ScheduleEvent(eAzzakelEvents::EventClawsOfArgus, 45s);
        events.ScheduleEvent(eAzzakelEvents::EventCurtainOfFlame, 14s);
        events.ScheduleEvent(eAzzakelEvents::EventMalevolentCrush01, 20s);
    ...
    void JustDied(Unit* /*p_Killer*/)
    {
        ...
        if (instance)
        {
            ...
            if (GameObject* l_Transport = instance->GetGameObject(eAuchindounDatas::DataSoulTransport1))
            {
                ...
                l_Transport->UseDoorOrButton(10, false, me);
            }
            //??
              if (m_Achievement)
            {
            if (me->GetMap() && me->GetMap()->IsHeroic())
            instance->DoCompleteAchievement(eAuchindounAchievements::AchievementDemonSouls);
            }
        }
    }
...
struct auchindoun_azzakel_mob_controller : public ScriptedAI
{
    ...
    void DoAction(int32 p_Action)
    {
        ...
        case eAuchindounActions::ActionCountPre3StBossKill:
        {
            ...
            if (m_Counting >= 6 && !m_Summoned)
            {
                ...
                events.ScheduleEvent(EventSummonAzzakel01, 2s);
            }
            break;
        }
        default:
    ...
    void UpdateAI(uint32 p_p_Diff) override
    {
        ...
        case eAzzakelEvents::EventSummonAzzakel01:
        {
            ...
            {
                if (GameObject* l_Portal = me->SummonGameObject(eAuchindounObjects::GameobjectDemonicPortal, *me, QuaternionData(0, 0, 0, 0), 0s))
                {
                    m_Azzakel = TempAzzakael->GetGUID();
                    l_Portal->SetFlag(GameObjectFlags(GO_FLAG_NOT_SELECTABLE | GameObjectFlags::GO_FLAG_NODESPAWN | GO_FLAG_INTERACT_COND));
            ...
            events.ScheduleEvent(eAzzakelEvents::EventSummonAzzakel02, 1s);
            break;
        }
            ...
            if (InstanceScript* l_Instance = GetCaster()->GetInstanceScript())
            {
                if (Creature* l_Azzakel = l_Instance->instance->GetCreature(l_Instance->GetGuidData(eAuchindounDatas::DataBossAzzakael)))
                {
                    if (l_Azzakel->IsAIEnabled())
                    {
                        ...
                        l_Azzakel->SetUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_IMMUNE_TO_PC | UnitFlags::UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_UNINTERACTIBLE));
                        Position pos;
                        pos.Relocate(l_Azzakel);
                        pos.m_positionZ += 45.0f;
                        l_Azzakel->GetMotionMaster()->MoveTakeoff(1000, pos);  //?????????
                    }
                }
            }
        }
            ...
            if (InstanceScript* l_Instance = GetCaster()->GetInstanceScript())
            {
                if (Creature* l_Azzakel = l_Instance->instance->GetCreature(l_Instance->GetGuidData(eAuchindounDatas::DataBossAzzakael)))
                {
                    if (l_Azzakel->IsAIEnabled())
                    {
                        ...
                        l_Azzakel->RemoveUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_UNINTERACTIBLE | UnitFlags::UNIT_FLAG_IMMUNE_TO_PC));
                        ...
                        {
                            if (Unit* l_Target = GetCaster()->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                            {
