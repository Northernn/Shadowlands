...
struct boss_kaathar : public BossAI
{
    ...
    void Reset() override
    {
       ...
       // me->GetAreatriggerListInRange(l_listAreaTriggers, 300.0f);
        ...
        if (m_False)
        {
            ...
            me->SetUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_UNINTERACTIBLE));
        }
    ...
    void JustDied(Unit* /*p_Killer*/) override
    {
        ...
        events.ScheduleEvent(Events::EVENT_POST_KAATHAR_1, 2s);

        me->SummonCreature(NPC_NYAMI, Position(1649.33f, 2964.08f, 35.3197f, 2.65524f), TEMPSUMMON_MANUAL_DESPAWN, 604800s);
    }
    ...
    void DoAction(int32 p_Action)
    {
        ...
        case eAuchindounActions::ActionCountPre1StBossKill:
        {
            ...
            if (m_Counting >= 12)
            {
                ...
                m_IntroDone = true;
                events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_1, 5s);
            }
            break;
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case Events::EVENT_POST_KAATHAR_1:
        {
            ...
            events.ScheduleEvent(Events::EVENT_POST_KAATHAR_2, 7s);
            events.ScheduleEvent(Events::EVENT_POST_KAATHAR_3, 55s);
            break;
        }
        case Events::EVENT_POST_KAATHAR_2:
        {
            if (Creature* l_Tuulani = me->FindNearestCreature(CreatureSoulBinderTuulani, 1000.0f))
            {
                ...
                l_Tuulani->SummonGameObject(eAuchindounObjects::GameobjectHolyWall, g_PositionWallInBackFromNyami.GetPositionX(), g_PositionWallInBackFromNyami.GetPositionY(), g_PositionWallInBackFromNyami.GetPositionZ(), g_PositionWallInBackFromNyami.GetOrientation(), QuaternionData(0, 0, 0, 0), 0s);
                ...
                for (int8 l_I = 0; l_I < 3; l_I++)
                {
                    if (Creature* l_Prisoners = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniSoulPriest, g_PositionThreePrisoners[l_I], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                    {
                       ...
                       // l_Prisoners->UNIT_NPC_EMOTESTATEUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                        l_Prisoners->SetUnitFlag3(UNIT_FLAG3_FAKE_DEAD);
                    }
                }
                ...
                for (int8 l_I = 0; l_I < 11; l_I++)
                {
                    if (Creature* l_Prisoners = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniDefender, g_PositionCorpsesNearNyomi[l_I], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                    {
                        ...
                        l_Prisoners->AddUnitMovementFlag(MovementFlags::MOVEMENTFLAG_ROOT);
                        l_Prisoners->SetUnitFlag3(UNIT_FLAG3_FAKE_DEAD);
                        l_Prisoners->SetUnitFlag(UnitFlags(UnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL));
                    }
                }
                ...
                for (int8 l_I = 0; l_I < 4; l_I++)
                {
                    if (Creature* l_Defenders = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniWarden, g_PositionDefenderBehindMiddleWallOfNyami[l_I], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                    {
                        ...
                        l_Defenders->SetUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC | UnitFlags::UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL));
        ...
        case Events::EVENT_NYAMI_ESCAPE_4:
        {
            if (Creature* l_Nyami = me->FindNearestCreature(CreatureSoulBinderNyami, 1000.0f))
            {
                ...
                events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_5, 5s);
            }
        ...
        case Events::EVENT_NYAMI_ESCAPE_5:
        {
            ...
            events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_6, 5s);
            break;
        }
        ...
        {
            if (Creature* l_Nyami = me->FindNearestCreature(CreatureSoulBinderNyami, 1000.0f))
                l_Nyami->DespawnOrUnsummon(5s);
            if (Creature* l_Tuulani = me->FindNearestCreature(CreatureSoulBinderTuulani, 1000.0f))
                l_Tuulani->AI()->Talk(eAuchindounTalks::TUULANITALK10);
            events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_8, 5s);
            break;
        }
        case Events::EVENT_NYAMI_ESCAPE_8:
        {
            ...
            events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_9, 5s);
            break;
        }
        ...
        case eKaatharEvents::EventFate:
        {
            ...
            events.ScheduleEvent(eKaatharEvents::EventFate, 37s);
            break;
        }
        case eKaatharEvents::EventHallowedGround:
        {
            ...
            me->SummonCreature(eKaatharCreatures::TriggerHallowedGround, l_Position, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 30s);
            events.ScheduleEvent(eKaatharEvents::EventHallowedGround, 8s);
            break;
        }
        case eKaatharEvents::EventHolyShield:
        {
            ...
            events.ScheduleEvent(eKaatharEvents::EventHolyShield, 47s);
            break;
        }
        ...
        case eKaatharEvents::EventSanctifiedStrike:
        {
            ...
            for (uint8 l_I = 0; l_I <= 10; l_I++)
            {
                ...
                me->SummonCreature(eKaatharCreatures::TriggerFissureSummoner, l_PosX, l_PosY, me->GetPositionZ(), me->GetOrientation(), TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 15s);
            }
            events.ScheduleEvent(eKaatharEvents::EventSanctifiedStrike, 8s);
            break;
        }
...
struct auchindoun_kaathar_mob_holy_shield : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        me->SetUnitFlag(UnitFlags(UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_UNINTERACTIBLE | UnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL));
        /// Knockback
