...
public:
    ...
    struct npc_generic_tanaan_guardianAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            events.ScheduleEvent(EVENT_CHECK_ENNEMY,    1s);
            events.ScheduleEvent(EVENT_RESTORE_HEALTH,  10s);
            events.ScheduleEvent(EVENT_RANDOM_SPELL,    5s, 10s);
        ...
        void SpawnEnnemy()
        {
         ...
         //   else
            ...
            if (TempSummon* ennemy = me->SummonCreature(ennemyId, ennemySpawnPos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10s))
            {
                if (ennemyId == NPC_SHADOWMOON_RITUALIST)
        ...
        void SummonedCreatureDespawn(Creature* summon) override
        {
            ...
            events.ScheduleEvent(EVENT_CHECK_ENNEMY, randtime(2s, 10s));
        }

        void DamageTaken(Unit* p_DoneBy, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_RANDOM_SPELL:
                    {
                        ...
                        events.ScheduleEvent(EVENT_RANDOM_SPELL, randtime(5s, 10s));
                        break;
                    }
                }
            }
...
struct gob_mark_of_tanaan : public GameObjectAI
{
    gob_mark_of_tanaan(GameObject* go) : GameObjectAI(go) { }
    ...
    bool OnGossipHello(Player* player) override
    {
        if (player->GetQuestStatus(TanaanQuests::QuestOnslaughtEnd) == QUEST_STATUS_INCOMPLETE)
        {
            ...
            if (me->GetEntry() == TanaanGameObjects::GobMarkOfShadowmoon)
            {
            ...
            else if (me->GetEntry() == TanaanGameObjects::GobMarkOfBleedingHollow)
            {
...
/// 233104 - Statis Rune
struct gob_static_rune : public GameObjectAI
{
    gob_static_rune(GameObject* go) : GameObjectAI(go) { }
