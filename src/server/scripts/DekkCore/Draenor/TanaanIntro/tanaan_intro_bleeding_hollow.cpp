...
struct go_bleeding_hollow_cage : public GameObjectAI
{
    go_bleeding_hollow_cage(GameObject* go) : GameObjectAI(go) { }
    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        {
            /// Eastern Cage
            if (me->GetEntry() == TanaanGameObjects::GobEasternCage)
            {
                ...
                player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneEasterCage, SceneFlag::None);

                return false;
            }
            /// Southest Cage
            else if (me->GetEntry() == TanaanGameObjects::GobSouthernCage)
            {
                ...
                player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneSouthernCage, SceneFlag::None);
    ...
    struct npc_tanaan_ariokAI : public ScriptedAI
    {
        ...
        bool OnGossipSelect(Player* player, uint32 sender, uint32 action) override
        {
                ...
                player->GetQuestStatus(TanaanQuests::QuestAltarAltercation) == QUEST_STATUS_FAILED)
            {
                ...
                if (TempSummon* l_Ariok = player->SummonCreature(me->GetEntry(), me->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0s))
                    if (l_Ariok->GetAI())
...
void AddSC_tanaan_intro_bleeding_hollow()
{
    ...
    RegisterGameObjectAI(go_bleeding_hollow_cage);
