...
public:
    ...
    void OnSceneTriggerEvent(Player* player, uint32 sceneInstanceID, std::string triggerEvent) override
    {
        ...
        if (triggerEvent == "Teleport")
        {
            ...
            player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneFromCaveToRidge, SceneFlag::None);
        }
    }
};
                ...
                if (m_SceneTimer <= diff)
                {
                    if (!playerGuid.IsEmpty())
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*me, playerGuid))
                        {
                            ...
                            player->GetSceneMgr().PlaySceneByPackageId(player->GetTeamId() == TEAM_ALLIANCE ? TanaanSceneObjects::SceneQuianaAndMaladaarEnter : TanaanSceneObjects::SceneLiadrinAndOlinEnter, SceneFlag::None);
                        }
                    }
                    m_SceneTimer = 0;
                }
...
struct npc_tanaan_yrel_summon : public FollowerAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        m_Events.ScheduleEvent(eEvents::EventCheckStopFollow, 6s);
    }
        ...
        switch (m_Events.ExecuteEvent())
        {
            case eEvents::EventCheckStopFollow:
            {
                    ...
                    || me->GetPositionX() < 4560.0f)
                {
                    ...
                    m_Events.ScheduleEvent(eEvents::EventContinueRun, 1s);
                }
                else
                    m_Events.ScheduleEvent(eEvents::EventCheckStopFollow, 8s);
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_maladaar_liadrin_tanaan_caveAI(creature);
    }

    struct npc_maladaar_liadrin_tanaan_caveAI : public ScriptedAI
    {
        npc_maladaar_liadrin_tanaan_caveAI(Creature* creature) : ScriptedAI(creature) { }
    };

    bool OnQuestAccept(Player* player, Creature* creature, const Quest* quest) override
    {
        if (quest->GetQuestId() == TanaanQuests::QuestKeliDanTheBreakerAlly || quest->GetQuestId() == TanaanQuests::QuestKeliDanTheBreakerHorde)
            player->GetSceneMgr().PlaySceneByPackageId(creature->GetEntry() == TanaanCreatures::NpcExarchMaladaar ? TanaanSceneObjects::SceneYrelLeaveAlliance : TanaanSceneObjects::SceneYrelLeaveHorde, SceneFlag::None);
...
public:
    ...
    struct go_iron_cage_doorAI : public GameObjectAI
    {
...
public:
    ...
    struct npc_keli_dan_the_breakerAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            m_events.ScheduleEvent(EVENT_VOID_BOLT_VOLLEY,  (8s));
            m_events.ScheduleEvent(EVENT_VOID_SHELL,        (10s));
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (m_events.ExecuteEvent())
            {
                case EVENT_VOID_BOLT_VOLLEY:
                {
                    ...
                    m_events.ScheduleEvent(EVENT_VOID_BOLT_VOLLEY, (15s));
                    break;
                }
                case EVENT_VOID_SHELL:
                {
                    DoCastAOE(SPELL_VOID_SHELL);
                    m_events.ScheduleEvent(EVENT_VOID_SHELL, (15s));
                    break;
                }
            }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            for (Player* player : playerList)
            {
                ...
                {
                    player->KilledMonsterCredit(me->GetEntry());
                    player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneNerZhulReveal, SceneFlag::NotCancelable);
                }
            }
        }
    };
