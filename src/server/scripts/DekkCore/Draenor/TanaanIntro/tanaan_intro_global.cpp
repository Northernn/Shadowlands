...
public:
    ...
    void OnUpdateArea(Player* player, uint32 newAreaId, uint32 /*oldAreaID*/) override
    {
        ...
        switch (newAreaId)
        {
            ...
            {
                player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneLoomingArmy, SceneFlag::None);
                player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneSoulTrain, SceneFlag::None);
                break;
            }
            case AREA_TANAAN_HEARTH_BLOOD:
            {
                player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneCuriousHatchets, SceneFlag::None);
                break;
            }
            case AREA_TANAAN_BLEEDING_ALTAR:
            {
                player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneEyeOfKilrogg, SceneFlag::None);
                player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneBloodBowl, SceneFlag::None);
                break;
            }
            case AREA_TANAAN_KARGATHAR_PROVING_GROUNDS:
            {
                if (player->GetQuestStatus(TanaanQuests::QuestAPotentialAlly) != QUEST_STATUS_COMPLETE &&
                    ...
                    player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneRingOfFire, SceneFlag::None);
                break;
            }
            case AREA_TANAAN_UMBRAL_HALLS:
            {
                player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneUmbralHallsPortals, SceneFlag::None);
                player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneKelidanVisual1, SceneFlag::None);
                player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneKelidanVisual2, SceneFlag::None);
                break;
            }
            case AREA_TANAAN_BLACKROCK_QUARRY:
            {
                if (player->GetQuestStatus(TanaanQuests::QuestTheBattleOfTheForge) == QUEST_STATUS_INCOMPLETE)
                {
                    ...
                    player->GetSceneMgr().RecreateScene(TanaanSceneObjects::SceneKhadgarAtDam, SceneFlag::None);
    ...
    void OnObjectiveValidate(Player* player, uint32 questId, uint32 objectiveId) override
    {
        if (questId == TanaanQuests::QuestThePortalPower && objectiveId == TanaanQuestObjectives::ObjEnterGulDanPrison)
            player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneGulDanReavel, SceneFlag::None);
    }
};
...
struct npc_archmage_khadgar : public ScriptedAI
{
    ...
    void OnQuestAccept(Player* player, Quest const* quest) override
    {
        switch (quest->GetQuestId())
        {
            ...
            {
                player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneCostOfWarEscort, SceneFlag::None);
            ...
            {
                player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneHoldout, SceneFlag::None);
