...
public:
    ...
    void StartBattle(Player* player, uint32 sceneInstanceId)
    {
        ...
        player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneKhadgarGoesToDam, SceneFlag::None);
    }
};
...
public:
    ...
    void OnSceneComplete(Player* player, uint32 sceneInstanceId) override
    {
        ...
        player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneKhadgarAtDam, SceneFlag::None);
    }
};
...
public:
    ...
    void OnQuestReward(Player* player, const Quest* quest) override
    {
        if (player && quest && quest->GetQuestId() == TanaanQuests::QuestTheBattleOfTheForge)
        {
            ...
            player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneDamExplosion, SceneFlag::None);
        }
    }
    ...
    void OnSceneCancel(Player* player, uint32 sceneInstanceId) override
    {
        ...
        player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneWaterPortal, SceneFlag::None);
    }
...
public:
    ...
    {
        return new npc_thrall_maladaar_blackrockAI(creature);
    }

    struct npc_thrall_maladaar_blackrockAI : public ScriptedAI
    {
        npc_thrall_maladaar_blackrockAI(Creature* creature) : ScriptedAI(creature) { }
    };
    ...
    bool OnQuestReward(Player* player, Creature* /*creature*/, const Quest* quest, uint32 /*p_Option*/) override
    {
        if (quest->GetQuestId() == TanaanQuests::QuestABattleToPrepareAlly)
            player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneArmingPrisonersAlly, SceneFlag::None);
        else if (quest->GetQuestId() == TanaanQuests::QuestABattleToPrepareHorde)
            player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneArmingPrisonersHorde, SceneFlag::None);
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* /*creature*/, const Quest* quest) override
    {
        if (quest->GetQuestId() == TanaanQuests::QuestTheBattleOfTheForge)
        {
            ...
            player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneBlackHandReveal, SceneFlag::None);
...
public:
    ...
    struct npc_ogron_warcrusherAI : public ScriptedAI
    {
        ...
        {
            m_Events.Reset();
            m_Events.ScheduleEvent(EventAddaura, 3s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (m_Events.ExecuteEvent())
            {
                case eEvents::EventAddaura:
                    ...
                    m_Events.ScheduleEvent(eEvents::EventAddaura, 12s);
                    break;
            }
        }
    };
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_tanaan_ganarAI(creature);
    }

    struct npc_tanaan_ganarAI : public ScriptedAI
    {
        npc_tanaan_ganarAI(Creature* creature) : ScriptedAI(creature) { }
    };

    bool OnQuestAccept(Player* player, Creature* /*creature*/, Quest const* quest) override
    {
        if (quest->GetQuestId() == TanaanQuests::QuestTheProdigalFrostwolf)
            player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneFreeGanar, SceneFlag::None);
...
public:
    ...
    struct npc_tanaan_overseer_gotriggAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* /*target*/) override
        {
            ...
            m_Events.ScheduleEvent(eDatas::EventBurningBody, 3s);
            m_Events.ScheduleEvent(eDatas::EventBurningBody, 10s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (m_Events.ExecuteEvent() == eDatas::EventBurningBody)
            {
                ...
                m_Events.ScheduleEvent(eDatas::EventBurningBody, 20s);
            }
            else if (m_Events.ExecuteEvent() == eDatas::EventBurningRadiance)
            {
                me->AddAura(eDatas::SpellBurningRadiance, me);
                m_Events.ScheduleEvent(eDatas::EventBurningRadiance, 20s);
            }
        }
    };
};
...
/// 231119 - Powder Keg
struct gob_powder_keg : public GameObjectAI
{
    gob_powder_keg(GameObject* go) : GameObjectAI(go) { }
...
struct gob_makeshift_plunger : public GameObjectAI
{
    gob_makeshift_plunger(GameObject* go) : GameObjectAI(go) { }
    ...
    bool OnGossipHello(Player* player) override
    {
        if (player->GetQuestStatus(TanaanQuests::QuestTheGunpowderPlot) == QUEST_STATUS_INCOMPLETE)
        {
            ...
            else
            {
                ...
                player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneBuildingExplosionFallBack, SceneFlag::None);
            }
        }
...
void AddSC_tanaan_intro_blackrock()
{
    ...
    RegisterGameObjectAI(gob_powder_keg);
    RegisterGameObjectAI(gob_makeshift_plunger);
