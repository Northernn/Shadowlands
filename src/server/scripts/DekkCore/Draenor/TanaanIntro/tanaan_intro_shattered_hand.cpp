...
public:
    ...
    void OnQuestAbandon(Player* player, const Quest* quest) override
    {
        switch (quest->GetQuestId())
        {
            ...
            case TanaanQuests::QuestAPotentialAllyHorde:
                player->GetSceneMgr().CancelSceneByPackageId(TanaanSceneObjects::SceneRingOfFire);
                player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneRingOfFire, SceneFlag::None);
            default:
                break;
        }
    }
    ...
    void OnSceneTriggerEvent(Player* player, uint32 sceneInstanceId, std::string triggerEvent) override
    {
        ...
        if (player->GetSceneMgr().HasScene(sceneInstanceId, TanaanSceneObjects::SceneEscapingTheArena))
        {
            ...
            else if (triggerEvent == "Credit")
            {
                ...
                // Scene flags taken from sniffs except SceneFlag::NotCancelable
                player->GetSceneMgr().PlaySceneByPackageId(TanaanSceneObjects::SceneCaveIn, SceneFlag::PlayerNonInteractablePhased | SceneFlag::NotCancelable | SceneFlag::FadeToBlackscreenOnCancel/* | SCENEFLAG_UNK16 */);
            }
        }
    }
};
...
public:
    ...
    struct npc_tanaan_mandragoraAI : public ScriptedAI
    {
        ...
        {
            m_Events.ScheduleEvent(eDatas::EventChomp, 3s);
            m_Events.ScheduleEvent(eDatas::EventImpactSplit, 10s);
        }
...
struct npc_shattered_hand_brawler : public ScriptedAI
{
    ...
    void Reset() override
    {
        PhasingHandler::AddPhase(me, 180, true);
        PhasingHandler::AddPhase(me, 183, true);
        PhasingHandler::AddPhase(me, 184, true);
