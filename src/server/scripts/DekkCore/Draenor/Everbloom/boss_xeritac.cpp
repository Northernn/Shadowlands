    ...
    struct xeri_tac_vehicleAI : public VehicleAI
    {
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            if (!descend)
            {
                ...
                {
                    chargeTimer = 5000;
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 500.0f))
                    {
                        if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == POINT_MOTION_TYPE)
                          //  me->GetMotionMaster()->MovementExpired();
    ...
    void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
        {
            switch (summon->GetEntry())
            {
               ...
               //     me->CastSpell(SPELL_TOXIC_GAS_XERITAC, true);
        ...
        void UpdateAI(uint32 const diff) override
        {
            ...
            switch (state)
            {
                case 1:
                    ...
                    {
                        addTimer = 15000;
                        me->SummonCreature(NPC_TOXIC_SPIDERLING, 935.9139f, 1446.727f, 86.68246f, 2.888725f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3s);
                        me->SummonCreature(NPC_VENOM_SPRAYER, 929.7065f, 1437.282f, 86.69052f, 3.35384f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3s);
                        // Todo : Gorged Burster
                    }
                    else
                        addTimer -= diff;
...
public:
    ...
    struct toxic_eggs_goAI : public GameObjectAI
    {
        toxic_eggs_goAI(GameObject* go) : GameObjectAI(go) { }

        bool OnGossipHello(Player* player) override
        {
            if (Creature* xeritac = GetClosestCreatureWithEntry(me, NPC_XERITAC, 100.0f))
            {
                xeritac->AI()->AttackStart(player);
                xeritac->AI()->DoZoneInCombat(xeritac);
            }

            if (Creature* vehicle = GetClosestCreatureWithEntry(me, NPC_XERITAC_VEHICLE, 100.0f))
            {
