                ...
                case 1:
                    ...
                    if (frozenRainTimer <= diff)
                    {
                        ...
                        if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 30.0f, true))
                        {
                            ...
                            frozenRain = me->SummonCreature(NPC_FROZEN_RAIN, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN, 30s);
                        }
                    }
                    else
...
public:
    ...
    struct npc_frozen_rainAI : public ScriptedAI
    {
        npc_frozen_rainAI(Creature* creature) : ScriptedAI(creature) { }
...
public:
    ...
    class aura_archmage_barrier_AuraScript : public AuraScript
    {
        ...
        void HandlePeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            for (uint8 i = 0; i < 5; i++)
                GetCaster()->CastSpell(GetCaster(), 170146, true);
        }
