    ...
    struct boss_witherbarkAI : public BossAI
    {
        ...
        void UpdateAI(uint32 const diff) override
        {
            ...
            if (BrittleBarkTimer <= diff)
            {
                ...
                for (uint8 i = 0; i < 3; i++)
                    me->CastSpell(me, SPELsummon_AQUEOUS_GLOBULE, true);
...
public:
    ...
    struct mob_enchanted_waterAI : public ScriptedAI
    {
        mob_enchanted_waterAI(Creature* creature) : ScriptedAI(creature)
        {
        ...
        void UpdateAI(const uint32 diff) override
        {
            if (timer)
            {
                if (timer <= diff)
                {
                    ...
                    if (Creature* witherbark = me->FindNearestCreature(81522, 1000.0f))
                    {
                        if (Unit* target = witherbark->AI()->SelectTarget(SelectTargetMethod::Random, 0, 50000.0f, true))
