                            ...
                            if (l_Zoggosh->IsAIEnabled() && l_Koramar->IsAIEnabled())
                            {
                                switch (m_Modifier)
                                {
                                    case 0:
                                        ...
                                        l_Koramar->m_Events.AddEvent(new basicevent_before_oshir(l_Koramar, 1), l_Koramar->m_Events.CalculateTime(8s));
                                        break;
                                    case 1:
                                        l_Koramar->AI()->Talk(eTalks::TalkKoramar09);
                                        l_Koramar->m_Events.AddEvent(new basicevent_before_oshir(l_Koramar, 2), l_Koramar->m_Events.CalculateTime(7s));
                                        break;
                                    case 2:
                                        l_Zoggosh->AI()->Talk(eTalks::TalkZoggosh08);
                                        l_Koramar->m_Events.AddEvent(new basicevent_before_oshir(l_Koramar, 3), l_Koramar->m_Events.CalculateTime(7s));
                                        break;
                                    case 3:
                                        l_Koramar->AI()->Talk(eTalks::TalkKoramar11);
                                        l_Koramar->m_Events.AddEvent(new basicevent_before_oshir(l_Koramar, 4), l_Koramar->m_Events.CalculateTime(7s));
                                        break;
                                    case 4:
                                        l_Zoggosh->AI()->Talk(eTalks::TalkZoggosh11);
                                        l_Koramar->m_Events.AddEvent(new basicevent_before_oshir(l_Koramar, 5), l_Koramar->m_Events.CalculateTime(7s));
                                        break;
                                    case 5:
        ...
        struct boss_oshirAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                if (!m_First)
                {
                    ...
                    me->SetUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_UNK_6 | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NON_ATTACKABLE));
                    ResetCages();
                }
            }
            ...
            void JustEngagedWith(Unit* p_Attacker) override
            {
                ...
                events.ScheduleEvent(eOshirEvents::EventFeedingFrenzy, 25s);
                events.ScheduleEvent(eOshirEvents::EventBreakout, 10s);
                events.ScheduleEvent(eOshirEvents::EventPrimalAssault, 36s);
                ...
                if (p_Who && p_Who->IsInWorld() && p_Who->GetTypeId() == TypeID::TYPEID_PLAYER && me->IsWithinDistInMap(p_Who, 50.0f) && !m_Intro)
                {
                    ...
                    for (uint8 l_I = 0; l_I < 3; l_I++)
                    {
                        if (Creature* l_Guard = me->SummonCreature(eCreatures::CreatureGuard, g_NpcSpawnPos, TempSummonType::TEMPSUMMON_MANUAL_DESPAWN))
                        {
                            ...
                            l_Guard->SetUnitFlag(UnitFlags(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_NPC));
            ...
            void DoAction(int32 const p_Action) override
            {
                switch (p_Action)
                {
                    ...
                    case eAction::ActionRelease:
                    {
                        ...
                        m_CosmeticEvent.ScheduleEvent(eCosmeticEvents::EventCosmetic01, 2s);
                        break;
                    }
                    ...
                    case eAction::ActionFinishSpell:
                    {
                        ...
                        if (Unit* l_Target = SelectTarget(SelectTargetMethod::MaxThreat, 0, 1000.0f, true))
                        {
                ...
                if (!m_ThunderilingWandlers.empty())
                {
                    switch (m_CosmeticEvent.ExecuteEvent())
                    {
                        case eCosmeticEvents::EventCosmetic01:
                        {
                            ...
                            m_CosmeticEvent.ScheduleEvent(eCosmeticEvents::EventCosmetic02, 3s);
                            break;
                        }
                        case eCosmeticEvents::EventCosmetic02:
                        {
                            ...
                            m_CosmeticEvent.ScheduleEvent(eCosmeticEvents::EventCosmetic03, 5s);
                            break;
                        }
                        case eCosmeticEvents::EventCosmetic03:
                        {
                            ...
                            m_CosmeticEvent.ScheduleEvent(eCosmeticEvents::EventCosmetic04, 6s);
                            break;
                        }
                        case eCosmeticEvents::EventCosmetic04:
                        {
                            ...
                            m_CosmeticEvent.ScheduleEvent(eCosmeticEvents::EventCosmetic05, 5s);
                            break;
                        }
                    ...
                    if (m_CurrentDestTarget != ObjectGuid::Empty)
                    {
                        if (Player* l_Nearest = ObjectAccessor::GetPlayer(*me, m_CurrentDestTarget))
                        {
                            if (me->IsWithinMeleeRange(l_Nearest))
                            {
                                ...
                                me->RemoveAura(eOshirSpells::SpellPrimalAssaultVisual);
                                events.ScheduleEvent(eOshirEvents::EventFeedingFrenzy, 20s);
                            }
                        }
                    }
                ...
                */
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case eOshirEvents::EventPrimalAssault:
                    {
                        ...
                        events.ScheduleEvent(eOshirEvents::EventPrimalAssault, 35s);
                        break;
                    }
                    case eOshirEvents::EventBreakout:
                    {
                        ...
                        events.ScheduleEvent(eOshirEvents::EventBreakout, 45s);
                        break;
                    }
                    default:
                        break;
...
public:
    ...
    struct iron_docks_mob_thundering_wandlerAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* p_Attacker) override
        {
            events.ScheduleEvent(eThunderWandlerEvents::EventCultTraps, 18s);
            events.ScheduleEvent(eThunderWandlerEvents::EventSpearThrow, 18s);
            events.ScheduleEvent(eThunderWandlerEvents::EventRendingCleave, 10s);
        }
            ...
            switch (events.ExecuteEvent())
            {
                case eThunderWandlerEvents::EventCultTraps:
                    if (Unit* l_Target = me->GetVictim())
                    {
                        ...
                        me->GetMotionMaster()->MoveKnockbackFrom(me->GetPositionX(), me->GetPositionY(), 10.0f);
                    }

                    events.ScheduleEvent(eThunderWandlerEvents::EventCultTraps, 25s);
                    break;
                case eThunderWandlerEvents::EventSpearThrow:
                    if (Unit* l_Target = SelectTarget(SelectTargetMethod::MaxDistance, 0, 50.0f, true))
                ...
                case eThunderWandlerEvents::EventRendingCleave:
                    ...
                    events.ScheduleEvent(eThunderWandlerEvents::EventRendingCleave, 12s);
                    break;
                default:
                    break;
            }
                    ...
                    case eAction::ActionRelease:
                    {
                        if (Creature* l_Oshir = m_Instance->instance->GetCreature(m_Instance->GetGuidData(eIronDocksDatas::DataOshir)))
                        {
                            if (!m_Released)
                            {
                                ...
                                events.ScheduleEvent(eEvents::EventRelease, 3s);
                            }
                        }
                        break;
                    }
                    ...
                    case eAction::ActionRelease:
                    {
                        if (Creature* l_Oshir = m_Instance->instance->GetCreature(m_Instance->GetGuidData(eIronDocksDatas::DataOshir)))
                        {
                            if (!m_Released)
                            {
                                ...
                                events.ScheduleEvent(eEvents::EventRelease, 1s);
                            }
                        }
                        break;
                    }
...
public:
    ...
    struct iron_docks_oshir_mob_eventAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* p_Who) override
        {
            ...
            {
                m_CanEvent = true;
                me->m_Events.AddEvent(new basicevent_before_oshir(me, 0), me->m_Events.CalculateTime(2s));
            }
        }
    };
    ...
    public:
        ...
        struct iron_docks_oshir_mob_rylakAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 const p_Diff) override
            {
                ...
                if (Unit* l_Victim = SelectTarget(SelectTargetMethod::MaxThreat, 0, 1000.0f, true))
                {
...
public:
    ...
    class iron_docks_oshir_spell_acid_spit_trigger_missile_SpellScript : public SpellScript
    {
        ...
        void OnSpellHit(SpellEffIndex)
        {
            ...
            l_Caster->SummonCreature(eAcidSpitCreatures::CreatureAcidSpitTrigger, *l_Dest, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 30s);
        }
                    ...
                    if (Creature* l_Oshir = l_InstanceScript->instance->GetCreature(l_InstanceScript->GetGuidData(eIronDocksDatas::DataOshir)))
                    {
                        if (!l_Oshir->HasAura(eSpells::SpellTimeToFeed))
                        {
                            ...
                            if (l_Oshir->IsAIEnabled())
                            {
                                ...
                                if (Unit* l_Target = l_Oshir->AI()->SelectTarget(SelectTargetMethod::MaxThreat, 0, 1000.0f, true))
                                {
