                ...
                if (Creature* l_Koramar = l_Instance->instance->GetCreature(l_Instance->GetGuidData(eIronDocksDatas::DataKoramar)))
                {
                    if (l_Koramar->IsAIEnabled())
                    {
                        ...
                        case 0:
                            ...
                            l_Koramar->m_Events.AddEvent(new basicevent_pre_boss(l_Koramar, 1), l_Koramar->m_Events.CalculateTime(8s));
                            break;
                        case 1:
                            l_Koramar->AI()->Talk(eTalks::TalkKoramar13);
                            l_Koramar->m_Events.AddEvent(new basicevent_pre_boss(l_Koramar, 2), l_Koramar->m_Events.CalculateTime(8s));
                            break;
                        case 2:
                        ...
                        if (l_Koramar->IsAIEnabled() && l_Skulloc->IsAIEnabled() && l_Zoggosh->IsAIEnabled())
                        {
                            switch (m_Modifier)
                            {
                                case 0:
                                {
                                    ...
                                    l_Zoggosh->m_Events.AddEvent(new basicevent_bombardment_bridge(l_Zoggosh, 1), l_Zoggosh->m_Events.CalculateTime(7s));
                                    break;
                                }
                                case 1:
                                {
                                    l_Koramar->AI()->Talk(eTalks::TalkKoramarBombardmenet01);
                                    l_Koramar->m_Events.AddEvent(new basicevent_bombardment_bridge(l_Koramar, 2), l_Koramar->m_Events.CalculateTime(8s));
                                    break;
                                }
                                case 2:
                                {
                                    l_Zoggosh->AI()->Talk(eTalks::TalkZoggoshBombardmenet02);
                                    l_Skulloc->m_Events.AddEvent(new basicevent_bombardment_bridge(l_Skulloc, 3), l_Skulloc->m_Events.CalculateTime(4s));
                                    break;
                                }
                                case 3:
                                {
                                    l_Skulloc->AI()->DoAction(eIronDocksActions::ActionBombardBridge);
                                    l_Skulloc->m_Events.AddEvent(new basicevent_bombardment_bridge(l_Skulloc, 3), l_Skulloc->m_Events.CalculateTime(8s));
                                    break;
                                }
                                default:
                                    break;
                    ...
                    if (Creature* l_Koramar = l_Instance->instance->GetCreature(l_Instance->GetGuidData(eIronDocksDatas::DataKoramar)))
                    {
                        if (Creature* l_Skulloc = l_Instance->instance->GetCreature(l_Instance->GetGuidData(eIronDocksDatas::DataSkulloc)))
                        {
                            if (l_Koramar->IsAIEnabled() && l_Skulloc->IsAIEnabled() && l_Zoggosh->IsAIEnabled())
                            {
                                ...
                                {
                                    l_Koramar->AI()->Talk(eTalks::TalkKoramarBombardmenet01);
                                    l_Zoggosh->m_Events.AddEvent(new basicevent_past_bombardment_bridge(l_Zoggosh, 1), l_Zoggosh->m_Events.CalculateTime(9s));
                                    break;
                                }
                                case 1:
                                {
                                    l_Zoggosh->AI()->Talk(eTalks::TalkZoggoshPastBombardment01);
                                    l_Koramar->m_Events.AddEvent(new basicevent_past_bombardment_bridge(l_Koramar, 2), l_Koramar->m_Events.CalculateTime(8s));
                                    break;
                                }
                                case 2:
                                {
        ...
        if (Creature* l_Koramar = p_Instance->instance->GetCreature(p_Instance->GetGuidData(eIronDocksDatas::DataKoramar)))
        {
            if (Creature* l_Zoggosh = p_Instance->instance->GetCreature(p_Instance->GetGuidData(eIronDocksDatas::DataZuggosh)))
            {
                if (l_Skulloc->isDead() && l_Koramar->isDead() && l_Zoggosh->isDead())
                {
                    ...
                    {
                        l_Skulloc->SetTappedBy(l_Player);
    ...
    public:
        ...
        struct boss_skullocAI : public BossAI
        {
            ...
            {            
                ...
                events.ScheduleEvent(eSkullocEvents::EventCannonBarrage, 50s);
                events.ScheduleEvent(eSkullocEvents::EventBackdraft, 20s);
                ...
                switch (p_Action)
                {
                    case eIronDocksActions::ActionActivateGrimrailEvent:
                    {
                        if (!m_GrimrailEvent)
                        {
                            ...
                            me->m_Events.AddEvent(new basicevent_past_bombardment_bridge(me, 0), me->m_Events.CalculateTime(1s));
                    ...
                    case eIronDocksActions::ActionActivateBridgeBombardmement:
                    {
                        if (!m_BridgeBombardmenet)
                        {
                            ...
                            if (Unit* l_Nearest = SelectTarget(SelectTargetMethod::MaxThreat, 0, 100.0f, true))
                                me->SetFacingToObject(l_Nearest);

                            me->m_Events.AddEvent(new basicevent_bombardment_bridge(me, 0), me->m_Events.CalculateTime(3s));
                        }
                        break;
                    }
            ...
            void MovementInform(uint32 /*p_Type*/, uint32 p_Id) override
            {
                if (p_Id == eSkullocMovementInformed::MovementInformedBeginBarrage)
                {
                    ...
                    events.ScheduleEvent(eSkullocEvents::EventCannonBarrageCancel, 1s);
                }
            }
            ...
            void UpdateAI(uint32 const p_Diff) override
            {
                ...
                if (m_BombardmentReady)
                {
                    if (m_CanBeginStoppingaBombardment)
                    {
                        if (Player* l_Nearest = me->SelectNearestPlayer(2.0f))
                        {
                            ...
                            {
                                m_BombardmentReady = false;
                                events.ScheduleEvent(eSkullocEvents::EventCannonBarrageStopAble, 1s);
                            }
                        }
                    }
                }
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    {           
                        ...
                        if (Unit* l_Victim = SelectTarget(SelectTargetMethod::MaxThreat, 0, 150.0f, true))
                        {
                    ...
                    case eSkullocEvents::EventCannonBarrageCancel:
                    {
                        ...
                        events.ScheduleEvent(eSkullocEvents::EventCannonBarrageCancel, 13s);
                        break;
                    }
                    case eSkullocEvents::EventBackdraft:
                    {
                        ...
                        events.ScheduleEvent(eSkullocEvents::EventBackdraft, 30s);
                        break;
                    }
                    default:
                        break;
    ...
    public:
        ...
        struct iron_docks_skulloc_mob_koramarAI : public ScriptedAI
        {
            ...
            {             
                ...
                me->m_Events.AddEvent(new basicevent_pre_boss(me, 0), me->m_Events.CalculateTime(4s));
                events.ScheduleEvent(eKoramarEvents::EventBladestorm, 40s);
                events.ScheduleEvent(eKoramarEvents::EventShatteringBlades, 15s);
                events.ScheduleEvent(eKoramarEvents::EventBerserkerLeap, 20s);
            }
            ...
            void UpdateAI(uint32 const p_Diff) override
            {
                ...
                case eKoramarEvents::EventShatteringBlades:
                    ...
                    if (Unit* l_Target = SelectTarget(SelectTargetMethod::MaxDistance, 0, 100.0f, true))
                        me->CastSpell(l_Target, eKoramarSpells::SpellShatteringBlades);
                    events.ScheduleEvent(eKoramarEvents::EventShatteringBlades, 12s);
                    break;
                case eKoramarEvents::EventBerserkerLeap:
                    me->CastSpell(me, eKoramarSpells::SpellBerserkerLeapDummy);
                    events.ScheduleEvent(eKoramarEvents::EventBerserkerLeap, 8s);
                    break;
                default:
                    break;
                }
    ...
    public:
        ...
        struct iron_docks_skulloc_mob_turretAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(eTurretEvents::EventInstallAccessories, 5s);
            }
            ...
            void JustEngagedWith(Unit* /*p_Who*/) override
            {
                ...
                events.Reset();
                events.ScheduleEvent(eTurretEvents::EventRapidFireChangeTarget, 2s);
            }
                    ...
                    case eIronDocksActions::ActionLeaveTurret:
                    {
                        if (m_Instance != nullptr)
                        {
                            if (Creature* l_Zoggosh = m_Instance->instance->GetCreature(m_Instance->GetGuidData(eIronDocksDatas::DataZuggosh)))
                            {
                                ...
                                l_Zoggosh->RemoveUnitFlag(UnitFlags(UNIT_FLAG_REMOVE_CLIENT_CONTROL | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NON_ATTACKABLE));

                                if (l_Zoggosh->IsAIEnabled())
                                {
                                    if (Unit* l_Target = l_Zoggosh->AI()->SelectTarget(SelectTargetMethod::MaxThreat, 0, 100.0f, true))
                                    {
            ...
            {       
                ...
                switch (events.ExecuteEvent())
                {
                    case eTurretEvents::EventRapidFireChangeTarget:
                    {
                        ...
                        if (Unit* l_Target = SelectTarget(SelectTargetMethod::Random, 0, 60.0f, true))
                        {
                            m_TargetGuid = l_Target->GetGUID();
                            l_Target->AddAura(eTurretSpells::SpellRapidFireTarget, l_Target);
                        }

                        events.ScheduleEvent(eTurretEvents::EventRapidFireChangeTarget, 8s);
                        events.ScheduleEvent(eTurretEvents::EventRapidFire, 1s);
                        break;
                    }
        ...
        class iron_docks_skulloc_spell_berserker_jump_AuraScript : public AuraScript
        {
            void HandlePeriodic(AuraEffect const* /*p_AurEff*/)
            {
                ...
                if (Unit* l_Caster = GetCaster())
                {
                    ...
                    {
                        if (Unit* l_Random = l_Caster->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
