                            ...
                            if (l_Zoggosh->IsAIEnabled() && l_Koramar->IsAIEnabled())
                            {
                                switch (m_Modifier)
                                {
                                    case 0:
                                        ...
                                        l_Zoggosh->m_Events.AddEvent(new basicevent_nokgar_death(l_Zoggosh, 1), l_Zoggosh->m_Events.CalculateTime(8s));
                                        break;
                                    case 1:
                                        l_Koramar->AI()->Talk(eTalks::TalkKoramar01);
                                        l_Koramar->m_Events.AddEvent(new basicevent_nokgar_death(l_Koramar, 2), l_Koramar->m_Events.CalculateTime(12s));
                                        break;
                                    case 2:
...
public:
    ...
    struct boss_nokgarAI : public BossAI
    {
        ...
        void DamageTaken(Unit* p_Attacker, uint32& p_Damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if (p_Damage && p_Damage > 0)
            {
                if (me->HealthBelowPct(50) && !m_DismountHeroic)
                {
                    ...
                    events.ScheduleEvent(eNokgarEvents::EventDismount, 1s);
                }
        ...
        void JustEngagedWith(Unit* p_Who) override
        {
            ...
            StopArchers();
            events.ScheduleEvent(eNokgarEvents::EventLaunchArchers, 10s);
            me->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
            if (!IsHeroic())
                events.ScheduleEvent(eNokgarEvents::EventDismount, 29s);
        }
        ...
        void JustDied(Unit* /*p_Killer*/) override
        {
            if (m_Instance != nullptr)
            {
                ...
                if (Creature* l_Wolf = m_Instance->instance->GetCreature(m_Instance->GetGuidData(eIronDocksDatas::DataMountWolf)))
                    l_Wolf->DespawnOrUnsummon(2s);           
            }

            _JustDied();
            StopArchers();
            Talk(eTalks::SayDeath);
            me->m_Events.AddEvent(new basicevent_nokgar_death(me, 0), me->m_Events.CalculateTime(2s));
        }
        ...
        void UpdateAI(uint32 const p_Diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case eNokgarEvents::EventLaunchArchers:
                    LaunchArchers();
                    events.ScheduleEvent(eNokgarEvents::EventStopArchers, 15s);
                    break;
                case eNokgarEvents::EventStopArchers:
                {
                    ...
                    events.ScheduleEvent(eNokgarEvents::EventLaunchArchers, 20s);
                    break;
                }
                case eNokgarEvents::EventDismount:
                {
                    ...
                    if (Unit* l_Target = SelectTarget(SelectTargetMethod::MaxThreat, 0, 100.0f, true))
                    {
                        ...
                        events.ScheduleEvent(eNokgarEvents::EventRecklessProvocation, 10s);
                        break;
                    }
                }
                case eNokgarEvents::EventRecklessProvocation:
                {
                    ...
                    events.ScheduleEvent(eNokgarEvents::EventRecklessProvocation, 20s);
                }
                default:
                    break;
                }
    ...
    public:
        ...
        struct iron_docks_nokgar_mob_dreadfangAI : public ScriptedAI
        {
            ...
            {             
                ...
                events.ScheduleEvent(eDreadfangEvents::EventInstallAccessories, 3s);
            }
            ...
            void JustEngagedWith(Unit* p_Who) override
            {
                ...
                m_BloodlettingHowlDiff = (12 * TimeConstants::IN_MILLISECONDS);
            ...
            void UpdateAI(uint32 const p_Diff) override
            {
               ...
               // if (me->HasUnitState(UnitState::UNIT_STATE_CASTING) || m_ShreddingStrikes)
                ...
                {
                    if (Unit* l_Target = SelectTarget(SelectTargetMethod::MaxThreat, 0, 50.0f, true))
                    {
                        me->Attack(l_Target, true);
                        me->GetMotionMaster()->MoveChase(l_Target);
                ...
                if (m_ShreddingAttackDiff <= p_Diff)
                {
                    ...
                    me->SetUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC));
                    ...
                    case eDreadfangEvents::EventSavageMauling:
                        {
                            ...
                            events.ScheduleEvent(eDreadfangEvents::EventSavageMauling, urand(8 * TimeConstants::IN_MILLISECONDS, 12s));
                            break;
                        }
                    case eDreadfangEvents::EventBloodlettingHowl:
                        {
                            me->CastSpell(me, eDreadfangSpells::SpellBloodlettingHowl);
                            events.ScheduleEvent(eDreadfangEvents::EventBloodlettingHowl, 25s);
                            break;
                        }
                    case eDreadfangEvents::EventShreddingSwipes:
                        {
                            ...
                            if (Unit* l_Target = SelectTarget(SelectAggroTarget::SELECT_TARGET_FARTHEST, 0, 50.0f, true))
                                me->GetMotionMaster()->MovePoint(eMovementInformed::MovementInformDreadfangShreddingStrikes, l_Target->GetPositionX(), l_Target->GetPositionY(), l_Target->GetPositionZ());
                            events.ScheduleEvent(eDreadfangEvents::EventShreddingSwipes, 50s);            
                            break;
                        }
                        default:
                            break;
        ...
        struct iron_docks_nokgar_mob_flameslingerAI : public ScriptedAI
        {
            ...
            void DoAction(int32 const p_Action) override
            {
                switch (p_Action)
                {
                    ...
                    case eIronDocksActions::ActionFireArrows:
                        ...
                        events.ScheduleEvent(eFlameslingerEvents::EventBurningArrow, 5s);
                        break;
            ...
            void UpdateAI(uint32 const p_Diff) override
            {
                ...
                case eFlameslingerEvents::EventBurningArrow:
                {
                    ...
                    events.ScheduleEvent(eFlameslingerEvents::EventBurningArrow, 2s);

                    if (roll_chance_i(50))
                    events.ScheduleEvent(eFlameslingerEvents::EventBarbedArrows, 5s);
                    break;
                }
