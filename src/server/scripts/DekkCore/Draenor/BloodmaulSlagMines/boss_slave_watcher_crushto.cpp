            ...
            public:
                ...
                struct boss_AI : public BossAI
                {
                    ...
                    void JustEngagedWith(Unit* who) override
                    {
                        ...
                        events.ScheduleEvent(uint32(Events::EarthCrush), 5s);
                        events.ScheduleEvent(uint32(Events::FerociousYell), 10s);
                        events.ScheduleEvent(uint32(Events::RaiseTheMiners), 16s);
                        events.ScheduleEvent(uint32(Events::CrushingLeap), 23s);
                        events.ScheduleEvent(uint32(Events::WildSlam), 27s);
                    ...
                    void UpdateAI(const uint32 diff) override
                    {
                        ...
                        switch (events.ExecuteEvent())
                        {
                            case uint32(Events::EarthCrush):
                            {
                                if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 40.0f))
                                {
                                  ...
                                  //  GetPositionWithDistInOrientation(me, 4.0f, me->GetAngle(target), summonPos);
                                    ...
                                    /// We summon the rock.
                                    TempSummon* summon = me->SummonCreature(uint32(MobEntries::EarthCrushStalker), summonPos, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 4s);
                                    if (summon != nullptr)
                                    {
                                ...
                                events.ScheduleEvent(uint32(Events::EarthCrush), 20s);
                                break;
                            }
                            case uint32(Events::FerociousYell):
                            {
                                ...
                                events.ScheduleEvent(uint32(Events::FerociousYell), 12s);
                                break;
                            }
                            case uint32(Events::RaiseTheMiners):
                            {
                                ...
                                events.ScheduleEvent(uint32(Events::RaiseTheMiners), 29s);
                                break;
                            }
                            ...
                            case uint32(Events::CrushingLeap):
                            {
                                ...
                                events.ScheduleEvent(uint32(Events::CrushingLeap), 20s);
                                break;
                            }
                            default:
                                break;
