...
public:
    ...
    struct boss_nerzulAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(eNerzulEvents::EventOmenOfDeath, 10s);
            events.ScheduleEvent(eNerzulEvents::EventRitualOfSouls, 15s);
            events.ScheduleEvent(eNerzulEvents::EventMalevolance, 8s);
        }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case eNerzulEvents::EventMalevolance:
                        ...
                        events.ScheduleEvent(eNerzulEvents::EventMalevolance, 15s);
                        break;
                    case eNerzulEvents::EventOmenOfDeath:
                        ...
                        events.ScheduleEvent(eNerzulEvents::EventOmenOfDeath, 12s);
                        break;
                            ...
                            case 0: // Right
                            {
                                for (int i = 0; i <= 6; i++)
                                {
                                    me->SummonCreature(eNerzulCreatures::CreatureRitualOfBones, l_InitialPositionRight.GetPositionX() + (m_X * i),
                                        ...
                                        l_InitialPositionRight.GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 40s);
                                }
                                break;
                            }
                            case 1: // Left
                            {
                                for (int i = 0; i <= 6; i++)
                                {
                                    me->SummonCreature(eNerzulCreatures::CreatureRitualOfBones, l_InitialPositionLeft.GetPositionX() + (m_X * i),
                                        ...
                                        l_InitialPositionLeft.GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 40s);
                                }
                                break;
                            }
                        }
        ...
        void DoAction(int32 const action) override
        {
            switch (action)
            {
                case eNerzulActions::ActionNerzulPropIntroduction:
                    if (!m_Intro)
                    {
                        events.ScheduleEvent(eShadowmoonBurialGroundsEvents::EventTalk01, 1s);
                    }
                    break;
            }
        }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case eShadowmoonBurialGroundsEvents::EventTalk01:
                        ...
                        events.ScheduleEvent(eShadowmoonBurialGroundsEvents::EventTalk02, 10s);
                        break;
