...
public:
    ...
    struct boss_nhalishAI : public BossAI
    {
        ...
        void JustReachedHome() override
        {
            ...
            {
                PhasingHandler::RemovePhase(itr, 50, true);
            }
        }
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(eNhalishEvents::EventPlanarShift, 15s);
            events.ScheduleEvent(eNhalishEvents::EventSoulSteal, 70s);
            events.ScheduleEvent(eNhalishEvents::EventVoidBlast, 6s);
            events.ScheduleEvent(eNhalishEvents::EventExhumeCrypt, 10s);
        }
        ...
        void KilledUnit(Unit* who) override
        {
            if (who->GetTypeId() == TypeID::TYPEID_PLAYER)
            {
                ...
                PhasingHandler::RemovePhase(who, 50, true); // Sets dead target back to normal phase.
            }
        }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            for (int i = 0; i < 10; i++)
            {
               Creature* creature = me->SummonCreature(eNhalishCreatures::CreatureDistrubedSouls, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 10s);
        ...
        void UpdateAI(uint32 const diff) override
        {
            ...
            if (m_Vortex)
            {
                if (m_VortexDiff <= diff)
                {
                    ...
                    for (auto itr : l_ListPlayers)
                    {
                        ...
                        {
                            if (itr->IsAlive() && !itr->GetMovementForces())
                                itr->ApplyMovementForce(me->GetGUID(), position, 3.0f, MovementForceType::SingleDirectional);
                            else if (!itr->IsAlive() && itr->GetMovementForces())
                                itr->RemoveMovementForce(me->GetGUID());
                        }
                        else if (itr->GetMovementForces())
                            itr->RemoveMovementForce(me->GetGUID());
                    }
            ...
            switch (events.ExecuteEvent())
            {
                case eNhalishEvents::EventExhumeCrypt:
                    ...
                    events.ScheduleEvent(eNhalishEvents::EventExhumeCrypt, 15s);
                    break;
                case eNhalishEvents::EventPlanarShift:
                    ...
                    {
                        case 0:
                            events.ScheduleEvent(eNhalishEvents::EventVoidDevastation, 5s);
                            break;
                        case 1:
                            events.ScheduleEvent(eNhalishEvents::EventVoidVortex, 5s);
                            break;
                    }

                    events.ScheduleEvent(eNhalishEvents::EventPlanarShift, 30s);
                    break;
                case eNhalishEvents::EventVoidDevastation:
                    ...
                    events.ScheduleEvent(eNhalishEvents::EventVoidDevastation, 22s);
                    break;
                case eNhalishEvents::EventSoulSteal:
                    ...
                    events.ScheduleEvent(eNhalishEvents::EventSoulSteal, 65s);
                    break;
                case eNhalishEvents::EventVoidBlast:
                    if (Unit* target = me->GetVictim())
                        me->CastSpell(target, eNhalishSpells::SpellVoidBlastSpellAuraDummy);

                    events.ScheduleEvent(eNhalishEvents::EventVoidBlast, 12s);
                    break;
                case eNhalishEvents::EventVoidVortex:
                    ...
                    events.ScheduleEvent(eNhalishEvents::EventVoidVortex, 35s);
                    break;
            }
...
//    //                    }
//
//    //                    PhasingHandler::AddPhase(victim, 50, true);
//
...
//        //    }
//
//       /*     events.ScheduleEvent(eNhalishEvents::EventRandomMovement, 1s);*/
//       // }
//
//        void JustDied(Unit* /*killer*/) override
//        {
...
//            me->SetDynamicFlag(UnitDynFlags::UNIT_DYNFLAG_DEAD);
//            me->SetUnitFlag(UNIT_FLAG_REMOVE_CLIENT_CONTROL);
    ...
    struct shadowmoon_burial_grounds_creaturesAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 const diff) override
        {
            ...
            if (m_Timer <= diff)
            {
                ...
                PL_list = me->SelectNearestPlayers(2.0f, true);
...
public:
    ...
    class spell_shadowmoon_burial_grounds_void_devestation_trigger_SpellScript : public SpellScript
    {
        ...
        void HandleTriggerMissile(SpellEffIndex effectIndex)
        {
           ...
           GetCaster()->SummonCreature(eNhalishCreatures::CreatureDevestationTrigger, l_WorldLocation->GetPositionX(), l_WorldLocation->GetPositionY(), l_WorldLocation->GetPositionZ(), l_WorldLocation->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN);
        }
    ...
    class spell_shadowmoon_burial_grounds_void_blast_SpellScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                {
                    if (Unit* target = caster->GetAI()->SelectTarget(SelectTargetMethod::MaxThreat))
                    {
    ...
    class spell_shadowmoon_burial_grounds_soul_steal_SpellScript : public SpellScript
    {
        ...
        void HandleForceCast(SpellEffIndex /*effIndex*/)
        {
            ...
            if (InstanceScript* m_Instance = GetCaster()->GetInstanceScript())
            {
                if (Creature* l_Nhalish = m_Instance->instance->GetCreature(m_Instance->GetGuidData(eShadowmoonBurialGroundsDatas::DataBossNhallish)))
                {
                    if (Creature* Soul = GetCaster()->SummonCreature(eNhalishCreatures::CreatureSoul, GetHitUnit()->GetPosition(), TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 20s, 0, true))
                    {
                        if (boss_nhalish::boss_nhalishAI* linkAI = CAST_AI(boss_nhalish::boss_nhalishAI, l_Nhalish->GetAI()))
                        {
                            ...
                            PhasingHandler::AddPhase(GetHitUnit(), m_phaseId, true);
                            PhasingHandler::AddPhase(Soul, m_phaseId, true);
...
public:
    ...
    class spell_shadowmoon_burial_grounds_teleport_SpellScript : public SpellScript
    {
        ...
        void HandleAfterCast()
        {
           ...
           // if (Unit* target = GetCaster()->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true))
