...
public:
    ...
    struct npc_carrion_wormAI : public ScriptedAI
    {
        npc_carrion_wormAI(Creature* creature) : ScriptedAI(creature){ }
            ...
            switch (scenePhase)
            {
                case 1:
                {
                    ...
                    for (Map::PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
                    {
                        ...
                        player->GetSceneMgr().PlaySceneByPackageId(SCENE_NERZHUL, SceneFlag::None);
