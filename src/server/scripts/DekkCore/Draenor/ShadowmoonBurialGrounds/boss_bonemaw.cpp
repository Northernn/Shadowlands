...
public:
    ...
    struct boss_bonemawAI : public BossAI
    {
        ...
        void Reset() override
        {
          ...
          //  me->SetUnitFlag(UNIT_FLAG2_DISABLE_TURN);
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.ScheduleEvent(eBoneMawEvents::EventNecroticPitch, 50ms);
            events.ScheduleEvent(eBoneMawEvents::EventBodySlam, 30ms);
            events.ScheduleEvent(eBoneMawEvents::EventCorpseBreath, 20ms);
            events.ScheduleEvent(eBoneMawEvents::EventInhale, 60ms);
            events.ScheduleEvent(eBoneMawEvents::EventFetidSpit, 10ms);
            events.ScheduleEvent(eBoneMawEvents::EventSubmerge, 70s);

            if (me->GetMap()->IsHeroic())
                events.ScheduleEvent(eBoneMawEvents::EventCarrionWorm, 70ms);
        }
        ...
        void UpdateAI(uint32 const diff) override
        {
                        ...
                        if (Creature* l_BonemawMouth = m_Instance->GetCreature(DataBonemawMouth))
                        {
                            ...
                            if (itr->IsWithinDist(l_BonemawMouth, 100.0f, true))
                            {
                                if (itr->IsAlive() && !itr->GetMovementForces())
                                    itr->ApplyMovementForce(l_BonemawMouth->GetGUID(), position, 3.0f, MovementForceType::SingleDirectional);
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case eBoneMawEvents::EventSubmerge:
                    ...
                    events.ScheduleEvent(eBoneMawEvents::EventCancelSubmerge, 8s);
                    break;
                case eBoneMawEvents::EventBodySlam:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                        me->CastSpell(target, eBoneMawSpells::SpellBodySlam);

                    events.ScheduleEvent(eBoneMawEvents::EventBodySlam, 20s);
                    break;
                case eBoneMawEvents::EventCarrionWorm:
                {
                    ...
                    events.ScheduleEvent(eBoneMawEvents::EventCancelSubmerge, 3s);
                    break;
                }
                case eBoneMawEvents::EventCancelSubmerge:
                {
                    ...
                    events.ScheduleEvent(eBoneMawEvents::EventNecroticPitch, 50s);
                    events.ScheduleEvent(eBoneMawEvents::EventBodySlam, 30s);
                ...
                case eBoneMawEvents::EventCorpseBreath:
                {
                    ...
                    events.ScheduleEvent(eBoneMawEvents::EventCorpseBreath, 15s);
                    break;
                }
                case eBoneMawEvents::EventInhale:
                {
                    ...
                    events.ScheduleEvent(eBoneMawEvents::EventInhale, 55s);
                    break;
                }
                case eBoneMawEvents::EventNecroticPitch:
                {
                    ...
                    events.ScheduleEvent(eBoneMawEvents::EventNecroticPitch, 25s);
                    break;
                }
            }
...
public:
    ...
    struct npc_shadowmoon_burial_grounds_necrotic_pitchAI : public ScriptedAI
    {
        npc_shadowmoon_burial_grounds_necrotic_pitchAI(Creature* creature) : ScriptedAI(creature)
        {
...
public:
    ...
    class spell_shadowmoon_burial_grounds_necrotic_pitch_SpellScript : public SpellScript
    {
        void HandleTriggerMissile(SpellEffIndex effectIndex)
        {
            ...
            GetCaster()->SummonCreature(eBoneMawCreatures::CreatureNecroticPitchTrigger, l_WorldLocation->GetPositionX(), l_WorldLocation->GetPositionY(), l_WorldLocation->GetPositionZ(), l_WorldLocation->GetOrientation(), TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 60s);
        }
