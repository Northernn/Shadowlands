    ...
    void DoAction(int32 action) override
    {
        switch (action)
        {
            case ACTION_FIRST_SPITTER_PLANTED:
                ...
                _events.ScheduleEvent(EVENT_CREATE_ZOMBIE, 1s);
            ...
            case ACTION_QUEST_FAILED:
                ...
                _events.Reset();
                _events.ScheduleEvent(EVENT_EJECT_PASSENGER, 2s + 1s);
                break;
            default:
                break;
        }
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_SUMMON_SOLAR_POWER:
                {
                    ...
                    if (Player* player = GetPlayer())
                        player->CastSpell(Position(pos.GetPositionX() + cos(angle) * 3, pos.GetPositionY() + sin(angle) * 3, pos.GetPositionZ() + 50.0f), SPELL_CREATE_RANDOM_SUN_POWER, true);
                ...
                case EVENT_CREATE_GHOUL:
                {
                    ...
                    if (Player* player = GetPlayer())
                        player->CastSpell(Position(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), angle), spellId, true);
                    break;
                }
...
private:
    ...
    void SetupSpawns()
    {
        ...
        for (uint8 i = 0; i < MAX_TARGET_POSITIONS; i++)
        {
            ...
            charmer->CastSpell(Position(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), LawnmowerOrientation), SPELL_CREATE_LAWMOWER, true);

            pos = GoalStalkerPositions[i];
            charmer->CastSpell(Position(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), GoalStalkerOrientation), SPELL_CREATE_GOAL_STALKER, true);

            pos = SpitterTargetPositions[i];
            charmer->CastSpell(Position(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), SpitterTargetOrientation), SPELL_CREATE_SPITTER_TARGET, true);
        }

        for (uint8 i = 0; i < MAX_EMPTY_SPOT_POSITIONS; i++)
        {
            Position pos = EmptySpotPositions[i];
            charmer->CastSpell(Position(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), EmptySpotOrientation), SPELL_CREATE_EMPTY_SPOT, true);
        }
    }
