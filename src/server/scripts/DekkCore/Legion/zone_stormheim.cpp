...
public:
    ...
    struct go_volatile_barrel_stormheimAI : public GameObjectAI
    {
        go_volatile_barrel_stormheimAI(GameObject* go) : GameObjectAI(go) { }
    ...
    {
        return new go_volatile_barrel_stormheimAI(go);
    }
};
...
public:
    ...
    {
        return new npc_grapple_pointAI(creature);
    }

    struct npc_grapple_pointAI : public ScriptedAI
    {
        npc_grapple_pointAI(Creature* creature) : ScriptedAI(creature) { }

        bool OnGossipHello(Player* player) override
        {
            if (player->GetQuestStatus(38613) == QUEST_STATUS_INCOMPLETE)
            {
                player->CastSpell(me, 182467, true);
                player->GetMotionMaster()->MoveJump(me->GetPosition(), 15.0f, 15.0f, 0, false);
    ...
    class spell_transponder_pack_180645_AuraScript : public AuraScript
    {
        void OnPeriodic(AuraEffect const* aurEff)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                {
                    if (caster->GetPower(POWER_ALTERNATE_POWER) > caster->ToPlayer()->GetQuestObjectiveProgress(38058, 1))
                        for (int32 i = 0; i < (std::min(caster->GetPower(POWER_ALTERNATE_POWER), 100) - caster->ToPlayer()->GetQuestObjectiveProgress(38058, 1)); ++i)
...
public:
    ...
    struct go_powered_console_stormheim_243802AI : public GameObjectAI
    {
        go_powered_console_stormheim_243802AI(GameObject* go) : GameObjectAI(go) { }
        ...
        bool OnGossipHello(Player* player) override
        {
            if (Creature* drugen = player->SummonCreature(96067, Position(3333.486f, 2230.95f, 315.7818f, 4.712389f), TEMPSUMMON_MANUAL_DESPAWN))
            {
                ...
                {
                    drugen->AddAura(190758, drugen);
                    drugen->AddAura(190549, drugen);

                    npc->CastSpell(npc, 190715, true);
                    drugen->DespawnOrUnsummon(30s);
                    npc->DespawnOrUnsummon(30s);
                }
            }
    ...
    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_powered_console_stormheim_243802AI(go);
    }
};
//243817
...
public:
    ...
    struct go_powered_console_stormheim_243817AI : public GameObjectAI
    {
        go_powered_console_stormheim_243817AI(GameObject* go) : GameObjectAI(go) { }
    ...
    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_powered_console_stormheim_243817AI(go);
    }
};
            ...
            if (summoner->ToPlayer()->HasQuest(38624) || summoner->ToPlayer()->HasQuest(41950) || summoner->ToPlayer()->HasQuest(41451))
            {
                ...
                summoner->GetScheduler().Schedule(Milliseconds(30000), [this, summoner](TaskContext context)
                {
                    if (Player* player = ObjectAccessor::GetPlayer(*me, _playerGuid))
                    {
                        ...
                        if (player->HasQuest(38624) && player->GetQuestObjectiveProgress(38624, 3) == 150)
                        {
                        ...
                        else if (player->HasQuest(41950) && player->GetQuestObjectiveProgress(41950, 3) == 50)
                        {
                            player->KilledMonsterCredit(91768);
                            Talk(2);
                            events.Reset();
                        ...
                        else if (player->HasQuest(41451) && player->GetQuestObjectiveProgress(41451, 0) == 15 && player->GetQuestObjectiveProgress(41451, 2) == 4)
                        {
                            events.Reset();
                            context.CancelAll();
                            me->GetMotionMaster()->Clear();
...
public:
    ...
    struct go_tideskorn_harpoon_launcher_251257AI : public GameObjectAI
    {
        go_tideskorn_harpoon_launcher_251257AI(GameObject* go) : GameObjectAI(go) { }
    ...
    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_tideskorn_harpoon_launcher_251257AI(go);
    }
};
...
public:
    ...
    struct go_rune_of_reformation_244450AI : public GameObjectAI
    {
        go_rune_of_reformation_244450AI(GameObject* go) : GameObjectAI(go) { }
    ...
    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_rune_of_reformation_244450AI(go);
    }
};
...
public:
    ...
    struct go_helyas_altar_244559AI : public GameObjectAI
    {
        go_helyas_altar_244559AI(GameObject* go) : GameObjectAI(go) { }
    ...
    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_helyas_altar_244559AI(go);
    }
};
//97480 97558
...
struct go_engraved_shield_quest_38878 : public GameObjectScript
{
public:
    go_engraved_shield_quest_38878() : GameObjectScript("go_engraved_shield_quest_38878") { }

    struct go_engraved_shield_quest_38878AI : public GameObjectAI
    {
        go_engraved_shield_quest_38878AI(GameObject* go) : GameObjectAI(go) { }
    ...
    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_engraved_shield_quest_38878AI(go);
    }
};
