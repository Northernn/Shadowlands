...
struct boss_the_amalgam_of_souls : public BossAI
{
    ...
    void DefaultEvents()
    {
        events.RescheduleEvent(EVENT_SWIRLING_SCYTHE, 8s);
        events.RescheduleEvent(EVENT_SOUL_ECHOES, 14s);
        events.RescheduleEvent(EVENT_REAP_SOUL, 20s);
    }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        if (me->HealthBelowPct(healthPct))
        {
            ...
            if (me->GetMap()->GetDifficultyID() != DIFFICULTY_LFR && me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
            {
                me->AttackStop();
                events.Reset();
                events.RescheduleEvent(EVENT_MOVE_CENTR_POS, 10s);
            }
        }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        events.RescheduleEvent(EVENT_CALL_SOULS, 10s);
    }
    ...
    void SpellHit(WorldObject* caster, const SpellInfo* spell) override
    {
        if (spell->Id == SPELL_CALL_SOULS)
        {
            ...
            soulTimer = 35000;
            events.RescheduleEvent(EVENT_CHECK_SOULS, 20s);
        }
    }
   ...
   void OnSpellCast(SpellInfo const* spell) override
    {
        ...
        if (spell->Id == SPELL_CALL_SOULS)
            events.RescheduleEvent(EVENT_REMOVE_SOULS, 5s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SWIRLING_SCYTHE:
                Talk(SAY_SCYTHE);
                if (auto pTarget = SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true, 0))
                    DoCast(pTarget, SPELL_SWIRLING_SCYTHE);
                else if (auto pTarget = SelectTarget(SelectTargetMethod::Random, 0, 60.0f, true))
                    DoCast(pTarget, SPELL_SWIRLING_SCYTHE);
                events.RescheduleEvent(EVENT_SWIRLING_SCYTHE, 20s);
                break;
            case EVENT_SOUL_ECHOES:
                ...
                events.RescheduleEvent(EVENT_SOUL_ECHOES, 28s);
                break;
            case EVENT_REAP_SOUL:
                ...
                me->SetReactState(REACT_AGGRESSIVE);
                events.RescheduleEvent(EVENT_REAP_SOUL, 13s);
                break;
...
struct npc_soul_echoes_stalker : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        events.RescheduleEvent(EVENT_1, 40s);
    }
