...
struct boss_latosius : public BossAI
{
    ...
    void JustEngagedWith(Unit* u)  //override
    {
        ...
        if (auto kurtalos = me->FindNearestCreature(NPC_KURTALOS, 30.0f))
            kurtalos->AI()->DoZoneInCombat(kurtalos);

        events.RescheduleEvent(EVENT_LATOSIUS_TP, 1s);
        events.RescheduleEvent(EVENT_DARK_BLAST, 10s);
    }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (!players.isEmpty())
        {
            ...
            instance->DoUpdateCriteria(CriteriaType::DefeatDungeonEncounter, 1835);
        }
        if (CheckAura())
        {
    ...
    void DoAction(int32 const action) override
    {
        if (action == ACTION_1 && instance->GetData(DATA_KURTALOS_STATE) != PHASE_2)
        {
            ...
            events.Reset();
            events.RescheduleEvent(EVENT_LATOSIUS_TP_2, 10s);
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_LATOSIUS_TP:
                ...
                events.RescheduleEvent(EVENT_SHADOW_BOLT, 5s);
                break;
            case EVENT_SHADOW_BOLT:
                DoCast(SPELL_SHADOW_BOLT);
                events.RescheduleEvent(EVENT_SHADOW_BOLT, 30s);
                break;
            case EVENT_DARK_BLAST:
                ...
                events.RescheduleEvent(EVENT_LATOSIUS_TP, 5s);
                events.RescheduleEvent(EVENT_DARK_BLAST, 18s);
                break;
            case EVENT_LATOSIUS_TP_2:
                DoCast(me, SPELL_TELEPORT_2, true);
                DoCast(me, SPELL_CONVERSATION , true);
                events.RescheduleEvent(EVENT_SAP_SOUL, 10s);
                break;
            case EVENT_SAP_SOUL:
            ...
            case EVENT_TRANSFORM:
                ...
                events.RescheduleEvent(EVENT_SHADOW_BOLT_VOLLEY, 14s);
                events.RescheduleEvent(EVENT_CLOUD_OF_HYPNOSIS, 26s);
                events.RescheduleEvent(EVENT_DREADLORDS_GUILE, 35s);
                if (me->GetMap()->GetDifficultyID() != DIFFICULTY_LFR && me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
                    events.RescheduleEvent(EVENT_STINGING_SWARM, 19s);
                break;
            case EVENT_SHADOW_BOLT_VOLLEY:
                DoCast(SPELL_SHADOW_BOLT_VOLLEY);
                events.RescheduleEvent(EVENT_SHADOW_BOLT_VOLLEY, 8s);
                break;
            case EVENT_CLOUD_OF_HYPNOSIS:
                Talk(SAY_CLOUD_OF_HYPNOSIS);
                if (auto pTarget = SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true))
                    DoCast(pTarget, SPELL_CLOUD_OF_HYPNOSIS);
                events.RescheduleEvent(EVENT_CLOUD_OF_HYPNOSIS, 32s);
                break;
            case EVENT_DREADLORDS_GUILE:
                ...
                for (auto id : { EVENT_SHADOW_BOLT_VOLLEY, EVENT_CLOUD_OF_HYPNOSIS, EVENT_STINGING_SWARM })
                    events.RescheduleEvent(id, 26s, true);
                events.RescheduleEvent(EVENT_DREADLORDS_GUILE, 8s);
                events.RescheduleEvent(EVENT_SUM_IMAGE, 4s);
                break;
            case EVENT_SUM_IMAGE:
                me->SummonCreature(NPC_IMAGE_OF_LATOSIUS, tpPos[imageSumCount], TEMPSUMMON_TIMED_DESPAWN, 4s);
                if (++imageSumCount > 10)
                {
                    events.RescheduleEvent(EVENT_IMAGE_END, 3s);
                    break;
                }
                events.RescheduleEvent(EVENT_SUM_IMAGE, 16s);
                break;
            ...
            case EVENT_STINGING_SWARM:
                if (auto target = SelectTarget(SelectTargetMethod::Random, 0, 90.0f, true, true, -SPELL_STINGING_SWARM))
                    DoCast(target, SPELL_STINGING_SWARM);
                events.RescheduleEvent(EVENT_STINGING_SWARM, 17s);
                break;
            }
        }
...
struct npc_kurtalos_ravencrest : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* u) override
    {
        ...
        if (me->GetOwner())
            if (auto latosius = me->GetOwner()->ToCreature())
                latosius->AI()->DoZoneInCombat(latosius);

        events.RescheduleEvent(EVENT_UNERRING_SHEAR, 6s);
        events.RescheduleEvent(EVENT_WHIRLING_BLADE, 10s);
    }
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        if (spell->Id == SPELL_WHIRLING_BLADE)
            if (auto blade = me->SummonCreature(NPC_KURTALOS_BLADE_TRIGGER, me->GetPosition(), TEMPSUMMON_TIMED_DESPAWN, 30s))
            {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_UNERRING_SHEAR:
                ...
                events.RescheduleEvent(EVENT_UNERRING_SHEAR, 13s);
                break;
            case EVENT_WHIRLING_BLADE:
                if (Unit* pTarget = SelectTarget(SelectTargetMethod::Random, 0, 40.0f, true))
...
struct npc_kurtalos_trigger : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        case NPC_SOUL_KURTALOS:
            ...
            events.RescheduleEvent(EVENT_3, 24s);
            break;
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        if (id == 1)
            events.RescheduleEvent(EVENT_1, 20s);
...
void AddSC_boss_lord_kurtalos_ravencrest()
{
    ...
    RegisterSpellScript(spell_kurtalos_stinging_swarm);
}
