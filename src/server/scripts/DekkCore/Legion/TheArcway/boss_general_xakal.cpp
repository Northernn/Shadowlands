...
public:
    ...
    struct boss_general_xakalAI : public BossAI
    {
            ...
            //37:50(N) | 58:24(H)
        {
            ...
            events.RescheduleEvent(EVENT_FEL_FISSURE, 6s); //37:56
            events.RescheduleEvent(EVENT_SHADOW_SLASH, 13s); //38:03
            events.RescheduleEvent(EVENT_WICKED_SLAM, 40s); //38:30
            events.RescheduleEvent(EVENT_SUM_FELBAT, 11s); //38:01, 38:23
        }
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            ...
            case SPELL_FEL_FISSURE:
                DoCast(target->ToUnit(), SPELL_FEL_FISSURE_SUM, true);
                break;
            case SPELL_SHADOW_SLASH:
                Position pos;
                angle = me->GetRelativeAngle(target);
                me->GetNearPosition(1.0f, angle);
                DoCast(target->ToUnit(), SPELL_SHADOW_SLASH_AT);
                break;
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FEL_FISSURE:
                    ...
                    Talk(SAY_FEL);
                    events.RescheduleEvent(EVENT_FEL_FISSURE, 14s);
                    break;
                case EVENT_SHADOW_SLASH:
                    shadowDist = 0;
                    shadowTimer = 500;
                    DoCast(SPELL_SHADOW_SLASH);
                    events.RescheduleEvent(EVENT_SHADOW_SLASH, 14s);
                    break;
                case EVENT_WICKED_SLAM:
                    Talk(SAY_WICKED);
                    Talk(SAY_WICKED_TEXT);
                    DoCast(SPELL_WICKED_SLAM);
                    events.RescheduleEvent(EVENT_WICKED_SLAM, 40s);
                    break;
                case EVENT_SUM_FELBAT:
                {
                    ...
                    events.RescheduleEvent(EVENT_SUM_FELBAT, 22s);
                    break;
                }
                }
            }
...
public:
    ...
    class spell_xakal_bombardment_AuraScript : public AuraScript
    {
        void OnPeriodic(AuraEffect const* aurEff)
        {
            ...
            if (Unit* target = caster->AI()->SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
