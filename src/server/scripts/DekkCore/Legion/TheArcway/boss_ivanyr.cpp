...
public:
    ...
    struct boss_ivanyrAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_VOLATILE_MAGIC, 8s);
            events.RescheduleEvent(EVENT_OVERCHARGE_MANA, 28s);
            events.RescheduleEvent(EVENT_NETHER_LINK, 56s);
            events.RescheduleEvent(EVENT_WITHERING_CONSUMPTION, 2s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_VOLATILE_MAGIC:
                    ...
                    Talk(SAY_MAGIC);
                    events.RescheduleEvent(EVENT_VOLATILE_MAGIC, 24s);
                    break;
                case EVENT_OVERCHARGE_MANA:
                    me->AttackStop();
                    me->GetMotionMaster()->MovePoint(1, me->GetHomePosition());
                    events.RescheduleEvent(EVENT_OVERCHARGE_MANA, 40s);
                    break;
                ...
                case EVENT_WITHERING_CONSUMPTION:
                    if (me->GetPower(POWER_MANA) > 0)
                        events.RescheduleEvent(EVENT_NETHER_LINK, 2s);
                    else
                    {
                        events.Reset();
                        DoCast(SPELL_WITHERING_CONSUMPTION);
                        events.RescheduleEvent(EVENT_CONSUME_ESSENCE, 8s);
                    }
                    break;
                case EVENT_CONSUME_ESSENCE:
                    DoCast(SPELL_CONSUME_ESSENCE);
                    events.RescheduleEvent(EVENT_CONSUME_ESSENCE, 16s);
                    break;
                }
            }
