...
public:
    ...
    struct boss_advisor_vandrosAI : public BossAI
    {
        ...
        void DefaultEvents()
        {
            events.RescheduleEvent(EVENT_A_BLAST, 3s);
            events.RescheduleEvent(EVENT_CHRONO_SHARDS, 6s);
            events.RescheduleEvent(EVENT_FORCE_BOMB, 16s);
        }
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_FORCE_BOMB)
                DoCast(target->ToUnit(), SPELL_FORCE_BOMB_AT, true);
        }
    ...
    void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if (me->HealthBelowPct(50) && !specialPhase)
            {
                ...
                events.Reset();
                me->AttackStop();
                events.RescheduleEvent(EVENT_BANISH_IN_TIME, 5s);
         ...
         void OnSpellCast(SpellInfo const* spell) override
        {
            ...
            case SPELL_BANISH_IN_TIME_REMOVE:
            {
                if (!specialPhaseComplete)
                {
                    ...
                    Talk(SAY_TIME_REM);
                    DefaultEvents();
                    events.RescheduleEvent(EVENT_UNSTABLE_MANA, 3s);
                }
                break;
            }
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_A_BLAST:
                    ...
                    events.RescheduleEvent(EVENT_A_BLAST, 6s);
                    break;
                ...
                case EVENT_UNSTABLE_MANA:
                    ...
                    events.RescheduleEvent(EVENT_UNSTABLE_MANA, 30s);
                    break;
                }
            }
...
public:
    ...
    struct npc_unstable_amalgamationAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            events.RescheduleEvent(EVENT_1, 30s); //193942
            if (me->GetEntry() == 98425)
                events.RescheduleEvent(EVENT_2, 15s); //193938
        }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            for (uint8 i = 0; i < 4; i++)
            {
                if (Unit* summon = me->SummonCreature(AddSum, x, y, z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s))
                {
                    float angle = i * static_cast<float>(M_PI);
                    Position posnear = me->GetNearPosition(5.0f, angle);
                    summon->CastSpell(posnear, 193852, true);
                }
            }
        }
