...
public:
    ...
    struct boss_naltiraAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_BLINK_STRIKES_1, 15s);
            events.RescheduleEvent(EVENT_NETHER_VENOM, 26s);
            events.RescheduleEvent(EVENT_TANGLED_WEB, 32s);

            if (me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
                events.RescheduleEvent(EVENT_SUM_MANAFANG, 25s);
        }
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            ...
            case SPELL_BLINK_STRIKES_TP:
                ...
                events.RescheduleEvent(EVENT_BLINK_STRIKES_2, 5s);
                break;
            case SPELL_TANGLED_WEB_DMG:
                if (checkWebPlayer)
                {
                    checkWebPlayer = false;
                    plrWebTarget = target->ToUnit();
                }
                else
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/)
        {
            if (summon->GetEntry() == NPC_ENCHANTED_BROODLING_I || summon->GetEntry() == NPC_VICIOUS_MANAFANG_I)
            {
                ...
                case 27:
                {
                    ...
                    {
                        me->GetClosePoint(x, y, z, me->GetCombatReach(), radius, angle);
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_BLINK_STRIKES_1:
                    ...
                    if (!blinkEvent)
                    {
                        ...
                        events.RescheduleEvent(EVENT_BLINK_STRIKES_1, 3s);
                    }
                    else
                    {
                        blinkEvent = false;
                        me->SetReactState(REACT_AGGRESSIVE);
                        events.RescheduleEvent(EVENT_BLINK_STRIKES_1, 30s);
                    }
                    break;
                ...
                case EVENT_NETHER_VENOM:
                    ...
                    events.RescheduleEvent(EVENT_NETHER_VENOM, 30s);
                    break;
