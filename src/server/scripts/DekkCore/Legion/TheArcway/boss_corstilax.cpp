...
public:
    ...
    struct boss_corstilaxAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_PROTOCOL, 5s);
            events.RescheduleEvent(EVENT_QUARANTINE, 20s);
            events.RescheduleEvent(EVENT_CLEANSING_FORCE, 28s); 
            events.RescheduleEvent(EVENT_ENERGY_BURST, 18s);
            events.RescheduleEvent(EVENT_DESTABILIZED_ORB, 13s);
        }
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            switch (spell->Id)
            {
                case SPELL_SUPPRESSION_PROTOCOL:
                {
                    ...
                    if (Creature* protocol = me->SummonCreature(NPC_SUPPRESSION_PROTOCOL, x, y, z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 12s))
                    {
                ...
                case SPELL_DESTABILIZED_ORB:
                {
                    ...
                    float y = target->GetPositionY();
                    float z = target->GetPositionZ();
                    if (Creature* orb = me->SummonCreature(NPC_DESTABLIZED_ORB, x, y, z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 1min))
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_PROTOCOL:
                        ...
                        events.RescheduleEvent(EVENT_PROTOCOL, 36s);
                        break;
                    case EVENT_QUARANTINE:
                        DoCast(SPELL_QUARANTINE);
                        events.RescheduleEvent(EVENT_QUARANTINE, 32s);
                        break;
                    case EVENT_CLEANSING_FORCE:
                        DoCast(SPELL_CLEANSING_FORCE);
                        events.RescheduleEvent(EVENT_CLEANSING_FORCE, 32s);
                        break;
                    case EVENT_ENERGY_BURST:
                        if (Creature* pipe = me->SummonCreature(NPC_PIPE_STALKER, 3101.87f, 4908.10f, 622.39f, 5.86f, TEMPSUMMON_TIMED_DESPAWN, 10s))
                        {
                            ...
                            for(uint8 i = 0; i < 10; i++)
                            {
                                ...
                                pipe->CastSpell(Position(x, y, 617.63f), SPELL_ENERGY_BURST_AT, TriggerCastFlags(TRIGGERED_IGNORE_GCD));
                            }
                        }
                        events.RescheduleEvent(EVENT_ENERGY_BURST, 32s);
                        break;
                    case EVENT_DESTABILIZED_ORB:
                        DoCast(SPELL_DESTABILIZED_ORB);
                        events.RescheduleEvent(EVENT_DESTABILIZED_ORB, 32s);
                        break;
                }
            }
