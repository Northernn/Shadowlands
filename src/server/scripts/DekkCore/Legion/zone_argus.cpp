...
struct boss_mistress_alluradel : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.RescheduleEvent(EVENT_HEART_BREAKER, 40s);
        events.RescheduleEvent(EVENT_FEL_LASH, 15s);
        events.RescheduleEvent(EVENT_BEGUILING_CHARM, 29s);
        events.RescheduleEvent(EVENT_PHANTASM, 33s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_HEART_BREAKER:
                ...
                events.RescheduleEvent(EVENT_HEART_BREAKER, 22s);
                break;
            case EVENT_FEL_LASH:
                DoCast(SPELL_FEL_LASH);
                events.RescheduleEvent(EVENT_FEL_LASH, 32s);
                break;
            case EVENT_BEGUILING_CHARM:
               // ZoneTalk(1, 0);
                DoCast(SPELL_BEGUILING_CHARM);
                events.RescheduleEvent(EVENT_BEGUILING_CHARM, 40s);
                break;
            default:
                break;
            }
...
struct boss_pit_lor_vilemus : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.RescheduleEvent(EVENT_FEL_BREATH, 20s);
        events.RescheduleEvent(EVENT_STOMP, 13s);
        events.RescheduleEvent(EVENT_DRAIN, 22s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_FEL_BREATH:
                ...
                events.RescheduleEvent(EVENT_FEL_BREATH, 19s);
                break;
            case EVENT_STOMP:
                DoCast(SPELL_STOMP);
                events.RescheduleEvent(EVENT_STOMP, 19s);
                break;
            case EVENT_DRAIN:
                DoCast(SPELL_DRAIN);
                events.RescheduleEvent(EVENT_DRAIN, 15s);
                break;
            default:
                break;
            }
...
struct boss_occularus : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.RescheduleEvent(EVENT_SEARING_GAZE, 7s);
        events.RescheduleEvent(EVENT_GUSHING_WOUND, 10s);
        events.RescheduleEvent(EVENT_LASH, 45s);
        events.RescheduleEvent(EVENT_EYE_SORE, 24s);
        events.RescheduleEvent(EVENT_PHANTASM, 27s);
    }
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
    {
        if (spell->Id == SPELL_PHANTASM)
        {
            ...
            {
                dist += 2.0f;
                Position pos = target->GetNearPosition(dist, 0.0f);
                me->CastSpell(pos, SPELL_PHANTASM_TRIGGER, true);
            }
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SEARING_GAZE:
                ...
                events.RescheduleEvent(EVENT_SEARING_GAZE, 10s);
                break;
            case EVENT_GUSHING_WOUND:
                if (auto target = me->GetVictim())
                    DoCast(target, SPELL_GUSHING_WOUND, false);
                events.RescheduleEvent(EVENT_GUSHING_WOUND, 11s);
            case EVENT_LASH:
                if (auto target = me->GetVictim())
                    DoCast(target, SPELL_LASH, false);
                events.RescheduleEvent(EVENT_LASH, 18s);
                break;
            case EVENT_EYE_SORE:
            ...
            case EVENT_PHANTASM:
                ...
                events.RescheduleEvent(EVENT_PHANTASM, 39s);
                break;
            default:
                break;
            }
...
struct boss_inquisitor_meto : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.RescheduleEvent(EVENT_REAP, 8s);
        events.RescheduleEvent(EVENT_SOW, 5s);
        events.RescheduleEvent(EVENT_DEATH_FIELD, 12s);
        events.RescheduleEvent(EVENT_SEEDS_OF_CHAOS, 26s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_REAP:
                DoCast(SPELL_REAP);
                events.RescheduleEvent(EVENT_REAP, 20s);
                break;
            case EVENT_SOW:
                DoCast(SPELL_SOW);
                DoCast(SPELL_SOW_DMG);
                events.RescheduleEvent(EVENT_SOW, 15s);
                break;
            case EVENT_DEATH_FIELD:
            {
                ...
                {
                    angle += 0.1f;
                    Position pos = me->GetNearPosition(35.0f, angle);
                    me->CastSpell(pos, SPELL_DEATH_FIELD_VISUAL, true);
                }
                DoCast(SPELL_DEATH_FIELD);
                events.RescheduleEvent(EVENT_DEATH_FIELD, 15s);
                break;
            }
            case EVENT_SEEDS_OF_CHAOS:
            {
                for (uint8 i = 0; i < 3; ++i)
                {
                    ...
                    Position pos = me->GetNearPosition(10.0f, angle);
                    me->CastSpell(pos, SPELL_SEEDS_OF_CHAOS, true);
                }
                events.RescheduleEvent(EVENT_SEEDS_OF_CHAOS, 30s);
                break;
            }
            default:
                break;
...
struct boss_sotanathor : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.RescheduleEvent(EVENT_SOUL_CLEAVE, 17s);
        events.RescheduleEvent(EVENT_SILENCE, 23s);
        events.RescheduleEvent(EVENT_CAVITATION, 21s);
        events.RescheduleEvent(EVENT_SEED_OF_DESTRUCTION, 16s);
    }
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
    {
        ...
        case SPELL_CAVITATION:
            if (target->IsPlayer() && target->ToPlayer()->IsInCombat())
            {
                float angle = 0.0f;
                for (uint8 i = 0; i < 4; i++)
                {
                    Position pos = target->GetNearPosition(5.0f, angle);
                    target->CastSpell(pos, SPELL_WAKE_OF_DESTRUCTION, CastSpellExtraArgs().SetOriginalCaster(me->GetGUID()));
                    angle += 1.57f;
                }
            }
            break;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SOUL_CLEAVE:
                ...
                events.RescheduleEvent(SPELL_SOUL_CLEAVE, 28s);
                break;
            case EVENT_SILENCE:
                DoCast(SPELL_SILENCE);
                events.RescheduleEvent(EVENT_SILENCE, 28s);
                break;
            case EVENT_CAVITATION:
              //  ZoneTalk(2, 0);
                DoCast(SPELL_CAVITATION);
                events.RescheduleEvent(EVENT_CAVITATION, 27s);
                break;
            case EVENT_SEED_OF_DESTRUCTION:
                DoCast(SPELL_SEED_OF_DESTRUCTION);
                events.RescheduleEvent(EVENT_SEED_OF_DESTRUCTION, 36s);
                break;
            default:
                break;
            }
...
struct boss_matron_folnuna : public ScriptedAI
{
        ...
        if (value == 100)
        {
            if (!me->HasUnitState(UNIT_STATE_CASTING))
            {
                if (!slimbering)
                {
                    ...
                    if (auto trigger = me->SummonCreature(124537, 4408.702f, 6490.092f, 40.38989f, 1.38464f, TEMPSUMMON_TIMED_DESPAWN, 15s))
    ...
    void StartEvents(uint8 phase)
    {
        ...
        case PHASE_1:
            slimbering = false;
            me->SetReactState(REACT_AGGRESSIVE);
            events.RescheduleEvent(EVENT_FEL_BLAST, 20s);
            events.RescheduleEvent(EVENT_GROTESQUE_SPAWN, 8s);
            break;
        case PHASE_2:
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_FEL_BLAST:
                ...
                events.RescheduleEvent(EVENT_FEL_BLAST, 10s);
                break;
            case EVENT_GROTESQUE_SPAWN:
                DoCast(SPELL_GROTESQUE_SPAWN);
                events.RescheduleEvent(EVENT_GROTESQUE_SPAWN, 35s);
                break;
            default:
                break;
            }
...
void AddSC_zone_argus()
{
    ...
    RegisterSpellScript(spell_sadist);
    RegisterSpellScript(spell_drain);
