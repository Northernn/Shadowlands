...
public:
    ...
    struct boss_ashgolmAI : public BossAI
    {
            ...
            //12:02
        {
            ...
            events.RescheduleEvent(EVENT_VOLCANO, 10s);     //12:12, 12:34
            events.RescheduleEvent(EVENT_LAVA_WREATH, 25s); //12:27, 13:22
            events.RescheduleEvent(EVENT_FISSURE, 40s);     //12:42, 13:29
            events.RescheduleEvent(EVENT_SUM_EMBER, 15s);
            events.RescheduleEvent(EVENT_SYSTEM_SECURITY, 45s);
        }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (checkaura())
                instance->DoUpdateCriteria(CriteriaType::BeSpellTarget, 215472);
        ...
        void SpellHit(WorldObject* caster, const SpellInfo* spell) override
        {
            ...
            if (spell->Id == 192517)
            {
                ...
                events.RescheduleEvent(EVENT_SYSTEM_SECURITY, 80s);
            }
        }
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_LAVA_WREATH)
            {
                ...
                DoCast(target->ToUnit(), SPELL_LAVA_WREATH_VIS, true);
                
                for (uint8 i = 0; i < 6; i++)
                {
                    Position pos= target->GetNearPosition(5.0f, i * 1.2);
                    me->CastSpell(pos, SPELL_LAVA_WREATH_TRIG, true);
                }
            }
            if (spell->Id == SPELL_FISSURE)
            {
                ...
                for (uint8 i = 0; i < 5; i++)
                {
                    ...
                    me->CastSpell(pos, SPELL_FISSURE_TRIG, true);
                }
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_VOLCANO:
                    ...
                    events.RescheduleEvent(EVENT_VOLCANO, 20s);
                    break;
                case EVENT_LAVA_WREATH:
                    DoCast(SPELL_LAVA_WREATH);
                    events.RescheduleEvent(EVENT_LAVA_WREATH, 55s);
                    break;
                case EVENT_FISSURE:
                    if (me->GetVictim())
                        DoCast(me->GetVictim(), SPELL_FISSURE);
                    events.RescheduleEvent(EVENT_FISSURE, 46s);
                    break;
                case EVENT_SUM_EMBER:
                    if (GameObject* volcano = me->FindNearestGameObject(GO_ASHGOLM_VOLCANO, 60.0f))
                        me->CastSpell(Position(volcano->GetPositionX(), volcano->GetPositionY(), volcano->GetPositionZ()), SPELL_SUM_EMBER, true);
                    events.RescheduleEvent(EVENT_SUM_EMBER, 15s);
                    break;
