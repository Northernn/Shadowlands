...
public:
    ...
    struct boss_inquisitor_tormentorumAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_SAP_SOUL, 15s);
        }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if (me->HealthBelowPct(healthPct) && healthPct > 40)
            {
                ...
                me->AttackStop();
                me->StopMoving();
                events.RescheduleEvent(EVENT_TELEPORT, 5s);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SAP_SOUL:
                    ...
                    me->StopMoving();
                    events.RescheduleEvent(EVENT_SAP_SOUL, 21s);
                    break;
                case EVENT_TELEPORT:
                    /*if someone can fix*/    //       me->CastSpell(tpPos[tpRand[tpCount]].GetPositionX(), tpPos[tpRand[tpCount]].GetPositionY(), tpPos[tpRand[tpCount]].GetPositionZ(), SPELL_TELEPORT);
                    Talk(SAY_TELEPORT);
                    events.RescheduleEvent(EVENT_OPEN_PRISON, 2s);
                    break;
                case EVENT_OPEN_PRISON:
                    DoCast(SPELL_OPEN_PRISONS_AURA);
                    me->SetFacingTo(tpPos[tpRand[tpCount]].GetOrientation());
                    events.RescheduleEvent(EVENT_SUM_PRISON_ADDS, 5s);
                    break;
