    ...
    struct boss_talixae_flamewreathAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            for (int8 i = 0; i < 3; i++)
                if (Creature* guard = me->GetCreature(*me, guardsGUID[i]))
                    if (guard && guard->IsAlive())
                        guard->AI()->DoZoneInCombat(guard);

            events.RescheduleEvent(EVENT_BURNING_INTENSITY, 6s);
            events.RescheduleEvent(EVENT_WITHERING_SOUL, 12s);
            events.RescheduleEvent(EVENT_INFERNAL_ERUPTION, 20s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_WITHERING_SOUL:
                    ...
                    events.RescheduleEvent(EVENT_WITHERING_SOUL, 15s);
                    break;
                case EVENT_INFERNAL_ERUPTION:
                    DoCast(SPELL_INFERNAL_ERUPTION);
                    events.RescheduleEvent(EVENT_INFERNAL_ERUPTION, 32s);
                    break;
                }
            }
    ...
    struct npc_talixae_guardsAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            if (me->GetEntry() == 104275)
                events.RescheduleEvent(EVENT_1, 3s); // 209378 17
            if (me->GetEntry() == 104274)
                events.RescheduleEvent(EVENT_2, 3s); // 207980 13
            if (me->GetEntry() == 104273)
                events.RescheduleEvent(EVENT_3, 7s); // 207979 11
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_1:
                    DoCast(209378);
                    events.RescheduleEvent(EVENT_1, 17s);
                    break;
                case EVENT_2:
                    DoCast(207980);
