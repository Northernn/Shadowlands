...
public:
    ...
    struct boss_advisor_melandrusAI : public BossAI
    {
            ...
            //48:11
        {
            ...
            events.RescheduleEvent(EVENT_PIERCING_GALE, 6s); //48:17, 48:37, 48:56
            events.RescheduleEvent(EVENT_SLICING_MAELSTROM, 11s); //48:22, 48:43, 49:01
            events.RescheduleEvent(EVENT_BLADE_SURGE, 19s); //48:31, 48:50, 49:09
            events.RescheduleEvent(EVENT_ENVELOPING_WINDS, 10s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_PIERCING_GALE:
                {
                    ...
                    events.RescheduleEvent(EVENT_PIERCING_GALE, 20s);
                    break;
                }
                case EVENT_SLICING_MAELSTROM:
                {
                    ...
                    Talk(SAY_MAELSTROM);
                    events.RescheduleEvent(EVENT_SLICING_MAELSTROM, 18s);
                    break;
                }
                case EVENT_BLADE_SURGE:
                    if (Unit* pTarget = SelectTarget(SelectTargetMethod::Random, 0, 40, true))
                        DoCast(pTarget, SPELL_BLADE_SURGE);
                    Talk(SAY_BLADE);
                    events.RescheduleEvent(EVENT_BLADE_SURGE, 19s);
                    break;
                case EVENT_ENVELOPING_WINDS:
                    DoCast(me, SPELL_ENVELOPING_WINDS_FILTER, true);
                    events.RescheduleEvent(EVENT_ENVELOPING_WINDS, 20s);
                    break;
                }
            }
...
public:
    ...
    struct npc_suspicious_nobleAI : public ScriptedAI
    {
        ...
        bool OnGossipHello(Player* player) override
        {
            ...
            {
                AddGossipItemFor(player, GossipOptionNpc::None, "We are here to help you!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        bool OnGossipSelect(Player* player, uint32 sender, uint32 action) override
        {
            ...
            if (me->GetGUID() == me->GetInstanceScript()->GetGuidData(107435))
            {
                me->AI()->Talk(1, player);
                CAST_AI(npc_suspicious_noble::npc_suspicious_nobleAI, (me->AI()))->start = true;
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_1, 3s); // 214688 25
            events.RescheduleEvent(EVENT_2, 9s); // 214690
            events.RescheduleEvent(EVENT_3, 16s); // 214692
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_1:
                    DoCast(214688);
                    events.RescheduleEvent(EVENT_1, 25s); // 214688 25
                    break;
                case EVENT_2:
                    DoCast(214690);
                    events.RescheduleEvent(EVENT_2, 25s); // 214690
                    break;
                case EVENT_3:
                    DoCast(214692);
                    events.RescheduleEvent(EVENT_3, 25s); // 214692
                    break;
                }
            }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_2:
                    if (Unit* summoner = me->GetOwner())
                        if (Unit* target = summoner->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 60.0f, true))
