...
public:
    ...
    struct boss_patrol_captain_gerdoAI : public BossAI
    {
            ...
            //06:03
        {
            ...
            events.RescheduleEvent(EVENT_RESONANT_SLASH, 7s);   //06:10, 06:32, 06:44
            events.RescheduleEvent(EVENT_ARCANE_LOCKDOWN, 15s); //06:18, 06:52, 07:20
            events.RescheduleEvent(EVENT_SIGNAL_BEACON, 23s);   //06:26
            events.RescheduleEvent(EVENT_STREETSWEEPER, 18s);
        }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if (me->HealthBelowPct(26) && !flaskEvent)
            {
                flaskEvent = true;
                Talk(SAY_FLASK);
                events.RescheduleEvent(EVENT_FLASK_SOLEMN_NIGHT, 5s);
            }
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (id == 1)
                events.RescheduleEvent(EVENT_FLASK_SOLEMN_NIGHT_END, 5s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_RESONANT_SLASH:
                {
                    ...
                    events.RescheduleEvent(EVENT_RESONANT_SLASH, 12s);
                    break;
                }
                case EVENT_ARCANE_LOCKDOWN:
                    DoCast(SPELL_ARCANE_LOCKDOWN);
                    Talk(SAY_ARCANE_LOCKDOWN);
                    events.RescheduleEvent(EVENT_ARCANE_LOCKDOWN, 28s);
                    break;
    ...
    struct npc_gerdo_arcane_beaconAI : public ScriptedAI
    {
        ...
        bool OnGossipHello(Player* player) override
        {
            if (!me->HasAura(SPELL_BEACON_DISABLED))
                player->CastSpell(me, SPELL_CAST_DISABLE_BEACON);
            return true;
        }
                ...
                case EVENT_1:
                    if (Unit* owner = me->GetOwner())
                        if (Creature* summoner = owner->ToCreature())
                            if (Unit* target = summoner->AI()->SelectTarget(SelectTargetMethod::Random, 0, 40.0f, true))
