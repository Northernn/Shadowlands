...
public:
    ...
    struct boss_thrashbite_the_scornfulAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_PULVERIZING_CUDGEL, 5s);
            events.RescheduleEvent(EVENT_HEAVE_CUDGEL, 14s);
            events.RescheduleEvent(EVENT_SCORNFUL_GAZE, 29s);
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (id == 1)
                events.RescheduleEvent(EVENT_SCORNFUL_DMG, 10s);
            else
                events.RescheduleEvent(EVENT_SCORNFUL_STUN, 10s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_PULVERIZING_CUDGEL:
                    ...
                    events.RescheduleEvent(EVENT_PULVERIZING_CUDGEL, 34s);
                    break;
                case EVENT_HEAVE_CUDGEL:
                    DoCast(SPELL_HEAVE_CUDGEL);
                    events.RescheduleEvent(EVENT_HEAVE_CUDGEL, 36s);
                    break;
                case EVENT_SCORNFUL_GAZE:
                    Talk(SAY_SCORNFUL);
                    DoCast(SPELL_SCORNFUL_GAZE);
                    events.RescheduleEvent(EVENT_SCORNFUL_GAZE, 36s);
                    events.DelayEvents(1s);
                    break;
                ...
                case EVENT_SCORNFUL_STUN:
                    ...
                    if (me->GetMap()->GetDifficultyID() != DIFFICULTY_HEROIC)
                    {
                        ...
                        for (uint8 i = 0; i < 2; ++i)
                        {
                            ...
                            me->SummonCreature(entry, me->GetPositionX() + frand(-5, 5), me->GetPositionY() + frand(-5, 5), me->GetPositionZ(), 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 300ms);
