    ...
    struct boss_mephistrothAI : public BossAI
    {
        ...
        void DefaultEvents(uint8 phase)
        {
            ...
            case 1:
                ...
                events.RescheduleEvent(EVENT_CHECK_ENERGY, 1s);
                events.RescheduleEvent(EVENT_DEMONIC_UPHEAVAL, 32s);
                events.RescheduleEvent(EVENT_DARK_SOLITUDE, 8s);
                events.RescheduleEvent(EVENT_CARRION_SWARM, 15s);
                break;
            case 2:
                events.RescheduleEvent(EVENT_PHASE_TWO_TICK, me->GetMap()->GetDifficultyID() == DIFFICULTY_HEROIC ? 5s : 4s);
                break;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_DEMONIC_UPHEAVAL:
                    ...
                    events.RescheduleEvent(EVENT_DEMONIC_UPHEAVAL, 27s);
                    break;
                case EVENT_DARK_SOLITUDE:
                    DoCast(SPELL_DARK_SOLITUDE);
                    events.RescheduleEvent(EVENT_DARK_SOLITUDE, 12s);
                    break;
                case EVENT_CARRION_SWARM:
                    DoCastVictim(SPELL_CARRION_SWARM);
                    events.RescheduleEvent(EVENT_CARRION_SWARM, 18s);
                    break;
    ...
    class spell_mephistroth_carrion_swarm_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            if (caster && target)
            {
                ...
                for (uint8 i = 0; i < 3; i++)
                {
                    caster->GetNearPoint2D(target, x,y, hit_range, angle);
