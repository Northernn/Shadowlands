...
public:
    ...
    struct boss_agronoxAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_TIMBER_SMASH, 5s);
            events.RescheduleEvent(EVENT_FLUMINATING, 10s);
            events.RescheduleEvent(EVENT_FLUMINATING_2, 40s);
            events.RescheduleEvent(EVENT_FLUMINATING_DEBUFF, 10s);
            events.RescheduleEvent(EVENT_CHOKING_VINES, 24s);

            if (me->GetMap()->GetDifficultyID() != DIFFICULTY_HEROIC) // mythic and other. normal mode isn't exist
                events.RescheduleEvent(EVENT_SUCCULENT_LAHSERS, 18s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_TIMBER_SMASH:
                    ...
                    events.RescheduleEvent(EVENT_TIMBER_SMASH, 31s);
                    break;
                ...
                case EVENT_FLUMINATING_2:
                {
                    ...
                    for (Map::PlayerList::const_iterator itr = pPlayers.begin(); itr != pPlayers.end(); ++itr)
                        if (Player* player = itr->GetSource())
                            me->SummonCreature(NPC_FLUMINATING_LASHERS, adds_pos[cur_position++], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3s);
                    break;
                }
                case EVENT_FLUMINATING_DEBUFF:
                    DoCastAOE(SPELL_FULMINATING_LASHERS_DEBUFF);
                    events.RescheduleEvent(EVENT_FLUMINATING_DEBUFF, 20s);
                    break;
                case EVENT_CHOKING_VINES:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 1, 0.0f, true))
                        if (Player* player = target->ToPlayer())
                            me->SummonCreature(NPC_CHOKING_VINES, vines_pos[urand(0, 3)], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3s);

                    Talk(SAY_VINES);
                    events.RescheduleEvent(EVENT_CHOKING_VINES, 35s);
                    break;
                case EVENT_SUCCULENT_LAHSERS:
                    DoCast(SPELL_SUCCULENT_LASHERS);
                    events.RescheduleEvent(EVENT_SUCCULENT_LAHSERS, 30s, 42s); // real random ?
                    break;
                }
            }
