...
struct boss_dargrul_the_underking : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_MAGMA_SCULPTOR, 10s);
        events.RescheduleEvent(EVENT_LANDSLIDE, 16s);
        events.RescheduleEvent(EVENT_CRYSTAL_WALL, 6s);
    }
    ...
    {
        if (me->GetMap()->GetDifficultyID() != DIFFICULTY_LFR && me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
            events.RescheduleEvent(EVENT_LAVA_GEYSER, 1s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_MAGMA_SCULPTOR:
                ...
                events.RescheduleEvent(EVENT_MAGMA_SCULPTOR, 70s);
                break;
            case EVENT_LANDSLIDE:
                listGuid.clear();
                DoCast(SPELL_LANDSLIDE);
                events.RescheduleEvent(EVENT_LANDSLIDE, 16s);
                events.RescheduleEvent(EVENT_MOLTEN_CRASH, 3s);
                break;
            case EVENT_CRYSTAL_WALL:
                DoCast(SPELL_CRYSTAL_WALL);
                events.RescheduleEvent(EVENT_CRYSTAL_WALL, 22s);
                break;
                ...
                if (!player || !player->IsAlive() || !player->HasAura(SPELL_FIXATE_PLR))
                {
                    if (me->GetOwner())
                        if (Creature* summoner = me->GetOwner()->ToCreature())
                            if (Unit* pTarget = summoner->AI()->SelectTarget(SelectTargetMethod::Random, checkPlayers(), 80.0f, true))
                            {
                                playerGuid.Clear();
                                me->AttackStop();
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
            {
                ...
                if (!player || !player->IsAlive() || !player->HasAura(SPELL_FIXATE_PLR))
                {
                    if (Unit* plrTarget = SelectTarget(SelectTargetMethod::Random, checkPlayers(), 80.0f, true))
                    {
                        playerGuid.Clear();
                        me->AttackStop();
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_2:
            {
                ...
                if (me->GetEntry() == 102287)
                    DoCast(201983);
                events.RescheduleEvent(EVENT_2, 21s);
                break;
            }
            case EVENT_3:
            {
                if (me->GetEntry() == 102287)
                    DoCast(226406);
                events.RescheduleEvent(EVENT_3, 13s);
                break;
            }
            }
        }
...
void AddSC_boss_dargrul_the_underking()
{
    ...
    RegisterSpellScript(spell_dargrul_gain_energy);
    RegisterSpellScript(spell_dargrul_magma_breaker);
