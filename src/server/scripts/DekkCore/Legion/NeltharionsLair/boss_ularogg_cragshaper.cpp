...
struct boss_ularogg_cragshaper : public BossAI
{
    ...
    {
        events.RescheduleEvent(EVENT_SUNDER, 8s);
        events.RescheduleEvent(EVENT_STRIKE_MOUNTAIN, 16s);
        events.RescheduleEvent(EVENT_BELLOW_DEEPS_1, 20s);
        events.RescheduleEvent(EVENT_STANCE_MOUNTAIN_1, enterCombat ? 50s : 120s);
    }
    ...
    void SpellFinishCast(const SpellInfo* spell)
    {
        ...
        case SPELL_STANCE_MOUNTAIN_SUM_2:
            if (me->GetMap()->GetDifficultyID() == DIFFICULTY_LFR || me->GetMap()->GetDifficultyID() == DIFFICULTY_NORMAL)
                events.RescheduleEvent(EVENT_STANCE_MOUNTAIN_2, 2s);
            else
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SUNDER:
                ...
                events.RescheduleEvent(EVENT_SUNDER, 10s);
                break;
            ...
            case EVENT_BELLOW_DEEPS_1:
                ...
                events.RescheduleEvent(EVENT_BELLOW_DEEPS_1, 32s);
                events.RescheduleEvent(EVENT_BELLOW_DEEPS_2, 3s);
                break;
            ...
            case EVENT_STANCE_MOUNTAIN_2:
                ...
                events.RescheduleEvent(EVENT_STANCE_MOUNTAIN_3, 2s);
                break;
            case EVENT_STANCE_MOUNTAIN_3:
                me->AddAura(SPELL_STANCE_MOUNTAIN_TICK, me);
                events.RescheduleEvent(EVENT_STANCE_MOUNTAIN_4, 10s);
                break;
...
struct npc_stonedark_slave : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        {
            // who->CastSpell(who, 209531, true);  ???? ??? ?? 7.0.3, ??????
            _endBarrel = true;
        }
...
void AddSC_boss_ularogg_cragshaper()
{
    ...
    RegisterSpellScript(spell_barrel_ride_plr_move);
    RegisterSpellScript(spell_ularogg_falling_debris);
