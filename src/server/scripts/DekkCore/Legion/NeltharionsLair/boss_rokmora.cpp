...
struct boss_rokmora : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_RAZOR_SHARDS, 30s);

        if (me->GetMap()->GetDifficultyID() != DIFFICULTY_LFR && me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
            events.RescheduleEvent(EVENT_CRYSTALLINE_GROUND, 4s);
    }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        Talk(SAY_DEATH);
        _JustDied();
        events.RescheduleEvent(EVENT_DEAD_CONVERSATION, 3s);
    }
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        {
            introDone = true;
            me->SummonCreature(NPC_NAVARROGG_INTRO, 2917.32f, 1402.29f, -2.28f, 2.744620f, TEMPSUMMON_TIMED_DESPAWN, 22s);
            if (auto ularogg = me->SummonCreature(NPC_ULAROGG_INTRO, 2900.33f, 1410.06f, -2.32f, 4.05f, TEMPSUMMON_TIMED_DESPAWN, 22s))
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_RAZOR_SHARDS:
                ...
                if (auto target = SelectTarget(SelectTargetMethod::MaxThreat))
                {
                    Position pos;
                    me->GetNearPosition(60.0f, me->GetRelativeAngle(target));
                    if (auto summon = me->SummonCreature(NPC_RAZOR_SHARDS_STALKER, pos, TEMPSUMMON_TIMED_DESPAWN, 10s))
                    {
                        summon->AttackStop();
                ...
                Talk(SAY_RAZOR);
                events.RescheduleEvent(EVENT_RAZOR_SHARDS, 26s);
                break;
...
struct npc_rokmora_blightshard_skitter : public ScriptedAI
{
    ...
    void JustDied(Unit* killer) override
    {
        if (auto owner = me->GetOwner())
            me->CastSpell(me, SPELL_CHOKING_DUST_AT, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(owner->GetGUID()));
    }
...
void AddSC_boss_rokmora()
{
    ...
    RegisterSpellScript(spell_nl_submerge);
    RegisterSpellScript(spell_entrance_run_plr_move);
}
