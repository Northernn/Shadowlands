...
struct boss_naraxas : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_RANCID_MAW, 8s);
        events.RescheduleEvent(EVENT_TOXIC_WRETCH, 12s);
        events.RescheduleEvent(EVENT_SUM_WORMSPEAKER, 7s);

        //Heroic
        events.RescheduleEvent(EVENT_CALL_ANGRY_CROWD, 4s); //?? ???????????
    }
...
void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    ...
    void SpellHit(WorldObject* caster, const SpellInfo* spell) override
    {
        ...
        case SPELL_TOXIC_WRETCH:
        {
            ...
            for (uint8 i = 0; i < 12; ++i)
            {
                ...
                Position pos = me->GetNearPosition(dist, frand(-2.0f, 2.0f));
                me->CastSpell(pos, SPELL_TOXIC_WRETCH_AT, true);
            }
            break;
        }
        case SPELL_FANATIC_SACRIFICE: //Trash Serach Boss
            DoCast(caster->ToUnit(), SPELL_DEVOUR_FANATIC);
            break;
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        ...
        if (spell->Id == SPELL_SPIKED_TONGUE_CHECK_R) //Check Radius
        {
            ...
            {
                target->ToUnit()->RemoveAurasDueToSpell(SPELL_SPIKED_TONGUE_CHANNEL);
                me->RemoveDynObject(SPELL_SPIKED_TONGUE_AT);
                DoCast(target->ToUnit(), SPELL_SPIKED_TONGUE_RIDE, true);
            }
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_RANCID_MAW:
                ...
                events.RescheduleEvent(EVENT_RANCID_MAW, 18s);
                break;
            case EVENT_TOXIC_WRETCH:
                DoCast(SPELL_TOXIC_WRETCH);
                events.RescheduleEvent(EVENT_TOXIC_WRETCH, 14s);
                break;
            case EVENT_SUM_WORMSPEAKER:
                ...
                events.RescheduleEvent(EVENT_SUM_WORMSPEAKER, 64s);
                break;
...
void AddSC_boss_naraxas()
{
    ...
    RegisterSpellScript(spell_naraxas_gain_energy);
