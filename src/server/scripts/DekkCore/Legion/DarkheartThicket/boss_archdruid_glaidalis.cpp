...
struct boss_arch_druid_glaidalis : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_GRIEVOUS_LEAP, 6s);
        events.RescheduleEvent(EVENT_PRIMAL_RAMPAGE, 14s);
        events.RescheduleEvent(EVENT_NIGHTFALL, 20s);
    }
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        ...
        case SPELL_GRIEVOUS_LEAP_AOE_2:
            if (me->GetGUID() != target->GetGUID())
                DoCast(target->ToUnit(), SPELL_GRIEVOUS_LEAP_DOT, true);
            break;
        }
    }
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        if (!_introDone && me->IsWithinDistInMap(who, 48.0f))
        {
            ...
            events.RescheduleEvent(EVENT_PRE_GRIEVOUS_LEAP, 3s);
        }
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_PRE_GRIEVOUS_LEAP:
                if (Creature* target = me->FindNearestCreature(NPC_DRUIDIC_PRESERVER, 200.0f, true))
                {
                    ...
                    me->Kill(me, target);
                    events.RescheduleEvent(EVENT_PRE_GRIEVOUS_LEAP, 7s);
                }
                else
                {
            ...
            case EVENT_GRIEVOUS_LEAP:
                ...
                events.RescheduleEvent(EVENT_GRIEVOUS_LEAP, 16s);
                break;
            case EVENT_PRIMAL_RAMPAGE:
                ...
                events.RescheduleEvent(EVENT_PRIMAL_RAMPAGE, 16s);
                break;
            case EVENT_NIGHTFALL:
                Talk(SAY_NIGHTFALL);
                DoCast(SPELL_NIGHTFALL);
                events.RescheduleEvent(EVENT_NIGHTFALL, 16s);
                break;
            }
        }
    ...
    void UpdateAI(uint32 diff)
    {
        ...
        if (fixateTimer)
        {
            if (fixateTimer <= diff)
            {
                ...
                {
                    if (auto target = SelectTarget(SelectTargetMethod::Random, 0, 60.0f, true))
                    {
...
class spell_glaidalis_grievous_leap : public SpellScript
{
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (Unit* target = caster->GetAI()->SelectTarget(SelectTargetMethod::MaxDistance, 0, 55.0f, true))
...
void AddSC_boss_archdruid_glaidalis()
{
    ...
    RegisterSpellScript(spell_glaidalis_grievous_tear);
    RegisterSpellScript(spell_nightmare_toxin);
    RegisterSpellScript(spell_nightclaw_grievous_rip);
}
