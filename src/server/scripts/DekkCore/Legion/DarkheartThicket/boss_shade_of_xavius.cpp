...
public:
    ...
    struct boss_shade_of_xaviusAI : public BossAI
    {
            ...
            //49:53
        {
            ...
            events.RescheduleEvent(EVENT_FESTERING_RIP, 3s);  //49:56, 50:13
            events.RescheduleEvent(EVENT_NIGHTMARE_BOLT, 9s);  //50:02, 50:19
            events.RescheduleEvent(EVENT_FEED_ON_THE_WEAK, 14s); //50:07, 51:01, 51:30, 52:00
            events.RescheduleEvent(EVENT_INDUCED_PARANOIA, 21s); //50:14, 51:00, 51:21, 51:42
        }
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            ...
            {
                finalPhase = true;
                events.RescheduleEvent(EVENT_FINAL_PHASE, 1s);
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FESTERING_RIP:
                    ...
                    events.RescheduleEvent(EVENT_FESTERING_RIP, 17s);
                    break;
                case EVENT_NIGHTMARE_BOLT:
                    DoCast(SPELL_NIGHTMARE_BOLT);
                    events.RescheduleEvent(EVENT_NIGHTMARE_BOLT, 17s);
                    break;
                case EVENT_FEED_ON_THE_WEAK:
                    if (Unit* pTarget = SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true))
                        DoCast(pTarget, SPELL_FEED_ON_THE_WEAK);
                    events.RescheduleEvent(EVENT_FEED_ON_THE_WEAK, 30s);
                    break;
                case EVENT_INDUCED_PARANOIA:
                    DoCast(SPELL_INDUCED_PARANOIA);
                    events.RescheduleEvent(EVENT_INDUCED_PARANOIA, 21s);
                    break;
