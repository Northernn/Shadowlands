...
public:
    ...
    struct boss_oakheartAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_SHATTERED_EARTH, 6s);
            events.RescheduleEvent(EVENT_STRANGLING_ROOTS, 14s);
            events.RescheduleEvent(EVENT_NIGHTMARE_BREATH, 22s);
            events.RescheduleEvent(EVENT_CRUSHING_GRIP, 30s);

            if (me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
                events.RescheduleEvent(EVENT_UPROOT, 34s);
        }
        ...
        void SpellHit(WorldObject* target, const SpellInfo* spell) override
        {
            if (spell->Id == 204644)
            {
                events.RescheduleEvent(EVENT_REMOVE_PASSENGERS, 6s);
                Unit* pTarget = SelectTarget(SelectTargetMethod::MaxDistance, 0, 0.0f, true);
                if (pTarget)
                {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SHATTERED_EARTH:
                    ...
                    Talk(SAY_EARTH);
                    events.RescheduleEvent(EVENT_SHATTERED_EARTH, 40s);
                    break;
                case EVENT_STRANGLING_ROOTS:
                    DoCast(SPELL_STRANGLING_ROOTS);
                    Talk(SAY_ROOTS);
                    events.RescheduleEvent(EVENT_STRANGLING_ROOTS, 22s);
                    break;
                case EVENT_NIGHTMARE_BREATH:
                    DoCastVictim(SPELL_NIGHTMARE_BREATH);
                    Talk(SAY_NIGHTMARE);
                    events.RescheduleEvent(EVENT_NIGHTMARE_BREATH, 32s);
                    break;
