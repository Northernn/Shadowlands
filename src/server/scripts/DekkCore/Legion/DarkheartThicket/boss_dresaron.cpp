...
public:
    ...
    struct boss_dresaronAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_BREATH_OF_CORRUPTION, 7s);
            events.RescheduleEvent(EVENT_EARTHSHAKING_ROAR, 14s);
            events.RescheduleEvent(EVENT_DOWN_DRAFT, 20s);
            abomination = false;
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_BREATH_OF_CORRUPTION:
                    ...
                    events.RescheduleEvent(EVENT_BREATH_OF_CORRUPTION, 27s);
                    break;
                case EVENT_EARTHSHAKING_ROAR:
                    DoCast(SPELL_EARTHSHAKING_ROAR);
                    events.RescheduleEvent(EVENT_EARTHSHAKING_ROAR, 19s);
                    break;
                case EVENT_DOWN_DRAFT:
                    DoCast(SPELL_DOWN_DRAFT);
                    events.RescheduleEvent(EVENT_DOWN_DRAFT, 30s);
                    break;
                }
            }
