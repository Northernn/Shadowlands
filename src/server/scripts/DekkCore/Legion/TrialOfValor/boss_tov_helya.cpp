...
struct boss_helya_tov : BossAI
{
    ...
    void Reset() override
    {
        ...
        events.RescheduleEvent(EVENT_REANIMATION, 1s);
    }
    ...
    {
        Talk(SAY_DEATH);
        instance->DoUpdateCriteria(CriteriaType::DefeatDungeonEncounter, 2008);
        me->RemoveAllAreaTriggers();
        RemoveAllDebaffsOnPlayer();
        DespawnAllSummons();
        _JustDied();
                ...
                if (!me->HasUnitState(UNIT_STATE_CASTING))
                {
                    ...
                    else if (IsLFR() || IsNormalRaid())
                    {
                        if (eventPhase == PHASE_2)
                        {
                            ...
                            events.RescheduleEvent(EVENT_FURY_OF_THE_MAW_CLEAN_AT, 4s);
                            me->SetPower(POWER_ALTERNATE_POWER, 0);
                        }
                    }
    ...
    void StartEvents(uint8 phase)
    {
        ...
        case PHASE_1:
            ...
            if (IsMythicRaid())
            {
                events.RescheduleEvent(EVENT_TAINT_OF_THE_SEA, 15s);
                events.RescheduleEvent(EVENT_TENTECLE_STRIKE, 35s);
                events.RescheduleEvent(EVENT_ORB_OF_CURRUPTION, 14s);
            }
            else if (IsHeroicRaid())
            {
                events.RescheduleEvent(EVENT_TAINT_OF_THE_SEA, 19s);
                events.RescheduleEvent(EVENT_TENTECLE_STRIKE, 36s);
                events.RescheduleEvent(EVENT_ORB_OF_CURRUPTION, 29s);
            }
            else if (IsNormalRaid() || IsLFR())
            {
                events.RescheduleEvent(EVENT_TAINT_OF_THE_SEA, 12s);
                events.RescheduleEvent(EVENT_TENTECLE_STRIKE, 53s);
                events.RescheduleEvent(EVENT_ORB_OF_CURRUPTION, 18s);
            }
            ...
            break;
        case PHASE_2:
            events.RescheduleEvent(EVENT_TELEPORT_OUTSIDE, 1s);
            checkrephase = true;
            ...
            me->SummonCreatureGroup(3);
                ...
                DoCast(SpellHelyaReuse);
                Talk(SAY_PHASE_2);
                events.RescheduleEvent(EVENT_TORRENT, 9s);
                events.RescheduleEvent(EVENT_REANIMATION, 30s);
                if (IsLFR() || IsNormalRaid())
                {
                    DoCast(SpellHelyaAltPowerRegen);
                    events.RescheduleEvent(EVENT_FURY_OF_THE_MAW, 2s);
                }
                else if (IsHeroicPlusRaid())
                    me->SetPower(POWER_ALTERNATE_POWER, 100);
            break;
        case PHASE_3:
            events.RescheduleEvent(EVENT_TELEPORT_HOME, 2s);
            DoCast(SpellHelyaReuse);
            me->CastStop();
            ...
            checkrephase = false;
                ...
                events.RescheduleEvent(EVENT_REANIMATION, 52s);
                if (IsMythicRaid())
                    events.RescheduleEvent(EVENT_ORB_OF_CORROSION, 6s);
                else if (IsHeroicRaid())
                    events.RescheduleEvent(EVENT_ORB_OF_CORROSION, 14s);
                else if (IsNormalRaid() || IsLFR())
                    events.RescheduleEvent(EVENT_ORB_OF_CORROSION, 18s);
            break;
        }
    }
...
void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    ...
    void SpellHit(WorldObject* owner, SpellInfo const* spell) 
    {
        switch (spell->Id)
        {
        case SpellBilewaterLiquefactionKill:
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(227992, DIFFICULTY_NONE))
            {
                ...
                float healthPct = owner->ToUnit()->GetHealthPct() > 5.0f ? owner->ToUnit()->GetHealthPct() : 5.0f;
                ...
                args.AddSpellMod(SPELLVALUE_BASE_POINT1, healthPct);
                me->CastSpell(owner, SpellBilewaterLiquefactionDmg, args);
            }
            break;
        case SpellOrbOfCorruption:
        {
            ...
            for (auto& player : playerList)
            {
                ...
                me->SummonCreature(Data::Creatures::OrbOfCorruption, pos, TEMPSUMMON_TIMED_DESPAWN, 9s);
            }
            break;
        }
        case SpellOrbOfCorrosion:
        {
            ...
            for (auto& player : playerList)
            {
                ...
                pos.m_positionX -= 6.0f;
                pos.m_positionY -= 6.0f;
                me->SummonCreature(Data::Creatures::OrbOfCorrosion, pos, TEMPSUMMON_TIMED_DESPAWN, 9s);
            }
            break;
        }
        case SpellBilewaterBreath:
        {
            ...
            for (uint8 i = 0; i < count; ++i)
            {
                ...
                Position pos = me->GetFirstCollisionPosition(dist, angle);
                me->CastSpell(pos, SpellBilewaterBreathNpc, true);
            }
            break;
        }
        case SpellCorruptedBreath:
        {
            ...
            float angle = me->GetAbsoluteAngle(me);
            float dist = 30.0f;
            for (uint8 i = 0; i < countTarget; ++i)
            {
                dist += 13.0f;

                Position pos = me->GetNearPosition(dist, angle);
                me->CastSpell(pos, SpellCorruptedAxion, true);
            }
            break;
        }
        case SpellOrbOfCorrosionShare:
            DoCast(owner->ToUnit(), SpellOrbOfCorrosionDmg, true);
            break;
        }
    }
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
    {
        ...
        case SpellBilewaterBreathDmg:
            DoCast(target->ToUnit(), SpellBilewaterRedox, true);
            break;
        case SpellCorruptedBreathDmg:
            DoCast(target->ToUnit(), SpellDarkHatred, false);
            break;
        }
    }
    ...
    void SpellFinishCast(SpellInfo const* spell)
    {
        ...
        case SpellFuryOfTheMawAT:
            if (IsHeroicPlusRaid())
            {
                ...
                {
                    events.RescheduleEvent(EVENT_BOAT, 5s);
                    events.RescheduleEvent(EVENT_SUMMON_SKELETION, 31s);
                    events.RescheduleEvent(EVENT_SUMMON_SKELETION, 41s);
                    FuryOfTheMawTrash = true;
                }
                events.RescheduleEvent(EVENT_FURY_OF_THE_MAW_CLEAN_AT, 3s);
            }
            else if (IsLFR() || IsNormalRaid())
            {
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_1:
            DoCast(SpellHelyaAltPowerRegen);
            events.RescheduleEvent(EVENT_TORRENT, 2s);
            break;
        case ACTION_2:
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
                ...
                if (eventPhase == PHASE_1 || eventPhase == PHASE_3)
                {
                 ...
                 //       if (threatlist.empty())
                    ...
                    if (IsMythicRaid())
                        ...
                        events.RescheduleEvent(EVENT_TAINT_OF_THE_SEA, 14s);
                    else if (IsNormalRaid() || IsLFR())
                        events.RescheduleEvent(EVENT_TAINT_OF_THE_SEA, 12s);
                }
                break;
            ...
            case EVENT_TENTECLE_STRIKE:
                ...
                {
                    me->SummonCreature(Data::Creatures::StrikingTentacle, StrikingTentacle[urand(0, 2)], TEMPSUMMON_TIMED_DESPAWN, 9s);

                    if (IsMythicRaid())
                        events.RescheduleEvent(EVENT_TENTECLE_STRIKE, 35s);
                    else
                        events.RescheduleEvent(EVENT_TENTECLE_STRIKE, 30s);
                }
                break;
            case EVENT_ORB_OF_CURRUPTION:
                if (eventPhase == PHASE_1)
                {
                    ...
                    if (IsMythicRaid())
                        events.RescheduleEvent(EVENT_ORB_OF_CURRUPTION, 23s);
                    else
                        events.RescheduleEvent(EVENT_ORB_OF_CURRUPTION, 26s);
                }
                break;
            case EVENT_ORB_OF_CORROSION:
                if (eventPhase == PHASE_3)
                {
                    ...
                    if (IsMythicRaid())
                        events.RescheduleEvent(EVENT_ORB_OF_CORROSION, 13s);
                    else
                        events.RescheduleEvent(EVENT_ORB_OF_CORROSION, 17s);
                }
                break;
            case EVENT_TORRENT:
                if (!me->HasUnitState(UNIT_STATE_CASTING))
                {
                    if (eventPhase == PHASE_2)
                    {
                        if (!me->HasAura(SpellFureOfTheMawLong))
                        {
                            ...
                            {
                                DoCast(SpellTorrent);
                                events.RescheduleEvent(EVENT_TORRENT, 10s);
                            }
                        }
                    }
                }
            ...
            case EVENT_REANIMATION:
                if (eventPhase == 0)
                {
                    ...
                    events.RescheduleEvent(EVENT_REANIMATION, 15s);
                }
                break;
            case EVENT_SUMMON_SKELETION:
                for (uint8 i = 0; i < 10; ++i)
                    me->SummonCreature(Data::Creatures::DecayingMinion, skeletionPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                break;
            case EVENT_BOAT:
                if (eventPhase == PHASE_2)
                {
                    me->SummonCreature(Data::Creatures::KvaldirLongboat, 569.39f, 857.6201f, 0.00213f, 4.433769f, TEMPSUMMON_TIMED_DESPAWN, 5s);
                    me->SummonCreature(Data::Creatures::KvaldirLongboat, 638.672f, 817.6201f, 0.00213f, 3.943812f, TEMPSUMMON_TIMED_DESPAWN, 5s);
                    me->SummonCreature(Data::Creatures::Grimelord, 458.674f, 639.483f, 7.53757f, 4.433769f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                    me->SummonCreature(Data::Creatures::NightWatchMariner, 516.007f, 614.179f, 5.5823f, 3.943812f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                    if (IsMythicRaid())
                    {
                        me->SummonCreature(Data::Creatures::HelarjarMistwatcher, 564.366f, 653.4237f, 17.9559f, 2.401197f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                        me->SummonCreature(Data::Creatures::HelarjarMistwatcher, 469.7802f, 703.5213f, 17.96658f, 6.12398f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                    }
                }
...
struct npc_grimelord_tov : ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_1, 14s);
        events.RescheduleEvent(EVENT_2, 16s);
        events.RescheduleEvent(EVENT_3, 9s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
                ...
                events.RescheduleEvent(EVENT_1, 12s);
                break;
            case EVENT_2:
           ...
           //     auto threatlist = me->CanHaveThreatList().getThreatList();
               ...
               //     if (auto itr = Trinity::Containers::SelectRandomContainerElement(threatlist))
                ...
                events.RescheduleEvent(EVENT_3, 17s);
                break;
            }
            }
        }
...
struct npc_night_watch_mariner_tov : ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_1, 8s);
          ...
          //      auto threatlist = me->CanHaveThreatList().getThreatList();
             ...
             //           if (Unit* target = itr->getTarget())
               ...
               //         threatlist.remove(itr);
                ...
                events.RescheduleEvent(EVENT_1, 9s);
                break;
            }
            case EVENT_2:
...
struct npc_decaying_minion_tov : ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* /*summoner*/) override
    {
        events.RescheduleEvent(EVENT_3, 2s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_3:
                ...
                events.RescheduleEvent(EVENT_2, 3s);
                break;
            }
        }
        DoMeleeAttackIfReady();
    ...
    void MovementInform(uint32 type, uint32 data) override
    {
        if (type == POINT_MOTION_TYPE)
        {
            ...
            case 3:
                ...
                break;
            case 4:
                events.RescheduleEvent(EVENT_4, 1s);
                break;
            case 5:
                events.RescheduleEvent(EVENT_5, 1s);
                break;
            }
        }
    }
...
void AddSC_boss_helya_tov()
{
    ...
    RegisterSpellScript(spell_helya_blue_power_regen);
    RegisterSpellScript(spell_helya_purple_power_regen);
    RegisterSpellScript(spell_helya_alt_power_regen);
    RegisterSpellScript(spell_tov_darkness);
    RegisterSpellScript(spell_taint_of_the_sea);
    RegisterSpellScript(spell_corrupted_axion_aura);
    RegisterSpellScript(spell_corrupted_mc);
    RegisterSpellScript(spell_fury_of_the_maw);
