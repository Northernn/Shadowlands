...
struct boss_tichonrius : BossAI
{
    ...
    void DefaultEvents()
    {
     ...
     /*   events.RescheduleEvent(EVENT_CARRION_PLAGUE, (5 - delay) * IN_MILLISECONDS);
        ...
        if (phase == 1 || phase == 2)
            events.RescheduleEvent(EVENT_SUM_GUARDS, 24s);
    }
    ...
    void SpellFinishCast(const SpellInfo* spell)
    {
        switch (spell->Id)
        {
            case SPELL_ILLUSIONARY_NIGHT:
                ...
                me->SummonCreature(NPC_COMBAT_STALKER, centrPos, TEMPSUMMON_TIMED_DESPAWN, 30s);
                me->SetVisible(false);
                break;
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (IllusionaryStartTimer)
        {
            if (IllusionaryStartTimer <= diff)
            {
                IllusionaryStartTimer = 0;
                events.Reset();
                events.RescheduleEvent(EVENT_ILLUSIONARY_NIGHT_START, 1s);
            }
            else
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_ECHOES_OF_THE_VOID:
                {
                    ...
                    float angle = centrPos.GetAbsoluteAngle(me) - 0.785f;
                    Position pos;
                    for (uint8 i = 0; i < 4; ++i)
                    {
                        angle += 3.14f / 2;
                    ...
                    events.RescheduleEvent(EVENT_ECHOES_OF_THE_VOID, 65s);
                    break;
                }
                ...
                case EVENT_SUM_GUARDS:
                    ...
                    events.RescheduleEvent(EVENT_SUM_GUARDS, 52s);
                    break;
        ...
        if (AttackTimer)
        {
            if (AttackTimer <= diff)
            {
                ...
                if (!me->GetVictim())
                {
                    ...
                    if (!targetFound)
                    {
                        if (Unit* newTarget = SelectTarget(SelectTargetMethod::MinDistance, 0, 100.0f, true))
...
struct npc_tichonrius_carrion_nightmare : ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        events.RescheduleEvent(EVENT_3, 6s);
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_2:
                    ...
                    events.RescheduleEvent(EVENT_2, 24s);
                    break;
            }
    ...
    void OnRemove(AuraEffect const* /*auraEffect*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (Unit* target = GetTarget())
        {
            if (InstanceScript* instance = target->GetInstanceScript())
                if (Creature* tichondrius = instance->instance->GetCreature(instance->GetGuidData(DATA_TICHONDRIUS)))
                    target->CastSpell(target, SPELL_NETHER_ZONE_AT, true);
        }
    }
...
class spell_tichonrius_burning_soul : public AuraScript
{
    ...
    void OnRemove(AuraEffect const* /*auraEffect*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (Unit* target = GetTarget())
        {
            if (InstanceScript* instance = target->GetInstanceScript())
                if (Creature* tichondrius = instance->instance->GetCreature(instance->GetGuidData(DATA_TICHONDRIUS)))
                    target->CastSpell(target, SPELL_BURNING_SOUL_DMG, true);
        }
    }
...
void AddSC_boss_tichondrius()
{
    ...
    RegisterSpellScript(spell_tichonrius_brand_of_argus);
    RegisterSpellScript(spell_tichonrius_echoes_void);
    RegisterSpellScript(spell_tichonrius_vampiric);
    RegisterSpellScript(spell_tichonrius_nether_zone);
    RegisterSpellScript(spell_tichonrius_burning_soul);
    RegisterSpellScript(spell_tichonrius_illusionary_night);
}
