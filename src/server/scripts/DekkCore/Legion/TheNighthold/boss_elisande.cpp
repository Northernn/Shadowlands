     ...
     //   if (!IsLFR())
      //      events.RescheduleEvent(EVENT_ARCANETIC_RING, arcaneticRingTimeList.front());
       ...
       /* if (phase > 1)
        ...
        {
            case 1:
                events.RescheduleEvent(EVENT_CASCADENT_STAR, 12s);
                events.RescheduleEvent(EVENT_ABLATION, 13s);
                break;
            case 2:
                events.RescheduleEvent(EVENT_ABLATING_EXPLOSION, 24s);
                break;
            case 3:
                events.RescheduleEvent(EVENT_ABLATIVE_PULSE, 24s);
...
void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        if (phase < 3)
        {
            ...
            if (!switchPhase && me->HealthBelowPct(11))
            {
                ...
                me->AttackStop();
                events.RescheduleEvent(EVENT_CHANGE_PHASE_1, 1s);
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spellInfo) override
    {
        switch (spellInfo->Id)
        {
            case SPELL_LEAVE_NIGHTWELL_FILTER:
            {
                ...
                DoCast(target->ToUnit(), SPELL_LEAVE_NIGHTWELL, true);
                break;
            }
            ...
            case SPELL_CASCADENT_STAR:
            {
                ...
                for (auto const& guid : cascadentTargetsList)
                {
                    ...
                    if (Unit* cascadentTarget = target->ToUnit()->GetVictim())
                        target->CastSpell(cascadentTarget, SPELL_CASCADENT_STAR_VISUAL, true);
                }
                break;
            }
            ...
            case SPELL_DELPHURIC_BEAM_FILTER:
                DoCast(target->ToUnit(), SPELL_DELPHURIC_BEAM_MARK, true);
                break;
            case SPELL_PERMELIATIVE_TORMENT_FILTER_2:
                DoCast(target->ToUnit(), SPELL_PERMELIATIVE_TORMENT_DOT, true);
                break;
            case SPELL_CONFLEXIVE_BURST_FILTER:
                DoCast(target->ToUnit(), SPELL_CONFLEXIVE_BURST_AURA, true);
                break;
        }
    }
    ...
    void SpellFinishCast(const SpellInfo* spell)
    {
        switch (spell->Id)
        {
            case SPELL_TIME_STOP:
                events.RescheduleEvent(EVENT_CHANGE_PHASE_2, 3s);
                break;
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_CHANGE_PHASE_2:
                    ...
                    events.RescheduleEvent(EVENT_CHANGE_PHASE_3, 2s);
                    break;
                case EVENT_CHANGE_PHASE_3:
                    ...
                    me->NearTeleportTo(me->GetHomePosition());
                    DoCast(me, SPELL_ETCHED_IN_TIME, true);
                    events.RescheduleEvent(EVENT_CHANGE_PHASE_4, 5s);
                    break;
               ...
               //         events.RescheduleEvent(EVENT_SUM_FAST_ELEMENTAL, SummonElementalTimeList[1][IsMythicRaid() ? phase : 0].front());
                ...
                case EVENT_CASCADENT_STAR:
                    ...
                    events.RescheduleEvent(EVENT_CASCADENT_STAR, 5s);
                    break;
                case EVENT_ABLATION:
                    if (me->GetReactState() != REACT_PASSIVE)
                        me->CastSpell(me->GetVictim(), SPELL_ABLATION);
                    events.RescheduleEvent(EVENT_ABLATION, 5s);
                    break;
                ...
                case EVENT_ABLATING_EXPLOSION:
                    ...
                    events.RescheduleEvent(EVENT_ABLATING_EXPLOSION, 21s);
                    break;
                case EVENT_ABLATIVE_PULSE:
                    me->CastSpell(me->GetVictim(), SPELL_ABLATIVE_PULSE);
                    events.RescheduleEvent(EVENT_ABLATIVE_PULSE, 6s);
                    break;
...
struct npc_elisande_echo_of_elisande : ScriptedAI
{
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spellInfo) override
    {
        ...
        {
            case SPELL_SPANNING_SINGULARITY_FILTER:
                DoCast(target->ToUnit(), GetSingularitySpellId());
                break;
            case SPELL_DELPHURIC_BEAM_FILTER:
                DoCast(target->ToUnit(), SPELL_DELPHURIC_BEAM_MARK, true);
                break;
...
struct npc_elisande_arcanetic_ring : ScriptedAI
{
    ...
    {
        me->SetReactState(REACT_PASSIVE);
        //me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_UNINTERACTIBLE);
    ...
    void SpellHit(WorldObject* caster, SpellInfo const* spellInfo) override
    {
        if (!activeMoving && spellInfo->Id == SPELL_ARCANETIC_RING_FILTER)
        {
           ...
           // centrPos.SimplePosXYRelocationByAngle(pos, 115.0f, me->GetAbsoluteAngle(caster));
...
class spell_elisande_epocheric_orb_dmg : public SpellScript
{
    ...
    void HandleDamage(SpellEffIndex effectIndex)
    {
        ...
        {
          //  if ( SpellInfo* info = sSpellMgr->GetSpellInfo(SPELL_EPOCHERIC_ORB_SMALL_DMG))
...
void AddSC_boss_elisande()
{
    ...
    RegisterSpellScript(spell_elisande_permeliative_torment);
    RegisterSpellScript(spell_elisande_time_slow_regulator);
    RegisterSpellScript(spell_elisande_delphuric_beam);
}
