...
public:
    ...
    struct boss_trilliaxAI : BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_ARCANE_SLASH, 7s);
            events.RescheduleEvent(EVENT_TOXIC_SLICE, 10s);
        ...
        void OnApplyOrRemoveAura(uint32 spellId, AuraRemoveMode mode, bool apply)
        {
            ...
            case SPELL_FORM_CLEANER:
            {
                ...
                events.RescheduleEvent(EVENT_ARCANE_SLASH, 7s);
                events.RescheduleEvent(EVENT_MANA_RUPTURE, 2s);
                events.RescheduleEvent(EVENT_ARCING_BONDS, 5s);
            ...
            case SPELL_FORM_MANIAC:
            {
                ...
                events.RescheduleEvent(EVENT_SUCCULENT_FEAST, 5s);
                events.RescheduleEvent(EVENT_TIDY_UP, 12s);
                break;
            }
            case SPELL_FORM_CARETAKER:
            {
                ...
                events.RescheduleEvent(EVENT_ARCANE_SLASH, 7s);
                events.RescheduleEvent(EVENT_TOXIC_SLICE, 10s);
                events.RescheduleEvent(EVENT_STERILIZE, 1s);
                events.RescheduleEvent(EVENT_CLEANSING_RAGE, 10s);
                if (IsMythicRaid())
                {
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == EFFECT_MOTION_TYPE)
            {
                if (id == SPELL_ANNIHILATION_JUMP)
                {
                    ...
                    events.RescheduleEvent(EVENT_ANNIHILATION, 2s);
                }
            }
        }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_ARCANE_SLASH:
                    ...
                    if (me->HasAura(SPELL_FORM_MANIAC))
                        events.RescheduleEvent(EVENT_ARCANE_SLASH, 8s);
                    else
                        events.RescheduleEvent(EVENT_ARCANE_SLASH, 11s);
                    break;
                ...
                case EVENT_TOXIC_SLICE:
                    ...
                    events.RescheduleEvent(EVENT_TOXIC_SLICE, 26s);
                    break;
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
        {
            if (summon->GetEntry() == NPC_PUTRID_SLUDGE)
            {
                ...
                if (putridDies == 8)
                {
                    ...
                    me->RemoveUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
                    me->RemoveUnitFlag(UNIT_FLAG_NOT_ATTACKABLE_1);
                    //me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_UNINTERACTIBLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_ATTACKABLE_1);
    ...
    struct npc_trilliax_scrubberAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* caster, const SpellInfo* spell) override
        {
            if (spell->Id == 207433 && !onClick) //Ride me
            {
                onClick = true;
                me->InterruptNonMeleeSpells(false);
                DoCast(caster->ToUnit(), SPELL_CLEANSING_DESTRUCTION_PLR, true);
                me->DespawnOrUnsummon(500ms);
            }
        }
    ...
    struct npc_trilliax_imprintAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* caster, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_TOXIC_SLICE)
            {
                Position pos;
                if (Unit* owner = me->GetOwner())
                    me->CastSpell(pos, SPELL_TOXIC_SLICE_TRIG);
            }
        }
