...
struct boss_high_botanist_telarn : BossAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        {
            case 1:
                events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 35s);
                events.RescheduleEvent(EVENT_PARASITIC_FETTER, 20s);
                events.RescheduleEvent(EVENT_SOLAR_COLLAPSE, 10s);
                break;
            case 2:
                Talk(SAY_PHASE_2);
                events.RescheduleEvent(EVENT_PHASE_2, 1s);
                events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 42s);
                events.RescheduleEvent(EVENT_PARASITIC_FETTER, 22s);
                break;
            case 3:
                Talk(SAY_PHASE_3);
                events.RescheduleEvent(EVENT_PHASE_3, 1s);
                events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 52s);
                events.RescheduleEvent(EVENT_CALL_OF_NIGHT, 20s);
                break;
        }
    }
                ...
                if (instance->GetBossState(DATA_TELARN) != NOT_STARTED)
                {
                    ...
                    for (auto const& guid : botanistList)
                    {
                        if (auto botanist = Unit::GetCreature(*me, guid))
                        {
                            ...
                            botanist->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
                            botanist->SetUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
                            botanist->AI()->DoAction(true);
                            botanist->DespawnOrUnsummon(200ms);
                        }
                    }
                    RemoveAurasPlayers();
                    DoAction(ACTION_1);
                    ...
                    for (auto const& guid : botanistList)
                    {
                        if (auto botanist = Unit::GetCreature(*me, guid))
                            if (botanist->IsAlive() && !botanist->IsInCombat())
                                botanist->AI()->DoZoneInCombat(botanist);
                    }
                }
                break;
            }
        ...
        switch (summon->GetEntry())
        {
            ...
            case NPC_NATURALIST_TELARN:
            {
                if (IsMythicRaid())
                {
                    ...
                    {
                        instance->SendEncounterUnit(ENCOUNTER_FRAME_INSTANCE_END, me );
                        instance->DoUpdateCriteria(CriteriaType::DefeatDungeonEncounter, 1886);
            ...
            switch (eventId)
            {
                case EVENT_CONTROLLED_CHAOS:
                    ...
                    if (healthPct == 0)          //Phase 3
                        events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 50s);
                    else if (healthPct == 51)    //Phase 2
                        events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 40s);
                    else                         //Phase 1
                        events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 35s);
                    break;
                case EVENT_PARASITIC_FETTER:
                    ...
                    if (healthPct == 51)         //Phase 2
                        events.RescheduleEvent(EVENT_PARASITIC_FETTER, 40s);
                    else                         //Phase 1
                        events.RescheduleEvent(EVENT_PARASITIC_FETTER, 35s);
                    break;
                case EVENT_SOLAR_COLLAPSE:
                    Talk(SAY_SOLAR_COLLAPSE);
                    DoCast(SPELL_SOLAR_COLLAPSE_FILTER);
                    events.RescheduleEvent(EVENT_SOLAR_COLLAPSE, 35s);
                    break;
                case EVENT_PHASE_2:
                ...
                case EVENT_CALL_OF_NIGHT:
                    ...
                    events.RescheduleEvent(EVENT_CALL_OF_NIGHT, 50s);
                    break;
            }
        }
...
struct npc_nh_solarist_telarn : ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        me->SetHealth(summoner->ToUnit()->GetHealth());
        DoZoneInCombat();
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        switch (phase)
        {
            case 0: // LFR/NORMAL/HEROIC
                ...
                events.RescheduleEvent(EVENT_FLARE, 7s);
                events.RescheduleEvent(EVENT_SOLAR_COLLAPSE, 32s);
                events.RescheduleEvent(EVENT_SUM_PLASMA_SPHERES, 10s);
                break;
            case 1:
                globalTimer = 64 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_SOLAR_COLLAPSE, 5s);
                events.RescheduleEvent(EVENT_FLARE, 7s);
                events.RescheduleEvent(EVENT_SUM_PLASMA_SPHERES, 45s);
                break;
            case 2:
                globalTimer = 55 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_FLARE, 2s);
                if (me->HasAura(SPELL_NATURE_INFUSION))
                {
                    events.RescheduleEvent(EVENT_SOLAR_COLLAPSE, 15s);
                    events.RescheduleEvent(EVENT_SUM_PLASMA_SPHERES, 25s);
                }
                else
                {
                    events.RescheduleEvent(EVENT_COLLAPSE_OF_NIGHT, 28s);
                    events.RescheduleEvent(EVENT_SUM_PLASMA_SPHERES, 40s);
                }
                break;
            case 3:
                globalTimer = 35 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_FLARE, 8s);
                events.RescheduleEvent(EVENT_COLLAPSE_OF_NIGHT, 20s);
                events.RescheduleEvent(EVENT_SUM_PLASMA_SPHERES, 35s);
                break;
        }
    }
        ...
        switch (spell->Id)
        {
            ...
            case SPELL_FLARE:
                if (IsMythicRaid() && _phase == 3)
                    DoCast(target->ToUnit(), SPELL_PARASITIC_FETTER_MARK, true);
                break;
        }
    }
...
struct npc_nh_arcanist_telarn : ScriptedAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        switch (phase)
        {
            case 1:
                globalTimer = 64 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 30s);
                events.RescheduleEvent(EVENT_CALL_OF_NIGHT, 57s);
                break;
            case 2:
                globalTimer = 55 * IN_MILLISECONDS;
                if (me->HasAura(SPELL_SOLAR_INFUSION))
                    events.RescheduleEvent(EVENT_SUMMON_CHAOS_SPHERES, 25s); // FIXME. Need Timing!
                else
                    events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 55s);
                events.RescheduleEvent(EVENT_CALL_OF_NIGHT, 42s);
                break;
            case 3:
                globalTimer = 35 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_CONTROLLED_CHAOS, 55s);
...
struct npc_nh_naturalist_telarn : ScriptedAI
{
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        switch (phase)
        {
            case 0:
                globalTimer = 64 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_TOXIC_SPORES, 8s);
                events.RescheduleEvent(EVENT_GRACE_OF_NATURE, 9s);
                events.RescheduleEvent(EVENT_PARASITIC_FETTER, 35s);
                break;
            case 1:
                globalTimer = 64 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_TOXIC_SPORES, 6s);
                events.RescheduleEvent(EVENT_GRACE_OF_NATURE, 64s);
                events.RescheduleEvent(EVENT_PARASITIC_FETTER, 16s);
                break;
            case 2:
                globalTimer = 55 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_TOXIC_SPORES, 6s);
            ...
            case 3: //FIXME. Need Valid Timing
                globalTimer = 35 * IN_MILLISECONDS;
                events.RescheduleEvent(EVENT_TOXIC_SPORES, 6s);
                events.RescheduleEvent(EVENT_CHAOTIC_SPHERE_NATURE, 35s);
                events.RescheduleEvent(EVENT_PARASITIC_FETTER, 16s);
                break;
        }
    }
...
struct npc_telarn_controlled_chaos_stalker : public ScriptedAI
{
    ...
    {
        Unit* summoner = owner->ToUnit();
        if (!summoner)
            return;

        events.RescheduleEvent(EVENT_1, 1s);

        naturSolar = summoner->HasAura(SPELL_NATURE_INFUSION) && summoner->HasAura(SPELL_SOLAR_INFUSION);
        arkanSolar = summoner->HasAura(SPELL_ARCANE_INFUSION) && summoner->HasAura(SPELL_SOLAR_INFUSION);
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_1:
                    ...
                    if (auto owner = me->GetOwner())
                    {
                        if (auto target = owner->GetAI()->SelectTarget(SelectTargetMethod::Random, 1, 100.0f, true, true, -SPELL_PARASITIC_FIXATE))
                        {
                        ...
                        else if (Unit* target = owner->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
...
struct npc_telarn_solar_collapse_stalker : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* owner) override
    {
        events.RescheduleEvent(EVENT_1, 1s); //12c-25y, 12c-20y, 6c-15y, 6c-10y, 6c-5y, 1c-0y
        events.RescheduleEvent(EVENT_2, 15s); //Despawn Event
...
struct npc_telarn_plasma_sphere : ScriptedAI
{
  ...
  void DamageTaken(Unit* /*attacker*/, uint32& damag, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        if (damag >= me->GetHealth())
        {
            if (!explosion)
            {
                ...
                //me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_UNINTERACTIBLE);
...
void AddSC_boss_high_botanist_telarn()
{
    ...
    RegisterSpellScript(spell_telarn_parasitic_fetter);
    RegisterSpellScript(spell_telarn_parasitic_fixate);
    RegisterSpellScript(spell_telarn_parasitic_fetter_mark);
    RegisterSpellScript(spell_telarn_rampant_growth);
    RegisterSpellScript(spell_telarn_plasma_explosion);
    RegisterSpellScript(spell_telarn_call_of_night_aura);
