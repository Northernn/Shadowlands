...
public:
    ...
    struct boss_spellblade_alurielAI : BossAI
    {
        ...
        void DefaultEvents(uint32 phase)
        {
            ...
            if (IsHeroicPlusRaid())
                events.RescheduleEvent(EVENT_FROZEN_TEMPEST_FILTER, 16s);

            if (IsMythicRaid())
                events.RescheduleEvent(EVENT_SUM_FEL_SOUL, 18s);

            if (IsMythicRaid() && phase == SPELL_MAGIC_ENCHANTMENT)
                events.RescheduleEvent(EVENT_ANNIHILATE, 5s);
            else
                events.RescheduleEvent(EVENT_ANNIHILATE, 7s);

            switch (phase)
            {
            case SPELL_ICY_ENCHANTMENT:
                DoCast(me, SPELL_ICY_ENCHANTMENT, true);
                events.RescheduleEvent(EVENT_MARK_OF_FROST, 18s);
                if (IsMythicRaid())
                {
                    events.RescheduleEvent(EVENT_REPLICATE_FROST, 28s);
                    events.RescheduleEvent(EVENT_DETONATE_FROST, 48s);
                    events.RescheduleEvent(EVENT_ANIMATE_FROST, 65s);
                }
                else
                {
                    events.RescheduleEvent(EVENT_REPLICATE_FROST, 38s);
                    events.RescheduleEvent(EVENT_DETONATE_FROST, 68s);
                    events.RescheduleEvent(EVENT_ANIMATE_FROST, 75s);
                }
                break;
            case SPELL_FIERY_ENCHANTMENT:
                DoCast(me, SPELL_FIERY_ENCHANTMENT, true);
                events.RescheduleEvent(EVENT_SEARING_BRAND, 18s);
                events.RescheduleEvent(EVENT_REPLICATE_SEARING_BRAND, 26s);
                if (IsMythicRaid())
                {
                    events.RescheduleEvent(EVENT_DETONATE_FIRE, 38s);
                    events.RescheduleEvent(EVENT_ANIMATE_FIRE, 54s);
                }
                else
                {
                    events.RescheduleEvent(EVENT_DETONATE_FIRE, 45s);
                    events.RescheduleEvent(EVENT_ANIMATE_FIRE, 62s);
                }
                break;
            case SPELL_MAGIC_ENCHANTMENT:
                DoCast(me, SPELL_MAGIC_ENCHANTMENT, true);
                events.RescheduleEvent(EVENT_ARCANE_ORB, 1s);
                events.RescheduleEvent(EVENT_REPLICATE_ARCANE_ORB, 15s);
                events.RescheduleEvent(EVENT_DETONATE_ARCANE_ORB, 35s);
                events.RescheduleEvent(EVENT_ANIMATE_ARCANE, 52s);
                break;
            }
        }
        ...
        void SpellFinishCast(const SpellInfo* spell)
        {
            ...
            case SPELL_ANIMATE_ARCANE_ORB:
                events.RescheduleEvent(EVENT_ANIMATE_DESPAWN, 1s);
                break;
            case SPELL_REPLICATE_SEARING_BRAND:
                events.RescheduleEvent(EVENT_REPLICATE_SEARING_BRAND_2, 1s);
                break;
            default:
                break;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_ANNIHILATE:
                    ...
                    if (IsMythicRaid())
                    {
                        if (me->HasAura(SPELL_ICY_ENCHANTMENT))
                            events.RescheduleEvent(EVENT_ANNIHILATE, 45s);
                        else if (me->HasAura(SPELL_FIERY_ENCHANTMENT))
                            events.RescheduleEvent(EVENT_ANNIHILATE, 38s);
                        else if (me->HasAura(SPELL_MAGIC_ENCHANTMENT))
                            events.RescheduleEvent(EVENT_ANNIHILATE, 43s);
                    }
                    else
                    {
                        if (me->HasAura(SPELL_MAGIC_ENCHANTMENT))
                            events.RescheduleEvent(EVENT_ANNIHILATE, 35s);
                        else
                            events.RescheduleEvent(EVENT_ANNIHILATE, 45s);
                    }
                    break;
                ...
                case EVENT_ANIMATE_FROST:
                    ...
                    if (IsHeroicPlusRaid())
                    {
                        events.RescheduleEvent(EVENT_FROZEN_TEMPEST_FILTER, 9s);
                    }
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            ...
            case NPC_FIERY_ENCHANTMENT:
            {
                ...
                if (IsHeroicPlusRaid())
                {
                    ...
                    events.RescheduleEvent(EVENT_2, 3s); // Bound in Flame
                    events.RescheduleEvent(EVENT_3, 7s); // Pyroblast
                }
                break;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                //        me->AddThreat(target, 1000.0f);
                    ...
                    events.RescheduleEvent(EVENT_3, 7s);
                    break;
                }
            }
                ...
                if (Player* player = ObjectAccessor::GetPlayer(*me, playerGUID))
                {
                    if (player->IsAlive() && player->HasAura(SPELL_SEARING_BRAND_MARK, me->GetGUID()))
                    {
                        if (Unit* owner = me->GetOwner())
                        {
                            ...
                            me->CastSpell(player, SPELL_SEARING_BRAND_DOT, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(owner->GetGUID()));
    ...
    struct npc_aluriel_arcane_orbAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* target, SpellInfo const* spell) override
        {
            ...
            case SPELL_DETONATE_ARCANE_ORB:
                ...
                break;
            case SPELL_ANIMATE_ARCANE_ORB:
                events.RescheduleEvent(EVENT_3, 1s); //Despawn
                break;
            }
        }
        ...
        void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
        {
            ...
            case SPELL_ARCANE_ORB_SEARCH_PLR:
                if (Unit* owner = me->GetOwner())
                    me->CastSpell(target, 213511, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(owner->GetGUID()));
                break;
            }
        }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == POINT_MOTION_TYPE)
            {
                ...
                {
                    if (Unit* owner = me->GetOwner())
                        me->CastSpell(me, SPELL_ARCANE_ORB_MISSILE, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(owner->GetGUID()));
                }
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_2:
                    if (Unit* owner = me->GetOwner())
                    {
                        if (Unit* target = owner->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 80.0f, true))
                        {
                            ...
                            me->CastSpell(target, SPELL_ARCANE_ORB_AT, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(owner->GetGUID()));
...
public:
    ...
    struct npc_aluriel_fel_soulAI : public ScriptedAI
    {
        ...
        void IsSummonedBy(WorldObject* summonerWO) override
        {
            ...
            despawnTimer = 45000;
            events.RescheduleEvent(EVENT_1, 1s);   // Cast SPELL_SEVERED_SOUL
            events.RescheduleEvent(EVENT_2, 2s);  // Check distance owner
            events.RescheduleEvent(EVENT_3, 20s); // Cast SPELL_DECIMATE

            if (summoner->HasAura(SPELL_ICY_ENCHANTMENT))
                events.RescheduleEvent(EVENT_4, 10s); // Cast SPELL_WHIRLING_BARRAGE

            if (summoner->HasAura(SPELL_FIERY_ENCHANTMENT))
                events.RescheduleEvent(EVENT_5, 10s); // Cast SPELL_FEL_STOMP_FILTER

            if (summoner->HasAura(SPELL_MAGIC_ENCHANTMENT)) //35:31, 35:42, 35:48, 36:00, 36:06
            {
                lashSwitcher = false;
                events.RescheduleEvent(EVENT_7, 9s); // Cast SPELL_FEL_LASH
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_7: //Arcane Phase
                    ...
                    {
                        lashSwitcher = true;
                        events.RescheduleEvent(EVENT_7, 11s);
                    }
                    else
                    {
                        lashSwitcher = false;
                        events.RescheduleEvent(EVENT_7, 6s);
                    }
                    break;
                }
            }
                    ...
                    if (InstanceScript* instance = target->GetInstanceScript())
                        if (instance->GetBossState(DATA_ALURIEL) == IN_PROGRESS)
                        {
                            if (Creature* aluriel = Creature::GetCreature(*target, instance->GetGuidData(DATA_ALURIEL)))
                                target->CastSpell(target, SPELL_DETONATE_SEARING_BRAND_AT, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(aluriel->GetGUID()));
                        }
            }
        }
            ...
            if (Unit* caster = GetCaster())
            {
                if (!targets.empty())
                {
                    if (Aura* flame = caster->GetAura(SPELL_BOUND_IN_FLAME))
                    {
                        ...
                        caster->CastSpell(caster, SPELL_BOUND_IN_FLAME, CastSpellExtraArgs(TRIGGERED_FULL_MASK).AddSpellMod(SPELLVALUE_AURA_STACK, targets.size()));
                }
                else
...
void AddSC_boss_aluriel()
{
    ...
    RegisterSpellScript(spell_aluriel_mark_of_frost);
