...
struct boss_skorpyron : BossAI
{
    ...
    void JustEngagedWith(Unit* who) override //37:22
    {
        ...
        events.RescheduleEvent(EVENT_FOCUSED_BLAST, 13s);
        events.RescheduleEvent(EVENT_CALL_SCORPID, 22s);
        events.RescheduleEvent(EVENT_ARCANOSLASH, 6s);
    }
...
void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    ...
    void DoAction(int32 const action) override
    {
        if (action == ACTION_1)
        {
            ...
            events.RescheduleEvent(EVENT_VULNERABILITY, 1s);
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_FOCUSED_BLAST:
                ...
                events.RescheduleEvent(EVENT_FOCUSED_BLAST, 33s);
                break;
            case EVENT_CALL_SCORPID:
                DoCast(me, SPELL_CALL_SCORPID, true);
                events.RescheduleEvent(EVENT_CALL_SCORPID, 20s);
                break;
            ...
            case EVENT_ARCANOSLASH:
                ...
                events.RescheduleEvent(EVENT_ARCANOSLASH, 10s);
                break;
            }
        }
...
struct npc_skorpyron_generic_scorp : public ScriptedAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        case NPC_ACIDMAW_SCORPID:
            if (Unit* owner = me->GetOwner())
                me->CastSpell(me, SPELL_TOXIC_CHITIN_AT, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(owner->GetGUID()));
            break;
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_3:
                if (Unit* owner = me->GetOwner())
                    me->CastSpell(me, SPELL_TOXIC_CHITIN, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(owner->GetGUID()));
...
struct npc_skorpyron_crystalline_fragments : public ScriptedAI
{
    explicit npc_skorpyron_crystalline_fragments(Creature* creature) : ScriptedAI(creature)
    {
        me->SetReactState(REACT_PASSIVE);
    }
...
//103682
struct npc_skorpyron_arcane_tether : public ScriptedAI
{
    explicit npc_skorpyron_arcane_tether(Creature* creature) : ScriptedAI(creature)
    {
        me->SetReactState(REACT_PASSIVE);
...
void AddSC_boss_skorpyron()
{
    ...
    RegisterSpellScript(spell_skorpyron_periodic_energize);
    RegisterSpellScript(spell_skorpyron_chitinous_exoskeleton);
