...
struct boss_helya : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_TAINT_SEA, 0s);
        events.RescheduleEvent(EVENT_SUM_DESTRUCTOR_TENTACLE, 26s);
       // events.RescheduleEvent(EVENT_SUM_PIERCING_TENTACLE, piercingTentacleTimer);

        if (me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
            events.RescheduleEvent(EVENT_BRACKWATER_BARRAGE, 15s);
        else
            events.RescheduleEvent(EVENT_TORRENT, 3s);
    ...
    void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
    {
        ...
        case NPC_GRASPING_TENTACLE_4:
        {
            if (++tentacleDiedCound == 6)
            {
                ...
                events.Reset();
                events.RescheduleEvent(EVENT_P1_SUBMERGED_START, 1s);
                Talk(SAY_87);
            }
...
void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        if (me->HealthBelowPct(71) && !encounterComplete)
        {
            ...
            Talk(SAY_DEATH);
            _JustDied();
            instance->DoUpdateCriteria(CriteriaType::DefeatDungeonEncounter, 1824);
            ...
            instance->DoOnPlayers([this](Player* player)
            {
                ...
                if (CheckAura())
                {
                    if (me->GetMap()->GetDifficultyID() == DIFFICULTY_MYTHIC || me->GetMap()->GetDifficultyID() == DIFFICULTY_MYTHIC_KEYSTONE)
                        instance->DoUpdateCriteria(CriteriaType::BeSpellTarget, 213415);
                }
            });
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        if (spell->Id == SPELL_SMASH)
            target->ToUnit()->GetMotionMaster()->MoveJump(smashPos[urand(0, 4)], 30.0f, 10.0f);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_TAINT_SEA:
                ...
                events.RescheduleEvent(EVENT_TAINT_SEA, 12s);
                Talk(SAY_TAINT_SEA);
                break;
            case EVENT_TORRENT:
                DoCast(SPELL_TORRENT);
                events.RescheduleEvent(EVENT_TORRENT, 5s);
                break;
            case EVENT_TORRENT_2: //Second Phase
                DoCast(SPELL_TORRENT_2);
                events.RescheduleEvent(EVENT_TORRENT_2, 8s);
                break;
            ...
            //    events.RescheduleEvent(EVENT_SUM_PIERCING_TENTACLE, piercingTentacleTimer = 2000);
                events.RescheduleEvent(EVENT_P1_SUBMERGED_END, 15s);
                break;
            case EVENT_P1_SUBMERGED_END:
                ...
                events.RescheduleEvent(EVENT_ACTION_SWIRLING_POOL, 1s);
                events.RescheduleEvent(EVENT_WING_BUFFET, 10s);
                events.RescheduleEvent(EVENT_TORRENT_2, 16s);
                events.RescheduleEvent(EVENT_CORRUPTED_BELLOW, 15s);
                events.RescheduleEvent(EVENT_P2_SUBMERGED_START, 70s);
                break;
            case EVENT_P2_SUBMERGED_START: //In Phase 2
                ...
                events.Reset();
                events.RescheduleEvent(EVENT_P2_SUBMERGED_END, 15s);
              //  events.RescheduleEvent(EVENT_SUM_PIERCING_TENTACLE, piercingTentacleTimer);
                break;
            case EVENT_P2_SUBMERGED_END: //In Phase 2
                ...
                events.RescheduleEvent(EVENT_ACTION_SWIRLING_POOL, 1s);
                events.RescheduleEvent(EVENT_WING_BUFFET, 10s);
                events.RescheduleEvent(EVENT_TORRENT_2, 14s);
                events.RescheduleEvent(EVENT_CORRUPTED_BELLOW, 16s);
                events.RescheduleEvent(EVENT_P2_SUBMERGED_START, 70s);
                break;
            case EVENT_WING_BUFFET:
                ...
                events.RescheduleEvent(EVENT_WING_BUFFET, 2s);
                break;
            ...
            case EVENT_BRACKWATER_BARRAGE:
            {
                ...
                events.RescheduleEvent(EVENT_BRACKWATER_BARRAGE, 22s);
                break;
            }
            case EVENT_CORRUPTED_BELLOW:
            {
                ...
                events.RescheduleEvent(EVENT_CORRUPTED_BELLOW, 21s);
...
struct npc_helya_tentacle : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        case 99801: //DESTRUCTOR_TENTACLE
            Talk(0);
            events.RescheduleEvent(EVENT_5, 2s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_3:
                ...
                events.RescheduleEvent(EVENT_4, 4s);
                break;
            case EVENT_4:
            ...
            case EVENT_5:
            {
                ...
                {
                    events.RescheduleEvent(EVENT_5, 3s);
                    break;
                }
...
struct npc_helya_tentacle_veh : public ScriptedAI
{
    ...
    void DoAction(int32 const action) override
    {
        ...
        {
            if (action == ACTION_1)
                owner->SummonCreature(NPC_PIERCING_TENTACLE, me->GetPosition(), TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 6s);
                ...
                if (!creatureList.empty())
                {
                    for (auto creature : creatureList)
                    {
                        if (creature->IsInCombat() && creature->HealthBelowPct(80))
                        {
                            ...
                            events.RescheduleEvent(EVENT_2, 19s);
                            break;
                        }
                        else
                            events.RescheduleEvent(EVENT_2, 1s);
...
struct npc_mos_helarjar_mistcaller : public ScriptedAI
{
    ...
    {
        events.RescheduleEvent(EVENT_1, 2s);
        events.RescheduleEvent(EVENT_2, 6s);
        events.RescheduleEvent(EVENT_3, 1s);
    }
                ...
                if (!creatureList.empty())
                {
                    for (auto creature : creatureList)
                    {
                        if (creature->IsInCombat() && creature->HealthBelowPct(40))
                        {
                            ...
                            events.RescheduleEvent(EVENT_3, 25s);
                            break;
                        }
                        else
                            events.RescheduleEvent(EVENT_3, 1s);
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        if (auto caster = GetCaster()->ToPlayer())
        {
            ...
            caster->GetScheduler().Schedule(1s, [this](TaskContext /*context*/)
            {
                ...
                {
                    float x, y, z;
                    caster->GetClosePoint(x, y, z, caster->GetCombatReach(), 5.0f, frand(0.0f, 6.28f));
...
void AddSC_boss_helya_maw()
{
    ...
    RegisterSpellScript(spell_brackwater);
    RegisterSpellScript(spell_helya_turbulent_waters);
    RegisterSpellScript(spell_helya_taint_of_the_sea);
    RegisterSpellScript(spell_whirpool_of_souls);
    RegisterSpellScript(spell_torrent_of_souls);
}
