...
struct boss_harbaron : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_COSMIC_SCYTHE, 4s);
        events.RescheduleEvent(EVENT_SUM_SHACKLED_SERVITOR, 7s);
        events.RescheduleEvent(EVENT_FRAGMENT, 18s);

        if (me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
            events.RescheduleEvent(EVENT_NETHER_RIP, 13s);
    }
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        ...
        if (spell->Id == SPELL_FRAGMENT)
        {
            ...
            for (uint8 i = 0; i < maxCount; ++i)
            {
                ...
                if (auto soul = target->SummonCreature(NPC_SOUL_FRAGMENT, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s))
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_COSMIC_SCYTHE:
                ...
                events.RescheduleEvent(EVENT_COSMIC_SCYTHE, 8s);
                break;
            case EVENT_SUM_SHACKLED_SERVITOR:
            {
                ...
                events.RescheduleEvent(EVENT_SUM_SHACKLED_SERVITOR, 23s);
                break;
            }
            case EVENT_FRAGMENT:
                Talk(SAY_FRAGMENT);
                ...
                events.RescheduleEvent(EVENT_FRAGMENT, 30s);
                break;
