...
struct boss_mistress_alluradel : public ScriptedAI
{
    ...
    {
        Talk(SAY_AGGRO);
        events.RescheduleEvent(EVENT_HEART_BREAKER, 4s);
        ...
        case SPELL_CAVITATION:
            if (target->IsPlayer()/* && target->IsInCombat()*/)
            {
                ...
                for (uint8 i = 0; i < 4; i++)
                {
                    ...
                    target->CastSpell(pos, SPELL_WAKE_OF_DESTRUCTION, true);
                    angle += 1.57f;
                }
            }
            break;
...
int32 RANDOM_GROTESQUE_SPAWN[2] =
{
...
struct boss_matron_folnuna : public ScriptedAI
{
    ...
    void StartEvents(uint8 phase)
    {
        ...
        case PHASE_1:
            slimbering = false;
            me->SetReactState(REACT_AGGRESSIVE);
            events.RescheduleEvent(EVENT_FEL_BLAST, 2s);
    ...
    void SpellHitTarget(WorldObject* /*target*/, SpellInfo const* spell) override
    {
        ...
        case SPELL_GROTESQUE_SPAWN:
            auto threatlist = me->GetThreatManager().GetThreatenedByMeList();
            if (!threatlist.empty())
            {
...
class spell_slimbering_gasp : public AuraScript
{
    ...
    void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        caster->GetAI()->DoAction(1);
    }

    void Register() override
    {
        OnEffectRemove += AuraEffectRemoveFn(spell_slimbering_gasp::OnRemove, EFFECT_1, SPELL_AURA_ADD_TARGET_TRIGGER, AURA_EFFECT_HANDLE_REAL);
    }
};
