...
struct boss_aggramar : BossAI
{
    ...
    void DefaultEvents(bool nextPhase = false)
    {
        ...
        case PHASE_1:
            if (IsMythicRaid())
            {
                events.RescheduleEvent(EVENT_RAVENOUS_BLAZE, 4s);
                events.RescheduleEvent(EVENT_WAKE_OF_FLAME, 11s);
            }
            else
            {
                events.RescheduleEvent(EVENT_SCORCHING_BLAZE, 5s);
                events.RescheduleEvent(EVENT_WAKE_OF_FLAME, 6s);
            }
            break;
        case PHASE_2:
            if (IsMythicRaid())
                events.RescheduleEvent(EVENT_RAVENOUS_BLAZE, 23s);
            else
                events.RescheduleEvent(EVENT_SCORCHING_BLAZE, 6s);
            events.RescheduleEvent(EVENT_FLARE, 10s);
            break;
        case PHASE_3:
            if (IsMythicRaid())
                events.RescheduleEvent(EVENT_RAVENOUS_BLAZE, 23s);
                ...
                events.RescheduleEvent(EVENT_SCORCHING_BLAZE, 6s);
            events.RescheduleEvent(EVENT_EMPOWERED_FLARE, 10s);
            break;
        }
    }
    ...
    void SpellFinishCast(const SpellInfo* spell)
    {
        ...
        case SPELL_TAESHALACH_TECHNIQUE:
            ...
            events.RescheduleEvent(EVENT_COMBO, 1s);
            break;
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
    {
        ...
        case SPELL_TAESHALACH_REACH_DMG:
            DoCast(target->ToUnit(), SPELL_TAESHALACH_REACH_MOD, true);
            break;
        case SPELL_WAKE_OF_FLAME_DUMMY:
        {
            ...
            for (uint8 i = 0; i < 3; ++i)
            {
                ...
                //pos.m_orientation = me->GetAbsoluteAngle(pos.GetPositionX(), pos.GetPositionY());
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        if (me->HealthBelowPct(healthPct))
        {
            ...
            events.Reset();
            events.RescheduleEvent(EVENT_PHASE_SWITCH, 1s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_COMBO:
                ...
                events.RescheduleEvent(EVENT_COMBO, IsMythicRaid() ? 35s : 5s);
                break;
            ...
            case EVENT_WAKE_OF_FLAME:
                ...
                events.RescheduleEvent(EVENT_WAKE_OF_FLAME, 24s);
                break;
            ...
            case EVENT_FLARE:
                ...
                if (!IsMythicRaid())
                    events.RescheduleEvent(EVENT_FLARE, 15s);
                break;
            case EVENT_EMPOWERED_FLARE:
                Talk(SAY_FLARE);
                DoCast(SPELL_EMPOWERED_FLARE_FILTER);
                if (!IsMythicRaid())
                    events.RescheduleEvent(EVENT_EMPOWERED_FLARE, 15s);
                break;
            case EVENT_RAVENOUS_BLAZE:
                DoCast(SPELL_RAVENOUS_BLAZE_FILTER);
                events.RescheduleEvent(EVENT_RAVENOUS_BLAZE, 22s);
                break;
            }
        }
    ...
    void DoAction(int32 const actionId)
    {
        ...
        if (actionId == ACTION_1)
        {
            ...
            if (auto owner = me->GetOwner())
                me->CastSpell(me, SPELL_BLAZING_ERUPTION, CastSpellExtraArgs(TRIGGERED_FULL_MASK).SetOriginalCaster(owner->GetGUID()));
...
class spell_aggramar_flame_rend_filter : public SpellScript
{
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_aggramar_flame_rend_filter::FilterTargets, EFFECT_1, TARGET_UNIT_CONE_CASTER_TO_DEST_ENEMY);
...
class spell_aggramar_command_empowered_flare : public SpellScript
{
    ...
    void HandleScript(SpellEffIndex /*effectIndex*/)
    {
        ...
        for (uint8 i = 0; i < maxCount; ++i)
        {
            ...
            //pos.m_orientation = GetHitDest()->GetAbsoluteAngle(pos.GetPositionX(), pos.GetPositionY());
...
void AddSC_boss_aggramar()
{
    ...
    RegisterSpellScript(spell_aggramar_energize_periodic);
    RegisterSpellScript(spell_aggramar_wrought_in_flame);
    RegisterSpellScript(spell_aggramar_meteor_swarm);
    RegisterSpellScript(spell_aggramar_ravenous_blaze);
