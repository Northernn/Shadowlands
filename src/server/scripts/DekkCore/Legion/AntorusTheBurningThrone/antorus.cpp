    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_2:
                targs.clear();
                DoCast(252621);
                events.RescheduleEvent(EVENT_2, 10s, 14s);
                break;
            }
        }
...
struct npc_atbt_battleship_controller : public ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
            case EVENT_1:
                me->SummonCreature(NPC_BATTLE_SHIP, -3268.71f, 10120.37f, -67.19f, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 9s);
                events.RescheduleEvent(EVENT_1, 3s, 5s);
                break;
            }
        }
    }
                ...
                AddDelayedEvent(100 * ++i, [this, y]() -> void
                    {
                        ...
                        Position targetedPosition = {x, y, -115.73f};
                    me->CastSpell(targetedPosition, 249933, TRIGGERED_FULL_MASK);
                });
            }
        });
    }
...
public:
    ...
    {
        return new npt_atbt_tele_gatesAI(creature);
    }

    struct npt_atbt_tele_gatesAI : public ScriptedAI
    {
        npt_atbt_tele_gatesAI(Creature* creature) : ScriptedAI(creature) { }
        ...
        bool OnGossipHello(Player* player) override
        {
            ...
            if (isDone && me->GetEntry() != 128303)
                AddGossipItemFor(player, GossipOptionNpc::None, "Imonar", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

            if (me->GetEntry() != 125720)
                AddGossipItemFor(player, GossipOptionNpc::None, "Start", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
            ...
            if (me->GetEntry() != 128304)
                AddGossipItemFor(player, GossipOptionNpc::None, "Varithmas", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_atbt_muradinAI(creature);
    }

    struct npc_atbt_muradinAI : public ScriptedAI
    {
        npc_atbt_muradinAI(Creature* creature) : ScriptedAI(creature) { }
...
void AddSC_antorus()
{
    ...
    RegisterSpellScript(spell_atbt_bound_by_fel);
    RegisterSpellScript(spell_atbt_soulburn);
