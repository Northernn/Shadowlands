        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_CRACKLING_JOLT:
                {
                    ...
                    if (TempSummon* tempSumm = me->SummonCreature(WORLD_TRIGGER, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 14, 0, TEMPSUMMON_TIMED_DESPAWN, 10s))
                    {
                        ...
                        tempSumm->CastSpell(tempSumm, SPELL_CRACKLING_JOLT_TARGET_PICKER, true); //If any of you knows how to make the game think Nithogg launched the spell, please tell me
                    }
                    events.ScheduleEvent(EVENT_CRACKLING_JOLT, 15s);
                    break;
                }
                case EVENT_ELECTRICAL_STORM:
                {
                    DoCastAOE(SPELL_ELECTRICAL_STORM_TARGET);
                    events.ScheduleEvent(EVENT_ELECTRICAL_STORM, 30s);
                    break;
                }
                case EVENT_STORM_BREATH:
                {
                    DoCastVictim(SPELL_STORM_BREATH);
                    events.ScheduleEvent(EVENT_STORM_BREATH, 23s);
                    break;
                }
                case EVENT_STATIC_CHARGE:
                {
                    ...
                    events.ScheduleEvent(EVENT_STATIC_CHARGE, 40s);
                    break;
                }
            }
        }
...
struct npc_nithogg_electrical_storm : public ScriptedAI
{
    ...
    void Reset() override
    {
        //me->SetUnitFlag(UNIT_FLAG_REMOVE_CLIENT_CONTROL); //TODO : Change this to a spline once those work/I know how to use them.
        ApplyBuff();
    }
...
class spell_nithogg_static_charge_summon : public SpellScript
{
    void HandleHitTarget(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* summoned = caster->SummonCreature(NPC_STATIC_ORB, *caster, TEMPSUMMON_TIMED_DESPAWN, 12s))
        {
...
struct npc_nithogg_static_orb : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        //me->SetUnitFlag(UnitFlags(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_UNINTERACTIBLE));  flags are now in db
    }
