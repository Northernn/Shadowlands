...
struct boss_humongris : public ScriptedAI
{
    ...
    {
        events.ScheduleEvent(EVENT_FIRE_BOOM, 14s);
        events.ScheduleEvent(EVENT_EARTHSHAKE_STOMP, 28s);
        events.ScheduleEvent(EVENT_ICE_FIST, 29s);
        events.ScheduleEvent(EVENT_MAKE_SNOW, 35s);
        events.ScheduleEvent(EVENT_YOU_GO_BANG, 24s);
    }
    ...
    void ExecuteEvent(uint32 eventId) //override
    {
        ...
        case EVENT_ICE_FIST:
            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0.0f, 10.0f, true))
            {
        ...
        case EVENT_YOU_GO_BANG:
            if (Unit* target = SelectTarget(SelectTargetMethod::MaxThreat, 0.0, 100.0f, true))
