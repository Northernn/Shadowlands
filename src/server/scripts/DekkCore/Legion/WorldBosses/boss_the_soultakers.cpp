    ...
    struct boss_soultakersAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* unit) override
        {
            ...
            if (me->GetEntry() == NPC_MEVRA)
                events.RescheduleEvent(3, 1s); // 9000

            events.RescheduleEvent(10, 1s); // check for energy
            ...
            if (me->GetEntry() == NPC_CAPITAN)
            {
                if (spell->Id == SPELL_SHATTER_CREWMEN)
                {
                    if (target->GetTypeId() == TYPEID_UNIT)
                    {
                        ...
                        {
                            if (target->ToPlayer()->IsAlive())
                            {
                                target->CastSpell(target, SPELL_SHATTER_CREWMEN_ADD);
                                target->ToCreature()->DespawnOrUnsummon(500ms);
                            }
                        }
                    }
                }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case 3:
                {
                    ...
                    me->CastSpell(tpPos[rand], SPELL_TELEPORT);
