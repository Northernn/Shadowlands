...
public:
    ...
    struct boss_flotsamAI : public WorldBossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_JETSAM, 10s);
            me->AI()->Talk(uint8(88247)); //On aggro text
            me->SetPower(POWER_MANA, 0);
            getsamCount = 0;
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (events.Empty())
            {
                events.ScheduleEvent(EVENT_JETSAM, 1s);
            }
            ...
            if (me->IsInCombat())
            {
                ...
                if (manaTimer >= 1000)
                {
                    if (me->GetPower(POWER_MANA) < 100)
                        ...
                        events.ScheduleEvent(EVENT_GETSAM, 2s);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_JETSAM:
                {
                    if (me->IsInCombat())
                    {
                        ...
                        getsamCount++;
                        events.ScheduleEvent(EVENT_BREAKSAM, 5s);
                    }
                    break;
                }
                case EVENT_BREAKSAM:
                {
                    me->CastSpell(SelectTarget(SelectTargetMethod::MaxThreat), SPELL_BREAKSAM_CAST, false);

                    if (getsamCount >= 4)
                    {
                        events.ScheduleEvent(EVENT_YAKSAM, 5s);
                        getsamCount = 0;
                    } else
                        events.ScheduleEvent(EVENT_JETSAM, 10s);
                ...
                case EVENT_YAKSAM:
                {
                    ...
                    events.ScheduleEvent(EVENT_JETSAM, 5s);
                    events.ScheduleEvent(EVENT_YAKSAM_SUMMON, 2s);
                    break;
                }
                case EVENT_GETSAM:
                {
...
public:
    ...
    class spell_flotsam_jetsam_target_SpellScript : public SpellScript
    {
        void HandleHitTarget(SpellEffIndex /*effIndex*/)
        {
            ...
            if (TempSummon* tempSumm = caster->SummonCreature(NPC_JETSAM, hit->GetPositionX(), hit->GetPositionY(), hit->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 2min))
            {
