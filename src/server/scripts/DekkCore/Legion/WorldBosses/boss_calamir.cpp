        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_PHASE_FIRE:
                    ...
                    events.ScheduleEvent(EVENT_CHANGE_PHASE, 20s);
                    events.ScheduleEvent(EVENT_BURNING_BOMB, 2s);
                    events.ScheduleEvent(EVENT_WRATHFUL_FLAMES, 8s);
                    lastPhase = PHASE_FIRE;
                    break;
                case EVENT_PHASE_FROST:
                    me->CastSpell(me, SPELL_AR_FROST, true);
                    events.ScheduleEvent(EVENT_HOWLING_GALE, 2s);
                    events.ScheduleEvent(EVENT_ICY_COMET, 8s);
                    events.ScheduleEvent(EVENT_CHANGE_PHASE, 20s);
                ...
                case EVENT_PHASE_ARCANE:
                    ...
                    events.ScheduleEvent(EVENT_CHANGE_PHASE, 20s);
                    events.ScheduleEvent(EVENT_ARCANE_DESOLATION, 2s);
                ...
                case EVENT_BURNING_BOMB:
                {
                    if (me->IsInCombat())
                        if (me->HasAura(SPELL_AR_FIRE))
                        {
                            ...
                            events.ScheduleEvent(EVENT_BURNING_BOMB, 16s);
                        }
                    break;
                }
                ...
                case EVENT_ARCANE_DESOLATION:
                {
                    if (me->IsInCombat())
                        ...
                        {
                            events.ScheduleEvent(EVENT_ARCANE_DESOLATION, 12s);
                            me->CastSpell(me, SPELL_A_AD_TARGET, false);
                            events.ScheduleEvent(EVENT_ARCANOPULSE, 1s);
                        }
                    break;
                }
            ...
            case PHASE_NEUTRAL:
                if (lastPhase == 0)
                {
                    events.ScheduleEvent(EVENT_PHASE_FIRE, 5s);
                    events.SetPhase(PHASE_FIRE);
                    lastPhase = PHASE_FIRE;
                }
                else if (lastPhase == PHASE_FIRE)
                {
                    events.ScheduleEvent(EVENT_PHASE_FROST, 5s);
                    events.SetPhase(PHASE_FROST);
                    lastPhase = PHASE_FROST;
                }
                else if (lastPhase == PHASE_FROST)
                {
                    events.ScheduleEvent(EVENT_PHASE_ARCANE, 5s);
...
public:
    ...
    struct at_calamir_arcanopulseAI : AreaTriggerAI
    {
        ...
        /*void OnSetCreatePosition(Unit* caster, Position& startPos, Position& endPos, std::list<Position>& path) override
        {
            ...
            float angle = center.GetAbsoluteAngle(trigger->GetPositionX(), trigger->GetPositionY());
