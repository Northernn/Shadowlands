...
public:
    ...
    struct boss_sharthosAI : public WorldBossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.ScheduleEvent(EventTailLash, 8s);
            events.ScheduleEvent(EventDreadFlame, 11s);
            events.ScheduleEvent(EventNightmareBreath, 16s);
            events.ScheduleEvent(EventBurningEarth, 28s);
            events.ScheduleEvent(EventCryOfTheTormented, 63s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case Events::EventTailLash:
                    ...
                    events.ScheduleEvent(Events::EventTailLash, 8s);
                    break;
                case Events::EventDreadFlame:
                    me->CastSpell(me->GetVictim(), Spells::DreadFlame);
                    events.ScheduleEvent(EventDreadFlame, 25s);
                    break;
                case Events::EventNightmareBreath:
                    DoCastAOE(Spells::NightmareBreath);
                    events.ScheduleEvent(Events::EventNightmareBreath, 18s);
                    break;
