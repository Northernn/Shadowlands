...
public:
    ...
    struct boss_fel_lord_betrugAI : public BossAI
    {
            ...
            //18:37
        {
            ...
            events.RescheduleEvent(EVENT_CHAOTIC_ENGERY, 6s); //18:43, 18:56, 19:35, 19:46
            events.RescheduleEvent(EVENT_FEL_SLASH, 12s); //18:49, 19:05, 20:01
            events.RescheduleEvent(EVENT_MIGHTY_SMASH, 32s); //19:09, 20:04, 20:58
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            {
                Talk(SAY_START);
                events.RescheduleEvent(EVENT_PRE_FIGHT, 1s);
            }
            ...
            if (id == SPELL_MIGHTY_SMASH_JUMP_1)
            {
                ...
                events.RescheduleEvent(EVENT_EXECUTION, 5s);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CHAOTIC_ENGERY:
                    ...
                    events.RescheduleEvent(EVENT_CHAOTIC_ENGERY, 12s);
                    break;
                case EVENT_FEL_SLASH:
                    DoCast(SPELL_FEL_SLASH);
                    events.RescheduleEvent(EVENT_FEL_SLASH, 16s);
                    break;
                case EVENT_MIGHTY_SMASH:
                    crystalDestroyed = false;
                    DoCast(SPELL_MIGHTY_SMASH);
                    events.RescheduleEvent(EVENT_MIGHTY_SMASH, 54s);
                    break;
                ...
                case EVENT_SEED_OF_DESTRUCTION:
                    ...
                    events.RescheduleEvent(EVENT_SEED_OF_DESTRUCTION, 22s);
                    break;
                }
            }
        ...
        void HandleOnHit()
        {
            ...
            else
            {
                ...
                if (Creature* trigger = GetCaster()->FindNearestCreature(92879, 50.0f))
                {
                    ...
                    GetHitUnit()->CastSpell(posLeft, SPELL_STASIS_CRYSTAL_SUM, true);
                    GetHitUnit()->CastSpell(posRight, SPELL_STASIS_CRYSTAL_SUM, true);
...
public:
    ...
    class spell_betrug_seed_of_destruction_AuraScript : public AuraScript
    {
        ...
        void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            ...
            for (uint8 i = 0; i < 4; i++)
            {
                Position pos = GetTarget()->GetNearPosition(5.0f, angle);
                GetTarget()->CastSpell(pos, SPELL_WAKE_OF_DESTRUCTION_AT, CastSpellExtraArgs().SetOriginalCaster(GetCaster()->GetGUID()));
                angle += 1.57f;
            }
        }
