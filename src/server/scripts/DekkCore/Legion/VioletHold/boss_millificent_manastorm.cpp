    ...
    struct boss_millificent_manastormAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
          //  ZoneTalk(SAY_AGGRO, nullptr);
            events.RescheduleEvent(EVENT_FINGER_LASER, 6s); //47:53, 48:00, 48:09, 48:19, 48:27
            events.RescheduleEvent(EVENT_ELEMENTIUM_BOMB, 7s); //47:54, 48:15, 48:33, 48:51
            events.RescheduleEvent(EVENT_ROCKET_CHICKEN, 24s); //48:11, 48:30, 48:48
            phase = 4;
            timer = 5000;
            event = true;
        }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            ...
            if (me->HealthBelowPct(9) && !end)
            {
              ...
              //  if (me->AI()->GetData(DATA_ANGRY) && end)
                //    instance->DoUpdateAchievementCriteria(CriteriaType::BeSpellTarget, 208122);
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FINGER_LASER:
                    if (Unit* pTarget = SelectTarget(SelectTargetMethod::Random, 0, 60.0f, true))
                        DoCast(pTarget, SPELL_FINGER_LASER);
                    events.RescheduleEvent(EVENT_FINGER_LASER, 7s);
                    break;
                case EVENT_ELEMENTIUM_BOMB:
                    ...
                    events.RescheduleEvent(EVENT_ELEMENTIUM_BOMB, 18s);
                    break;
                case EVENT_ROCKET_CHICKEN:
                    ...
                    events.RescheduleEvent(EVENT_ROCKET_CHICKEN, 18s);
                    break;
                }
            }
    ...
    struct npc_millificent_mechanical_bomb_squirrelAI : public ScriptedAI
    {
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_SQUIRREL_SEARCH)
                DoCast(target->ToUnit(), SPELL_SQUIRREL_CHARGE, true);
        }
