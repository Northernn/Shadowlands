...
public:
    ...
    {
        AddGossipItemFor(player, GossipOptionNpc::None, "Start", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        AddGossipItemFor(player, GossipOptionNpc::None, "Im stuck! Teleport me!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
    ...
    bool OnGossipSelect(Player* player, uint32 sender, uint32 action) override
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 1:
            ...
            me->AI()->DoAction(ACTION_START_DUNGEON);
            break;
        ...
        void SummonedCreatureDies(Creature* creature, Unit* killer)
        {
            ...
            case NPC_EREDAR_INVADER_2:
                ...
                if (waveCounterKilled >= 5)
                {
                    ...
                    if (waveCounter >= 2)
                    {
                        events.ScheduleEvent(EVENT_CALCULATE_NEXT_BOSS, 10s); // at 2 packs of mobs, select one random boss
                    }
                    else
                        events.ScheduleEvent(EVENT_CALCULATE_NEXT_SPAWNING, 10s);

                    if(waveCounter >= 6) // at 4 packs of mobs 
                        events.ScheduleEvent(EVENT_CALCULATE_NEXT_FINAL_BOSS, 10s);
            ...
            if (!creatureList.empty())
            {
                for (auto creature : creatureList)
                {
                    if (auto creature = validCreature)
                    {
                        ...
                        validCreature->RemoveUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
                ...
                if (!bPortalGuardianOrKeeperOrEliteSpawn)
                {
                    if (uiSpawnTimer <= diff)
                    {
                        ...
                        for (uint8 i = 0; i < k; ++i)
                        {
                            ...
                            DoSummon(entry, me, 2.0f, 20s, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                        }
                        me->SetVisible(false);
                    }
            ...
            case 1:
                if (uiSpawnTimer <= diff)
                {
                    ...
                    else
                    {
                        ...
                        if (Creature* pPortalKeeper = DoSummon(entry, me, 2.0f, 20s, TEMPSUMMON_CORPSE_TIMED_DESPAWN))
                        {
...
public:
    ...
    struct npc_lord_malgathAI : public ScriptedAI
    {
        ...
        {
            events.RescheduleEvent(EVENT_6, 17s); //205046
            events.RescheduleEvent(EVENT_7, 30s); //204962
            events.RescheduleEvent(EVENT_8, 18s); //204963 
            events.RescheduleEvent(EVENT_9, 19s); //204966
        }
...
public:
    ...
    struct npc_violet_hold_trashAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            {
                events.RescheduleEvent(EVENT_1, 4s); // 204028
                events.RescheduleEvent(EVENT_2, 25s); // 204032
                events.RescheduleEvent(EVENT_3, 20s);// while under 204140 and get damage - cast 204208
            }
            if (me->GetEntry() == 102335)
            {
                Talk(1);
                events.RescheduleEvent(EVENT_4, 16s); // 204517
                events.RescheduleEvent(EVENT_5, 18s); // 204493   
            }
            if (me->GetEntry() == 102302)
            {
                events.RescheduleEvent(EVENT_6, 31s); // 204722  33
                events.RescheduleEvent(EVENT_7, 33s); // 204876
                events.RescheduleEvent(EVENT_8, 23s); // 204895
            }
            if (me->GetEntry() == 102336)
            {
                events.RescheduleEvent(EVENT_9, 9s); // 204951
                events.RescheduleEvent(EVENT_10, 27s); // 204901
                events.RescheduleEvent(EVENT_11, 14s); // 204948
            }
            if (me->GetEntry() == 102368 || me->GetEntry() == 102380 || me->GetEntry() == 102272 || me->GetEntry() == 102370 || me->GetEntry() == 102270)
                events.RescheduleEvent(EVENT_12, 15s); // 205115 205123
            if (me->GetEntry() == 102395)
                events.RescheduleEvent(EVENT_13, 27s); // 205097 and cast 205093
            if (me->GetEntry() == 102400)
            {
                DoCast(205099);
                events.RescheduleEvent(EVENT_14, 20s); //205103
                events.RescheduleEvent(EVENT_15, 13s); //205102
            }
            if (me->GetEntry() == 102397)
            {
                DoCast(182405);
                events.RescheduleEvent(EVENT_16, 20s); // 205082
                events.RescheduleEvent(EVENT_17, 21s); // 205080
            }
            if (me->GetEntry() == 102369 || me->GetEntry() == 102269)
                events.RescheduleEvent(EVENT_18, 22s); // 205108
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                {
                    DoCast(204722);
                    events.RescheduleEvent(EVENT_6, 33s); // 204722  33
                    break;
                }
                case EVENT_7:
                {
