...
public:
    ...
    struct boss_blood_princess_thalenaAI : public BossAI
    {
            ...
            //30:22
        {
            ...
            events.RescheduleEvent(EVENT_VAMPYR_KISS, 6s); //30:28. ?? ???????????.
            events.RescheduleEvent(EVENT_BLOOD_SWARM, 20s); //30:42, 31:03, 31:24

            if (me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
                events.RescheduleEvent(EVENT_BLOOD_CALL, 30s);
        }
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            if (target->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                {
                    DoCast(target->ToUnit(), SPELL_VAMPYR_KISS_DMG, true);
                    target->CastSpell(target, SPELL_VAMPYR_ESSENCE_MOD, true);
                }
            }

            if (spell->Id == SPELL_BLOOD_CALL_VISUAL)
                target->CastSpell(target, SPELL_BLOOD_CALL_SUM, true);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_BLOOD_SWARM:
                    ...
                    events.RescheduleEvent(EVENT_BLOOD_SWARM, 21s);
                    break;
