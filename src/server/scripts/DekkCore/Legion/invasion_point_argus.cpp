...
struct npc_baldrazar : ScriptedAI
{
    ...
    void JustEngagedWith (Unit* /*who*/) override
    {
        ...
        events.RescheduleEvent(2, 11s);
        events.RescheduleEvent(3, 29s);
    }
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        case 9127:
        case 9126:
            me->CastSpell(me, 251003, false);
            break;
        case 9100:
        case 9128:
            me->CastSpell(me, 251005, false);
            break;
        case 9102:
            DoCast(252113);
            me->CastSpell(me, 251004, false);
            break;
        default:
            break;
        }
    ...
    //    //me->RemoveAllAreaObjects();
      ...
      //  if (!playerListAchiv.empty())
        ...
        case 9180:
            for (uint8 i = 0; i < 5; ++i)
                me->SummonCreature(127108, DemonHunterBonich[i], TEMPSUMMON_TIMED_DESPAWN, 20s);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(276221, GameObjectsFlagsBonich[i], QuaternionData(), 20s);
            me->SummonCreature(124884, -3724.07f, -7999.76f, 3.26834f, 1.61817f, TEMPSUMMON_TIMED_DESPAWN, 20s);
            DoCast(251015);
            break;
        case 9100:
            for (uint8 i = 0; i < 5; ++i)
                me->SummonCreature(127108, DemonHunterAurinor[i], TEMPSUMMON_TIMED_DESPAWN, 20s);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(276221, GameObjectsFlagsAurinor[i], QuaternionData(), 20s);
            me->SummonCreature(124884, -4066.52f, -4659.22f, 80.5446f, 1.61817f, TEMPSUMMON_TIMED_DESPAWN, 20s);
            DoCast(251010);
            break;
        case 9126:
            for (uint8 i = 0; i < 5; ++i)
                me->SummonCreature(127108, DemonHunterCengar[i], TEMPSUMMON_TIMED_DESPAWN, 20s);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(276221, GameObjectsFlagsCengar[i], QuaternionData(), 20s);
            me->SummonCreature(124884, 654.082f, 596.453f, 40.4031f, 4.22246f, TEMPSUMMON_TIMED_DESPAWN, 20s);
            DoCast(251013);
            break;
        case 9127:
            for (uint8 i = 0; i < 5; ++i)
                me->SummonCreature(127108, DemonHunterVal[i], TEMPSUMMON_TIMED_DESPAWN, 20s);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(276221, GameObjectsFlagsVal[i], QuaternionData(), 20s);
            me->SummonCreature(124884, -4033.44f, 568.951f, 0.531898f, 5.59048f, TEMPSUMMON_TIMED_DESPAWN, 20s);
            DoCast(251013);
            break;
        case 9102:
            for (uint8 i = 0; i < 5; ++i)
                me->SummonCreature(127108, DemonHunterNaigtal[i], TEMPSUMMON_TIMED_DESPAWN, 20s);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(276221, GameObjectsFlagsNaigtal[i], QuaternionData(), 20s);
            me->SummonCreature(124884, -1833.56f, -1603.79f, 0.902228f, 4.73205f, TEMPSUMMON_TIMED_DESPAWN, 20s);
            DoCast(251014);
            break;
        case 9128:
            for (uint8 i = 0; i < 5; ++i)
                me->SummonCreature(127108, DemonHunterSangua[i], TEMPSUMMON_TIMED_DESPAWN, 20s);
            for (uint8 i = 0; i < 2; ++i)
                me->SummonGameObject(276221, GameObjectsFlagsSangua[i], QuaternionData(), 20s);
            me->SummonCreature(124884, -1390.74f, 761.028f, 62.5843f, 5.47791f, TEMPSUMMON_TIMED_DESPAWN, 20s);
...
struct npc_dreadbringer_valus : ScriptedAI
{
    ...
    void EnterCombat(Unit* /*who*/)
    {
        ...
        events.RescheduleEvent(2, 9s);
        events.RescheduleEvent(3, 85s);
    }
...
struct npc_vogrethar_the_defiled : ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case 2:
                DoCast(249257);
                events.RescheduleEvent(2, 34s);
                break;
            case 3:
                DoCast(249259);
                events.RescheduleEvent(3, 22s);
                break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_flamebringer_azrothel : ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case 2:
                DoCast(249457);
                events.RescheduleEvent(2, 40s);
                break;
            case 3:
                DoCast(249440);
                events.RescheduleEvent(3, 43s);
                break;
            case 4:
                me->CastSpell(me, 249391, false);
                events.RescheduleEvent(4, 21s);
                break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_gorgoloth : ScriptedAI
{
    ...
    void EnterCombat(Unit* /*who*/) 
    {
        ...
        events.RescheduleEvent(1, 11s);
        events.RescheduleEvent(2, 21s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case 1:
                DoCast(248641);
                events.RescheduleEvent(1, 28s);
                break;
            case 2:
                DoCast(248508);
                events.RescheduleEvent(2, 16s);
                break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_velthrak_the_punisher : ScriptedAI
{
    ...
    void EnterCombat(Unit* /*who*/) 
    {
        ...
        events.RescheduleEvent(1, 5s);
        events.RescheduleEvent(2, 12s);
        events.RescheduleEvent(3, 20s);
        events.RescheduleEvent(4, 11s);
    }
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        case 125638:
            ...
            break;
        case 125636:
            summon->CastSpell(summon, 249394, false);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case 3:
                DoCast(249392);
                events.RescheduleEvent(3, 32s);
                break;
            case 4:
                me->CastSpell(me, 249391, false);
                events.RescheduleEvent(4, 20s);
                break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_malphazel_argus : ScriptedAI
{
    ...
    void EnterCombat(Unit* /*who*/) 
    {
        ...
        events.RescheduleEvent(1, 12s);
        events.RescheduleEvent(2, 17s);
        events.RescheduleEvent(3, 25s);
    }
  ...
  //  }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case 1:
                DoCast(249235);
                events.RescheduleEvent(1, 64s);
                break;
                /*case 2:
                DoCast();
                events.RescheduleEvent(2, 35s);
                break;*/
            case 3:
                DoCast(249245);
                events.RescheduleEvent(3, 36s);
                break;
            }
        }
        DoMeleeAttackIfReady();
...
struct npc_harbinger_drelnathar : ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case 1:
                DoCast(249071);
                events.RescheduleEvent(1, 30s);
                break;
            case 2:
                DoCast(249066);
                events.RescheduleEvent(2, 24s);
                break;
            case 3:
                DoCast(249068);
                events.RescheduleEvent(3, 26s);
...
struct npc_dread_knight_zakgal : ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case 1:
                DoCast(248663);
                events.RescheduleEvent(1, 10s);
                break;
            case 2:
                if (auto target = SelectTarget(SelectTargetMethod::Random, 0, 200.0f, true))
...
struct npc_explosive_orb : ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* /*summoner*/) override
    {
        ...
        me->CastSpell(me, 249447, false);
        me->DespawnOrUnsummon(13s);
    }
};
...
struct npc_invasion_point_bunny : ScriptedAI
{
    ...
    void Reset() override
    {
        if (!useit)
        {
            ...
            if (me->GetZoneId() == 9128 || me->GetZoneId() == 9299)
                timer2 = 16000;

            me->AddDelayedEvent(5000, [=]() -> void
            {
                if (me->GetZoneId() == 9127 || me->GetZoneId() == 9298)
                    timer1 = 34000;
            });
        }
    }
...
struct npc_felflame_invader : ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        me->AddDelayedEvent(5000, [=]() -> void
        {
            ...
            else if (me->GetZoneId() == 9128 || me->GetZoneId() == 9299)
            {
...
struct npc_felflame_subjugator : ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        me->AddDelayedEvent(5000, [=]() -> void
        {
            ...
            else if (me->GetZoneId() == 9126 || me->GetZoneId() == 9295)
            {
                DoCast(251888);
...
class spell_creeping_doom_argus_dummy : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        ...
        caster->CastSpell(target, 248507, false);
    }
   ...
   /* void HandlePlayerEnterZone(ObjectGuid guid, uint32 zone) override
    ...
    void HandleKill(Player* killer, Unit* killed) override
    {
        ...
        case 125197:
            ...
            case 9100:
            case 9127:
               // m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 6, 0, nullptr, killer);
                break;
            case 9180:
             //   m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 9, 0, nullptr, killer);
                break;
            case 9128:
              //  m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 18, 0, nullptr, killer);
                break;
            }
            break;
        case 125666:
        ...
        case 126446:
            ...
            break;
        case 126231:
         //   m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 19, 0, nullptr, killer);
            break;
        case 125921:
        case 125755:
            switch (zoneid)
            ...
            case 9128:
            case 9102:
            //    m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 3, 0, nullptr, killer);
                break;
            case 9100:
            //    m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 5, 0, nullptr, killer);
                break;
            case 9180:
             //   m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 10, 0, nullptr, killer);
                break;
            }
            break;
        case 125936:
            ...
            case 9102:
             //   m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 3, 0, nullptr, killer);
                break;
            case 9100:
              //  m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 12, 0, nullptr, killer);
                break;
            }
            break;
        case 125759:
            ...
            {
            case 9102:
            //    m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 4, 0, nullptr, killer);
                break;
            case 9180:
              //  m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 7, 0, nullptr, killer);
                break;
            }
            break;
        case 125776:
        ...
        case 125963:
            ...
            break;
        case 125970:
          //  m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 28, 0, nullptr, killer);
            break;
        case 125939:
         //   m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 22, 0, nullptr, killer);
            break;
        case 125779:
            switch (zoneid)
            {
            case 9128:
             //   m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 13, 0, nullptr, killer);
                break;
            case 9127:
             //   m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, 59201, 6, 0, nullptr, killer);
                break;
     ...
     //   if (criteria)
         //   m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, criteria, 0, 0, nullptr, killer);
    }
    ...
    //    if (criteria)
        //    m_scenarios[zoneid]->UpdateAchievementCriteria(CRITERIA_TYPE_SCRIPT_2, criteria, 0, 0, nullptr, player);
    }
          ...
          //      if (Creature* cre = ObjectAccessor::GetObjectInMap(guid, m_map, (Creature*)nullptr))
                {
                    ...
                    case 9100:
                        if (cre->GetEntry() == 125856)
                            cre->SetNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
                        break;
                    case 9127:
...
public:
    ...
    {
        return new OutdoorPVPArgusInvasion(map);
    }
};
