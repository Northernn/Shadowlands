...
public:
    ...
    struct boss_maiden_of_virtue_legionAI : public BossAI
    {
        ...
        void DefaultEvents()
        {
            events.RescheduleEvent(EVENT_HOLY_BOLT, 9s);
            events.RescheduleEvent(EVENT_SACRED_GROUND, 11s);
            events.RescheduleEvent(EVENT_HOLY_SHOCK, 16s);
            events.RescheduleEvent(EVENT_MASS_REPENTANCE, 50s);
        }
       ...
       void OnSpellCast(SpellInfo const* spell) override
        {
            ...
            if (spell->Id == SPELL_MASS_REPENTANCE)
                events.RescheduleEvent(EVENT_HOLY_BULWARK, 1s);

            if (spell->Id == SPELL_HOLY_BULWARK)
                events.RescheduleEvent(EVENT_HOLY_WRATH, 1s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_HOLY_BOLT:
                    ...
                    events.RescheduleEvent(EVENT_HOLY_BOLT, 14s);
                    break;
                case EVENT_SACRED_GROUND:
                    ...
                    events.RescheduleEvent(EVENT_SACRED_GROUND, 23s);
                    break;
                case EVENT_HOLY_SHOCK:
                    if (urand(0, 1))
                        Talk(SAY_HOLY_SHOCK);
                    DoCastVictim(SPELL_HOLY_SHOCK);
                    events.RescheduleEvent(EVENT_HOLY_SHOCK, 13s);
                    break;
