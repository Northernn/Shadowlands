...
public:
    ...
    struct boss_rtk_moroesAI : public BossAI
    {
            ...
            //41:14
        {
            ...
            events.RescheduleEvent(EVENT_VANISH, 7s); //41:21, 41:42 42:02
            events.RescheduleEvent(EVENT_COAT_CHECK, 30s); //41:44 42:20
        }
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_VANISH)
                DoCast(target->ToUnit(), SPELL_GARROTE, true);

            if (spell->Id == SPELL_COAT_CHECK)
                DoCast(target->ToUnit(), SPELL_COAT_CHECK_RESIST, true);
        }
    ...
    struct npc_moroes_trash_genericAI : public ScriptedAI
    {
            ...
            //41:14
        {
            ...
            case NPC_BARONESS_DOROTHEA:
                events.RescheduleEvent(EVENT_1, 1s); //41:15
                events.RescheduleEvent(EVENT_2, 9s); //41:23
                break;
            case NPC_LADY_CATRIONA:
                ...
                break;
            case NPC_BARON_RAFE:
                events.RescheduleEvent(EVENT_5, 5s); //41:19
                break;
            case NPC_LADY_KEIRA:
                events.RescheduleEvent(EVENT_6, 10s);
                break;
            case NPC_LORD_ROBIN:
                events.RescheduleEvent(EVENT_7, 1s);
                break;
            case NPC_LORD_CRISPIN:
                events.RescheduleEvent(EVENT_8, 11s); //41:25
                break;
            }
            DoZoneInCombat();
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_3:
                    ...
                    events.RescheduleEvent(EVENT_3, 4s);
                    break;
                case EVENT_4:
                    DoCast(SPELL_HEALING_STREAM);
                    events.RescheduleEvent(EVENT_4, 30s);
                    break;
                case EVENT_5:
                ...
                case EVENT_6:
                    ...
                    events.RescheduleEvent(EVENT_6, 12s);
                    break;
                case EVENT_7:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 80.0f, true))
                    {
                        if (Unit* owner = me->GetOwner())
                            me->CastSpell(target, SPELL_WHIRLING_EDGE, false);
                    }
                    events.RescheduleEvent(EVENT_7, 18s);
                    break;
                case EVENT_8:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 80.0f, true))
                    {
                    ...
                    events.RescheduleEvent(EVENT_8, 18s);
                    break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_moroes_whirling_edgeAI : public ScriptedAI
    {
        npc_moroes_whirling_edgeAI(Creature* creature) : ScriptedAI(creature)
        {
    ...
    struct npc_moroes_after_sceneAI : public ScriptedAI
    {
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_UNLEASHED_POWER && target->GetEntry() == NPC_NIELAS_ARAN)
            {
                ...
                target->ToUnit()->SetUnitFlag2(UNIT_FLAG2_FEIGN_DEATH);
                DoCast(me, SPELL_KNOCKBACK_SELF, true);
                events.RescheduleEvent(EVENT_4, 3s);
            }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            if (type == POINT_MOTION_TYPE && id == 1)
                events.RescheduleEvent(EVENT_6, 1s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_1: //42:56
                    ...
                    DelayTalk(me, 9, 1s, nullptr);
                    break;
                case EVENT_2: //42:56
                    DoCast(me, SPELL_UNCONTROLLABLE_ENERGY, true);
                    DelayTalk(me, 13, 1s, nullptr);
