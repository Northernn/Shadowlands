...
public:
    ...
    struct boss_shade_of_medivhAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_ALTERNATING_ABILITY, 5s);
        }
       ...
       void OnSpellCast(SpellInfo const* spell) override
        ...
        void DoAction(int32 const action) override
        {
            if (action == ACTION_1 && !specialAbility)
            {
                ...
                case 0: //Special Frost
                    events.RescheduleEvent(EVENT_SPECIAL_FROST, 5s);
                    break;
                case 1: //Special Fire
                    events.RescheduleEvent(EVENT_SPECIAL_FIRE, 5s);
                    break;
                case 2: //Special Arcane
                    events.RescheduleEvent(EVENT_SPECIAL_ARCANE, 5s);
                    break;
                }
            }
        }
        ...
        void CastDefAbility()
        {
            Unit* target = SelectTarget(SelectTargetMethod::Random, 1, 60.0f, true);
            if (!target)
                target = SelectTarget(SelectTargetMethod::Random, 0, 60.0f, true);
            if (!target)
                return;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_ALTERNATING_ABILITY:
                    CastDefAbility();
                    events.RescheduleEvent(EVENT_ALTERNATING_ABILITY, 4s);
                    break;
...
public:
    ...
    struct npc_medivh_guardian_imageAI : public ScriptedAI
    {
        npc_medivh_guardian_imageAI(Creature* creature) : ScriptedAI(creature) {}
