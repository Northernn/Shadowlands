...
public:
    ...
    struct boss_rtk_nightbaneAI : public BossAI
    {
        ...
        void DefaultEvents(uint8 phase)
        {
            ...
            {
            case 2:
                events.RescheduleEvent(EVENT_ROAR, 20s);
                // no break
            case 1:
                events.RescheduleEvent(EVENT_CINDER_BREATH, 8s);
                events.RescheduleEvent(EVENT_IGNITE_SOUL, 20s);
                events.RescheduleEvent(EVENT_REVERBERATING_SHADOWS, 17s);
                events.RescheduleEvent(EVENT_CHARRED_GROUND, 3s);
                events.RescheduleEvent(EVENT_BURNING, 19s);
                events.RescheduleEvent(EVENT_TAIL, 12s);

                events.RescheduleEvent(EVENT_POWER, 30s);
                events.RescheduleEvent(EVENT_POWER_1, 53s);
                break;
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CINDER_BREATH:
                    ...
                    events.RescheduleEvent(EVENT_CINDER_BREATH, 20s);
                    break;
                case EVENT_IGNITE_SOUL:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 1, 70.0f, true))
                    {
                        me->CastSpell(target, SPELL_IGNITE_SOUL);
                        Talk(SAY_SOUL, target);
                        Talk(SAY_SOUL_1);
                    }
                    events.RescheduleEvent(EVENT_IGNITE_SOUL, 25s);
                    break;
                case EVENT_REVERBERATING_SHADOWS:
                    DoCast(SPELL_REVERBERATING_SHADOWS);
                    events.RescheduleEvent(EVENT_REVERBERATING_SHADOWS, 13s);
                    break;
                case EVENT_BURNING:
                    DoCast(SPELL_BURNING_BONES);
                    Talk(SAY_BURNING);
                    events.RescheduleEvent(EVENT_BURNING, 18s);
                    break;
                case EVENT_TAIL:
                ...
                case EVENT_ROAR:
                    ...
                    Talk(SAY_ROAR);
                    events.RescheduleEvent(EVENT_ROAR, 42s);
                    break;
...
public:
    ...
    struct npc_karazhan_part_of_soulAI : public ScriptedAI
    {
    ...
    struct npc_karazhan_nighbane_starterAI : public ScriptedAI
    {
        ...
        bool OnGossipSelect(Player* player, uint32 sender, uint32 action) override
        {
            ...
            if (Creature* nightbane = me->SummonCreature(115213, -11170.30f, -1868.32f, 106.51f, 5.37f, TEMPSUMMON_DEAD_DESPAWN))
            {
                ...
                nightbane->DespawnOrUnsummon(3s);
