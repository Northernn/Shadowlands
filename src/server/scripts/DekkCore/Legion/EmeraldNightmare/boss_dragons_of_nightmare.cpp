...
enum Spells
{
    ...
    SPELL_UNRELENTING = 221419, //Mythic. ????? ???? ? ???, ??? ????? ?????????? ??? ???????????. ???? ??? ??? ? 10 ??????.
...
struct boss_dragon_ysondre : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_CORRUPTED_BREATH, 16s);
        events.RescheduleEvent(EVENT_SWITCH_DRAGONS, 1s);
        events.RescheduleEvent(EVENT_NIGHTMARE_BLAST, 40s);

        if (IsMythicRaid())
            events.RescheduleEvent(EVENT_TAIL_LASH, 5s);
    ...
    void DoAction(int32 action)
    {
        ...
        case ACTION_4: //Special Attack
            events.RescheduleEvent(EVENT_CALL_DEFILED_SPIRIT, 1s);
            break;
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_CORRUPTED_BREATH:
                ...
                events.RescheduleEvent(EVENT_CORRUPTED_BREATH, 32s);
                break;
            ...
            case EVENT_NIGHTMARE_BLAST:
                ...
                events.RescheduleEvent(EVENT_NIGHTMARE_BLAST, 15s);
                break;
...
struct boss_dragon_taerar : public ScriptedAI
{
    ...
    void DefaultEvents()
    {
        ...
        events.RescheduleEvent(EVENT_CORRUPTED_BREATH, 16s);
        events.RescheduleEvent(EVENT_SEEPING_FOG, 26s);
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_3:
            ...
            break;
        case ACTION_4: //Special Attack
            events.RescheduleEvent(EVENT_SHADES_OF_TAERAR, 1s);
            break;
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SEEPING_FOG:
                ...
                events.RescheduleEvent(EVENT_SEEPING_FOG, 16s);
                break;
            case EVENT_TAIL_LASH:
                DoCast(SPELL_TAIL_LASH);
                events.RescheduleEvent(EVENT_TAIL_LASH, 5s);
...
struct boss_dragon_lethon : public ScriptedAI
{
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_3:
            ...
            break;
        case ACTION_4: //Special Attack
            events.RescheduleEvent(EVENT_SIPHON_SPIRIT, 1s);
            break;
        }
    }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        if (id == 1)
        {
            ...
            if (IsHeroicPlusRaid())
                events.RescheduleEvent(EVENT_SHADOW_BURST, 15s);
        }
        else if (id == 2)
        {
            me->SetCanFly(false);
...
struct boss_dragon_emeriss : public ScriptedAI
{
    ...
    void DefaultEvents()
    {
        ...
        DoCast(me, SPELL_ENERGIZE_DRAGONS, true);
        events.RescheduleEvent(EVENT_CORRUPTED_BREATH, 16s);
        events.RescheduleEvent(EVENT_ESSENCE_OF_CORRUPTION, 30s);
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_3:
            ...
            break;
        case ACTION_4: //Special Attack
            events.RescheduleEvent(EVENT_VOLATILE_INFECTION, 1s);
            break;
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
                ...
                events.RescheduleEvent(EVENT_1, 30s);
                break;
            case EVENT_TAIL_LASH:
...
struct npc_en_rothos : public ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
                DoCastVictim(223697);
                events.RescheduleEvent(EVENT_1, 15s);
                break;
            case EVENT_2:
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        if (GetCaster() && targets.empty())
            if (auto owner = GetCaster()->GetOwner())
                GetCaster()->CastSpell(GetCaster(), 203667, true);
    }
...
class spell_ysondre_shadow_burst : public AuraScript
{
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        GetTarget()->CastSpell(GetTarget(), 204044, true);
    }
...
void AddSC_boss_dragons_of_nightmare()
{
    ...
    RegisterSpellScript(spell_ysondre_shadow_burst);
    RegisterSpellScript(spell_ysondre_periodic_energize);
    RegisterSpellScript(spell_ysondre_marks);
    RegisterSpellScript(spell_ysondre_nightmare_bloom);
    RegisterSpellScript(spell_ysondre_bellowing_roar);
