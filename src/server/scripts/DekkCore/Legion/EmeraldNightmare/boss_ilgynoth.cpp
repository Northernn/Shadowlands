...
struct boss_ilgynoth : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_SUMMON_1, 20s); //Sum Deathglare
        events.RescheduleEvent(EVENT_CHECK_TARGET_DISTANCE, 2s);
    }
    ...
    void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
    {
        if (summon->GetEntry() == NPC_EYE_OF_ILGYNOTH)
        {
            ...
            events.RescheduleEvent(EVENT_DARK_RECONSTITUTION, 10s);
            events.RescheduleEvent(EVENT_CURSED_BLOOD, 15s);

            if (me->GetMap()->GetDifficultyID() == DIFFICULTY_MYTHIC_RAID)
            {
                if (finalTorpor)
                    events.RescheduleEvent(EVENT_SUM_SHRIVELED_EYESTALK, 16s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SUMMON_1:
                ...
                events.RescheduleEvent(EVENT_SUMMON_2, 70s); //Next: 2 Corruptors
                break;
            case EVENT_SUMMON_2:
                SummomRandPos(2, NPC_CORRUPTOR_TENTACLE);
                events.RescheduleEvent(EVENT_SUMMON_3, 6s); //Next: 2 Deathglare
                break;
            case EVENT_SUMMON_3:
                SummomRandPos(2, NPC_DEATHGLARE_TENTACLE);
                events.RescheduleEvent(EVENT_SUMMON_4, 84s); //Next: 1 Deathglare
                break;
            case EVENT_SUMMON_4:
                SummomRandPos(1, NPC_DEATHGLARE_TENTACLE);
                events.RescheduleEvent(EVENT_SUMMON_5, 4s); //Next: 2 Corruptors
                break;
            case EVENT_SUMMON_5:
                SummomRandPos(2, NPC_CORRUPTOR_TENTACLE);
                events.RescheduleEvent(EVENT_SUMMON_6, 40s); //Next: 1 Corruptors
                break;
            case EVENT_SUMMON_6:
                SummomRandPos(1, NPC_CORRUPTOR_TENTACLE);
                events.RescheduleEvent(EVENT_SUMMON_7, 15s); //Next: 1 Deathglare
                break;
            case EVENT_SUMMON_7:
                SummomRandPos(1, NPC_DEATHGLARE_TENTACLE);
                events.RescheduleEvent(EVENT_SUMMON_6, 30s); //Next: ??? ????. ?????????
                break;
            case EVENT_DARK_RECONSTITUTION:
                ...
                finalTorpor = true;
                events.RescheduleEvent(EVENT_ANNOUNCE_FINAL, 45s);
                events.RescheduleEvent(EVENT_EYE_RESPAWN, 51s);
                events.RescheduleEvent(EVENT_ANNOUNCE, 10s);
                AnnText = 1;
                break;
            ...
            case EVENT_SUM_SHRIVELED_EYESTALK:
            {
                ...
                events.RescheduleEvent(EVENT_SUM_SHRIVELED_EYESTALK, 20s);
                break;
            }
            ...
            case EVENT_ANNOUNCE:
                ...
                {
                    ++AnnText;
                    events.RescheduleEvent(EVENT_ANNOUNCE, 20s);
                }
                break;
...
struct npc_eye_of_ilgynoth : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_2, 65s); //Summon Nightmare Horror

        if (me->GetMap()->GetDifficultyID() == DIFFICULTY_MYTHIC_RAID)
            events.RescheduleEvent(EVENT_DEATH_BLOSSOM, 60s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
            case EVENT_1:
                if (Unit* pTarget = SelectTarget(SelectTargetMethod::Random, 0, 120.0f, true))
                {
            ...
            case EVENT_2:
                if (Unit* summoner = me->GetOwner())
                    if (Creature* horror = summoner->SummonCreature(NPC_NIGHTMARE_HORROR, sumPos[3]))
                    {
                        ...
                        horror->SetUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
                        horror->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_PC);
                        horror->SetUnitFlag(UNIT_FLAG_NOT_ATTACKABLE_1);
                    }
                Talk(0);
                events.RescheduleEvent(EVENT_2, 3s); //3m40s
                break;
            case EVENT_DEATH_BLOSSOM:
                ...
                if (auto summoner = me->GetOwner())
                {
                    ...
                    for (auto idx : randList)
                        if (auto blossom = summoner->SummonCreature(NPC_DEATH_BLOSSOM, blossPos[idx], TEMPSUMMON_TIMED_DESPAWN, 16s))
                        {
                            blossom->CastSpell(blossom, SPELL_DEATH_BLOSSOM_AT, true);
                            blossom->CastSpell(blossom, SPELL_DEATH_BLOSSOM_VISUAL, true);
                        }
                }
                events.RescheduleEvent(EVENT_DEATH_BLOSSOM, 100s); //1m40s
                break;
            }
        }
    }
...
struct npc_ilgynoth_tentacles : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        case NPC_DOMINATOR_TENTACLE:
            ...
            events.RescheduleEvent(EVENT_2, 13s);
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (auto summoner = me->GetOwner())
            for (uint8 i = 0; i < bloodCount; ++i)
                me->CastSpell(me, SPELL_SPAWN_BLOOD, true);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (rupturingRoarTimer)
        {
            if (rupturingRoarTimer <= diff)
            {
                ...
                if (auto target = SelectTarget(SelectTargetMethod::MinDistance, 0, 100.0f, true))
                {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
                ...
                events.RescheduleEvent(EVENT_1, 11s);
                break;
            case EVENT_2:
                ...
                events.RescheduleEvent(EVENT_2, 22s);
                break;
            case EVENT_3:
                if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 120.0f, true))
...
void AddSC_ilgynoth()
{
    ...
    RegisterSpellScript(spell_ilgynoth_cursed_blood);
