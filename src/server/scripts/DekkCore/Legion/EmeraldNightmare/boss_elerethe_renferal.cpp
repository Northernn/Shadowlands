...
struct boss_elerethe_renferal : public BossAI
{
    ...
    void Intro()
    {
        ...
        {
            fakeBoss->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
            fakeBoss->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_PC);
            fakeBoss->SetUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
    ...
    void EventsForm(uint8 form)
    {
        ...
        {
        case SPIDER_FORM:
            events.RescheduleEvent(EVENT_WEB_OF_PAIN, 6s);
            events.RescheduleEvent(EVENT_NECROTIC_VENOM, 12s);
            events.RescheduleEvent(EVENT_FEEDING_TIME, 16s);
            break;
        case DARKWING_FORM:
            events.RescheduleEvent(EVENT_TWISTING_SHADOWS, 7s);
            events.RescheduleEvent(EVENT_GATHERING_CLOUDS, 16s);
            events.RescheduleEvent(EVENT_DARK_STORM, 26s);
            events.RescheduleEvent(EVENT_RAZOR_WING, 59s);
            //if (IsMythicRaid())
    ...
    void SpellHit(WorldObject* caster, const SpellInfo* spell) override
    {
        ...
        case SPELL_ENERGY_TRANSFORM_COST:
            ...
            if (me->HasAura(SPELL_DARKWING_FORM))
                events.RescheduleEvent(EVENT_SPIDER_TRANSFORM, 0s);
            else
                events.RescheduleEvent(EVENT_DARKWING_FORM, 0s);
            break;
        case SPELL_SPIDER_TRANSFORM_DUMMY:
        case SPELL_DARKWING_FORM_DUMMY:
            events.RescheduleEvent(EVENT_REACT_AGGRESSIVE, 5s);
            break;
        }
    }
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        ...
        case SPELL_FEEDING_TIME_FILTER:
            DoCast(target->ToUnit(), SPELL_FEEDING_TIME_SUM_STALKER, true);
        ...
        case SPELL_VILE_AMBUSH_FILTER:
            ...
            DoCast(target->ToUnit(), SPELL_VILE_AMBUSH_MISSILE, true);
            break;
        case SPELL_RAZOR_WING_FILTER:
            me->AttackStop();
            me->SetFacingToObject(target);
            DoCast(target->ToUnit(), SPELL_RAZOR_WING);
            events.RescheduleEvent(EVENT_REACT_AGGRESSIVE, 5s);
            break;
        case SPELL_RAZOR_WING:
            DoCast(target->ToUnit(), SPELL_RAZOR_WING_VISUAL, true);
            DoCast(target->ToUnit(), SPELL_RAZOR_WING_DMG, true);
            break;
        }
    }
    ...
    void SpellFinishCast(const SpellInfo* spell)
    {
        ...
        {
            if (auto victim = SelectTarget(SelectTargetMethod::MaxThreat, 0, 15.0f, true))
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        if (type == WAYPOINT_MOTION_TYPE)
        {
            if (id == 4)
            {
                ...
                checkEvadeTimer = 2000;
                events.RescheduleEvent(EVENT_CHECH_PLAYER, 5s);
            }
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_CHECH_PLAYER:
                ...
                {
                    me->SetReactState(REACT_AGGRESSIVE);
                    events.RescheduleEvent(EVENT_RAKING_TALONS, 7s);
                }
                else
                    events.RescheduleEvent(EVENT_CHECH_PLAYER, 1s);
                break;
            }
        }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
                if (Player* player = ObjectAccessor::GetPlayer(*me, playerGUID))
                    player->KnockbackFrom(me->GetPosition(), 30, 10);
                break;
            }
        }
    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_2:
            {
                ...
                if (Unit* target = SelectTarget(SelectTargetMethod::MaxThreat))
                {
                    ...
                    jumpTimer = 6000;
                  //  events.DelayEvents(EVENT_1, 7s);
                }
                events.RescheduleEvent(EVENT_2, 29s);
                break;
            }
            }
        }
...
struct npc_en_shadowfeather : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        events.RescheduleEvent(EVENT_1, 35s);
        events.RescheduleEvent(EVENT_2, 4s);
    }
...
void AddSC_elerethe_renferal()
{
    ...
    RegisterSpellScript(spell_elerethe_energy_tracker_transform);
    RegisterSpellScript(spell_elerethe_web_of_pain_filter);
    RegisterSpellScript(spell_elerethe_web_of_pain);
    RegisterSpellScript(spell_elerethe_vile_ambush);
    RegisterSpellScript(spell_elerethe_feeding_time_filter);
    RegisterSpellScript(spell_elerethe_necrotic_venom);
    RegisterSpellScript(spell_elerethe_shimmering_feather_proc);
