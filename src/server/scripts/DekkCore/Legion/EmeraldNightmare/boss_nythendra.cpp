...
struct boss_nythendra : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_CHECK_POWER, 5s);
        events.RescheduleEvent(EVENT_ALLOW_ROT_CAST, 1s);
        DefaultEvents(true);
    ...
    {
        events.RescheduleEvent(EVENT_ROT, enterCombat ? 6s : 14s);
        events.RescheduleEvent(EVENT_VOLATILE_ROT, enterCombat ? 24s : 35s);
        events.RescheduleEvent(EVENT_INFESTED_BREATH, (enterCombat ? 24s : 35s));
    }
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        ...
        case SPELL_INFESTED_BREATH_FILTER:
        {
            ...
            if (auto stalker = me->SummonCreature(NPC_BREATH_STALKER, pos, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10s))
            {
                for (uint8 i = 0; i < 5; ++i)
        ...
        case SPELL_TRANSFORM_BUGS:
            target->ToUnit()->SetVisible(true);
            break;
        case SPELL_INFESTED_MIND_FILTER:
            if (target->ToUnit()->GetAuraCount(SPELL_INFESTED) >= 10)
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_CHECK_POWER:
                if (me->GetPower(POWER_ENERGY) == 0)
                {
                    ...
                    events.RescheduleEvent(EVENT_HEART_SWARM_END, 20s);
                }
                events.RescheduleEvent(EVENT_CHECK_POWER, 10s);
                break;
            case EVENT_ALLOW_ROT_CAST:
                DoCast(me, SPELL_ALLOW_ROT_CAST, true);
                events.RescheduleEvent(EVENT_ALLOW_ROT_CAST, 120s);
                break;
            case EVENT_ROT:
                if (me->GetPower(POWER_ENERGY) > 0)
                {
                ...
                events.RescheduleEvent(EVENT_ROT, 16s);
                break;
            case EVENT_VOLATILE_ROT:
                ...
                events.RescheduleEvent(EVENT_VOLATILE_ROT, 25s);
                break;
            case EVENT_INFESTED_BREATH:
                ...
                Talk(SAY_BREATH);
                events.RescheduleEvent(EVENT_INFESTED_BREATH, 36s);
                if (IsMythicRaid())
                    events.RescheduleEvent(EVENT_INFESTED_MIND, 1s);
                break;
...
struct npc_nythendra_gelatinized_decay : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_3, 22s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_3:
                ...
                events.RescheduleEvent(EVENT_3, 30s);
                break;
            }
        }
        DoMeleeAttackIfReady();
            ...
            case EVENT_2:
                ...
                else
                {
                    ...
                    if (++castCount == 3)
                    {
                        ...
                        events.RescheduleEvent(EVENT_1, 18s);
                        break;
                    }
                }
                events.RescheduleEvent(EVENT_2, 2s);
...
void AddSC_nythendra()
{
    ...
    RegisterSpellScript(spell_nythendra_volatile_rot);
    ...
    RegisterSpellScript(spell_nythendra_infested_mind);
