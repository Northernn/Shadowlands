...
struct boss_cenarius : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_NIGHTMARE_BRAMBLES, 25s);
        events.RescheduleEvent(EVENT_CHECH_EVADE, 2s);

        if (IsMythicRaid())
            events.RescheduleEvent(EVENT_NIGHTMARE_BLAST_OR_BEAST, 30s);
    ...
    void SpellHit(WorldObject* target, const SpellInfo* spell) override
    {
        if (spell->Id == SPELL_FORCES_OF_NIGHTMARE)
        {
            ...
            {
            case 1:
                me->SummonCreature(NPC_TORMENTED_SOULS, forcesPos[0], TEMPSUMMON_TIMED_DESPAWN, 13s);
                me->SummonCreature(NPC_NIGHTMARE_SAPLING, forcesPos[1], TEMPSUMMON_TIMED_DESPAWN, 13s);
                me->SummonCreature(NPC_CORRUPTED_EMERALD_EGG, forcesPos[3], TEMPSUMMON_TIMED_DESPAWN, 13s);
                break;
            case 2:
                me->SummonCreature(NPC_TORMENTED_SOULS, forcesPos[0], TEMPSUMMON_TIMED_DESPAWN, 13s);
                me->SummonCreature(NPC_CORRUPTED_NATURE, forcesPos[2], TEMPSUMMON_TIMED_DESPAWN, 13s);
                me->SummonCreature(NPC_CORRUPTED_EMERALD_EGG, forcesPos[3], TEMPSUMMON_TIMED_DESPAWN, 13s);
                break;
            case 3:
                me->SummonCreature(NPC_NIGHTMARE_SAPLING, forcesPos[1], TEMPSUMMON_TIMED_DESPAWN, 13s);
                me->SummonCreature(NPC_CORRUPTED_NATURE, forcesPos[2], TEMPSUMMON_TIMED_DESPAWN, 13s);
                me->SummonCreature(NPC_CORRUPTED_NATURE, forcesPos[3], TEMPSUMMON_TIMED_DESPAWN, 13s);
                break;
            case 4:
                me->SummonCreature(NPC_TORMENTED_SOULS, forcesPos[0], TEMPSUMMON_TIMED_DESPAWN, 13s);
                me->SummonCreature(NPC_CORRUPTED_EMERALD_EGG, forcesPos[2], TEMPSUMMON_TIMED_DESPAWN, 13s);
                me->SummonCreature(NPC_CORRUPTED_EMERALD_EGG, forcesPos[3], TEMPSUMMON_TIMED_DESPAWN, 13s);
                break;
            case 5:
                me->SummonCreature(NPC_NIGHTMARE_SAPLING, forcesPos[1], TEMPSUMMON_TIMED_DESPAWN, 13s);
    ...
    void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        if (me->HealthBelowPct(35) && !secondPhase)
        {
            secondPhase = true;
            me->AttackStop();
            events.RescheduleEvent(EVENT_OVERWHELMING_NIGHTMARE, 1s);
            events.RescheduleEvent(EVENT_SPEAR_OF_NIGHTMARES, 21s);
            me->SummonCreature(NPC_MALFURION_STORMRAGE, me->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN);
        ...
        {
            firstConv = true;
            events.RescheduleEvent(EVENT_SCENE, 20s);
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
            ...
            if (powerTimer <= diff)
            {
                ...
                else
                {
                    if (!me->HasUnitState(UNIT_STATE_CASTING))
                        events.RescheduleEvent(EVENT_FORCES_OF_NIGHTMARE, 1s);
                }
            }
            else
                powerTimer -= diff;
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_NIGHTMARE_BRAMBLES:
                if (auto pTarget = SelectTarget(SelectTargetMethod::Random, 1, 100.0f, true))
                    DoCast(pTarget, SPELL_NIGHTMARE_BRAMBLES_SUM);
                Talk(SAY_NIGHTMARE_BRAMBLES);
                events.RescheduleEvent(EVENT_NIGHTMARE_BRAMBLES, 30s);
                break;
            case EVENT_OVERWHELMING_NIGHTMARE:
                DoCast(me, SPELL_OVERWHELMING_NIGHTMARE, true);
                events.RescheduleEvent(EVENT_NIGHTMARE_BLAST_SAVER, 4s);
                break;
            ...
            case EVENT_SPEAR_OF_NIGHTMARES:
                ...
                events.RescheduleEvent(EVENT_SPEAR_OF_NIGHTMARES, 18s);
                break;
...
struct npc_cenarius_force_summoner : public ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
            case EVENT_1:
                me->CastSpell(me, alies ? SPELL_ALLIES_GERMINATE : SPELL_NIGHTMARE_GERMINATE, true);
                break;
            case EVENT_2:
            {
                Position pos;
                for (uint8 i = 0; i < 5; ++i)
                    me->CastSpell(me, alies ? SPELL_SUM_ALLIES_WISPS : SPELL_SUM_NIGHTMARE_WISPS, true);
                break;
            }
            case EVENT_3:
                me->CastSpell(me, alies ? SPELL_SUM_ALLIES_DRAKE : SPELL_SUM_NIGHTMARE_DRAKE, true);
                break;
            case EVENT_4:
                me->CastSpell(me, alies ? SPELL_SUM_ALLIES_DRYAD : SPELL_SUM_NIGHTMARE_DRYAD, true);
                break;
            case EVENT_5:
...
struct npc_cenarius_forces_of_nightmare : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        case NPC_TWISTED_SISTER:
            ...
            events.RescheduleEvent(EVENT_3, 11s);
            break;
        }
    }
...
void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    ...
    void SwitchTarget(bool allowTank = false)
    {
        ...
        auto pTarget = SelectTarget(SelectTargetMethod::Random, 1, [this](Unit const* t) { return t->IsPlayer(); });
        if (!pTarget)
            pTarget = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true);
...
struct npc_cenarius_allies_of_nature : public ScriptedAI
{
    ...
    void DefaultEvents()
    {
        ...
        events.RescheduleEvent(EVENT_3, 58s);
        instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, me);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
            case EVENT_1:
                me->CastSpell(me, SPELL_CLEANSING_DETONATE_AT, false);
                break;
            case EVENT_2:
            {
                ...
                events.RescheduleEvent(EVENT_2, 36s);
                break;
            }
            case EVENT_3:
                events.Reset();
...
struct npc_cenarius_malfurion_stormrage : public ScriptedAI
{
    ...
    void DoAction(int32 const action) override
    {
        ...
        case ACTION_3:
            ...
            break;
        case ACTION_4:
            events.RescheduleEvent(EVENT_5, 4s); // outro
            break;
        }
    }
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        if (spell->Id == SPELL_CLEANSING_GROUND_FILTER)
            DoCast(target->ToUnit(), SPELL_CLEANSING_GROUND_AT, true);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_4:
                if (auto owner = me->GetOwner())
                    if (auto target = owner->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 80.0f, true))
            ...
            case EVENT_5: // outro
                ...
                events.RescheduleEvent(EVENT_6, 10s);
                break;
            case EVENT_6:
                Talk(4);
                if (auto owner = me->GetOwner())
                    me->AddAura(225790, owner);
                events.RescheduleEvent(EVENT_7, 10s);
                break;
            case EVENT_7:
                Talk(5);
                events.RescheduleEvent(EVENT_8, 10s);
                break;
            case EVENT_8:
                Talk(6);
                events.RescheduleEvent(EVENT_9, 10s);
                break;
            case EVENT_9:
                Talk(7);
                break;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_2:
            {
                ...
                if (auto owner = me->GetOwner())
                    if (auto target = owner->GetAI()->SelectTarget(SelectTargetMethod::Random, 1, 80.0f, true))
...
void AddSC_boss_cenarius()
{
    ...
    RegisterSpellScript(spell_cenarius_allies_periodic_energize);
    RegisterSpellScript(spell_cenarius_ancient_dream_filter);
    RegisterSpellScript(spell_cenarius_ancient_dream);
    ...
    RegisterSpellScript(spell_cenarius_dread_thorns_reflects);
