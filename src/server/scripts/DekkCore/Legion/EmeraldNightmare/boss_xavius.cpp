...
struct boss_xavius : public BossAI
{
    ...
    void StartEvents(uint8 phase)
    {
        ...
        {
        case PHASE_1:
            events.RescheduleEvent(EVENT_ABOMINATION_CRUSHING, 12s);
            events.RescheduleEvent(EVENT_DARKENING_SOUL, 7s);
            events.RescheduleEvent(EVENT_NIGHTMARE_BLADES, 18s);
            break;
        case PHASE_2:
            events.RescheduleEvent(EVENT_BLACKENING_SOUL, 7s);
            events.RescheduleEvent(EVENT_CORRUPTION_METEOR, 21s);
            events.RescheduleEvent(EVENT_NIGHTMARE_INFUSION, 32s);
            if (me->GetMap()->GetDifficultyID() == DIFFICULTY_HEROIC_RAID || me->GetMap()->GetDifficultyID() == DIFFICULTY_MYTHIC_RAID)
                events.RescheduleEvent(EVENT_BONDS_OF_TERROR, 12s);
            break;
        case PHASE_3:
            events.RescheduleEvent(EVENT_BLACKENING_SOUL, 7s);
            events.RescheduleEvent(EVENT_NIGHTMARE_BLADES, 18s);
        ...
        events.RescheduleEvent(EVENT_CHECK_ALT_POWER, 2s);
    }
...
void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        ...
        {
            if (auto pTarget = SelectTarget(SelectTargetMethod::Random, checkPlayers(), 150.0f, true, true, -SPELL_NIGHTMARE_BLADES_MARK))
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_ABOMINATION_CRUSHING:
            {
                ...
                events.RescheduleEvent(EVENT_ABOMINATION_CRUSHING, 10s);
                break;
            }
            case EVENT_DARKENING_SOUL:
                DoCastVictim(SPELL_DARKENING_SOUL);
                events.RescheduleEvent(EVENT_DARKENING_SOUL, 8s);
                if (urand(0, 3) == 3)
                    Talk(SAY_DARKENING_SOUL);
                break;
            case EVENT_NIGHTMARE_BLADES:
                if (auto target = SelectTarget(SelectTargetMethod::Random, checkPlayers(), 150.0f, true, true, -SPELL_NIGHTMARE_BLADES_MARK_2))
                {
                    DoCast(target, SPELL_NIGHTMARE_BLADES_MARK, true);
                    DoCast(target, SPELL_NIGHTMARE_BLADES_MARK_2, true);
                }
                events.RescheduleEvent(EVENT_NIGHTMARE_BLADES, 16s);
                break;
            ...
            case EVENT_NIGHTMARE_INFUSION:
                ...
                events.RescheduleEvent(EVENT_NIGHTMARE_INFUSION, 62s);
                break;
            case EVENT_CORRUPTION_METEOR:
                DoCast(me, SPELL_CORRUPTION_METEOR, true);
                events.RescheduleEvent(EVENT_CORRUPTION_METEOR, 29s);
                break;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_3:
                ...
                events.RescheduleEvent(EVENT_4, 5s);
                break;
            case EVENT_4:
                me->SetVisible(false);
                break;
...
struct npc_xavius_lurking_terror : public ScriptedAI
{
    ...
    void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
    {
        ...
        {
            DoCast(target->ToUnit(), SPELL_TORMENTING_FIXATION, true);
...
struct npc_xavius_corruption_horror : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.RescheduleEvent(EVENT_2, 14s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_2:
                ...
                events.RescheduleEvent(EVENT_2, 20s);
                break;
            }
        }
        DoMeleeAttackIfReady();
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
            {
                if (auto owner = me->GetOwner())
                    if (auto target = owner->GetAI()->SelectTarget(SelectTargetMethod::Random, 0, 150.0f, true, true, SPELL_NIGHTMARE_BLADES_MARK))
                    {
                        Position pos;
...
struct npc_xavius_nightmare_tentacle : public ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
            case EVENT_1:
                if (auto target = SelectTarget(SelectTargetMethod::Random, 0, 150.0f, true))
...
struct npc_xavius_event_conroller : public ScriptedAI
{
    npc_xavius_event_conroller(Creature* creature) : ScriptedAI(creature), summons(me)
    {
        ...
        if (instance->GetBossState(DATA_PRE_EVENT_XAVIUS) == NOT_STARTED)
        {
            for (int i = 0; i < 9; ++i)
            {
                me->SummonCreature(NPC_SMALL_TRASH, eventPos[6].GetPositionX() + irand(-15, 15), eventPos[6].GetPositionY() + irand(-15, 15), eventPos[6].GetPositionZ(), 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
            }
            for (int i = 0; i < 9; ++i)
            {
                me->SummonCreature(NPC_SMALL_TRASH, eventPos[7].GetPositionX() + irand(-15, 15), eventPos[7].GetPositionY() + irand(-15, 15), eventPos[7].GetPositionZ(), 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
            }
            for (int i = 0; i < 9; ++i)
            {
                me->SummonCreature(NPC_SMALL_TRASH, eventPos[8].GetPositionX() + irand(-15, 15), eventPos[8].GetPositionY() + irand(-15, 15), eventPos[8].GetPositionZ(), 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
            }
            for (int i = 0; i < 9; ++i)
            {
                me->SummonCreature(NPC_SMALL_TRASH, eventPos[9].GetPositionX() + irand(-15, 15), eventPos[9].GetPositionY() + irand(-15, 15), eventPos[9].GetPositionZ(), 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
            }
        }
    }
    ...
    void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
    {
        ...
        case 6:
            ...
            break;
        case 12:
            events.RescheduleEvent(EVENT_5, 5s);
            break;
        default:
            break;
        }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_1:
                for (uint8 i = 0; i < 3; ++i)
                    me->SummonCreature(NPC_IN_THE_SHADOW, eventPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                events.RescheduleEvent(EVENT_6, 5s);
                break;
            case EVENT_2:
                for (uint8 i = 0; i < 3; ++i)
                    me->SummonCreature(NPC_NIGHTMARE_AMALGAMATION, eventPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                events.RescheduleEvent(EVENT_6, 5s);
                break;
            case EVENT_3:
                for (uint8 i = 0; i < 2; ++i)
                    me->SummonCreature(NPC_NIGHTMARE_AMALGAMATION, eventPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                me->SummonCreature(NPC_IN_THE_SHADOW, eventPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                events.RescheduleEvent(EVENT_4, 7s);
                events.RescheduleEvent(EVENT_6, 5s);
                break;
            case EVENT_4:
                for (uint8 i = 3; i < 6; ++i)
                    me->SummonCreature(NPC_SHADOW_POUNDER, eventPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);
                break;
            case EVENT_5:
            {
        ...
        if (auto player = GetTarget()->ToPlayer())
        {
            ...
            for (GuidList::const_iterator iter = summonList->begin(); iter != summonList->end(); ++iter)
            {
                if (auto summon = ObjectAccessor::GetCreature(*player, (*iter)))
                {
                    ...
                    summon->DespawnOrUnsummon(100ms);
                }
            }
        }
    }
...
void AddSC_xavius()
{
    ...
    RegisterSpellScript(spell_xavius_periodic_energize);
    RegisterSpellScript(spell_xavius_dream_simulacrum);
    RegisterSpellScript(spell_xavius_nightmare_torment_alt_power);
    RegisterSpellScript(spell_xavius_crushing_shadows);
    RegisterSpellScript(spell_xavius_unfathomable_reality);
    RegisterSpellScript(spell_xavius_descent_into_madness);
    RegisterSpellScript(spell_xavius_darkening_soul);
    RegisterSpellScript(spell_xavius_nightmare_blades);
    RegisterSpellScript(spell_xavius_nightmare_blades_dmg_filter);
    RegisterSpellScript(spell_xavius_bonds_of_terror);
    ...
    RegisterSpellScript(spell_teleport_to_rift);}
