...
public:
    ...
    struct boss_odyn_hov_AI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            achievement = true;
            events.RescheduleEvent(EVENT_SPEAR_OF_LIGHT, 8s);
            events.RescheduleEvent(EVENT_RADIANT_TEMPEST, 24s);
            events.RescheduleEvent(EVENT_SHATTER_SPEARS, 40s);
            events.RescheduleEvent(EVENT_RUNIC_BRAND, 44s);

            if (me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
                events.RescheduleEvent(EVENT_SUMMON_STORMFORGED, 18s);
        ...
        void DoAction(int32 const action) override
        {
            ...
            events.RescheduleEvent(EVENT_SKOVALD_DONE_1, 5s);
        }
            ...
            if (spell->Id == SPELL_SHATTER_SPEARS)
                if (target->GetEntry() == NPC_SPEAR_OF_LIGHT)
                {
                    ...
                    for (uint8 i = 0; i < 3; i++)
                    {
                        ...
                        Position pos = target->GetNearPosition(1.0f, angle);
                        me->CastSpell(pos, SPELL_GLOWING_FRAGMENT, true);
                    }
                    target->ToCreature()->DespawnOrUnsummon(1s);
                }
            if (spell->Id == SPELL_RUNIC_BRAND)
            {
                DoCast(target->ToUnit(), SpellsRunicColour[runicBrandCount], true);
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if (me->HealthBelowPct(81) && !encounterComplete)
            {
                ...
                Talk(SAY_DEATH);
                _JustDied();
                instance->DoUpdateCriteria(CriteriaType::DefeatDungeonEncounter, 1809);
                instance->DoRemoveAurasDueToSpellOnPlayers(200988);
                ...
                if (GetData(DATA_ACHIEVEMENT))
                    instance->DoUpdateCriteria(CriteriaType::BeSpellTarget, 208534);
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SKOVALD_DONE_1:
                    ...
                    events.RescheduleEvent(EVENT_SKOVALD_DONE_2, 10s);
                    break;
                ...
                case EVENT_RADIANT_TEMPEST:
                    ...
                    Talk(SAY_RADIANT);
                    events.RescheduleEvent(EVENT_RADIANT_TEMPEST, 48s);
                    break;
                case EVENT_SHATTER_SPEARS:
                    DoCast(SPELL_SHATTER_SPEARS);
                    Talk(SAY_SPEARS);
                    events.RescheduleEvent(EVENT_SHATTER_SPEARS, 56s);
                    break;
                case EVENT_RUNIC_BRAND:
                    runicBrandCount = 0;
                    DoCast(SPELL_RUNIC_BRAND);
                    Talk(SAY_RUNIC);
                    events.RescheduleEvent(EVENT_RUNIC_BRAND, 56s);
                    break;
                case EVENT_SUMMON_STORMFORGED:
                   // Talk();
                    me->CastSpell(Position(centrPos.GetPositionX(), centrPos.GetPositionY(), centrPos.GetPositionZ()), SPELL_SUMMON_STORMFORGED, true);
                    events.RescheduleEvent(EVENT_SUMMON_STORMFORGED, 52s);
                    break;
                }
            }
