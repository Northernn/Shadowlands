...
public:
    ...
    struct boss_fenryrAI : public BossAI
    {
            ...
            //43:41(N) | 34:01(H)
        {
            ...
            events.RescheduleEvent(EVENT_UNNERVING_HOWL, 3s); //43:44, 44:23, 45:02, 45:38
            events.RescheduleEvent(EVENT_RAVENOUS_LEAP, 28s); //44:09, 44:49, 45:26
            events.RescheduleEvent(EVENT_CLAW_FRENZY, 40s);   //44:20, 44:47, 45:00, 45:36, 45:44
            events.RescheduleEvent(EVENT_SCENT_OF_BLOOD, 9s); //43:50, 44:28, 45:07, 45:47

            if (me->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
                events.RescheduleEvent(EVENT_SPAWN_WOLVES, 4s); //34:05, 34:41
        }
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            ...
            case SPELL_RAVENOUS_LEAP_T_1:
                ...
                {
                    DoCast(target->ToUnit(), SPELL_RAVENOUS_LEAP_JUMP, true);
                    jumpCount++;
                }
                break;
            case SPELL_RAVENOUS_LEAP_AOE:
                DoCast(target->ToUnit(), SPELL_RAVENOUS_LEAP_DOT, true);
                break;
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_UNNERVING_HOWL:
                    ...
                    events.RescheduleEvent(EVENT_UNNERVING_HOWL, 36s);
                    break;
                case EVENT_RAVENOUS_LEAP:
                    ...
                    events.RescheduleEvent(EVENT_RAVENOUS_LEAP, 36s);
                    break;
                case EVENT_CLAW_FRENZY:
                    DoCast(SPELL_CLAW_FRENZY);
                    events.RescheduleEvent(EVENT_CLAW_FRENZY, 20s);
                    break;
                case EVENT_SCENT_OF_BLOOD:
                    DoCast(me, SPELL_SCENT_OF_BLOOD, true);
                    events.RescheduleEvent(EVENT_SCENT_OF_BLOOD, 38s);
                    break;
                case EVENT_SPAWN_WOLVES:
                {
                    for (uint8 i = 0; i < 3; i++)
                    {
                        ...
                        Position pos = me->GetNearPosition(40.0f, i);
                        me->CastSpell(pos, SPELL_SPAWN_WOLVES, true);
                    }
                    events.RescheduleEvent(EVENT_SPAWN_WOLVES, 36s);
                    break;
                }
                }
            }
...
public:
    ...
    struct npc_fenryrAI : public ScriptedAI
    {
        ...
        {
            DoZoneInCombat();
            events.RescheduleEvent(EVENT_UNNERVING_HOWL, 7s);
            events.RescheduleEvent(EVENT_RAVENOUS_LEAP, 12s);
            events.RescheduleEvent(EVENT_CLAW_FRENZY, 20s);
        }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            if (me->HealthBelowPct(61) && !loseFight)
            {
                ...
                events.RescheduleEvent(EVENT_LOW_HP_MOVE_LAIR, 5s);
                instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_RAVENOUS_LEAP_DOT);
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_UNNERVING_HOWL:
                    Talk(SAY_UNNERVING_HOWL);
                    DoCast(SPELL_UNNERVING_HOWL);
                    events.RescheduleEvent(EVENT_UNNERVING_HOWL, 32s);
                    break;
                case EVENT_RAVENOUS_LEAP:
                    jumpCount = 0;
                    jumpMaxCount = 0;
                    DoCast(me, SPELL_RAVENOUS_LEAP_FILTER, true);
                    DoCast(SPELL_RAVENOUS_LEAP);
                    events.RescheduleEvent(EVENT_RAVENOUS_LEAP, 34s);
                    break;
                case EVENT_CLAW_FRENZY:
                    DoCast(SPELL_CLAW_FRENZY);
                    events.RescheduleEvent(EVENT_CLAW_FRENZY, 10s);
                    break;
