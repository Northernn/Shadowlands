...
public:
    ...
    struct boss_god_king_skovaldAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_FELBLAZE_RUSH, 7s);
            events.RescheduleEvent(EVENT_SAVAGE_BLADE, 24s);
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            if (id == 1)
            {
                ...
                events.RescheduleEvent(EVENT_INTRO_COMPLETE, 24s);
            }
            else if (id == 2)
            {
        ...
        void SpellHitTarget(WorldObject* target, const SpellInfo* spell) override
        {
            ...
            case SPELL_INFERNAL_FLAMES_FIND_N:
                if (!me->isInBack(target, 3.14f * 0.8f))
                    DoCast(target->ToUnit(), SPELL_INFERNAL_FLAMES_TRIG_AT, true);
                break;
            case SPELL_INFERNAL_FLAMES_FIND_H:
                DoCast(target->ToUnit(), SPELL_INFERNAL_FLAMES_TRIG_SUM, true);
                break;
            }
        }
        ...
        {
            events.RescheduleEvent(EVENT_PICK_AEGIS, 1s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->IsInCombat())
            {
                if (powerTimer <= diff)
                {
                    ...
                    {
                        powerTimer = 4000;
                        events.RescheduleEvent(EVENT_RAGNAROK, 5s);
                    }
                }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FELBLAZE_RUSH:
                    ...
                    events.RescheduleEvent(EVENT_FELBLAZE_RUSH, 26s);
                    break;
                case EVENT_RAGNAROK:
...
public:
    ...
    struct npc_skovald_aegis_of_aggramarAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* caster, const SpellInfo* spell) override
        {
            ...
            {
                DoCast(caster->ToUnit(), 193988, true);
                me->DespawnOrUnsummon();
            }
        }
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            if (InstanceScript* instance = caster->GetInstanceScript())
                if (instance->GetBossState(DATA_SKOVALD) == IN_PROGRESS)
                    caster->CastSpell(Position(loc->GetPositionX(), loc->GetPositionY(), loc->GetPositionZ()), 193781, true);
        }
