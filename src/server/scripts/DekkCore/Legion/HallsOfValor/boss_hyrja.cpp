...
public:
    ...
    struct boss_hyrjaAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.RescheduleEvent(EVENT_EXPEL_LIGHT, 3s);
            events.RescheduleEvent(EVENT_SHIELD_OF_LIGHT, 10s);
            events.RescheduleEvent(EVENT_SPECIAL_ABILITY, 20s);
            events.RescheduleEvent(EVENT_ARCING_BOLT, 12s);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_EXPEL_LIGHT:
                    ...
                    events.RescheduleEvent(EVENT_EXPEL_LIGHT, 30s);
                    break;
                ...
                case EVENT_SPECIAL_ABILITY:
                {
                    ...
                    events.RescheduleEvent(EVENT_SPECIAL_ABILITY, 30s);
                    break;
                }
                ...
                if (hyrja->GetMap()->GetDifficultyID() != DIFFICULTY_NORMAL)
                {
                    if (hyrja->GetAI())
                    {
                        ...
                        if (stack > 1)
                        {
                            ...
                            if (Unit* randTarget = hyrja->AI()->SelectTarget(SelectTargetMethod::Random, 1, 100.0f, true, true, -SPELL_EXPEL_LIGHT_DUMMY))
                                target->CastSpell(randTarget, GetId(), true);
                            else if (Unit* randTarget = hyrja->AI()->SelectTarget(SelectTargetMethod::Random, 0, 80.0f, true))
                                target->CastSpell(randTarget, GetId(), true);
                        }
                    }
                }
          ...
          //  absorbSave = GetSpellInfo()->GetEffect(EFFECT_0, GetCaster()->GetMap()->GetDifficultyID())->CalcValue(GetCaster());
            ...
            if (auto instance = GetCaster()->GetInstanceScript())
            {
                if (auto hyrja = instance->instance->GetCreature(instance->GetGuidData(DATA_HYRJA)))
                    if (auto effect = hyrja->GetAuraEffect(206150, EFFECT_1)) //challengers might
