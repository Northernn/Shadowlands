	...
	struct bfa_boss_warlord_parjesh_AI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_THROW_SPEAR:
				{
					...
					//SelectTargetList(targets, 5, SelectTargetMethod::Random, 500.0f, true); // 5 players, random targets, alive > non-tank spec selector Nontankselector selects the current target
        ...
        void OnSpellCast(SpellInfo const* spell) override
        {
			...
			case SPELL_CRASHING_WAVE_CAST:
				me->CastSpell(Position(me->GetPositionX() + 5.0f, me->GetPositionY(), me->GetPositionZ()), SPELL_QUICKSAND_AT, true);
				me->CastSpell(Position(me->GetPositionX() + 10.0f, me->GetPositionY(), me->GetPositionZ()), SPELL_QUICKSAND_AT, true);
				me->CastSpell(Position(me->GetPositionX() + 15.0f, me->GetPositionY(), me->GetPositionZ()), SPELL_QUICKSAND_AT, true);
	...
	struct bfa_npc_hatecoil_crestrider_AI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				{
					Unit* target = SelectTarget(SelectTargetMethod::Random, 0, NonTankTargetSelector(me));
