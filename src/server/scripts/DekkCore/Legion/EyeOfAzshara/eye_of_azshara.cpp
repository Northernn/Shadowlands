...
class spell_animated_storm_water_spout : public SpellScript
{
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        {
            float x, y, z;
            target->GetNearPoint(target, x, y, z, 3.0f, target->GetAbsoluteAngle(caster));
...
class aura_eoa_violent_winds_force_move : public AuraScript
{
    ...
    {
        Unit* target = GetTarget();
        target->ApplyMovementForce(target->GetGUID(), Position(-3486.264f, 4386.87f, -3.580416f), 2.0f, MovementForceType::SingleDirectional);
    }
...
void AddSC_eye_of_azshara()
{
    ...
    RegisterSpellScript(aura_hatecoil_wavebinder_bubble_shield);
    RegisterSpellScript(aura_makrana_hardshell_armorshell);
    RegisterSpellScript(aura_eoa_violent_winds_broadcast);
    RegisterSpellScript(aura_eoa_violent_winds_force_move);
    RegisterSpellScript(aura_eoa_lightning_strikes);
