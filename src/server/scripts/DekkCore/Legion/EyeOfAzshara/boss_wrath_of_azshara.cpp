...
public:
	...
	struct bfa_npc_mystic_tornado_AI : public ScriptedAI
	{
		...
		void Reset() override
		{
			...
			events.ScheduleEvent(EVENT_BORN, 2s);
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_CHECK_DISTANCE:
					CheckDist();
					events.ScheduleEvent(EVENT_CHECK_DISTANCE, 5s);
					break;
				case EVENT_BORN:
			...
			for (auto & it : me->GetMap()->GetPlayers())
			{
				if (Player* player = it.GetSource())
				{
					if (player->HasAura(SPELL_ARCANE_BOMB))
					{
						...
						_events.ScheduleEvent(EVENT_ARCANE_BOMB, 1s);
						break;
					}
				}
			}
...
public:
	...
	public:
		void HandleSummon()
		{
			...
			GetCaster()->SummonCreature(NPC_ARCANE_BOMB, summon_pos, TEMPSUMMON_TIMED_DESPAWN, 20s);
		}
