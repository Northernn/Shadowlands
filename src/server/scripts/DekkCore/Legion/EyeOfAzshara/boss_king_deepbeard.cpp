		...
		void HandleEffectRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
		{
			...
			if (removeMode == AURA_REMOVE_BY_EXPIRE)
			{
				if (owner->GetMap() && owner->GetMap()->IsHeroic() || owner->GetMap()->IsMythic())
					owner->CastSpell(owner, SPELL_GASEOUS_EXPLOSION, CastSpellExtraArgs(TRIGGERED_FULL_MASK).AddSpellBP0(aurEff->GetBaseAmount() - _amountAbsorbed));
				else
	...
	public:
		...
		void HandleOnCast()
		{
			...
			for (uint8 i = 0; i < 40; i++)
			{
				...
				GetCaster()->SummonCreature(NPC_CALL_OF_THE_SEA, random, TEMPSUMMON_TIMED_DESPAWN, 21s);
			}
		}
