...
//
			if (me->GetMap()->IsHeroic() || me->GetMap()->IsMythic())
				events.ScheduleEvent(EVENT_MONSOON, 50s);
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_STATIC_NOVA:
				{
					...
					events.ScheduleEvent(EVENT_FOCUSED_LIGHTING, 15s);
					break;
				}
				case EVENT_FOCUSED_LIGHTING:
				{
					...
					events.ScheduleEvent(EVENT_STATIC_NOVA, 20s);
					break;
				}
				...
				//	SelectTargetList(targets, 5, SelectTargetMethod::Random, 500.0f, true);

					if (!targets.empty())
						for (std::list<Unit*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
							me->CastSpell((*itr), SPELL_BECKON_STORM);
				//	events.ScheduleEvent(EVENT_BECKON_STORM, 45s);
					break;
				}
				}
			}
...
public:
	...
	struct bfa_go_sand_dune_AI : public GameObjectAI
	{
		...
		void Reset() override
		{
			me->HasFlag(GO_FLAG_NOT_SELECTABLE);
		}
	};
...
public:
	...
	struct bfa_npc_mosoon_AI : public ScriptedAI
	{
		...
		void HandleEvents()
		{
			...
			events.ScheduleEvent(EVENT_CHECK_PLAYER_DISTANCE, 3s);
			events.ScheduleEvent(EVENT_CHECK_DUNE_DISTANCE, 2s);
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_CHECK_PLAYER_DISTANCE:
				{
					...
					events.ScheduleEvent(EVENT_CHECK_PLAYER_DISTANCE, 1s);
					break;
				}
