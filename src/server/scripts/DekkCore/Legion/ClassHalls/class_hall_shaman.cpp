...
struct npc_trall_91731 : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        if (!player)
            return;
        if (TempSummon* personalCreature = player->SummonCreature(_insideNpc, _positon, TEMPSUMMON_TIMED_DESPAWN))
        {
    ...
    void JustEngagedWith(Unit* who) override
        {
            ...
            DoZoneInCombat();
            me->SetFullHealth();
            events.ScheduleEvent(EVENT_SHATTERING_AXE, 6s);
            events.ScheduleEvent(EVENT_FEL_SPIKES, 13s);
        }
  ...
  void UpdateAI(uint32 diff) override
   ...
   while (uint32 eventId = events.ExecuteEvent())
     ...
     case EVENT_SHATTERING_AXE:
                {
                    Unit* target = SelectTarget(SelectTargetMethod::Random, 0.0f, 10.0f, true);
                    if (!target)
                        break;
                ...
                {
                    Unit* target = SelectTarget(SelectTargetMethod::MaxThreat, 0.0, 0.0, true);
                    if (!target)
                        break;
