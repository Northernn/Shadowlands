...
struct npc_meryl_felstorm_102700 : public ScriptedAI
{
    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        if (player->HasQuest(QUEST_FINDING_BONCHILL) /*&& (player->GetQuestObjectiveData(QUEST_FINDING_BONCHILL, 0) ) && !player->GetQuestObjectiveData(QUEST_FINDING_BONCHILL, 6)*/)
        {
            ...
            AddGossipItemFor(player, GossipOptionNpc::None, "Tell Merrill about the information you found.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        if (!HasPlayer(player->GetGUID()) && (player->HasQuest(QUEST_THE_DREADLORDS_PRIZE)) /*&& (player->GetQuestObjectiveData(QUEST_THE_DREADLORDS_PRIZE, 0))*/ /*&& (!player->GetQuestObjectiveData(QUEST_THE_DREADLORDS_PRIZE, 2))*/)
        {
            ...
            events.ScheduleEvent(EVENT_THE_DREADLORDS_PRIZE_LAST_TALK, 1s);
        }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        if (type == POINT_MOTION_TYPE)
            ...
            {
             events.ScheduleEvent(EVENT_TALK_1, 5s);
                break;
            }
    ...
    void QuestAccept(Player* player, Quest const* quest)
    {
        if (quest->GetQuestId() == QUEST_THE_DREADLORDS_PRIZE)
        {
            ...
            events.ScheduleEvent(EVENT_TALK_1, 10s);
         //   player->AddConversationDelayedTeleport(14s, CONVERSATION_THE_DREADLORDS_PRIZE, 1494, 1276.16f, -263.27f, 44.36f, 6.275331f);
        ...
        if (quest->GetQuestId() == QUEST_FINDING_BONCHILL)
        {
            ...
            PhasingHandler::AddPhase(player, PHASE_2, true);
            PhasingHandler::AddPhase(player, PHASE_3, true);
        }
    }
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_TALK_1:
                {
                    ...
                    Talk(0);
                    events.ScheduleEvent(EVENET_TELE_TO_LFG_DUNGENON, 2s);
                    break;
                }
