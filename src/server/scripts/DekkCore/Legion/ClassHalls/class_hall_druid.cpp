    ...
    struct npc_class_hall_druid_gatewardenAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (me->isInBackInMap(who, 12.0f))   // In my line of sight, "outdoors", and behind me
            {
                ...
                {
                  //  if (!who->ToPlayer()->GetQuestObjectiveDatas(QUEST_TO_THE_DREAMGROVE, 1))
                    {
    ...
    struct npc_nightmare_blight_103246 : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer) override
        {
            if (killer->IsPlayer())
            {
                ...
                killer->ToPlayer()->SummonCreature(NPC_KEEPER_REMULOS_103488, Position(1720.639f, 1347.214f, 2.08268f, 4.830286f), TEMPSUMMON_TIMED_DESPAWN);
        ...
        void DoAction(int32 param)
        {
            ...
            case ACTION_QUEST_ITS_RIGHTFUL_PLACE:
                if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                {
                    ...
                    {
                        issay = true;
                        PhasingHandler::RemovePhase(player, QUEST_THE_SCYTHE_OF_ELUNE_PHASE, true);
                        PhasingHandler::AddPhase(player, QUEST_ITS_RIGHTFUL_PLACE_PHASE_1, true);
                        //summon 101657
                        ...
                        {
                            PhasingHandler::AddPhase(player, QUEST_ITS_RIGHTFUL_PLACE_PHASE_2, true);
                            PhasingHandler::RemovePhase(player, QUEST_ITS_RIGHTFUL_PLACE_PHASE_1, true);
                            PhasingHandler::InheritPhaseShift(ariden, player);
                        });
