...
public:
    ...
    private:
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_CHECK_ENERGY, 3s);
            events.ScheduleEvent(EVENT_GUST, 10s);

            if (IsHeroic() || IsMythic())
                events.ScheduleEvent(EVENT_ARCING_BLADE, 12s);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CHECK_ENERGY:
                {
                    if (shielded)
                    {
                        ...
                        {
                            castSpecial = true;
                            events.ScheduleEvent(EVENT_A_PEAL_OF_THUNDER, 10s);
                        }
...
public:
    ...
    private:
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            if (IsHeroic() || IsMythic())
                events.ScheduleEvent(EVENT_GALE_FORCE, 22s);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CHECK_ENERGY:
                {
                    if (shielded)
                    {
                        ...
                        {
                            castSpecial = true;
                            events.ScheduleEvent(EVENT_CONDUCTION, 15s);
                        }
                ...
                case EVENT_CYCLONE_STRIKE:
                    ...
                    events.ScheduleEvent(EVENT_CYCLONE_STRIKE, 25s);
                    break;
                case EVENT_JOLT:
                    if (shielded)
                        if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                            me->CastSpell(target, SPELL_JOLT);
                    events.ScheduleEvent(EVENT_JOLT, 5s);
                    break;
                }
            }
        }
