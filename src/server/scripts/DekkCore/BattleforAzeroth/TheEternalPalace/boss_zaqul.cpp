    ...
    struct bfa_boss_zaqul_AI : public BossAI
    {
        ...
        void SummonQueenAndPorta()
        {
            if (Creature* portal = me->SummonCreature(NPC_PORTAL_TO_AZSHARA, 195.06f, 2053.93f, 756.05f, TEMPSUMMON_MANUAL_DESPAWN))
                portal->SetNpcFlag(UNIT_NPC_FLAG_GOSSIP);
    ...
    class bfa_spell_delirium_realm_cast_SpellScript : public SpellScript
    {
        ...
        void HandleDummy()
        {
            ...
            for (uint8 i = 0; i < 3; ++i)
            {
                ...
                caster->CastSpell(Position(x,y,z), SPELL_CAUSTIC_DELIRIUM_AT, true);
            }
        }
...
public:
    ...
    class bfa_spell_fear_realm_hysteria_AuraScript : public AuraScript
    {
        ...
        void HandlePeriodic(AuraEffect const* aureff)
        {
            ...
            target->AddAura(SPELL_HYSTERIA, target);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_TENTACLE_SLAM:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 20.0f, true, 0))
    ...
    class bfa_spell_mind_tether_damage_share_AuraScript : public AuraScript
    {
        ...
        void HandleOnProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            if (validTarget)
            {
                ...
                if (target->GetMap()->GetMapDifficulty()->DifficultyID == Difficulty::DIFFICULTY_NORMAL)
                    args.AddSpellMod(SPELLVALUE_BASE_POINT0, (sharedDamage / 2));
                else
                    args.AddSpellMod(SPELLVALUE_BASE_POINT0, sharedDamage);
                args.SetTriggerFlags(TRIGGERED_FULL_MASK);

                target->CastSpell(validTarget, SPELL_MIND_TETHER_LINKAGE_DAMAGE, args);
            }
        }
