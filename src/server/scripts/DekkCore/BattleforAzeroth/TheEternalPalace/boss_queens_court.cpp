        ...
        void DespawnCreature(uint32 entry, uint32 timer)
        {
            ...
            if (clist.size())
            {
                for (auto creature : clist)
                    creature->DespawnOrUnsummon(Seconds(timer));
            }
        }
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
                if (Unit* target = GetHitUnit())
                    caster->CastSpell(Position(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ()), SPELL_MIGHTY_RUPTURE_AT, true);
        }
