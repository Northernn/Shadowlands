    ...
    struct bfa_boss_orgozoa_AI : public BossAI
    {
       ...
       void OnSpellCast(SpellInfo const* spell) override
        ...
        void MovementInform(uint32 /*type*/, uint32 pointId) override
        {
            ...
            {
            case POINT_1:
                events.ScheduleEvent(EVENT_ACTION_MOVE_2, 5s);
                break;
            case POINT_2:
                events.ScheduleEvent(EVENT_ACTION_MOVE_3, 5s);
                break;
            case POINT_3:
                events.ScheduleEvent(EVENT_ACTION_MOVE_4, 5s);
                break;
            case POINT_4:
                events.ScheduleEvent(EVENT_ACTION_MOVE_5, 5s);
                break;
            case POINT_5:
                events.ScheduleEvent(EVENT_ACTION_MOVE_6, 5s);
                break;
            case POINT_6:
                events.ScheduleEvent(EVENT_ACTION_MOVE_7, 5s);
                break;
            case POINT_7:
                events.ScheduleEvent(EVENT_ACTION_MOVE_8, 5s);
                break;
            case POINT_8_FINAL:
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_ENERGY_CHECK:
                {
                    ...
                    {
                        if (phase1_check)
                            events.ScheduleEvent(EVENT_DRIBBLING_ICHOR_PHASE_1, 5s);
                        else if (phase2_check)
                            events.ScheduleEvent(EVENT_DRIBBLING_ICHOR_PHASE_2, 5s);
                        me->SetPower(POWER_ENERGY, 0);
                    }
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SHOCKING_LIGHTNING:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true, 0))
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
                if (WorldLocation const* target = GetExplTargetDest())
                    caster->CastSpell(target->GetPosition(), SPELL_AQUA_LANCE_AT);
        }
