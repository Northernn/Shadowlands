...
class instance_eternal_palace : public InstanceMapScript
{
public:
    instance_eternal_palace() : InstanceMapScript("instance_eternal_palace", 2164) { }


    InstanceScript* GetInstanceScript(InstanceMap* map) const
    {
        return new instance_eternal_palace_InstanceMapScript(map);
    }

    struct instance_eternal_palace_InstanceMapScript : public InstanceScript
    {
        instance_eternal_palace_InstanceMapScript(InstanceMap* map) : InstanceScript(map), intro_conv(false)
        {
...
struct npc_generic_ashvane : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* u) override
    {
        ...
        case NPC_ZANJIR_MYRMIDON:
            events.ScheduleEvent(EVENT_AQUA_LANCE, 1500ms);
            events.ScheduleEvent(EVENT_RAGE_OF_AZSHARA, 3s);
            events.ScheduleEvent(EVENT_SKEWER, 5s);
            break;

        case NPC_AZSHARI_WITCH:
            events.ScheduleEvent(EVENT_CONDUCTIVE_PULSE, 1500ms);
            events.ScheduleEvent(EVENT_RAGE_OF_AZSHARA, 3s);
            events.ScheduleEvent(EVENT_SHOCKING_LIGHTNING, 5s);
            break;
