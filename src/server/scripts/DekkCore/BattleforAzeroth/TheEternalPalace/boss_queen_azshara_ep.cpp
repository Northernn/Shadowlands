...
struct bfa_boss_queen_azshara : public BossAI
{
    ...
    public:
        ...
        void Reset() override
        {
            ...
            {
                titanConsole->SetNpcFlag(UNIT_NPC_FLAG_GOSSIP);
                titanConsole->RemoveUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
            }
        ...
        void JustDied(Unit* /*target*/) override
        {
            ...
            me->SummonGameObject(GO_AZSHARA_CHEST, Position(216.6f, 2475.3f, 710.779f, 4.70423f), QuaternionData(0.0f, 0.0f, -0.709986f, 0.704216f), 0s);
...
private:
    ...
    {
        AddGossipItemFor(player, GossipOptionNpc::None, "Attempt to activate the device.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 0);
...
struct bfa_npc_cyranus : public ScriptedAI
{
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case EVENT_CHARGED_SPEAR:
        {
            ...
            Position pos;
            me->GetNearPoint(nullptr, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 1.0f, orientation);
            me->CastSpell(pos.m_positionX, pos.m_positionY, SPELL_CHARGED_SPEAR);
            */
            me->CastSpell(target->GetPosition(), SPELL_CHARGED_SPEAR);
            //events.ScheduleEvent(EVENT_CHARGED_SPEAR, TIMER_CHARGED_SPEAR);
            break;
        }
...
struct bfa_at_piercing_gaze : AreaTriggerAI
{
    ...
    bool IsOnBeam(WorldObject* obj, Position source, Position destination, float heightDiff, float beamWidth, float beamLenght)
    {
        ...
        float angleSourceDest = source.GetAbsoluteAngle(&destination);
        float angleSourcePoint = source.GetAbsoluteAngle(obj);
...
class bfa_spell_divide_and_conquer_cast : public SpellScript
{
    void DummyScript()
    {
        ...
        */
        Position centerPos = { 216.65f, 2444.94f, 710.65f };
        caster->CastSpell(centerPos, SPELL_DIVIDE_AND_CONQUER_AT_2, true);
            ...
            if (playerList.size())
            {
                for (auto player : playerList)
                {
                    if (IsOnBeam(at, at->GetPosition(), (at->GetPositionY() > 100.0f && at->GetPositionY() < 100.0f), 2.0f, 5.0f, 20.0f))
                    {
                        ...
                        at->GetCaster()->Kill(at->GetCaster(), player);
                    }
                }
            }
            timer = 2000;
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case EVENT_CHARGED_SPEAR:
        {
            ...
            else
            {
                ...
                float orientation = me->GetOrientation();
                Position pos;
                me->GetNearPoint(NULL, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 180.0f, orientation);
                */
                me->CastSpell(me->GetRandomNearPosition(180.0f), SPELL_CHARGED_SPEAR);
            }
...
void AddSC_boss_queen_azshara_ep()
{
    ...
    RegisterSpellScript(bfa_spell_arcane_burst_aura);
    RegisterSpellScript(bfa_spell_arcane_burst_dummy);
    RegisterSpellScript(bfa_spell_arcane_jolt);
    RegisterSpellScript(bfa_spell_arcane_mastery_periodic);
    new bfa_spell_beckon_cast(); //RegisterSpellAndAuraScriptPair
    RegisterSpellScript(bfa_spell_cursed_heart);
    RegisterSpellScript(bfa_spell_divide_and_conquer_cast);
    RegisterSpellScript(bfa_spell_ground_pound);
    RegisterSpellScript(bfa_spell_longing);
    RegisterSpellScript(bfa_spell_longing_aura);
    RegisterSpellScript(bfa_spell_march);
    RegisterSpellScript(bfa_spell_stay);
    RegisterSpellScript(bfa_spell_painful_memories);
    RegisterSpellScript(bfa_spell_painful_memories_aura);
