        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_ENERGY_CHECK:
                    ...
                    events.ScheduleEvent(EVENT_ENERGY_CHECK, 5s);
                    break;
        ...
        void AfterCast(SpellEffIndex index)
        {
            ...
            if (!playerList.empty())
            {
                ...
                for (auto player : playerList)
                {
                    ...
                    caster->CastSpell(Position(x, y, z), SPELL_ARCANADO_MISSILE, true);
                }
            }
        }
...
public:
    ...
    struct bfa_at_arcanado_AI : AreaTriggerAI
    {
        ...
        void FillCirclePath(Position const& center, float radius, float z, Movement::PointsArray& path, bool clockwise)
        {
            ...
            float angle = center.GetAbsoluteAngle(at->GetPositionX(), at->GetPositionY());
...
public:
    ...
    public:
        ...
        void HandleSummon()
        {
            ...
            GetCaster()->SummonCreature(NPC_ARCANE_BOMB_TRIGGER, summon_pos, TEMPSUMMON_TIMED_DESPAWN, 20s);
        }
            ...
            for (auto& it : me->GetMap()->GetPlayers())
            {
                if (Player* player = it.GetSource())
                {
                    if (player->HasAura(SPELL_ARCANE_BOMB_VEH))
                    {
                        ...
                        events.ScheduleEvent(EVENT_ARCANE_BOMB, 1s);
                        break;
                    }
                }
            }
...
public:
    ...
    class bfa_spell_ancient_tempest_players_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes mode)
        {
            ...
            target->ApplyMovementForce(caster->GetGUID(), caster->GetPosition(), -5.0f, MovementForceType::SingleDirectional);
        }
