        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if (canApplyForce)
            {
                ...
                for (Map::PlayerList::const_iterator i = playerList.begin(); i != playerList.end(); ++i)
                    if (Player* player = i->GetSource())
                    {
                        if (!player->IsGameMaster())
                        {
                            player->ApplyMovementForce(me->GetGUID(), me->GetPosition(), -5.0f, MovementForceType::SingleDirectional);
                        }
                    }
            }
            else
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CHANGE_PHASE:
                {
                    ...
                    HandleJellyFish();
                    isIntermission = true;
                    events.ScheduleEvent(EVENT_REMOVE_INTERMISSION, 35s);
                    me->SetPower(POWER_ENERGY, 0);
                   // events.ScheduleEvent(EVENT_CHANGE_PHASE, TIMER_CHANGE_PHASE);
                    break;
                }
                ...
                case EVENT_SHOCK_PULSE:
                {
                    ...
                    canApplyForce = true;
                    events.ScheduleEvent(EVENT_REMOVE_MOVEMENT_FORCE, 5s);
                  //  events.ScheduleEvent(EVENT_SHOCK_PULSE, TIMER_SHOCK_PULSE);
                    break;
                }
