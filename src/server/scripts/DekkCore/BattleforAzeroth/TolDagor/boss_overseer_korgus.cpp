...
struct boss_overseer_korgus : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.ScheduleEvent(EVENT_MUNITION, 5s);
        events.ScheduleEvent(EVENT_EXPLOSIVE_BURST, 11s);
        events.ScheduleEvent(EVENT_CROSSIGNITION, 16s);
        events.ScheduleEvent(EVENT_MASSIVE_BLAST, 17s);
        events.ScheduleEvent(EVENT_DEADEYE, 23s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_MUNITION:
                ...
                events.ScheduleEvent(EVENT_MUNITION, 44s);
                break;
            case EVENT_CROSSIGNITION:
               ...
               /* me->CastSpellWithOrientation(me, SPELL_CROSSIGNITION_VISUAL, true, 6.065254f);
                ...
                events.ScheduleEvent(EVENT_CROSSIGNITION, 44s);
                break;
            case EVENT_DEADEYE:
            ...
            case EVENT_MASSIVE_BLAST:
                ...
                events.ScheduleEvent(EVENT_MASSIVE_BLAST, 22s);
                break;
            case EVENT_EXPLOSIVE_BURST:
                ...
                if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 50.0f, true))
                {
                    ...
                    events.ScheduleEvent(EVENT_EXPLOSIVE_BURST, 44s);
                    break;
                }
            default:
                break;
...
class spell_generic_deadeye : public AuraScript
{
    ...
    void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        caster->CastSpell(target, SPELL_DEADEYE_BULLET, TRIGGERED_CAST_DIRECTLY);
    }
...
class spell_explosive_burst : public AuraScript
{
    ...
    void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        caster->CastSpell(target, SPELL_EXPLOSIVE_BURST_DAMAGE, TRIGGERED_CAST_DIRECTLY);
    }
...
void AddSC_boss_overseer_korgus()
{
    ...
    RegisterSpellScript(spell_generic_deadeye);
    RegisterSpellScript(spell_explosive_burst);
}
