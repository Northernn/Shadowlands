...
struct boss_jes_howlis : public BossAI
{
    ...
    {
        events.ScheduleEvent(EVENT_CRIP_SHIV, 7s);
        events.ScheduleEvent(EVENT_HOWLING_FEAR, 8s);
        events.ScheduleEvent(EVENT_FLASHING_DAGGER, 12s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_CRIP_SHIV:
                ...
                events.ScheduleEvent(EVENT_CRIP_SHIV, 16s);
                break;
            case EVENT_HOWLING_FEAR:
                DoCastVictim(SPELL_HOWLING_FEAR);
                events.ScheduleEvent(EVENT_HOWLING_FEAR, 13s);
                break;
            case EVENT_FLASHING_DAGGER:
                DoCastVictim(SPELL_FLASHING_DAGGER);
                events.ScheduleEvent(EVENT_FLASHING_DAGGER, 31s);
                break;
            default:
                break;
            }
