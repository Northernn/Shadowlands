...
struct boss_sand_queen : public BossAI
{
    ...
    {
        events.ScheduleEvent(EVENT_SAND_TRAP, 8s);
        events.ScheduleEvent(EVENT_UPHEAVAL, 20s);
        events.ScheduleEvent(EVENT_SANDSTORM, 30s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SAND_TRAP:
                ...
                events.ScheduleEvent(EVENT_SAND_TRAP, 14s);
                break;
            case EVENT_UPHEAVAL:
                me->CastSpell(me, SPELL_UPHEAVAL_CAST);
                events.ScheduleEvent(EVENT_UPHEAVAL, 43s);
                events.ScheduleEvent(EVENT_UPHEAVAL_TARGET_SELECTION, 2s);
                break;
            case EVENT_SANDSTORM:
                me->CastSpell(me, SPELL_SANDSTORM);
                events.ScheduleEvent(EVENT_SANDSTORM, 34s);
                break;
...
class spell_sand_trap : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        caster->SummonCreature(NPC_BUZZING_DRONE, target->GetPosition(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5s);
    }
...
class spell_upheaval_target_selector : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        caster->RemoveUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
    }
...
class spell_upheaval : public AuraScript
{
    ...
    void HandleEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (Creature* sandqueen = caster->ToCreature())
            sandqueen->RemoveUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
    }
