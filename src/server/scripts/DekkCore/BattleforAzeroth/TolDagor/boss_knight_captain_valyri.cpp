...
struct boss_knight_captain_valyri : public BossAI
{
    ...
    {
        events.ScheduleEvent(EVENTS_IGNITION, 6s);
        events.ScheduleEvent(EVENTS_FUSELIGHTER, 14s);
        events.ScheduleEvent(EVENTS_CINDER_FLAME, 18s);
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
    {
        ...
        {
            if (target->ToUnit()->HasAura(SPELL_CARRYING_MUNITIONS))
            {
                target->CastSpell(nullptr,  256578);
                target->ToUnit()->RemoveAura(SPELL_CARRYING_MUNITIONS);

                if (Unit* barrel = me->SummonCreature(NPC_MUNITIONS_BARREL, target->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 1min))
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENTS_CINDER_FLAME:
                ...
                events.ScheduleEvent(EVENTS_CINDER_FLAME, 20s);
                break;
            ...
            case EVENTS_IGNITION:
                ...
                events.ScheduleEvent(EVENTS_IGNITION, 32s);
                break;
            default:
                break;
            }
    ...
    void SpellHit(WorldObject* caster, SpellInfo const* spell) override
    {
        if (spell->Id == 256963 || spell->Id == 256978 || spell->Id == 256931 || spell->Id == 256931) //256931?
        {
            if (hit == false)
            {
                ...
                events.ScheduleEvent(EVENT_DESPAWN, 6s);
                hit = true;
            }
        }
        if (spell->Id == SPELL_PICKING_UP)
        {
            if (!me->HasUnitState(UNIT_STATE_CASTING))
            {
                if (!caster->ToUnit()->HasAura(SPELL_CARRYING_MUNITIONS))
                {
                    if (me->IsAlive())
                    {
                        me->ForcedDespawn();
...
void AddSC_boss_knight_captain_valyri()
{
    ...
    RegisterSpellScript(spell_fuselighter);
}
