...
//				int32 damageToRune = damage / 0.75f;
		...
		void SelectRelic()
		{
			...
			events.ScheduleEvent(EVENT_SUMMON_VISAGE, 3s);
		}
...
//					me->DealDamage(rune, damageToRune);
		...
		void SelectRelic()
		{
			...
			events.ScheduleEvent(EVENT_SUMMON_ELDRITCH, 3s);
		}
...
public:
	...
	struct bfa_npc_visage_from_beyond_AI : public ScriptedAI
	{
		...
		void JustEngagedWith(Unit*) override
		{
			...
			if(me->GetMap()->IsHeroic() || me->GetMap()->IsMythic())
				events.ScheduleEvent(EVENT_KILL, 90s);
			else
				events.ScheduleEvent(EVENT_KILL_2, 60s);
			events.ScheduleEvent(EVENT_TERRIFYING_ECHO, 3s);
		}
...
public:
	...
	struct bfa_at_void_essence_AI : AreaTriggerAI
	{
		...
		{
			unit->ApplyMovementForce(at->GetGUID(), at->GetPosition(), -5.0f, MovementForceType::SingleDirectional);
		}
