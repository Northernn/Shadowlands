			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_ENERGY_REGEN:
				{
					...
					if (me->GetPower(POWER_ENERGY) == 100)
					{
						me->SetPower(POWER_ENERGY, 0);
						if (phase1)
							events.ScheduleEvent(EVENT_GIFT_OF_NZOTH_OBSCURITY, 1s);
						else if (phase2)
							events.ScheduleEvent(EVENT_GIFT_OF_NZOTH_HYSTERIA, 1s);
						else if (phase3)
							events.ScheduleEvent(EVENT_GIFT_OF_NZOTH_LUNACY, 1s);
					}
				//	events.ScheduleEvent(EVENT_ENERGY_REGEN, TIMER_ENERGY_REGEN);
					break;
				}
				...
				case EVENT_UNKNOWABLE_TERROR:
					me->CastSpell(me, SPELL_UNKNOWABLE_TERROR_CAST);
					//events.ScheduleEvent(EVENT_UNKNOWABLE_TERROR, TIMER_UNKNOWABLE_TERROR);
					break;
				}
			}
...
public:
    ...
    struct bfa_npc_undying_guardian_AI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit*)
        {
            ...
            events.ScheduleEvent(EVENT_FIXATE_TANK, 2s);
            events.ScheduleEvent(EVENT_SIGHTLESS_BOND, 3s);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FIXATE_TANK:
                {
                    ...
                    events.ScheduleEvent(EVENT_FIXATE_TANK, 10s); // just to make sure its still on tank
                    break;
                }
                case EVENT_SIGHTLESS_BOND:
                {
                    ...
                    events.ScheduleEvent(EVENT_SIGHTLESS_BOND, 2s);
                    break;
                }
                case EVENT_ENERGY_REGEN:
                    me->SetPower(POWER_ENERGY, me->GetPower(POWER_ENERGY) + urand(1, 5));
                    ...
                    events.ScheduleEvent(EVENT_ENERGY_REGEN, 3s);
                    break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
	...
	struct bfa_npc_primordial_mindbender_AI : public ScriptedAI
	{
		...
		void JustEngagedWith(Unit*)
		{
			...
			events.ScheduleEvent(EVENT_CONSUME_ESSENCE, 1s);
		}
	...
	struct bfa_npc_void_stone_relic_AI : public ScriptedAI
	{
		...
		void DoAction(int32 action)
		{
			...
			case ACTION_REGEN_RELIC:
				events.ScheduleEvent(EVENT_RELIC_REGEN, 60s);
				break;
			}
		}
...
public:
	...
	struct bfa_npc_trident_of_deep_ocean_relic_AI : public ScriptedAI
	{
		...
		void DoAction(int32 action)
		{
			...
			{
			case ACTION_REGEN_RELIC:
				events.ScheduleEvent(EVENT_RELIC_REGEN, 120s);
				break;
			}
		}
...
public:
	...
	struct bfa_npc_ocean_rune_trigger_AI : public ScriptedAI
	{
		...
		{
			events.Reset();
			events.ScheduleEvent(EVENT_ABYSSAL_COLLAPSE, 20s);
			events.ScheduleEvent(EVENT_ADD_FRAME, 1s);
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_ABYSSAL_COLLAPSE:
					...
					events.ScheduleEvent(EVENT_REMOVE_FRAME, 5s);
					me->DespawnOrUnsummon(1s);
					break;
				}
			}
