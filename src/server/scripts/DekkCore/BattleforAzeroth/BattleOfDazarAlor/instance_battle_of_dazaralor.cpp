...
class instance_battle_of_dazaralor : public InstanceMapScript
{

public:
    instance_battle_of_dazaralor() : InstanceMapScript("instance_battle_of_dazaralor", 2070) {}

    InstanceScript* GetInstanceScript(InstanceMap* map) const override
    {
        return new instance_battle_of_dazaralor_InstanceMapScript(map);
    }

    struct instance_battle_of_dazaralor_InstanceMapScript : public InstanceScript
    {
        instance_battle_of_dazaralor_InstanceMapScript(InstanceMap* map) : InstanceScript(map)
        {
        ...
        void OnPlayerEnter(Player* player)
        {
            ...
            else
            {
                if (player->GetTeam() == HORDE)
                {
                    ...
                    player->GetSceneMgr().RecreateScene(SCENE_CORPSES, SceneFlag::NotCancelable/* | SCENEFLAG_UNK16 */);
                    player->GetSceneMgr().RecreateScene(SCENE_BATTLEFIELD_ACTION, SceneFlag::NotCancelable/* | SCENEFLAG_UNK16 */);
                ...
                if (player->GetTeam() == ALLIANCE)
                {
                    ...
                    player->GetSceneMgr().RecreateScene(SCENE_ALLIANCE_LANDING, SceneFlag::NotCancelable/* | SCENEFLAG_UNK16 */);
                    player->GetSceneMgr().RecreateScene(SCENE_ALLIANCE_LANDING_2, SceneFlag::NotCancelable/* | SCENEFLAG_UNK16 */);
                    player->GetSceneMgr().RecreateScene(SCENE_CORPSES, SceneFlag::NotCancelable/* | SCENEFLAG_UNK16 */);
                    player->GetSceneMgr().RecreateScene(SCENE_BATTLEFIELD_ACTION, SceneFlag::NotCancelable/* | SCENEFLAG_UNK16 */);
...
{
    new instance_battle_of_dazaralor();
}
