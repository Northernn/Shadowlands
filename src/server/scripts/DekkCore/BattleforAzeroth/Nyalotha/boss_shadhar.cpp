	...
	void ExecuteEvent(uint32 eventId) override
	{
		...
		if (me->IsInWater())
			if (!me->HasAura(SPELL_CAUSTIC_COATING_BUFF))
				me->AddAura(SPELL_CAUSTIC_COATING_BUFF, me);
		...
		if (IsMythic())
		{
			if (Aura* hungry = me->GetAura(SPELL_HUNGRY_STACK))
			{
				if (hungry->GetStackAmount() == 10)
				{
					...
					me->AddAura(SPELL_UNCONTROLLABLY_RAVENOUS, me);
				}
			}
		}
		...
		case EVENT_HUNGRY:
		{
			me->AddAura(SPELL_HUNGRY_STACK, me);
...
void AddSC_shadhar()
{
	...
	RegisterSpellScript(aura_debilatiting_spit);
