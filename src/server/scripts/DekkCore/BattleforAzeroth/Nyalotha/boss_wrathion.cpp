...
struct boss_wrathion : public BossAI
{
    ...
    {
        me->AddAura(SPELL_WRATHION_INITIAL_TRANSFORM, me);
   ...
   //     me->RemoveUnitFlag2(UnitFlags2(UNIT_FLAG2_SELECTION_DISABLED));
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
           ...
           //     me->SetUnitFlag2(UnitFlags2(UNIT_FLAG2_SELECTION_DISABLED));
            ...
            case SPELL_SCALES_OF_WRATHION_MISSILE:
                ...
                if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_SCALES_OF_WRATHION_MISSILE, DIFFICULTY_NONE))
                {
                    if (spellInfo->Difficulty)
                    {
    ...
    void JustDied(Unit* /*attacker*/) override
    {
     ...
     //   instance->DoModifyPlayerCurrencies(CURRENCY_ECHOES_OF_NYALOTHA, 12);
        ...
        instance->DoNearTeleportPlayers(annex_of_prophecy_platform_pos, true);
    }
...
class spell_wrathion_burning_cataclysm_teleport : public SpellScript
{
    void HandleHitTarget(SpellEffIndex /*effIndex*/)
    {
        ...
        caster->StopMoving();
        caster->SetFacingTo(caster->GetAbsoluteAngle(wrathion_room_center));
...
void AddSC_wrathion()
{
    ...
    RegisterSpellScript(aura_wrathion_incineration);
    RegisterSpellScript(aura_wrathion_burning_cataclysm);
    RegisterSpellScript(spell_wrathion_burning_cataclysm_teleport);
    RegisterSpellScript(aura_wrathion_smoke_and_mirrors);
    RegisterSpellScript(spell_wrathion_creeping_madness_targets);
    RegisterSpellScript(aura_wrathion_creeping_madness);
