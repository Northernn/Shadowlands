...
//156523
struct boss_maut : public CreatureAI
{
	boss_maut(Creature* creature) : CreatureAI(creature, DATA_MAUT) { }
...
private:
	...
	//	if (IsHeroic() || IsMythic()) WHYYY NOT WORK=?
		{
...
void AddSC_maut()
{
	...
	RegisterSpellScript(aura_consuming_shadows);
	RegisterSpellScript(aura_devour_magic);
	RegisterSpellScript(aura_drain_essence);
	RegisterSpellScript(aura_ancient_curse);
	RegisterCreatureAI(npc_mana_orb);
	RegisterSpellScript(aura_forbidden_mana);
