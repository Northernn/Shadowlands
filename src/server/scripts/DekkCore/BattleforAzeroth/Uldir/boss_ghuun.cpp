...
private:
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        {
            bool taked = false;
            for (auto players : me->SelectNearestPlayers(200, true))
            {
            ...
            for (auto idx : me->FindNearestCreatures(NPC_POWER_MATRIX, 500.0f))
            {
                ...
                idx->SetNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
            }
            events.Repeat(25s);
            break;
        }
...
struct npc_dark_young : public ScriptedAI
{
    ...
    {
        if (summonerWO->ToUnit()->IsInCombat())
            me->AI()->DoZoneInCombat(nullptr);       
    }
...
struct npc_matrix_surge : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        ...
        {            
            if (Unit* matrixHolder = SelectTarget(SelectTargetMethod::Random, 0, 25.0f, true))
            {
