...
struct boss_mother : public BossAI
{
    ...
    void OnSpellCast(SpellInfo const* spell) override
    {
        ...
        if (spell->Id == SPELL_SANITIZING_STRIKE)
        {
            if (Unit* tar = SelectTarget(SelectTargetMethod::MaxDistance, 0, 100.0f, true))
            {
...
struct npc_corners_purifying_flame : public ScriptedAI
{
    ...
    void OnSpellCast(SpellInfo const* spell) override
    {
        if (spell->Id == SPELL_PURIFYING_FLAME_DAMAGE)
        {
            me->DespawnOrUnsummon();
        }
    }
...
struct at_wind_tunnel_l_to_r : public AreaTriggerAI
{
    ...
    void OnPeriodicProc() override
    {
        ...
        {
            if (player->HasAura(SPELL_FIRST_ROOM_OCCUPANT))
                player->ApplyMovementForce(at->GetGUID(), wind_tunnel_push_positions[1], -5.f, MovementForceType::SingleDirectional);

            else if (player->HasAura(SPELL_SECOND_ROOM_OCCUPANT))
                player->ApplyMovementForce(at->GetGUID(), wind_tunnel_push_positions[3], -5.f, MovementForceType::SingleDirectional);

            else if (player->HasAura(SPELL_THIRD_ROOM_OCCUPANT))
                player->ApplyMovementForce(at->GetGUID(), wind_tunnel_push_positions[5], -5.f, MovementForceType::SingleDirectional);
        }
    }
...
struct at_wind_tunnel_r_to_l : public AreaTriggerAI
{
    ...
    void OnPeriodicProc() override
    {
        ...
        {
            if (player->HasAura(SPELL_FIRST_ROOM_OCCUPANT))
                player->ApplyMovementForce(at->GetGUID(), wind_tunnel_push_positions[0], -5.f, MovementForceType::SingleDirectional);

            else if (player->HasAura(SPELL_SECOND_ROOM_OCCUPANT))
                player->ApplyMovementForce(at->GetGUID(), wind_tunnel_push_positions[2], -5.f, MovementForceType::SingleDirectional);

            else if (player->HasAura(SPELL_THIRD_ROOM_OCCUPANT))
                player->ApplyMovementForce(at->GetGUID(), wind_tunnel_push_positions[4], -5.f, MovementForceType::SingleDirectional);
        }
    }
