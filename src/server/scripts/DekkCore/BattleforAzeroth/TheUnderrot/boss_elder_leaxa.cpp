...
public:
    ...
    struct bfa_boss_elder_leaxa_AI : public BossAI
    {
        ...
        void HandleBloodMirrorSpawns()
        {
            ...
            me->CastSpell(Position(x, y, me->GetPositionZ()), SPELL_BLOOD_MIRROR_MISSILE, true);
       ...
       //     SelectTargetList(targets, 5, SELECT_TARGET_RANDOM, 500.0f, true);
            ...
            for (auto target : targets)
            {
                ...
                if (Creature* trigger = me->SummonCreature(NPC_CREEPING_ROG_TRIGGER, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), TEMPSUMMON_MANUAL_DESPAWN))
                {
                    ...
                    trigger->GetNearPoint(NULL, pos.m_positionX, pos.m_positionY, pos.m_positionZ, DEFAULT_PLAYER_BOUNDING_RADIUS, orientation);
        ...
        void OnTick(AuraEffect const* /*aurEff*/)
        {
            if (Unit* caster = GetTarget())
            {
                if (AuraEffect* aurEff = GetAura()->GetEffect(EFFECT_1))
                    if (AuraEffect* aurEff = GetAura()->GetEffect(EFFECT_1))
                        aurEff->SetAmount(caster->SpellDamageBonusDone(GetTarget(), GetSpellInfo(), 0, DOT, aurEff->GetSpellEffectInfo(), GetStackAmount()) * aurEff->GetAmount());
            }
        }
