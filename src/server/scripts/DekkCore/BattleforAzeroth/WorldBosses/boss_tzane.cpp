...
struct boss_tzane : public BossAI
{
    ...
    {
        events.ScheduleEvent(EVENT_CRUSHING_SLAM, 22s);
        events.ScheduleEvent(EVENT_TERROR_WALL, 11s);
        events.ScheduleEvent(EVENT_COALSECED_ESSENCE, 8s);
        events.ScheduleEvent(EVENT_CONSUMING_SPIRITS, 19s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_CRUSHING_SLAM:
                ...
                events.ScheduleEvent(EVENT_CRUSHING_SLAM, 23s);
                break;
            case EVENT_TERROR_WALL:
                DoCastVictim(SPELL_TERROR_WALL);
                events.ScheduleEvent(EVENT_TERROR_WALL, 23s);
                break;
            case EVENT_COALSECED_ESSENCE:
                DoCast(SPELL_COALESCED_ESSENCE);
                events.ScheduleEvent(EVENT_COALSECED_ESSENCE, 24s);
                break;
            case EVENT_CONSUMING_SPIRITS:
                ...
                events.ScheduleEvent(EVENT_CONSUMING_SPIRITS, 21s);
                break;
            default:
                break;
            }
