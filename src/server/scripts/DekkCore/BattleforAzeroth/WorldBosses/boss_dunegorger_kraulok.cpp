...
struct boss_dunegorger_kraulok : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        //EVENTS
        events.ScheduleEvent(EVENT_SHAKE_LOSE, 25s);
        events.ScheduleEvent(EVENT_PRIMAL_RAGE, 39s);
        events.ScheduleEvent(EVENT_SONIC_BELLOW, 9s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SHAKE_LOSE:
            {
                ...
                {
                    events.ScheduleEvent(EVENT_SHAKE_LOSE, 15s);
                }
                else
                {
                    events.ScheduleEvent(EVENT_SHAKE_LOSE, 28s);
                }
                break;
            }
            case EVENT_PRIMAL_RAGE:
            {
                ...
                events.ScheduleEvent(EVENT_PRIMAL_RAGE, 33s);
                break;
            }
            case EVENT_SONIC_BELLOW:
            {
                ...
                if (me->HasAura(SPELL_PRIMAL_RAGE))
                    events.ScheduleEvent(EVENT_SONIC_BELLOW, 20s);
                else
                {
                    events.ScheduleEvent(EVENT_SONIC_BELLOW, 30s);
                }
                break;
            }
            default:
