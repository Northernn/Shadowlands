...
struct boss_jiarak : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        //EVENTS
        events.ScheduleEvent(EVENT_STORM_WING, 11s);
        events.ScheduleEvent(EVENT_MATRIARCHS_CALL, 30s);
        events.ScheduleEvent(EVENT_HURRICANE_CRASH, 45s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_STORM_WING:
            {
                ...
                if (urand(0, 1) == 0)
                {
                    events.ScheduleEvent(EVENT_STORM_WING, 15s);
                }
                else
                {
                    events.ScheduleEvent(EVENT_STORM_WING, 45s);
                }
                break;
            }
            case EVENT_MATRIARCHS_CALL:
            {
                ...
                if (urand(0, 1) == 0)
                {
                    events.ScheduleEvent(EVENT_MATRIARCHS_CALL, 12s);
                }
                else
                {
                    events.ScheduleEvent(EVENT_MATRIARCHS_CALL, 45s);
                }
                break;
            }
