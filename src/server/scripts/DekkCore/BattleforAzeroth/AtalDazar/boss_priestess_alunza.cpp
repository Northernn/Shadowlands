...
struct boss_priestess_alunza : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.ScheduleEvent(EVENT_GILDED_CLAWS, 10s);
        events.ScheduleEvent(EVENT_MOLTEN_GOLD, 16s);
        events.ScheduleEvent(EVENT_TAINTED_BLOOD, 18s);

        if (IsMythic() || IsHeroic())
        {
            events.ScheduleEvent(EVENT_SPAWN_CORRUPTED_GOLD, 2s);
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_TAINTED_BLOOD:
                    ...
                    events.ScheduleEvent(EVENT_TAINTED_BLOOD_CAST, 1s);
                    events.ScheduleEvent(EVENT_TAINTED_BLOOD, 44s);
                    break;
                case EVENT_TAINTED_BLOOD_CAST:
                    DoCastSelf(SPELL_TAINTED_BLOOD_TARGET_CAULDRON);
                    ++taintedCounter;
                    if (taintedCounter < 5)
                        events.ScheduleEvent(EVENT_TAINTED_BLOOD_CAST, 2s);
                    else
                    {
                        if (IsHeroic() || IsMythic())
                            events.ScheduleEvent(EVENT_SPIRIT_OF_GOLD, 8s);
                        events.ScheduleEvent(EVENT_TRANSFUSION, 13s);
                    }
                    break;
                case EVENT_SPAWN_CORRUPTED_GOLD:
                {
                    scheduler.Schedule(0s, [this](TaskContext context)
                    {
                        ...
                        {
                            cauldron = DoSummon(NPC_CORRUPTED_GOLD, cauldron->GetPosition(), 40s, TEMPSUMMON_TIMED_DESPAWN);
                        }
...
struct npc_blood_tainted_cauldron : public ScriptedAI
{
    ...
    void SpellHitTarget(WorldObject*  /*caster*/, SpellInfo const* spell) override
    {
...
struct npc_corrupted_gold : public ScriptedAI
{
    ...
    void MoveForward(float distance)
    {
        ...
        me->GetNearPoint2D(nullptr, x, y, distance, ori);
        movePos = { x, y, z, ori };
    ...
    void DoEffectHitTarget(SpellEffIndex /*effIndex*/)
    {
        if (Unit* hitUnit = GetHitUnit())
            if (UnitAI* ai = GetCaster()->GetAI()) //@HACK TODO
                if (Unit* target = ai->SelectTarget(SelectTargetMethod::Random, 0))
    ...
    void DoEffectHitTarget(SpellEffIndex /*effIndex*/)
    {
        if (Unit* hitUnit = GetHitUnit())
            if (InstanceScript* atalDazarScript = GetCaster()->GetInstanceScript())
                GetCaster()->CastSpell(hitUnit, SPELL_MOLTEN_GOLD_MISSILE, CastSpellExtraArgs().SetOriginalCaster(atalDazarScript->GetGuidData(DATA_PRIESTESS_ALUNZA)));
    }
...
void AddSC_boss_priestess_alunza()
{
    ...
    RegisterSpellScript(spell_npc_cauldron_purify);
    RegisterSpellScript(spell_priestess_energy_regen);
    RegisterSpellScript(spell_priestess_transfusion);
