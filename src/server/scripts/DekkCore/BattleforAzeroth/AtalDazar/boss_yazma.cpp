...
struct boss_ataldazar_yazma : public BossAI
{
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
    {
        ...
        if (spell->Id == SPELL_SOULREND)
            me->SummonCreature(NPC_SOULREND, target->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN, 0s);
    }
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        Talk(TALK_AGGRO);
        // Events
        events.ScheduleEvent(EVENT_WRACKING_PAIN, 3s);
        events.ScheduleEvent(EVENT_SKEWER, 5s);
        events.ScheduleEvent(EVENT_SOULREND, 10s);
        events.ScheduleEvent(EVENT_ECHOES_OF_SHADOW, 16s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SOULREND:
            {
                ...
                events.ScheduleEvent(EVENT_SOULREND, 41s);
                break;
            }
            case EVENT_WRACKING_PAIN:
            {
                ...
                events.ScheduleEvent(EVENT_WRACKING_PAIN, 23s);
                break;
            }
            case EVENT_SKEWER:
            {
                me->CastSpell(me->GetVictim(), SPELL_SKEWER);
                events.ScheduleEvent(EVENT_SKEWER, 32s);
                break;
            }
            case EVENT_ECHOES_OF_SHADOW:
            {
                ...
                events.ScheduleEvent(EVENT_ECHOES_OF_SHADOW, 12s);
                break;
            }
            default:
                break;
...
struct npc_ataldazar_soulspawn : public ScriptedAI
{
    npc_ataldazar_soulspawn(Creature* pCreature) : ScriptedAI(pCreature) {
        ...
        me->SetDisplayId(me->SelectNearestPlayer(10)->GetDisplayId());
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SHADOW_EMPOWERED:
            {
                ...
                events.ScheduleEvent(EVENT_SHADOW_EMPOWERED_STOP, 3s);
                break;
            }
            case EVENT_SHADOW_EMPOWERED_STOP:
            {
                me->RemoveAura(SPELL_ECHOES_OF_SHADOW_EMPOWERED);
                events.ScheduleEvent(EVENT_SHADOW_EMPOWERED, 6s);
                me->GetMotionMaster()->Clear();
                me->GetMotionMaster()->MoveRandom(urand(5, 15));
                closestplayer = NULL;
            }
