...
struct boss_ataldazar_rezan : public BossAI
{
    ...
    {
        // Events
        events.ScheduleEvent(EVENT_TAIL, 20s); //timed event like onyxia
        events.ScheduleEvent(EVENT_TERRIFYING_VISAGE, 12s);
        events.ScheduleEvent(EVENT_SERRATHED_TEETH, 6s);;
        events.ScheduleEvent(EVENT_PURSUIT, 21s);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_TAIL:
            {
                ...
                events.ScheduleEvent(EVENT_TAIL, 20s);
                break;
            }
            case EVENT_TERRIFYING_VISAGE:
            {
                me->CastSpell(me, SPELL_TERRIFYING_VISAGE);
                events.ScheduleEvent(EVENT_TERRIFYING_VISAGE, 40s);
                break;
            }
            case EVENT_SERRATHED_TEETH:
            {
                DoCastVictim(SPELL_SERRATHED_TEETH);
                events.ScheduleEvent(EVENT_SERRATHED_TEETH, 38s);;
                break;
            }
            case EVENT_PURSUIT:
            {
                ...
                events.ScheduleEvent(EVENT_PURSUIT, 44s);
                events.ScheduleEvent(EVENT_PURSUIT_TARGET, 1s);
                events.ScheduleEvent(EVENT_PURSUIT_STOP, 18s);
                break;
            }
            case EVENT_PURSUIT_TARGET:
            {
                if(storedtarget)
                {
                    ...
                    else
                    {
                        ...
                        events.ScheduleEvent(EVENT_PURSUIT_STOP, 0s);
                        events.ScheduleEvent(EVENT_DISMOUNT, 8s);
                    }
                }
                events.ScheduleEvent(EVENT_PURSUIT_TARGET, 5s);
                break;
            }
