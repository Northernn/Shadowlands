...
struct boss_dread_captain_lockwood : public BossAI
{
	...
	void ExecuteEvent(uint32 eventid) override
	{
		...
		{
		case EVENT_SHOOT:
			if (Unit* tar = SelectTarget(SelectTargetMethod::Random, 0, 30.0f, true))
			{
		...
		case EVENT_GUT_SHOT:
		{
			Talk(SAY_GUT);
			UnitList tarlist;
			/*SelectTargetList(tarlist, 3, SelectTargetMethod::Random, 45.0f, true);*/
		...
		case EVENT_CLEAR_THE_DECK:
			if (Unit* tar = SelectTarget(SelectTargetMethod::MaxThreat, 0, 30.0f, true))
			{
