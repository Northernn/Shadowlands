...
struct boss_council_captain : public BossAI
{
        ...
        if (Creature* jolly = m_Instance->instance->GetCreature(m_Instance->GetGuidData(FreeholdCreature::NpcCaptainJolly)))
        {
            if (Creature* raoul = m_Instance->instance->GetCreature(m_Instance->GetGuidData(FreeholdCreature::NpcCaptainRaoul)))
            {
                if (Creature* eudora = m_Instance->instance->GetCreature(m_Instance->GetGuidData(FreeholdCreature::NpcCaptainEudora)))
                {
                    ...
                    {
                        jolly->Respawn();
                        jolly->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
                        jolly->GetMotionMaster()->MoveTargetedHome();
                    }
                    if (raoul->isDead())
                    {
                        raoul->Respawn();
                        raoul->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
                        raoul->GetMotionMaster()->MoveTargetedHome();
                    }
                    if (eudora->isDead())
                    {
                        eudora->Respawn();
                        eudora->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);
    ...
    void JustEngagedWith(Unit*) override
    {
        ...
        case FreeholdCreature::NpcCaptainJolly:
        {
            ...
            {
                events.ScheduleEvent(CouncilCaptainEvents::EventCuttingSurge, 5s);
                events.ScheduleEvent(CouncilCaptainEvents::EventWhirlpoolofBlades, 8s);
            }
            else
                events.ScheduleEvent(CouncilCaptainEvents::EventAlliedBuff, 8s);
            break;
        }
        case FreeholdCreature::NpcCaptainEudora:
        {
            ...
            if (me->GetFaction() == FreeHoldFaction::FactionEnemy)
            {
                events.ScheduleEvent(CouncilCaptainEvents::EventPowderShot, 5s);
                events.ScheduleEvent(CouncilCaptainEvents::EventGrapeshotJump, 8s);
            }
            else
                events.ScheduleEvent(CouncilCaptainEvents::EventAlliedBuff, 8s);
            break;
        ...
        case FreeholdCreature::NpcCaptainRaoul:
        {
            ...
            if (me->GetFaction() == FreeHoldFaction::FactionEnemy)
            {
                events.ScheduleEvent(CouncilCaptainEvents::EventBlackoutBarrel, 5s);
                events.ScheduleEvent(CouncilCaptainEvents::EventBarrelSmash, 8s);
            }
            else
                events.ScheduleEvent(CouncilCaptainEvents::EventAlliedBuff, 8s);
            break;
        ...
        events.ScheduleEvent(CouncilCaptainEvents::EventCheckPlayers, 1s);
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        {
            events.ScheduleEvent(CouncilCaptainEvents::EventGrapeshotFire, 5s);
            break;
        }
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
      ...
      //  UpdateOperations(diff);
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
                if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0.0, 0.0, true))
            ...
            case CouncilCaptainEvents::EventGrapeshotFire:
            {
                ...
                else
                {
                    ...
                    grapershotcount = 0;
                    events.ScheduleEvent(CouncilCaptainEvents::EventPowderShot, 5s);
                    events.ScheduleEvent(CouncilCaptainEvents::EventGrapeshotJump, 15s);
                }
                break;
            }
...
struct npc_rummy_mancomb : public ScriptedAI
{
    ...
    void DoAction(int32 const action) override
    {
        ...
        {
            events.ScheduleEvent(CouncilCaptainEvents::EventLaunchBrew, 8s);
            break;
        }
