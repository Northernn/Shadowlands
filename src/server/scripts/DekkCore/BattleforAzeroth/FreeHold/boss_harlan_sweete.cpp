...
struct boss_harlan_sweete : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.ScheduleEvent(HarlanSweeteEvents::EventCannonBarrage, 8s);
        events.ScheduleEvent(HarlanSweeteEvents::EventSwiftwindSaber, 12s);
        events.ScheduleEvent(HarlanSweeteEvents::EventSummonIrontideGrenadier, 15s);
    }
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            for (uint8 i = 0; i < count; i++)
                caster->SummonCreature(FreeholdCreature::NpcSwiftwindSaber, caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ(), caster->GetOrientation() - (float(M_PI) / 6.0f) * (i), TEMPSUMMON_TIMED_DESPAWN, 5s);
        }
    }
