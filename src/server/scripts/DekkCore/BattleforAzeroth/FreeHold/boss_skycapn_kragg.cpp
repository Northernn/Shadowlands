...
struct boss_skycap_kragg : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        events.ScheduleEvent(KraggEvents::EventPistolShot, 5s);
        events.ScheduleEvent(KraggEvents::EventChaaarrge, 8s);
    }
    ...
    void OnSpellCastInterrupt(SpellInfo const* spell)
    {
        if (spell->Id == KraggSpells::RevitalizingBrewSkyCap)
        {
            ...
            me->SummonCreature(FreeholdCreature::NpcRevitalizingBrew, GetRandomPositionAround(me, 3.0f, 5.0f), TEMPSUMMON_TIMED_DESPAWN, 15s);
        }
    }
     ...
     //   if (me->HealthWillBeBelowPctDamaged(75, damage) && phase == PhaseMount)
        {
            ...
            events.ScheduleEvent(KraggEvents::EventAzeritePowderShot, 2s);
            events.ScheduleEvent(KraggEvents::EventRevitalizingBrew, 15s);
        }
    }
...
struct npc_sharkbait : public ScriptedAI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        case KraggMovementPoint::MovementPointMiddle:
        {
            ...
            events.Reset();
            events.ScheduleEvent(KraggEvents::EventVileBombadment, 5s);
            if (IsHeroic())
                events.ScheduleEvent(KraggEvents::EventDiveBombs, 17s);
            break;
        }
...
class spell_dive_bomb : public SpellScript
{
    ...
    void HandleOnCast()
    {
        if (Unit* caster = GetCaster())
        {
            ...
            {
                if (Unit* target = caster->ToCreature()->AI()->SelectTarget(SelectTargetMethod::Random, 0.0, 0.0, true))
                {
                    Position ChargePosition;
                    //GetPositionWithDistInOrientation(target, 10.0f, caster->GetAbsoluteAngle(target), ChargePosition);
