...
struct npc_gukguk : public ScriptedAI
{
    ...
    void DoAction(int32 const action) override
    {
        ...
        {
            m_Instance->SetBossState(FreeholdData::DataRingOfBooty, DONE);
            break;
        }
        }
    }
...
struct npc_davey : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        events.Reset();
        me->SetWalk(true);
        events.ScheduleEvent(RingOfBootyEvents::EventsCheckPlayers, 1s);
    }
...
struct npc_gurgthock : public ScriptedAI
{
    ...
    void Reset() override
    {
        events.Reset();
        resetFight = true;
        if (m_Instance->GetBossState(FreeholdData::DataRingOfBooty) != DONE)
        {
            ...
            AddTimedDelayedOperation(15000, [this]() -> void
                {
                    ...
                    events.ScheduleEvent(RingOfBootyEvents::EventsCheckReset, 1s);
...
struct npc_lightning : public ScriptedAI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        case RingOfBootyPoint::MovementPointLightningCenter:
        {
            ...
            events.ScheduleEvent(RingOfBootyEvents::EventsMoveNextPosition, 5s);
            break;
        }
...
struct npc_ludwig_von_tortollan : public ScriptedAI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        case RingOfBootyPoint::MovementPointLightningCenter:
        {
            ...
            events.ScheduleEvent(RingOfBootyEvents::EventsShellBounce, 5s);
            break;
        }
        }
    }
...
struct boss_trothak : public BossAI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        case RingOfBootyPoint::MovementPointFight:
        {
            ...
            events.ScheduleEvent(RingOfBootyEvents::EventsRipperPunch, 3s);
            events.ScheduleEvent(RingOfBootyEvents::EventsSharkTornado, 10s);
            events.ScheduleEvent(RingOfBootyEvents::EventsSharkToss, 8s);
            if (IsHeroic())
                events.ScheduleEvent(RingOfBootyEvents::EventsThrowChum, 8s);
            break;
        }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case RingOfBootyEvents::EventsSharkToss:
            {
                ...
                events.ScheduleEvent(RingOfBootyEvents::EventsRearm, 15s);
                break;
            }
            case RingOfBootyEvents::EventsRearm:
            {
                ...
                events.ScheduleEvent(RingOfBootyEvents::EventsSharkToss, 10s);
                break;
            }
        ...
        case EVENT_CHARGE:///SPELL_EFFECT_CHARGE_DEST use this id to movement  Inform
            if (delayShark)
            {
                ...
                AddTimedDelayedOperation(1500, [this]() -> void
                    {
                        if (Unit* target = SelectTarget(SelectTargetMethod::MinDistance, 0, 0, true))
...
struct at_shell_bounce : AreaTriggerAI
{
    ...
    void OnCreate(Spell const* /*creatingSpell*/) override
    {
        ...
        if (!listPlayer.empty())
        {
            ...
            {
                rotation = at->GetAbsoluteAngle(target);
