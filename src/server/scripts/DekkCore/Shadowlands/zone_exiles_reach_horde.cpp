...
struct warlord_breka_grimaxe_166906 : public ScriptedAI
{
    ...
    void OnQuestAccept(Player* player, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_H_COOKING_MEAT)
            ...
            {
                wansa->AI()->Talk(0);
                //wansa->PlayDirectSound(156959, player, 195978);
    ...
    void SpellHit(WorldObject* caster, SpellInfo const* spell) override
    {
        ...
        if (caster->IsPlayer())
        {
            ...
            if (player->HasQuest(QUEST_H_ENHANCED_COMBAT_TACTICS) && itwilldo == 1)
            {
...
void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/)override
    {
        if ((me->GetHealth() <= damage) || (me->GetHealth() <= me->GetHealth() * 0.2) && attacker->IsPlayer())
        {
            Player* player = attacker->ToPlayer();
            if (player->HasQuest(QUEST_H_ENHANCED_COMBAT_TACTICS))
            {
                if (Quest const* quest = sObjectMgr->GetQuestTemplate(QUEST_H_ENHANCED_COMBAT_TACTICS))
                    for (QuestObjective const& obj : quest->GetObjectives())
                    {
                    ...
                    if (!event59949)
                    {
                        if (Creature* wonsa = me->FindNearestCreature(167226, 30.0f, true))
                            player->GetScheduler().Schedule(Milliseconds(6000), [this, wonsa](TaskContext context)
                                {
                                    Start(true, false);
                                    wonsa->GetMotionMaster()->MoveFollow(me, PET_FOLLOW_DIST, wonsa->GetFollowAngle(), MOTION_SLOT_ACTIVE);
                                    wonsa->AI()->Talk(1);
