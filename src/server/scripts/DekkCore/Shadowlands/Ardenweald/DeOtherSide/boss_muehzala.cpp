...
struct boss_muehzala : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        instance->SetBossState(DATA_MUEHZALA, IN_PROGRESS);
    ...
    {
        CastSpellExtraArgs SPELL_COSMIC_ARTIFICE_args;

        switch (eventId)
        {
        case SPELL_SOULCRASHER:
            if (Unit* target = SelectTarget(SelectTargetMethod::MaxThreat, 0, 30.0f, true))
        ...
        case SPELL_COSMIC_ARTIFICE:
            SPELL_COSMIC_ARTIFICE_args.AddSpellMod(SPELLVALUE_RADIUS_MOD, 100.0f);
            me->CastSpell(nullptr, SPELL_COSMIC_ARTIFICE, SPELL_COSMIC_ARTIFICE_args);
    ...
    {
        _JustDied();
        instance->SetBossState(DATA_MUEHZALA, DONE);
        me->RemoveAllAreaTriggers();
...
struct npc_bwonsamdi_portal : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        me->SetUnitFlag(UnitFlags(UNIT_FLAG_NON_ATTACKABLE | UNIT_NPC_FLAG_SPELLCLICK));
    }
