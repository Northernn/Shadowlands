...
private:
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case SPELL_FEEDING_TIME_MARK:
        {
            ...
            {
                UnitList targetList;
                SelectTargetList(targetList, 3, SelectTargetMethod::Random, 100.0f, true, true, 0);
        ...
        case SPELL_SHATTERING_PAIN_TRIGGER:
            ...
            if (Unit* target = SelectTarget(SelectTargetMethod::MaxThreat, 0, 25.0f))
            {
        ...
        case SPELL_FATAL_FINESSE_PERIODIC_DAMAGE:
            UnitList targetList;
            SelectTargetList(targetList, (urand(3, 4)), SelectTargetMethod::Random, 100.0f, true, true, 0);
            for (Unit* targets : targetList)
            {
   ...
   /* void OnPeriodicProc() override
    {
        ...
        if (Unit* target = ObjectAccessor::GetUnit(*caster, *getPlayers.begin()))
            if (target->IsPlayer() && target != caster)
                at->GetCaster()->Kill(at->GetCaster(), target, true);
    }*/
};
