...
private:
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case SPELL_EVASIVE_LUNGE_TELEPORT:
            if (Unit* target = SelectTarget(SelectTargetMethod::MaxThreat, 0, 100.0f))
            {
        ...
        {
            UnitList targetList;
            SelectTargetList(targetList, 5, SelectTargetMethod::Random, 150.0f, true, true, 0);
...
struct npc_afterimage : public ScriptedAI
{
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        if (summoner->ToUnit()->IsInCombat())
            me->AI()->JustEngagedWith(summoner->ToUnit()->GetVictim());
