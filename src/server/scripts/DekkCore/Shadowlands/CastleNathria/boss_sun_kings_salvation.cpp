    ...
    void DoAction(int32 action) override
    {
        ...
        case ACTION_SPAWN_ADDS:
            me->GetScheduler().Schedule(1s, [this](TaskContext context)
            {
                ...
                if (!me->HasUnitFlag(UnitFlags(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_UNINTERACTIBLE)))
                    context.Repeat(60s);
            });
            break;
    ...
    void JustEngagedWith(Unit* who) override
    {
        if (Creature* kaelthas = me->FindNearestCreature(NPC_SUN_KING_SALVATION, 100.0f, true))
        {
           // instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, kaelthas);
            kaelthas->RemoveUnitFlag(UnitFlags(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_UNINTERACTIBLE));
...
void AddSC_boss_sun_kings_salvation()
{
    ...
    RegisterSpellScript(aura_smoldering_plumage);
}
