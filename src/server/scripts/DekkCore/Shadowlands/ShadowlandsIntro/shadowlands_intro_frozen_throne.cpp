...
public:
    ...
    {
        if (player->GetAreaId() == AREA_THE_FORZEN_THRONE)
        {
    ...
    {
        if (newArea == AREA_THE_FORZEN_THRONE)
        {
                if (!player->GetPhaseShift().HasPhase(10010))
                 PhasingHandler::AddPhase(player, 10010, true);
        }
...
{
    npc_highlord_bolvar_fordragon_169076(Creature* c) : ScriptedAI(c) { Initialize(); }
private:
        ...
        bool OnGossipHello(Player* player) override
        {
            ...
            else if (player->HasQuest(QUEST_CHILLING_SUMMONS_H))
                player->ForceCompleteQuest(QUEST_CHILLING_SUMMONS_H);
                        ...
                        player->GetScheduler().Schedule(37s, [this](TaskContext context)
                            {
                                ...
                                if (Creature* SD1 = me->FindNearestCreature(169095, 30.f, true))
                                    SD1->SetNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
                            });
                    }
                    break;
        ...
        void domination2(Player* player)
        {
            ...
            if (Creature* SD1 = me->FindNearestCreature(169095, 40.0f, true))
                SD1->RemoveNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
            ...
            player->GetScheduler().Schedule(23s, [this](TaskContext context)
                {
                    ...
                    if (Creature* SD2 = me->FindNearestCreature(169098, 30.f, true))
                        SD2->SetNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
                });
        }
        ...
        void domination4(Player* player)
        {
            ...
            if (Creature* SD3 = me->FindNearestCreature(169100, 30.f, true))
                SD3->RemoveNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
            ...
            player->GetScheduler().Schedule(21s, [this](TaskContext context)
                {
                    ...
                    if (Creature* SD4 = me->FindNearestCreature(169101, 30.f, true))
                        SD4->SetNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
                });
        }
        ...
        void domination5(Player* player)
        {
            ...
            if (Creature* SD4 = me->FindNearestCreature(169101, 30.f, true))
                SD4->RemoveNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
            player->GetScheduler().Schedule(12s, [this](TaskContext context)
                {
                    Talk(14);
                });
            player->GetScheduler().Schedule(24s, [this](TaskContext context)
                {
                    ...
                    if (Creature* SD5 = me->FindNearestCreature(169109, 50.f, true))
                        SD5->SetNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
                });
        }
        ...
        void domination6(Player* player)
        {
            ...
            if (Creature* SD5 = me->FindNearestCreature(169109, 50.f, true))
                SD5->RemoveNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
            ...
            player->GetScheduler().Schedule(29s, [this, player](TaskContext context)
                {
                    if (Creature* darion = me->FindNearestCreature(169070, 50.0f))
                        darion->ForcedDespawn(3000);
                    player->RemoveAura(SPELL_SHATTERED_SKY_SKYBOX);
                });
        }
    };
                ...
                if (Player* player = caster->ToPlayer())
                    if (Unit* target = GetHitUnit())
                        if (Creature* creTarget = target->ToCreature())
                        {
                            ...
                            {
                            case 169095:
                                player->CastSpell(player, SPELL_CARRYING_SHARD_01);
                                if (Creature* bolvar = creTarget->FindNearestCreature(169076, 50.0f))
                                    bolvar->AI()->DoAction(1);
                                break;
                            case 169098:
                                player->CastSpell(player, SPELL_CARRYING_SHARD_02);
                                if (Creature* bolvar = creTarget->FindNearestCreature(169076, 50.0f))
                                    bolvar->AI()->DoAction(2);
                                break;
                            case 169100:
                                player->CastSpell(player, SPELL_CARRYING_SHARD_03);
                                if (Creature* bolvar = creTarget->FindNearestCreature(169076, 50.0f))
                                    bolvar->AI()->DoAction(3);
                                break;
                            case 169101:
                                player->CastSpell(player, SPELL_CARRYING_SHARD_04);
                                if (Creature* bolvar = creTarget->FindNearestCreature(169076, 50.0f))
