...
private:
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        {
            UnitList targetList;
        //    SelectTargetList(targetList, 5, SelectTargetMethod::Random, 100.0f, true);
        ...
        case SPELL_HEART_REND:
        {
            ...
            UnitList targetList;
          //  SelectTargetList(targetList, 4, SelectTargetMethod::Random, 100.0f, true);
            for (Unit* targets : targetList)
            {
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spellInfo) override
    {
        ...
        case SPELL_REVERBERATING_ERUPTION_DAMAGE_SUMMON:
            if (target->IsUnit())
                me->AddAura(SPELL_REVERBERATING_VULNERABILITY, target->ToUnit());
            break;
        }
    }
...
void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    ...
    void JustDied(Unit* /*victim*/) override
    {
        ...
        if (Creature* generalGrashaal = me->FindNearestCreature(NPC_GENERAL_GRASHAAL, 150.0f, true))
            generalGrashaal->AddAura(SPELL_SOLDIERS_OATH, generalGrashaal);

        if (Creature* generalKaal = me->FindNearestCreature(NPC_GENERAL_KAAL, 150.0f, true))
            generalKaal->AddAura(SPELL_SOLDIERS_OATH, generalKaal);
    }
};
