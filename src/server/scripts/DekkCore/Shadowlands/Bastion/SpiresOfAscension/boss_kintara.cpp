...
struct boss_kintara : public BossAI
{
    ...
    {
        BossAI::Reset();
        me->AddAura(SPELL_LIFE_LINK, me);
        if (Creature* azules = me->FindNearestCreature(NPC_AZULES, 100.0f, true))
        {
            azules->AddAura(SPELL_LIFE_LINK, azules);
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        if (Creature* azules = me->FindNearestCreature(NPC_AZULES, 100.0f, true))
        {
            ...
            azules->RemoveUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_UNINTERACTIBLE | UNIT_FLAG_NON_ATTACKABLE));
...
struct npc_azules : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        _events.ScheduleEvent(SPELL_INSIDIOUS_VENOM, 5s);
        _events.ScheduleEvent(SPELL_ATTENUATED_BARRAGE, 10s);
    }
