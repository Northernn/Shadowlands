...
private:
    ...
    void OnUpdateZone(Player* player, uint32 newzone, uint32 /*oldzone*/) override
    {
        ...
        if (newzone == 10534 && !player->GetPhaseShift().HasPhase(PHASE_GENERIC))
...
private:
    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        if (player->HasQuest(QUEST_TROUBLE_IN_PARADISE))
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (Player* player = who->ToPlayer())
            ...
            {
                if (player->HasQuest(QUEST_A_SOULBIND_IN_NEED) == QUEST_STATUS_COMPLETE && !save_pelagos)
                {
                    save_pelagos = true;
                    Talk(8);
