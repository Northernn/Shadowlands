...
public:
    ...
    static bool HandleBypassQuestCommand(ChatHandler* handler, char const* args)
    {
        ...
        QueryResult checkQuest = WorldDatabase.PQuery("SELECT * FROM quest_bypass WHERE questId = {} AND bypass = '1'", quest->GetQuestId());
        if (!checkQuest) {
    ...
    static bool HandleReportQuestCommand(ChatHandler* handler, char const* args)
    {
        ...
        QueryResult checkQuest = WorldDatabase.PQuery("SELECT * FROM quest_bypass WHERE questId = {} AND bypass = '1'", questId);
        if (checkQuest) {
        ...
        QueryResult checkQuest2 = WorldDatabase.PQuery("SELECT * FROM quest_report WHERE questId = {}", questId);
        if (checkQuest2) {
        ...
        QueryResult reportQuest = WorldDatabase.PQuery("INSERT INTO quest_report (`questId`, `questName`) VALUES ({}, \"{}\")", questId, quest->GetLogTitle());
        if (!reportQuest) {
