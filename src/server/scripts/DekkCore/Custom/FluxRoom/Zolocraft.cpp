/*
..::ZOLOCRAFT (modified solocraft) by Fluxurion (2022)::..
...
public:
	...
	{
        if (sConfigMgr->GetBoolDefault("ZoloCraft.Enable", true))
        {
...
class spell_zolocraft_aura_167349 : public AuraScript
{
	...
	{
		ChatHandler(player->GetSession()).PSendSysMessage("Welcome to the %s! Power Increase System activated. Difficulty: %u", map->GetMapName(), int(difficulty));

		TC_LOG_INFO("entities.player", "[ZoloCraft] Player %s entered map: %s (mapid: %u), difficulty is %u.", player->GetName().c_str(), map->GetMapName(), map->GetId(), int(difficulty));
	...
	void SwitchIncreasedStats(Player* player, bool onoff, float difficulty, int numInGroup)
	{
		...
		TC_LOG_INFO("entities.player", "[ZoloCraft] Player %s's multiplier for stats: %f, multiplier for ratings: %f.", player->GetName().c_str(), final_multiplier_stat, final_multiplier_rat);
