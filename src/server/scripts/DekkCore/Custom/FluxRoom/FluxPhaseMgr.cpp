...
public:
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        void AddPhase(Player* player, uint32 phaseID)
        {
            if (!player->GetPhaseShift().HasPhase(phaseID))
            {
                ...
                if (sConfigMgr->GetBoolDefault("FluxPhaseMgr.ConsoleDebug", false))
                    TC_LOG_INFO("server.FluxPhaseMgr", "[FluxPhaseMgr]: AddPhase %u for %s.", phaseID, player->GetName().c_str());
            }
        }
        ...
        void RemovePhase(Player* player, uint32 phaseID)
        {
            if (player->GetPhaseShift().HasPhase(phaseID))
            {
                ...
                if (sConfigMgr->GetBoolDefault("FluxPhaseMgr.ConsoleDebug", false))
                    TC_LOG_INFO("server.FluxPhaseMgr", "[FluxPhaseMgr]: RemovePhase %u for %s.", phaseID, player->GetName().c_str());
            }
        }
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        {
            if (sConfigMgr->GetBoolDefault("FluxPhaseMgr.ConsoleDebug", false))
                TC_LOG_DEBUG("server.FluxPhaseMgr", "[FluxPhaseMgr]: Checking Phase fluxPhaseID %u for %s.", fluxPhaseID, player->GetName().c_str());
            ...
            if (sConfigMgr->GetBoolDefault("FluxPhaseMgr.ConsoleDebug", false))
                TC_LOG_DEBUG("server.FluxPhaseMgr", "[FluxPhaseMgr]: fluxPhaseID: %u with PhaseID: %u and Method %u available.", fp->ID, fp->PhaseID, fp->Method);
