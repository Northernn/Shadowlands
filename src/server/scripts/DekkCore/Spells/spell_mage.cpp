...
enum MageSpells
{
    ...
    SPELL_MAGE_SUN_KINGS_BLESSING                   = 383886,
    SPELL_MAGE_SUN_KINGS_BLESSING_COUNTER           = 383882,
    SPELL_MAGE_SUN_KING_FURY                        = 383883,
    //Azerite Traits
...
void HandleHotStreakRemove(Unit* caster)
{
    if (!caster)
        return;
    ...
    if (Aura* sunKingsBlessing = caster->GetAura(SPELL_MAGE_SUN_KINGS_BLESSING))
    {
        caster->CastSpell(nullptr, SPELL_MAGE_SUN_KINGS_BLESSING_COUNTER, true);

        if (Aura* blessingCounter = caster->GetAura(SPELL_MAGE_SUN_KINGS_BLESSING_COUNTER))
        {
            if (blessingCounter->GetStackAmount() >= sunKingsBlessing->GetEffect(EFFECT_0)->GetAmount())
            {
                blessingCounter->Remove();
                caster->CastSpell(nullptr, SPELL_MAGE_SUN_KING_FURY, true);
            }
        }
    }
}
...
class spell_mage_pyroblast : public SpellScript
{
    ...
    {
        HandleHotStreakRemove(GetCaster());
    }
...
class spell_mage_flurry : public SpellScript
{
    ...
    public:
        ...
        bool Execute(uint64 time, uint32 /*diff*/) override
        {
            ...
            _caster->CastSpell(target, SPELL_MAGE_FLURRY_VISUAL, CastSpellExtraArgs(TRIGGERED_IGNORE_CAST_IN_PROGRESS).SetOriginalCastId(_originalCastId));
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_MAGE_FLURRY_VISUAL });
    }
