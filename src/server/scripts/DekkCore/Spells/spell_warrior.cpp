...
enum WarriorSpells
{
    ...
    SPELL_WARRIOR_SIEGEBREAKER_BUFF                 = 280773,
...
class spell_warr_siegebreaker : public SpellScript
{
    void HandleOnHit()
    {
        ...
        caster->CastSpell(nullptr, SPELL_WARRIOR_SIEGEBREAKER_BUFF, true);
    }
    ...
    void CountTargets(std::list<WorldObject*>& targets)
    {
        if (Unit* caster = GetCaster())
        {
            if (Aura* inForTheKill = caster->GetAura(248621)) // In For The Kill
            {
                ...
                caster->CastSpell(caster, 248622, &hastePct); // In For The Kill
            }
        }
    }
        ...
        void HandleOnHit(SpellEffIndex effIndex)
        {
            ...
            if (roll_chance_i(sSpellMgr->GetSpellInfo(SPELL_WARRIOR_DEVASTATE, DIFFICULTY_NONE)->GetEffect(effIndex).BasePoints))
                if (Player* player = caster->ToPlayer())
                    player->GetSpellHistory()->ModifyCooldown(SPELL_WARRIOR_SHIELD_SLAM, (Milliseconds)-40 * IN_MILLISECONDS);
...
private:
    ...
    void HandleOnHit()
    {
        ...
        if (Aura* whirlWind = GetCaster()->GetAura(SPELL_WARRIOR_WHIRLWIND_PASSIVE))
            whirlWind->ModStackAmount(-1, AuraRemoveMode::AURA_REMOVE_BY_DEFAULT, false);
    }
...
class spell_warr_execute : public SpellScript
{
    ...
    void HandleTakePower(SpellPowerCost& powerCost)
    {
        ...
        uint32 requiredAmount = powerCost.Amount - powerCost.Amount;
...
class spell_warr_execute_damages : public SpellScript
{
    ...
    {
        float damageMultiplier = GetCaster()->VariableStorage.GetValue<float>("spell_warr_execute_damages::multiplier", 1.f);
...
//        {
//            ChrSpecialization spec = player->GetSpecializationId();
//
...
//                    player->GetSpellHistory()->ModifyCooldown(167105, (Milliseconds)-1000); // Colossus Smash
//                }
...
//                    player->GetSpellHistory()->ModifyCooldown(12975, (Milliseconds)-1000); // Last Stand
                ...
                for (auto& targets : allies)
                {
                    if (targets->IsFriendlyTo(owner) && targets->IsPlayer())
                    {
                        if (!targets->HasAura(SPELL_WARRIOR_WAR_BANNER_BUFF))
                            targets->AddAura(SPELL_WARRIOR_WAR_BANNER_BUFF, targets);
...
void AddSC_DekkCore_warrior_spell_scripts()
{
    ...
    RegisterSpellScript(spell_defensive_state);
