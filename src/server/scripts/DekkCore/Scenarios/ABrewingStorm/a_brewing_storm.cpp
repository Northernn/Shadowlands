    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        if (InstanceScript* instance = creature->GetInstanceScript())
        {
            if (instance->GetData(DATA_MAKE_BOOMERS_BREW) < DONE)
               AddGossipItemFor(player, GossipOptionNpc::None, TEXT_CHAPTER_MAKE_BOOMERS_BREW, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

            if (instance->GetData(DATA_ROAD_TO_THUNDERPAW) < DONE && instance->GetData(DATA_ROAD_TO_THUNDERPAW - 1) == DONE)
                AddGossipItemFor(player, GossipOptionNpc::None, TEXT_CHAPTER_ROAD_TO_THUNDERPAW, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);

            if (instance->GetData(DATA_SAVE_THUNDERPAW_REFUGE) < DONE && instance->GetData(DATA_SAVE_THUNDERPAW_REFUGE - 1) == DONE)
                AddGossipItemFor(player, GossipOptionNpc::None,  TEXT_CHAPTER_SAVE_THUNDERPAW_REFUGE, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
        }
    ...
    struct npc_brewmaster_blancheAI : public ScriptedAI
    {
        ...
        void DoAction(int32 actionId) override
        {
            ...
            case ACTION_MAKE_BOOMERS_BREW:
                ...
                nonCombatEvents.ScheduleEvent(EVENT_PREPARE_TO_STORM, 4s);
                nonCombatEvents.ScheduleEvent(EVENT_BLANCHE_POTION, 59s);
                break;
            ...
            case ACTION_CHAPTER_ONE_COMPLETED:
                ...
                nonCombatEvents.ScheduleEvent(EVENT_CHAPTER_2_PREPARE, 8s);
                break;
            case ACTION_ROAD_TO_THUNDERPAW:
                ...
                me->SetNpcFlag(UNIT_NPC_FLAG_GOSSIP);
                sauroksCount = 0;
                nonCombatEvents.ScheduleEvent(EVENT_ESCORT, 2s);
                break;
        ...
        void MovementInform(uint32 type, uint32 pointId) override
        {
            ...
            case 18:
                Talk(TALK_BREATHING);
                nonCombatEvents.ScheduleEvent(EVENT_BREATHING, 7s);
                break;
            case 34:
                me->SetNpcFlag(UNIT_NPC_FLAG_GOSSIP);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = nonCombatEvents.ExecuteEvent())
            {
                ...
                case EVENT_PREPARE_TO_STORM:
                    Talk(TALK_KEGS);
                    BrewOnKegs();
                    nonCombatEvents.ScheduleEvent(EVENT_LIGHTNING_ROD, 5s);
                    nonCombatEvents.ScheduleEvent(EVENT_VILETONGUE_SAUROKS, 12s);
                    break;
                ...
                case EVENT_ESCORT:
                    if (sauroksCount)
                    {
                        nonCombatEvents.RescheduleEvent(EVENT_ESCORT, 1s);
                        break;
                    }
                ...
                case EVENT_BREATHING:
                    ...
                    nonCombatEvents.ScheduleEvent(EVENT_ESCORT, 1s);
                    break;
                }
            }
...
struct npc_viletongue_sauroks : public ScriptedAI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        {
            me->RemoveUnitFlag(UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_PACIFIED);
            me->SetInCombatWithZone();

            events.ScheduleEvent(EVENT_TORCH_TOSS, 5s);
        }
    }
...
struct npc_brewkeg : public ScriptedAI
{
    ...
    void DoAction(int32 actionId) override
    {
        if (actionId == ACTION_MAKE_BOOMERS_BREW)
            nonCombatEvents.ScheduleEvent(EVENT_PREPARE_TO_STORM, 5s);
    }
...
struct npc_borokhula_the_destroyer : public ScriptedAI
{
    ...
    void InitializeAI() override
    {
        ...
        x = 0.0f; y = 0.0f;
        instance = me->GetInstanceScript();
        me->SetUnitFlag(UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_PACIFIED);
        Reset();
    }
    ...
    void EnterCombat(Unit* /*who*/) //override
    {
        events.ScheduleEvent(EVENT_SWAMP_SMASH, 3s);
        events.ScheduleEvent(EVENT_EARTH_SHATTERING, 12s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SWAMP_SMASH:
                ...
                events.ScheduleEvent(EVENT_SWAMP_SMASH, 7s);
                break;
            case EVENT_EARTH_SHATTERING:
                DoCast(me, SPELL_EARTH_SHATTERING);
                events.ScheduleEvent(EVENT_EARTH_SHATTERING, 15s);
                break;
            }
        }
...
struct npc_viletongue_decimator : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*who*/) //override
    {
        events.ScheduleEvent(EVENT_TORCH_TOSS, 7s);
        events.ScheduleEvent(EVENT_WIND_SLASH, 5s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_WIND_SLASH:
                ...
                events.ScheduleEvent(EVENT_WIND_SLASH, 12s);
                break;
            }
        }
...
class spell_brewing_storm_lightning_impact : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (targets.size() > affectedCount)
            Trinity::Containers::RandomResize(targets, affectedCount);
    }
...
//class sat_blanches_elexir : public AreaTriggerScript
//{
...
void AddSC_a_brewing_storm()
{
    ...
    RegisterCreatureAI(npc_viletongue_sauroks);
    RegisterCreatureAI(npc_brewkeg);
    RegisterCreatureAI(npc_viletongue_stalker);
    RegisterCreatureAI(npc_borokhula_the_destroyer);
    RegisterCreatureAI(npc_viletongue_decimator);

    new spell_brewing_storm_lightning_impact();
  //  new spell_brewing_storm_lightning_channel();
    new spell_boomer_brew_strike();
   // new sat_blanches_elexir();
}
