...
public:
    ...
    struct instance_broken_islands_InstanceMapScript : public InstanceScript
    {
        ...
        void OnPlayerEnter(Player* player) override
        {
                ...
                /*player->AddDelayedEvent(5000, [player]() -> void
                    {
                        ...
                        player->PlayConversation(player->GetTeam() == ALLIANCE ? 923 : 3598);
                        player->AddDelayedEvent(15000, [player]() -> void
                            {
                                if (player->GetMapId() != 1460)
                                    return;

                                player->PlayConversation(player->GetTeam() == ALLIANCE ? 924 : 3599);
                                player->AddDelayedEvent(13100, [player]() -> void
                                    {
                                        if (player->GetMapId() != 1460)
                                            return;

                                        player->PlayConversation(player->GetTeam() == ALLIANCE ? 925 : 3600);
                                    });
                            });
                    });*/
            }
            ...
            player->SetPlayerFlag(PLAYER_FLAGS_PVP_TIMER);
        ...
        void OnUnitDeath(Unit* unit) override
        {
            ...
            case 110618:    // horde
            {
                ...
                if (c >= 3)
                {
                    ...
                    if (auto cr = unit->GetOwner()->ToCreature())
        ...
        void massive_cast(uint32 spell)
        {
            ...
            if (spell == 208513) // alliance
                quest = sObjectMgr->GetQuestTemplate(39691);
            if (spell == 208516) // horde
                quest = sObjectMgr->GetQuestTemplate(40976);
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            case 105170:
                ...
                break;
            case 101667:
                creature->SetUnitFlag(UNIT_FLAG_REMOVE_CLIENT_CONTROL);
        ...
        void releaseWaveCre(Creature* creature, bool Final)
        {
            ...
            if (!Final)
            {
                creature->RemoveUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC));
        ...
        void onScenarionNextStep(uint32 newStep)// override TODO LATER
        {
