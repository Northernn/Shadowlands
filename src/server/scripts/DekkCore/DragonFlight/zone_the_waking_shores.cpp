...
//382100 Repaired titan orb
class go_repaired_titan_orb : public GameObjectScript
{
public:
    go_repaired_titan_orb() : GameObjectScript("go_repaired_titan_orb") {}

    struct go_repaired_titan_orbAI : public GameObjectAI
    {
        go_repaired_titan_orbAI(GameObject* go) : GameObjectAI(go)
        {
        }

        bool OnGossipHello(Player* player) override
        {
            if (player->HasQuest(QUEST_A_TINKERS_CHANCE))
            {
                player->ForceCompleteQuest(QUEST_A_TINKERS_CHANCE);
            }
    ...
    GameObjectAI* GetAI(GameObject* go) const override
    {
        return new go_repaired_titan_orbAI(go);
    }
};
