            ...
            void DoAction(int32 const action) override
            {
                switch (action)
                {
                    case ACTION_ENTER_COMBAT:
                    {
                        ...
                        events.ScheduleEvent(EVENT_PETRIFICATION, 7s);
                        events.ScheduleEvent(EVENT_CHECK_PETRIFICATION, 8s);

                        if (IsHeroic())
                            events.ScheduleEvent(EVENT_CRYSTALS, 7s);
                    ...
                    {
                        events.ScheduleEvent(EVENT_PETRIFICATION, 2s);
                    }
                    default:
                        break;
                }
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_CHECK_PETRIFICATION:
                    {
                        ...
                        if (petrifiedCount == playerList.size())
                            ...
                            events.ScheduleEvent(EVENT_CHECK_PETRIFICATION, 1s);
            ...
            void DoAction(int32 const action) override
            {
                switch (action)
                {
                    case ACTION_ENTER_COMBAT:
                    {
                        ...
                        events.ScheduleEvent(EVENT_CHECK_NEAR_GUARDIANS,    2s);
                        events.ScheduleEvent(EVENT_CHECK_ENERGY,            1s);
                        events.ScheduleEvent(EVENT_REND_FLESH,              5s);
                        events.ScheduleEvent(EVENT_MAIN_ATTACK,             10s);
                        events.ScheduleEvent(EVENT_ENRAGE,                  420s); // 7 minutes enrage timer
                        break;
                    }
            ...
            void UpdateAI(const uint32 diff) override
            {
                ...
                switch(events.ExecuteEvent())
                {
                    ...
                    case EVENT_REND_FLESH:
                    {
                        ...
                        events.ScheduleEvent(EVENT_REND_FLESH, 20s);
                        break;
                    }
                        ...
                        if (isInTrueForm)
                        {
                            switch (me->GetEntry())
                            {
                                case NPC_JADE:
                                {
                                    ...
                                    events.ScheduleEvent(EVENT_MAIN_ATTACK, 8s);
                                    break;
                                }
                                case NPC_COBALT:
                                case NPC_AMETHYST:
    ...
    public :
        ...
        struct mob_tiling_creatureAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                events.Reset();
                activated = false;
                events.ScheduleEvent(EVENT_TILING, 1s);
            }
