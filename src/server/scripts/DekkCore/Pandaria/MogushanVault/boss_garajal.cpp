    ...
    public:
        ...
        struct boss_garajalAI : public BossAI
        {
            ...
            void JustEngagedWith(Unit* attacker) override
            {
                ...
                events.ScheduleEvent(EVENT_SECONDARY_ATTACK, 5s);
                events.ScheduleEvent(EVENT_SUMMON_TOTEM, 27s);
                events.ScheduleEvent(EVENT_SUMMON_SHADOWY_MINION, 10s);
                events.ScheduleEvent(EVENT_BANISHMENT, 90s);
                events.ScheduleEvent(EVENT_VOODOO_DOLL, 2s);
                events.ScheduleEvent(EVENT_FINAL_DESTINATION, 60s); 
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_TOTEM:
                        {
                           ...
                           // GetRandPosFromCenterInDist(4277.08f, 1341.35f, frand(0.0f, 30.0f), x, y);
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_TOTEM, 47s);
                            break;
                        }
                        ...
                        case EVENT_VOODOO_DOLL:
                        {
                            ...
                            {
                                if (Unit* target = SelectTarget(i == 0 ? SelectTargetMethod::MaxThreat : SelectTargetMethod::Random, 0, 0, true, true, -SPELL_VOODOO_DOLL_VISUAL))
                                {
                        ...
                        case EVENT_BANISHMENT:
                        {
                            if (Unit* target = SelectTarget(SelectTargetMethod::MaxThreat))
                            {
                                ...
                                for (uint8 i = 0; i < mobCount; ++i)
                                    if (Creature* soulCutter = me->SummonCreature(NPC_SOUL_CUTTER, target->GetPositionX() + 2.0f, target->GetPositionY() + 2.0f, target->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 30s))
                                    {
    ...
    public:
        ...
        struct mob_garajal_ghostAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_GROWTH, 2s);
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_DISAPPEAR:
                            if (Creature* spiritualPortal = me->FindNearestCreature(NPC_SPIRITUAL_PORTAL, 50.0f))
                                spiritualPortal->DespawnOrUnsummon();
        ...
        struct mob_shadowy_minionAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                if (me->GetEntry() == NPC_SHADOWY_MINION_REAL)
                {
                    ...
                    me->SetUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
                    events.ScheduleEvent(EVENT_SPIRITUAL_GRASP, 2s);
                }
                else
            ...
            void UpdateAI(const uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0, false, true, SPELL_CROSSED_OVER))
                                me->CastSpell(target, SPELL_SHADOW_BOLT, false);
                            else if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0, false, true, SPELL_SOUL_CUT_SUICIDE))
                                me->CastSpell(target, SPELL_SHADOW_BOLT, false);
                        ...
                        {
                            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0, false, true, -SPELL_SOUL_CUT_SUICIDE))
                                me->CastSpell(target, SPELL_SPIRITUAL_GRASP, false);
                            else if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0, false, true, -SPELL_CROSSED_OVER))
                                me->CastSpell(target, SPELL_SPIRITUAL_GRASP, false);

                            events.ScheduleEvent(EVENT_SPIRITUAL_GRASP, 8s);
                            break;
                        }
                    }
                }
