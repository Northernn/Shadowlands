...
public:
    ...
    struct boss_wing_leader_neronokAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who)
        {
            ...
            events.ScheduleEvent(EVEMNT_FLY_TO_OPPOSITE_SIDE_OF_BRIDGE, 20s);
            events.ScheduleEvent(EVENT_CAST_HURL_BRICK, 1s);
            events.ScheduleEvent(EVENT_CAST_CAUSTIC_PITCH, 5s);
            events.ScheduleEvent(EVENT_CAST_QUICK_DRY_RESIN, 19s);
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            case EVEMNT_FLY_TO_OPPOSITE_SIDE_OF_BRIDGE:
                ...
                if (!isOnOtherSideOfBridge)
                {
                    ...
                    events.ScheduleEvent(EVEMNT_FLY_TO_OPPOSITE_SIDE_OF_BRIDGE, 30s);
                }
                else
                    MoveToPoint(POINT_OTHER_SIDE_AIR_FROM_GROUND, BridgeSidePositions[3]);

                events.ScheduleEvent(EVENT_CHECK_IF_REACHED_WP, 5s);
                events.ScheduleEvent(EVENT_CAST_GUSTING_WINDS_1, 8s);
                events.ScheduleEvent(EVENT_CAST_GUSTING_WINDS_2, 12s);
                events.ScheduleEvent(EVENT_CAST_GUSTING_WINDS_3, 15s);
                break;
            ...
            case EVENT_CAST_HURL_BRICK:
                ...
                events.ScheduleEvent(EVENT_CAST_HURL_BRICK, 2s);
                break;
            case EVENT_CAST_CAUSTIC_PITCH:
                me->CastSpell(me, SPELL_CAUSTIC_PITCH_SELECTOR, false);
                events.ScheduleEvent(EVENT_CAST_CAUSTIC_PITCH, 8s);
                break;
