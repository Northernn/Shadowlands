    ...
    public:
        ...
        struct boss_vizier_jinbakAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                if (!evaded)
                    if (Creature* puddle = me->SummonCreature(NPC_SAP_PUDDLE, middlePos, TEMPSUMMON_CORPSE_DESPAWN))
            ...
            void JustEngagedWith(Unit* who)
            {
                ...
                events.ScheduleEvent(EVENT_SUMMON_GLOBULES, 15s);
                events.ScheduleEvent(EVENT_DETONATE, 30s);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SUMMON_GLOBULES:
                            me->Yell(ANN_GLOBULES, LANG_UNIVERSAL);
                            DoCast(me, SPELL_SUMMON_GLOBULE);
                            events.ScheduleEvent(EVENT_SUMMON_GLOBULES, 45s);
                            break;

                        case EVENT_DETONATE:
                            me->Yell(ANN_DETONATE, LANG_UNIVERSAL);
                            if (Creature* puddle = me->FindNearestCreature(NPC_SAP_PUDDLE, 100.0f, true))
                                DoCast(puddle, SPELL_DETONATE);
                            events.ScheduleEvent(EVENT_DETONATE, 45s);
                            break;
    ...
    public:
        ...
        struct npc_sap_globuleAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(WorldObject* summoner)
            {
                ...
                events.ScheduleEvent(EVENT_MOVE_PUDDLE, 1s);
            }
    ...
    public:
        ...
        class spell_jinbak_sap_residue_se_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                /*if (Aura grow = caster->GetAura(SPELL_GROW))
                {
