...
class SummonSikthikAdsEvent : public BasicEvent
{
    public:
        ...
        {
            _owner.SummonCreature(_entry, _owner.GetPositionX(), _owner.GetPositionY(), _owner.GetPositionZ(), 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
            return true;
        }
...
class boss_commander_vojak : public CreatureScript
{
    public:
        ...
        struct boss_commander_vojakAI : public BossAI
        {
            ...
            {
                events.RescheduleEvent(EVENT_CHECK_FOR_ALIVE_PLAYERS, 1s);
            ...
            void DoAction(int32 action)
            {
                switch (action)
                {
                    case ACTION_VOJAK_START_ENCOUNTER:
                        DoZoneInCombat();
                        events.ScheduleEvent(EVENT_VOJAK_SAY_INTRO_LINE_ONE, 3s);
                        events.ScheduleEvent(EVENT_CHECK_FOR_ALIVE_PLAYERS, 1s);
                        break;
                }
            }
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    case EVENT_CHECK_FOR_ALIVE_PLAYERS:
                        ...
                        events.ScheduleEvent(EVENT_CHECK_FOR_ALIVE_PLAYERS, 5s);
                        break;
                    case EVENT_VOJAK_SAY_INTRO_LINE_ONE:
                        Talk(0);
                        events.ScheduleEvent(EVENT_VOJAK_SAY_INTRO_LINE_TWO, 6s);
                        break;
                    case EVENT_VOJAK_SAY_INTRO_LINE_TWO:
                        Talk(1);
                        events.ScheduleEvent(EVENT_VOJAK_SUMMON_SIKTHIK_SWARMER, 6s);
                        break;
                    case EVENT_VOJAK_SUMMON_SIKTHIK_SWARMER:
                        DoDelayedSummonSikthikSwarmers();
                        events.ScheduleEvent(EVENT_VOJAK_SUMMON_AIR_SUPPORT, 37s);
                        break;
                    case EVENT_VOJAK_SUMMON_AIR_SUPPORT:
                        Talk(2);

                        me->SummonCreature(NPC_SIKTHIK_AMBERWING, SikthikAmberwingWaypoint[0].GetPositionX(), SikthikAmberwingWaypoint[0].GetPositionY(), SikthikAmberwingWaypoint[0].GetPositionZ(), 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN);

                        if (Creature* ironclaw = me->FindNearestCreature(NPC_YANG_IRONCLAW, 150.0f))
                            ironclaw->AI()->Talk(9);

                        events.ScheduleEvent(EVENT_VOJAK_SUMMON_SIKTHIK_DEMOLISHER, 37s);
                        break;
                    case EVENT_VOJAK_SUMMON_SIKTHIK_DEMOLISHER:
                        DoDelayedSummonSikthikDemolishers();
                        events.ScheduleEvent(EVENT_VOJAK_SUMMON_SIKTHIK_SWARMER_AND_WARRIOR, 45s);
                        break;
                    case EVENT_VOJAK_SUMMON_SIKTHIK_SWARMER_AND_WARRIOR:
                        ...
                        // 500 - 1000 - 1500
                        for (int i = 0; i < 3; ++i)
                            me->m_Events.AddEvent(new SummonSikthikAdsEvent(*me, NPC_SIKTHIK_SWARMER), me->m_Events.CalculateTime((i + 1) * 5s));

                        me->m_Events.AddEvent(new SummonSikthikAdsEvent(*me, NPC_SIKTHIK_WARRIOR), me->m_Events.CalculateTime(3s));

                        for (int i = 0; i < 3; ++i)
                            me->m_Events.AddEvent(new SummonSikthikAdsEvent(*me, NPC_SIKTHIK_SWARMER), me->m_Events.CalculateTime(((i + 1) * 5s)) + 4s);
                        ...
                        events.ScheduleEvent(EVENT_VOJAK_SUMMON_SIKTHIK_DEMOLISHER_AND_WARRIOR, 45s);
                        break;
                    case EVENT_VOJAK_SUMMON_SIKTHIK_DEMOLISHER_AND_WARRIOR:
                        for (int i = 0; i < 3; ++i)
                            me->m_Events.AddEvent(new SummonSikthikAdsEvent(*me, NPC_SIKTHIK_DEMOLISHER), me->m_Events.CalculateTime((i + 1) * 5s));

                        me->m_Events.AddEvent(new SummonSikthikAdsEvent(*me, NPC_SIKTHIK_WARRIOR), me->m_Events.CalculateTime(3s));

                        for (int i = 0; i < 3; ++i)
                            me->m_Events.AddEvent(new SummonSikthikAdsEvent(*me, NPC_SIKTHIK_DEMOLISHER), me->m_Events.CalculateTime(((i + 1) * 5s)) + 4s);

                        if (Creature* ironclaw = me->FindNearestCreature(NPC_YANG_IRONCLAW, 150.0f))
                            ironclaw->AI()->Talk(14);

                        events.ScheduleEvent(EVENT_VOJAK_MOVE_TO_PLATFORM, 55s);
                        break;
                    case EVENT_VOJAK_MOVE_TO_PLATFORM:
                        ...
                        events.ScheduleEvent(EVENT_CHECK_FOR_SAP_PUDDLE, 1s);
                        break;
                    case EVENT_CHECK_FOR_SAP_PUDDLE:
                        ...
                        events.ScheduleEvent(EVENT_CHECK_FOR_SAP_PUDDLE, 2s);
                        break;
                    case EVENT_CAST_DASHING_STRIKE:
                        if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0.0f, true))
                            DoCast(target, SPELL_DASHING_STRIKE);

                        events.ScheduleEvent(EVENT_CAST_DASHING_STRIKE, 13s);
                        events.ScheduleEvent(EVENT_CAST_THOUSAND_BLADES, 5s);
                        break;
        ...
        struct npc_commander_vojak_yang_ironclawAI : public ScriptedAI
        {
            ...
            void MovementInform(uint32 type, uint32 pointId)
            {
                ...
                if (pointId == POINT_YANG_IRONCLAW_INTRO)
                    events.ScheduleEvent(EVENT_YANG_DO_IRONCLAW_INTRO_EVENT, 5s);
            }
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    case EVENT_YANG_DO_IRONCLAW_BOSS_ORDER_CHECK:
                        ...
                        events.ScheduleEvent(EVENT_YANG_DO_IRONCLAW_BOSS_ORDER_CHECK, 5s);
                        break;
                        ...
                        switch (talkProgress)
                        {
                            ...
                            case 7: //! Make Vojak speak, then continue
                                if (Creature* vojak = me->FindNearestCreature(61634, 300.0f))
                                {
                                    ...
                                    events.ScheduleEvent(EVENT_YANG_DO_IRONCLAW_INTRO_EVENT, 19s);
                                }
                            ...
                            case 5: //! Tell the Chu's to start placing barrels
                                ...
                                events.ScheduleEvent(EVENT_YANG_DO_IRONCLAW_INTRO_EVENT, 6s);
                                break;
                            case 3:
            ...
            void MovementInform(uint32 type, uint32 pointId)
            {
                ...
                switch (pointId)
                {
                    case POINT_LO_LI_CHU_BARREL_PICKUP_SPOT:
                        me->SetFacingTo(4.79f);
                        events.ScheduleEvent(EVENT_PICK_UP_BARREL, 4s);
                        ...
                        if (Vehicle* veh = me->GetVehicleKit())
                        {
                            if (Unit* passenger = veh->GetPassenger(0))
                            {
                                if (passenger->GetTypeId() == TYPEID_UNIT)
                                {
                                    ...
                                    me->MovePositionToFirstCollision(pos, 5.0f, me->GetAbsoluteAngle(&pos));
                                    passenger->GetMotionMaster()->MoveJump(pos, 20.0f, 10.0f);
                                }
                            }
                        }

                        events.ScheduleEvent(EVENT_MOVE_BACK_TO_BARREL_PICKUP_SPOT, 1s);
                        break;
                }
            }
            ...
            void DoAction(int32 action)
            {
                switch (action)
                {
                    case ACTION_CHU_START_PLANTING_BARRELS:
                        events.ScheduleEvent(EVENT_PICK_UP_BARREL, 5s);
                        break;
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    case EVENT_PICK_UP_BARREL:
                        ...
                        events.ScheduleEvent(EVENT_MOVE_TO_BARREL_DROP_SPOT, 5s);
                        break;
        ...
        struct npc_commander_vojak_sikthik_amberwingAI : public ScriptedAI
        {
            ...
            void MovementInform(uint32 type, uint32 pointId)
            {
                ...
                if (pointId == POINT_AMBERWING_WAYPOINT)
                {
                    ...
                    events.ScheduleEvent(EVENT_AMBERWING_RETREAT, 25s);
                    me->SetCanFly(true);
                }
                else if (pointId == POINT_AMBERWING_RETREAT)
                    events.ScheduleEvent(EVENT_AMBERWING_START_MOVING, 27s);

                {
                    if (waypointProgress == 6)
                        events.ScheduleEvent(EVENT_AMBERWING_RETREAT, 15s);
                    else
                        me->GetMotionMaster()->MovePoint(POINT_AMBERWING_WAYPOINT, SikthikAmberwingWaypoint[waypointProgress].GetPositionX(), SikthikAmberwingWaypoint[waypointProgress].GetPositionY(), SikthikAmberwingWaypoint[waypointProgress].GetPositionZ(), false);
