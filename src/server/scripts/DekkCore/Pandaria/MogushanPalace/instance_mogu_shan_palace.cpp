        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            switch (go->GetEntry())
            {
                ...
                case GO_TRIAL_CHEST:
                    ...
                    PhasingHandler::AddPhase(go, 176, true);
                    break;
...
public:
    ...
    struct go_mogushan_palace_temp_portalAI : public GameObjectAI
    {
        go_mogushan_palace_temp_portalAI(GameObject* go) : GameObjectAI(go) { }

        bool OnGossipHello(Player* player) override
        {
            if (me->GetPositionZ() < 0.0f)
                player->NearTeleportTo(me->GetPositionX(), me->GetPositionY(), 22.31f, me->GetOrientation());
            else
                player->NearTeleportTo(me->GetPositionX(), me->GetPositionY(), -39.0f, me->GetOrientation());
    ...
    {
        return new go_mogushan_palace_temp_portalAI(go);
    }
};
