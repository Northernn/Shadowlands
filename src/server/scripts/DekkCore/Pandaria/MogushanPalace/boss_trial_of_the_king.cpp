    ...
    struct mob_xian_the_weaponmaster_trigger_AI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (!event_go && who->ToPlayer() && who->GetAreaId() == AREA_MOGUSHAN_PALACE_CRIMSON_ASSEMBLY_HALL)
            {
                ...
                event_go = true;
                events.ScheduleEvent(EVENT_TALK_0, 3s);
            }
        }
...
public:
    ...
    struct boss_ming_the_cunning_AI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_WHIRLING_DERVISH, 10s);
            events.ScheduleEvent(EVENT_MAGNETIC_FIELD, 30s);
        }
        ...
        void DoAction(int32 action) override
        {
            switch (action)
            {
                ...
                case ACTION_OUTRO_01:
                    Talk(TALK_OUTRO_01);
                    events.ScheduleEvent(EVENT_OUTRO_01, 3s);
                    break;
                case ACTION_OUTRO_02:
                    Talk(TALK_OUTRO_02);
                    events.ScheduleEvent(EVENT_OUTRO_02, 3s);
                    break;
            }
        }
        ...
        void DamageTaken(Unit* /*attacker*/, uint32 &damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            ...
            if (int(me->GetHealth()) - int(damage) <= 0)
            {
                ...
                me->AttackStop();
                events.Reset();
                events.ScheduleEvent(EVENT_BOSS_RETIRE, 4s);
                if (me->GetInstanceScript())
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_LIGHTNING_BOLT:
                        ...
                        events.ScheduleEvent(EVENT_LIGHTNING_BOLT, 6s);
                        break;
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case 1:
                        ...
                        me->Attack(SelectTarget(SelectTargetMethod::Random), false);
        ...
        void DoAction(int32 action) override
        {
            switch (action)
            {
                case ACTION_ENCOURAGE:
                {
                    ...
                    events.ScheduleEvent(EVENT_APPLAUSE, 5s);
                    break;
                }
                case ACTION_RETIRE:
                {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SHOCKWAVE:
                {
                    ...
                    events.ScheduleEvent(EVENT_SHOCKWAVE_2, 4s);
                }
                break;
                case EVENT_BOSS_RETIRE:
                    if (me->GetInstanceScript())
...
public:
    ...
    struct boss_haiyan_the_unstoppable_AI : public BossAI
    {
        ...
        {
            Talk(TALK_AGGRO);
            events.ScheduleEvent(EVENT_TRAUMATIC_BLOW, 3s);
            events.ScheduleEvent(EVENT_CONFLAGRATE, 10s);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_TRAUMATIC_BLOW:
                        ...
                        events.ScheduleEvent(EVENT_TRAUMATIC_BLOW, 6s);
                        break;
                    case EVENT_CONFLAGRATE:
                        me->CastSpell(me->GetVictim(), SPELL_CONFLAGRATE, false);
                        events.ScheduleEvent(EVENT_CONFLAGRATE, 10s);
                        events.ScheduleEvent(EVENT_CONFLAGRATE_2, 2s);
                        break;
                    case EVENT_METEOR:
                        me->CastSpell(SelectTarget(SelectTargetMethod::Random), SPELL_METEOR, false);
                        events.ScheduleEvent(EVENT_METEOR, 30s);
                        break;
                    case EVENT_BOSS_RETIRE:
                        if (me->GetInstanceScript())
