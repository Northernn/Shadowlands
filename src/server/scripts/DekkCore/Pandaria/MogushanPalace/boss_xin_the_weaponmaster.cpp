...
class boss_xin_the_weaponmaster : public CreatureScript
{
    public:
        ...
        struct boss_xin_the_weaponmaster_AI : public BossAI
        {
            ...
            {
                events.ScheduleEvent(EVENT_RING_OF_FIRE, 3s);
                events.ScheduleEvent(EVENT_HEURT, 15s);
                events.ScheduleEvent(EVENT_INCITING_ROAR, 25s);
                events.ScheduleEvent(EVENT_SWORD_THROWER, 30s);
                events.ScheduleEvent(EVENT_AXES_ACTIVATE, 15s);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_RING_OF_FIRE:
                            ...
                            events.ScheduleEvent(EVENT_RING_OF_FIRE, 20s);
                            break;
                        case EVENT_HEURT:
                        ...
                        case EVENT_INCITING_ROAR:
                            ...
                            events.ScheduleEvent(EVENT_INCITING_ROAR, 30s);
                            break;
                        case EVENT_SWORD_THROWER:
                            if (pInstance)
                                pInstance->SetData(TYPE_ACTIVATE_SWORD, 1);
                            events.ScheduleEvent(EVENT_SWORD_THROWER_STOP, 10s);
                            break;
                        case EVENT_SWORD_THROWER_STOP:
                            if (pInstance)
                                pInstance->SetData(TYPE_ACTIVATE_SWORD, 0);
                            events.ScheduleEvent(EVENT_SWORD_THROWER, 20s);
                            break;
                        case EVENT_AXES_ACTIVATE:
                            if (pInstance)
                                pInstance->SetData(TYPE_ACTIVATE_ANIMATED_AXE, 1);

                            events.ScheduleEvent(EVENT_AXES_DESACTIVATE, 10s);
                            break;
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    case ACTION_ACTIVATE:
                        ...
                        events.ScheduleEvent(EVENT_SUMMON_RING_OF_FIRE, 5s);
                        break;
                }
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            events.ScheduleEvent(EVENT_UNSUMMON, 9s);
                            Unit* target = nullptr;
                            std::list<Unit*> units;
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_RING_TRIGGER, 1s);
                            break;
                        }
                        case EVENT_UNSUMMON:
                            Reset();
                        ...
                        case EVENT_SUMMON_RING_TRIGGER:
                        {
                            if (point >= 11)
                            {
                                if (TempSummon* tmp = me->SummonCreature(CREATURE_RING_OF_FIRE, _x, _y, me->GetMap()->GetHeight(me->GetPhaseShift(), _x, _y, me->GetPositionZ()), 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 10s))
                                {
                            ...
                            me->SummonCreature(CREATURE_RING_OF_FIRE, x, y, me->GetMap()->GetHeight(me->GetPhaseShift(), x, y, me->GetPositionZ()), 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 10s);
                            ++point;
                            events.ScheduleEvent(EVENT_SUMMON_RING_TRIGGER, 4s);
                            break;
                        }
                    }
                }
