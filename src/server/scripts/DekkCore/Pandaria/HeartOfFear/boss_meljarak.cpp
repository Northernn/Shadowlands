...
public:
    ...
    struct boss_wind_lord_meljarakAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* attacker) override
        {
            ...
            // Normal events.
            events.ScheduleEvent(EVENT_WHIRLING_BLADE, 36s);
            events.ScheduleEvent(EVENT_RAIN_OF_BLADES, 60s);
            events.ScheduleEvent(EVENT_BERSERK_MELJARAK, 80s);
        ...
        void DoAction(int32 const action) override
        {
            switch (action)
            {
                case ACTION_KORTHIK_DIED:
                {
                    ...
                    if (IsHeroic())
                        events.ScheduleEvent(EVENT_SUMMON_KORTHIK, 45s);
                    break;
                }
                case ACTION_SRATHIK_DIED:
                {
                    ...
                    if (IsHeroic())
                        events.ScheduleEvent(EVENT_SUMMON_SRATHIK, 45s);
                    break;
                }
                case ACTION_ZARTHIK_DIED:
                {
                    ...
                    if (IsHeroic())
                        events.ScheduleEvent(EVENT_SUMMON_ZARTHIK, 45s);
                    break;
                }
        ...
        void UpdateAI(uint32 const diff) override
        {
            ...
            {
                events.ScheduleEvent(EVENT_WIND_BOMB, 6s);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_KORTHIK_STRIKE:
                    {
                        ...
                        events.ScheduleEvent(EVENT_KORTHIK_STRIKE, 34s);
                        break;
                    }
                    // Normal events.
                    case EVENT_WHIRLING_BLADE:
                    {
                        ...
                        events.ScheduleEvent(EVENT_WHIRLING_BLADE, 48s);
                        break;
                    }
                    case EVENT_RAIN_OF_BLADES:
                    {
                        ...
                        events.ScheduleEvent(EVENT_RAIN_OF_BLADES, 48s);
                        break;
                    }
                    case EVENT_WIND_BOMB:
                    {
                        if (Unit* target = SelectTarget(SelectTargetMethod::MaxDistance, 0, 150.0f, true))
                            DoCast(target, SPELL_WIND_BOMB);
                        events.ScheduleEvent(EVENT_WIND_BOMB, 18s);
                        break;
                    }
...
public:
    ...
    struct npc_srathik_amber_trapperAI: public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* attacker) override
        {
            ...
            events.ScheduleEvent(EVENT_AMBER_PRISON, 13s);
            events.ScheduleEvent(EVENT_CORROSIVE_RESIN, 8s);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_AMBER_PRISON:
                        ...
                        events.ScheduleEvent(EVENT_AMBER_PRISON, 35s);
                        break;

                    case EVENT_CORROSIVE_RESIN:
                        if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 150.0f, true))
                            DoCast(target, SPELL_CORROSIVE_RESIN);
                        events.ScheduleEvent(EVENT_CORROSIVE_RESIN, 25s);
                        break;
...
public:
    ...
    struct npc_zarthik_battle_menderAI: public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* attacker) override
        {
            ...
            events.ScheduleEvent(EVENT_MENDING, 30s);
            events.ScheduleEvent(EVENT_QUICKENING, 12s);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_QUICKENING:
                    {
                        ...
                        events.ScheduleEvent(EVENT_QUICKENING, 33s);
                        break;
                    }
                    default:
                        break;
...
public:
    ...
    struct npc_whirling_bladeAI : public ScriptedAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_WB_BACK, 3s);
...
class go_krithik_weapon_rack : public GameObjectScript
{
    public:
        ...
        struct go_krithik_weapon_rackAI : public GameObjectAI
        {
            go_krithik_weapon_rackAI(GameObject* go) : GameObjectAI(go) { }
        };

        GameObjectAI* GetAI(GameObject* go) const override
        {
            return new go_krithik_weapon_rackAI(go);
        }
};
