    ...
    public:
        ...
        struct boss_zorlokAI : public BossAI
        {
            ...
            void SetLanding(uint8 dest)
            {
                ...
                me->SetHover(false);
                me->RemoveUnitFlag(UNIT_FLAG_IMMUNE_TO_PC);
                events.ScheduleEvent(EVENT_INHALE, 15s);
                ...
                if (dest)
                {
                    switch (dest)
                    {
                        ...
                        {
                            events.ScheduleEvent(EVENT_FORCE_AND_VERVE, 10s);
                            break;
                        }
                        // Attenuation Platform
                        case PLATFORM_ZORLOK_NE:
                        {
                            events.ScheduleEvent(EVENT_ATTENUATION, 10s);
                            break;
                        }
                        // Convert Platform
                        case PLATFORM_ZORLOK_SE:
                        {
                            events.ScheduleEvent(EVENT_CONVERT, 10s);
                            break;
                        }
                        ...
                        case PHASE_ZORLOK2:
                        {
                            ...
                            events.ScheduleEvent(ChooseAction(), 25s);
                            events.ScheduleEvent(EVENT_PULL_RAID, 2s);
                            break;
                        }
                        default:
                            break;
            ...
            void JustEngagedWith(Unit* /*p_Attacker*/) override
            {
                ...
                for (uint8 i = 0; i < 3; ++i)
                    // Arena walls
                    me->SummonGameObject(GOB_ARENA_WALLS, finalPhaseWalls2[i].GetPositionX(), finalPhaseWalls2[i].GetPositionY(), finalPhaseWalls2[i].GetPositionZ(), finalPhaseWalls2[i].GetOrientation(), QuaternionData(), 7s);


                // 10 minutes enrage timer
                if (!isEcho)
                    events.ScheduleEvent(EVENT_BERSERK, 60s);

                if (isAttEcho)
                    events.ScheduleEvent(EVENT_ATTENUATION, 15s);
                if (isFaVEcho)
                    events.ScheduleEvent(EVENT_FORCE_AND_VERVE, 15s);
            }
                ...
                switch (action)
                {
                    case ACTION_SONIC_RING:
                    {
                        ...
                        //    me->RemoveUnitFlag2(UNIT_FLAG2_DISABLE_TURN);
                          //  me->SetUnitFlag2(UNIT_FLAG2_ALLOW_ENEMY_INTERACT);
                        }
                        else
                        {
                            ++sonicRingCount;
                            ringOrientation = CorrectAngle(ringOrientation + (clocksideRings ? -M_PI / 6 : M_PI / 6));
                            events.ScheduleEvent(EVENT_SUMMON_RINGS, 1s);
                        }
                    ...
                    case ACTION_SONIC_PULSE:
                    {
                        ...
                        else
                        {
                            ...
                            events.ScheduleEvent(EVENT_SONIC_PULSE, 1s);
                        }
            ...
            void MovementInform(uint32 type, uint32 id) override
            {
                ...
                if (id == 4)
                {
                    ...
                    pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_PHEROMONES_CLOUD);
                    events.ScheduleEvent(EVENT_PULL_RAID, 7s);
                }
            }
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_INHALE:
                    {
                        ...
                        if (me->HasUnitState(UNIT_STATE_CASTING))
                            events.RescheduleEvent(EVENT_INHALE, 5s);
                        else
                        {
                            ...
                            events.ScheduleEvent(EVENT_INHALE, 25s);
                        }
                        break;
                    }
                    // Attenuation platform
                    case EVENT_ATTENUATION:
                    {
                        ...
                        {
                            events.RescheduleEvent(EVENT_ATTENUATION, 10s);
                            return;
                        }
                        me->CastSpell(me, SPELL_ATTENUATION, true);
                        me->SetReactState(REACT_PASSIVE);
                        me->SetUnitFlag(UnitFlags(UNIT_FLAG_PACIFIED|UNIT_FLAG_REMOVE_CLIENT_CONTROL|UNIT_FLAG_IMMUNE_TO_PC|UNIT_FLAG_STUNNED));
                    //    me->SetUnitFlag2(UNIT_FLAG2_DISABLE_TURN);
                     //   me->RemoveUnitFlag2(UNIT_FLAG2_ALLOW_ENEMY_INTERACT);
                        ...
                        events.ScheduleEvent(action, 40s);
                    ...
                    case EVENT_CONVERT:
                    {
                        if (me->HasUnitState(UNIT_STATE_CASTING))
                        {
                            events.RescheduleEvent(EVENT_CONVERT, 10s);
                            return;
                        }
                    ...
                    case EVENT_FORCE_AND_VERVE:
                    {
                        if (me->HasUnitState(UNIT_STATE_CASTING))
                        {
                            events.RescheduleEvent(EVENT_FORCE_AND_VERVE, 10s);
                            return;
                        }
                        ...
                        events.ScheduleEvent(EVENT_CAST_FANDV, 2s);
                        break;
                    }
                    case EVENT_CAST_FANDV:
                    {
                        ...
                        if (!hasSummonedLastEcho && phase == PHASE_ZORLOK2)
                        {
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_LAST_ECHO, 10s);
                        }
                        events.ScheduleEvent(action, 40s);
                        break;
                    }
                    ...
                    case EVENT_PULL_RAID:
                    {
                        ...
                        for (uint8 i = 0; i < 3; ++i)
                            me->SummonGameObject(GOB_FINAL_PHASE_WALLS, finalPhaseWalls1[i].GetPositionX(), finalPhaseWalls1[i].GetPositionY(), finalPhaseWalls1[i].GetPositionZ(), finalPhaseWalls1[i].GetOrientation(), QuaternionData(), 7s);
    ...
    public:
        ...
        struct mob_sonic_ringAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                m_Events.ScheduleEvent(EVENT_SONIC_MOVE, 5s);
...
class spell_zorlok_exhale_damage : public SpellScriptLoader
{
    public:
        ...
        class spell_zorlok_exhale_damage_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_zorlok_exhale_damage_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_CONE_180_DEG_ENEMY);
            }
        };
