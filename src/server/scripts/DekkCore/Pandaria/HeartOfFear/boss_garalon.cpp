...
public:
    ...
    struct boss_garalonAI : public BossAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_FURIOUS_SWIPE, 11s);
            events.ScheduleEvent(EVENT_PHEROMONES, 3s);
            events.ScheduleEvent(EVENT_CRUSH, 30s); // First Crush always seems to have this timer, on any difficulty.
            events.ScheduleEvent(EVENT_GARALON_BERSERK, 70s); // 1 min enrage timer.
                ...
                case ACTION_GARALON_VISIBLE:
                {
                    ...
                    if (me->IsAlive())
                    {
                        if (instance)
                        {
                            ...
                            {
                                l_Leg->RemoveUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_NON_ATTACKABLE));
                                instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, l_Leg);
                            }
                        }
                    }
            ...
            if (action == ACTION_LEG_IS_DEAD)
            {
                ...
                if (legs.size() < 4)
                {
                    ...
                    if (search)
                    {
                        ...
                        events.ScheduleEvent(EVENT_MEND_LEG, 10s);
                    }
                }
        ...
        void SetGUID(ObjectGuid const& guid, int32 p_Id) override
        {
            if (p_Id == GUID_MENDED_LEG)
            {
                if (Creature* l_Leg = ObjectAccessor::GetCreature(*me, guid))
                {
                    std::map<ObjectGuid, uint8>::iterator l_Itr = legsIndexMap.find(guid);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_FURIOUS_SWIPE:
                    {
                        ...
                        events.ScheduleEvent(EVENT_FURIOUS_SWIPE, 8s);
                        break;
                    }
                    ...
                    case EVENT_CRUSH:
                    {
                        ...
                        if (me->GetMap()->IsHeroic()) // In Heroic Difficulty, Garalon periodically crushes foes.
                            events.ScheduleEvent(EVENT_CRUSH, 37s);
                        castingCrush = false;
                        break;
                    }
                    case EVENT_MEND_LEG:
...
class spell_garalon_furious_swipe: public SpellScriptLoader
{
    public:
        ...
        class spell_garalon_furious_swipeSpellScript: public SpellScript
        {
            ...
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_garalon_furious_swipeSpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_CONE_ALLY);
            }
        };
