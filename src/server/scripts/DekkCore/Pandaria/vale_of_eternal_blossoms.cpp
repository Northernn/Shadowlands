    ...
    public:
        ...
        struct mob_zhao_jinAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_LIGHTNING_SPEAR, 15s);
            }
    ...
    public:
        ...
        struct mob_shao_tien_behemothAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_CRUSHING_STOMP, 8s);
                events.ScheduleEvent(EVENT_GROUND_STOMP, 12s);
                events.ScheduleEvent(EVENT_PARALYZED, 17s);
                events.ScheduleEvent(EVENT_THUNDERING_SLAM, 25s);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CRUSHING_STOMP:
                            ...
                            events.ScheduleEvent(EVENT_CRUSHING_STOMP, 320s);
                            break;
                        case EVENT_GROUND_STOMP:
                            me->CastSpell(me, SPELL_GROUND_STOMP, false);
                            events.ScheduleEvent(EVENT_GROUND_STOMP, 17s);
                            break;
                        case EVENT_PARALYZED:
                            if (Unit* target = SelectTarget(SelectTargetMethod::MaxThreat))
                                me->CastSpell(target, SPELL_PARALYZED, false);
                            events.ScheduleEvent(EVENT_PARALYZED, 21s);
                            break;
