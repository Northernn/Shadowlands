    ...
    public:
        ...
        struct boss_striker_gadokAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_PREY_TIME, 10s, PHASE_MAIN);
                events.ScheduleEvent(EVENT_IMPALING_STRIKE, 19s, PHASE_MAIN);
            }
            ...
            void DoStrafingEvent()
            {
                switch (strafingEventProgress)
                {
                    ...
                    case 12: // POINT_END, End Strafing Event, go back to fight
                        ...
                        else if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0.0f, true))
                        ...
                        events.ScheduleEvent(EVENT_PREY_TIME, 5s, PHASE_MAIN);
                        events.ScheduleEvent(EVENT_IMPALING_STRIKE, 9s, PHASE_MAIN);
                        break;
                    default:
                        break;
                }
...
struct npc_krikthik : public ScriptedAI
{
    ...
    {
        nextMovementTimer = 0;
        actualAngle = me->GetAbsoluteAngle(CenterPos.GetPositionX(), CenterPos.GetPositionY());
        direction = urand(0, 1);
...
public:
    ...
    struct npc_krikthik_disruptorAI : public npc_krikthik
    {
        ...
        void Reset() override
        {
            ...
            events.ScheduleEvent(EVENT_DISRUPTOR_BOMBARD, 5s, 20s);
        }
            ...
            if (spell->Id == 116554) // Fire Flak Cannon
            {
                for (uint8 i = 0; i < 5; ++i)
                {
                    if (Creature* bombarder = pInstance->instance->GetCreature(pInstance->GetGuidData(DATA_RANDOM_BOMBARDER)))
                    {
                        ...
                        bombarder->DespawnOrUnsummon(2s);
                    }
                }
            }
        }
