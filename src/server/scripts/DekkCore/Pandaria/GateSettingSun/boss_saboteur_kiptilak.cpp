    ...
    public:
        ...
        struct boss_saboteur_kiptilakAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_EXPLOSIVES, 10s);
                events.ScheduleEvent(EVENT_SABOTAGE, 30s);
                ...
                switch(events.ExecuteEvent())
                {
                    case EVENT_EXPLOSIVES:
                        for (uint8 i = 0; i < urand(1, 3); ++i)
                            me->CastSpell(Position(frand(702, 740), frand(2292, 2320), 388.5f), SPELL_PLANT_EXPLOSIVE, true);

                        events.ScheduleEvent(EVENT_EXPLOSIVES, 12s);
                        break;
                    case EVENT_SABOTAGE:
