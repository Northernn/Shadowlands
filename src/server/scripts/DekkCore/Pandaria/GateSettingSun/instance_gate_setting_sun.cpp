    ...
    struct instance_gate_setting_sun_InstanceMapScript : public InstanceScript
    {
        ...
        void OnPlayerEnter(Player* player) override
        {
            if (GetData(DATA_BRASIER_CLICKED) == NOT_STARTED)
                ...
                PhasingHandler::AddPhase(player, 50, true);
        }
                ...
                case DATA_RAIGONN:
                {
                    for (auto itr: artilleryGUIDs)
                    {
                        if (Creature* artillery = instance->GetCreature(itr))
                        {
                            ...
                            {
                                artillery->SetUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
                                artillery->RemoveNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
                            }
                            else
                            {
                                artillery->RemoveUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
                                artillery->SetNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
                            }
                        }
                    }
                    break;
            ...
            switch (type)
            {
                case DATA_OPEN_FIRST_DOOR:
                {
                    ...
                    if (GameObject* firstDoor = instance->GetGameObject(firstDoorGUID))
                    {
                        ...
                        if (Creature* trigger = firstDoor->SummonTrigger(firstDoor->GetPositionX(), firstDoor->GetPositionY(), firstDoor->GetPositionZ(), 0, 500ms))
                        {
                            ...
                            for (auto itr: defensorList)
                            {
                                ...
                                itr->KnockbackFrom(Position(posX, posY, itr->GetPositionZ(), itr->GetOrientation()), 25.0f, 20.0f);
                                itr->DespawnOrUnsummon(1s);
                            }
                        }
                    }
                    break;
