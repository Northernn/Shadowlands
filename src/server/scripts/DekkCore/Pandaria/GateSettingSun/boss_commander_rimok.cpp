    ...
    public:
        ...
        struct boss_commander_rimokAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_FRENZIED_ASSAULT, 10s);
                events.ScheduleEvent(EVENT_VISCOUS_FLUID, 15s);
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch(events.ExecuteEvent())
                {
                    case EVENT_FRENZIED_ASSAULT:
                        ...
                        events.ScheduleEvent(EVENT_FRENZIED_ASSAULT, 15s);
                        break;
                    case EVENT_VISCOUS_FLUID:
                        me->CastSpell(me, 107077, true);
                        events.ScheduleEvent(EVENT_VISCOUS_FLUID, 10s);
                        break;
                }
                    ...
                    if (Unit* target = me->SelectNearestPlayer(5.0f))
                    {
                        
                        {
                            if (Creature* rimok = pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_RIMOK)))
                            {
                                ...
                                me->AddAura(SPELL_VISCOUS_FLUID_DMG_DOWN, me);
                            }
                        }
                    }
                ...
                if (Unit* caster = GetCaster())
                {
                    if (InstanceScript* pInstance = caster->GetInstanceScript())
                    {
                        ...
                        if (Player* player = it->GetSource())
                            caster->CastSpell(player, GetSpellInfo()->GetEffect(SpellEffIndex::EFFECT_0).TriggerSpell, true);
                    }
                }
            }
