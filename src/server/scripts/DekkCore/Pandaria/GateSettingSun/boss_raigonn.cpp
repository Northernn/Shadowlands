    ...
    public:
        ...
        struct boss_raigonnAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_RAIGONN_CHARGE, 1s, PHASE_WEAK_SPOT);
                ...
                if (Vehicle* meVehicle = me->GetVehicleKit())
                {
                    ...
                    if (Creature* weakSpot = me->SummonCreature(NPC_WEAK_SPOT, 0, 0, 0))
                    {
                        ...
                        if (Vehicle* vehicleWeakSpot = weakSpot->GetVehicleKit())
                            vehicleWeakSpot->RemoveAllPassengers();
                    }
                }
            }
            ...
            void MoveInLineOfSight(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_SUMMON_PROTECTORAT, 30s);
                events.ScheduleEvent(EVENT_SUMMON_ENGULFER, 30s);
                events.ScheduleEvent(EVENT_SUMMON_SWARM_BRINGER, 30s);
                events.ScheduleEvent(EVENT_CHECK_WIPE, 1s);
            ...
            void DoAction(int32 action) override
            {
                if (action == ACTION_WEAK_SPOT_DEAD)
                {
                    ...
                    events.ScheduleEvent(EVENT_FIXATE, 30s, PHASE_VULNERABILITY);
                    events.ScheduleEvent(EVENT_STOMP, 16s, PHASE_VULNERABILITY);
            ...
            void DoEventCharge()
            {
                ...
                switch (eventChargeProgress)
                {
                    ...
                    case 2:
                        ...
                        events.ScheduleEvent(EVENT_RAIGONN_CHARGE, 3s, PHASE_WEAK_SPOT);
                        ++eventChargeProgress;
                        break;
                    case 3:
                    default:
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch(events.ExecuteEvent())
                {
                    case EVENT_CHECK_WIPE:
                           if (!UpdateVictim() || (pInstance != nullptr/* && m_Instance->IsWipe()*/))
                            Reset();
                        else
                            events.ScheduleEvent(EVENT_CHECK_WIPE, 1s);
                        break;
                    ...
                    {
                        for (uint8 i = 0; i < 8; ++i)
                            if (Creature* summon = me->SummonCreature(NPC_KRIKTHIK_PROTECTORAT, frand(941.0f, 974.0f), 2374.85f, 296.67f, 4.73f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s))
                                if (Unit* target = SelectTarget(SelectTargetMethod::Random))
                                    if (summon->IsAIEnabled())
                                        summon->AI()->AttackStart(target);

                        events.ScheduleEvent(EVENT_SUMMON_PROTECTORAT, 45s, PHASE_WEAK_SPOT);
                        break;
                    }
                    case EVENT_SUMMON_ENGULFER:
                    {
                        for (uint8 i = 0; i < 3; ++i)
                            me->SummonCreature(NPC_KRIKTHIK_ENGULFER, frand(941.0f, 974.0f), me->GetPositionY(), me->GetPositionZ() + 30.0f, 4.73f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s);

                        events.ScheduleEvent(EVENT_SUMMON_ENGULFER, 105s, PHASE_WEAK_SPOT);
                        break;
                    }
                    case EVENT_SUMMON_SWARM_BRINGER:
                    {
                        if (Creature* summon = me->SummonCreature(NPC_KRIKTHIK_SWARM_BRINGER, frand(941.0f, 974.0f), 2374.85f, 296.67f, 4.73f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5s))
                            if (Unit* target = SelectTarget(SelectTargetMethod::Random))
                                if (summon->AI())
                                    summon->AI()->AttackStart(target);

                        events.ScheduleEvent(EVENT_SUMMON_ENGULFER, 50s, PHASE_WEAK_SPOT);
                        break;
                    }
                    case EVENT_FIXATE:
                    {
                        ...
                        events.ScheduleEvent(EVENT_FIXATE_STOP, 15s, PHASE_VULNERABILITY);
                        break;
                    }
                    ...
                    case EVENT_STOMP:
                    {
                        ...
                        events.ScheduleEvent(EVENT_STOMP, 30s, PHASE_VULNERABILITY);
                        break;
                    }
                    default:
                        break;
