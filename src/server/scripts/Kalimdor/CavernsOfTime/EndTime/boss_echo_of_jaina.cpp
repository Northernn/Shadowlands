    ...
    public:
        ...
        struct boss_echo_of_jainaAI : public BossAI
        {
            ...
            void JustEngagedWith(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_PYROBLAST, 1s);
                events.ScheduleEvent(EVENT_BLINK, 3s);
                events.ScheduleEvent(EVENT_FLARECORE, 5s);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_PYROBLAST:
                            ...
                            events.ScheduleEvent(EVENT_PYROBLAST, 5s);
                            break;
                        case EVENT_BLINK:
                            ...
                            events.ScheduleEvent(EVENT_FROST_BLADES, 1s);
                            break;
                        case EVENT_FROST_BLADES:
                        {
                            ...
                            for(int i = 0; i < 3; i++)
                            {
                                if(Creature* summon = me->SummonCreature(NPC_FROST_BLADE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 5s))
                                {
                                    summon->SetReactState(REACT_PASSIVE);
                        ...
                        case EVENT_FROSTBOLT_VOLLEY:
                            if(volley < 3)
                            {
                                ...
                                volley++;
                                events.ScheduleEvent(EVENT_FROSTBOLT_VOLLEY, 1s);
                            }
                            else
                            {
                                events.CancelEvent(EVENT_FROSTBOLT_VOLLEY);
                                events.ScheduleEvent(EVENT_PYROBLAST, 1s);
                                events.ScheduleEvent(EVENT_BLINK, 2s);
                            }
                            break;
