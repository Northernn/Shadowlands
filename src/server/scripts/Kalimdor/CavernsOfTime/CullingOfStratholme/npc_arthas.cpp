...
enum Arthas1Gossip
{
    GOSSIP_MENU_ARTHAS_FIRST = 26499,
    GOSSIP_MENU_ARTHAS_FIRST_START = -2471168
};
    ...
    struct npc_arthas_stratholmeAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            case RP2_POINTID_ARTHAS3: 
            {
                ...
                events.ScheduleEvent(RP2_EVENT_SUMMON_ZOMBIES, 18s);
                events.ScheduleEvent(RP2_EVENT_MALGANIS2, 22s);
                const Milliseconds Offset = 2s;
                events.ScheduleEvent(RP2_EVENT_MALGANIS_LEAVE1, 36s + Offset);
                events.ScheduleEvent(RP2_EVENT_MALGANIS_LEAVE2, 37s + Offset);
                events.ScheduleEvent(RP2_EVENT_ARTHAS4, 37s + Offset);
                events.ScheduleEvent(RP2_EVENT_ARTHAS4_2, 41s + Offset);
                events.ScheduleEvent(RP2_EVENT_ARTHAS_MOVE_5, 47s + Offset);
                events.ScheduleEvent(RP2_EVENT_ARTHAS5, 47s + Offset);
                events.ScheduleEvent(RP2_EVENT_ARTHAS5_2, 53s + Offset);
                events.ScheduleEvent(RP2_EVENT_ARTHAS5_3, 59s + Offset);
                events.ScheduleEvent(RP2_EVENT_WAVE_START, 66s + Offset);
                break;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 event = events.ExecuteEvent())
            {
                ...
                case RP2_EVENT_REACT5:
                {
                    ...
                    me->GetCreatureListWithEntryInGrid(nearbyVictims, urand(0, 1) ? NPC_CITIZEN : NPC_RESIDENT, 200.f);
                ...
                case RP2_EVENT_MALGANIS1:
                {
                    ...
                    me->GetCreatureListWithEntryInGrid(citizens, NPC_RESIDENT, 60.f);
                    me->GetCreatureListWithEntryInGrid(citizens, NPC_CITIZEN, 60.f);
                    for (Creature* unit : citizens)
                        if (unit->IsAlive())
                            unit->SetEmoteState(EMOTE_STATE_COWER);
                }
                    break;
                ...
                break;
                case RP2_EVENT_SUMMON_ZOMBIES:
                {
                    std::list<Creature*> citizens;
                    me->GetCreatureListWithEntryInGrid(citizens, NPC_RESIDENT, 200.f);
                    me->GetCreatureListWithEntryInGrid(citizens, NPC_CITIZEN, 200.f);
                    for (Creature* unit : citizens)
                    {
                        if (unit->IsAlive())
                        {
                            TempSummon* Zombie = unit->SummonCreature(NPC_RISEN_ZOMBIE, unit->GetPosition(), TEMPSUMMON_TIMED_DESPAWN, 3600s);
...
struct npc_stratholme_rp_dummy : NullCreatureAI
{
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        if (type == POINT_MOTION_TYPE || type == EFFECT_MOTION_TYPE || type == SPLINE_CHAIN_MOTION_TYPE)
            ...
            {
                if (self->GetSummonerCreatureBase())
                {
...
class spell_stratholme_crusader_strike : public SpellScript
{
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        if (Unit* target = GetHitUnit())
        {
            ...
            {
                Unit::DealDamage(GetCaster(), target, 9999, nullptr, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, nullptr, false);
            }
        }
    }
