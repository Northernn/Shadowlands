    ...
    struct boss_high_prophet_barimAI : public ScriptedAI
    {
        ...
        void JustReachedHome() override
        {
            if (me->IsInCombat())
            {
                ...
                events.ScheduleEvent(EVENT_REPENTANCE, 1s, 0, PHASE_REPENTANCE);
                return;
            }
        }
            ...
            if (action == ACTION_REPENTANCE_DONE)
            {
                ...
                for (SummonList::const_iterator itr = lSummons.begin(); itr != lSummons.end(); ++itr)
                    if (Creature* pSummon = ObjectAccessor::GetCreature(*me, (*itr)))
                        if (pSummon->GetEntry() == NPC_REPENTANCE_MIRROR)
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            if (IsHeroic())
                events.ScheduleEvent(EVENT_SUMMON_BLAZE_OF_THE_HEAVENS, 3s);

            Talk(SAY_START);
            events.SetPhase(PHASE_BARIM);
            events.ScheduleEvent(EVENT_FIFTY_LASHINGS, 5s, 0, PHASE_BARIM);
            events.ScheduleEvent(EVENT_HEAVENS_FURY, 5s, 0, PHASE_BARIM);
            events.ScheduleEvent(EVENT_PLAGUE_OF_AGES, 6s, 0, PHASE_BARIM);
        }
                    ...
                    case EVENT_REPENTANCE:
                        {
                            ...
                            switch(uiEventPhase)
                            {
                                ...
                                case 2:
                                    ...
                                    events.ScheduleEvent(EVENT_REPENTANCE, 6s, 0, PHASE_REPENTANCE);
                                    break;
                                case 3:
                                ...
                                case 4:
                                    {
                                        events.ScheduleEvent(EVENT_REPENTANCE, 3s, 0, PHASE_REPENTANCE);
                    ...
                    case EVENT_SUMMON_BLAZE_OF_THE_HEAVENS:
                        {
                            ...
                            switch (uiEventPhase)
                            {
                                case 1:
                                    ...
                                    events.ScheduleEvent(EVENT_SUMMON_BLAZE_OF_THE_HEAVENS, 2s);
                                    break;
                                case 2:
                                    {
                                        events.ScheduleEvent(EVENT_SUMMON_BLAZE_OF_THE_HEAVENS, 3s);
                    ...
                    case EVENT_HEAVENS_FURY:
                        {
                            ...
                            events.ScheduleEvent(EVENT_HEAVENS_FURY, 20s, 0, PHASE_BARIM);
                        }
                        break;
                    case EVENT_FIFTY_LASHINGS:
                        events.ScheduleEvent(EVENT_FIFTY_LASHINGS, 20s, 0, PHASE_BARIM);
                        me->CastSpell(me, SPELL_FIFTY_LASHINGS, false);
                        break;
                    case EVENT_PLAGUE_OF_AGES:
                        {
                            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                                me->CastSpell(target, SPELL_PLAGUE_OF_AGES, false);

                            events.ScheduleEvent(EVENT_PLAGUE_OF_AGES, 18s, 0, PHASE_BARIM);
                        }
                        break;
                }
            }
...
public:
    ...
    void DamageTaken(Unit* /*who*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_SUMMON_BLAZE_OF_THE_HEAVENS_GROUND, 3s, 0, PHASE_BLAZE);
        }
                    ...
                    case EVENT_SUMMON_BLAZE_OF_THE_HEAVENS_GROUND:
                        {
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_BLAZE_OF_THE_HEAVENS_GROUND, 1s, 0, PHASE_BLAZE);
...
void DamageTaken(Unit* /*who*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        ...
        void JustEngagedWith(Unit* who) override
        {
            events.ScheduleEvent(EVENT_SOUL_SEVER, 1s);
            events.ScheduleEvent(EVENT_WAIL_OF_DARKNESS, 2s);
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SOUL_SEVER:
                        {
                            events.ScheduleEvent(EVENT_SOUL_SEVER, 11s);
...
public:
    ...
    struct npc_soul_fragmentAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* /*unit*/, const SpellInfo* spell) override
        {
            if (spell->Id == 91277)
            {
                ...
                if (Creature* soul = me->SummonCreature(49219, x, y, z, 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 5s))
