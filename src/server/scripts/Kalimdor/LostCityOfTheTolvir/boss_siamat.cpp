...
public:
    ...
    struct boss_siamatAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_STATIC_SHOCK, 2s, 0, PHASE_DEFLECTING_WINDS);
            events.ScheduleEvent(EVENT_DEFLECTING_WINDS, 5s, 0, PHASE_DEFLECTING_WINDS);
            events.ScheduleEvent(EVENT_CALL_OF_SKY, 15s);
            events.ScheduleEvent(EVENT_CLOUD_BURST, 7s, 0, PHASE_DEFLECTING_WINDS);
            events.ScheduleEvent(EVENT_STORM_BOLT_DW, 2s, 0, PHASE_DEFLECTING_WINDS);
        ...
        void DoAction(const int32 action) override
        {
            if (action == ACTION_SERVANT_DEATH)
            {
                ...
                events.ScheduleEvent(EVENT_WAILING_WINDS, 1s, 0, PHASE_WAILING_WINDS);
                events.ScheduleEvent(EVENT_STORM_BOLT_S, 15s, 0, PHASE_SIAMAT);
            }
        }
                    ...
                    case EVENT_STATIC_SHOCK:
                        {
                           ...
                           // me->CastSpell(x, y, FLOR_COORD_Z, StaticShock[uiStaticShockId], false);
                            ...
                            if (uiStaticShockId < 3)
                                events.ScheduleEvent(EVENT_STATIC_SHOCK, 32s, 0, PHASE_DEFLECTING_WINDS);
                        }
                        break;
                    ...
                    case EVENT_CALL_OF_SKY:
                        ...
                        //    me->GetNearPoint2D(x, y, (float)dist, angle);
                         //   me->CastSpell(x, y, FLOR_COORD_Z, SPELL_CALL_OF_SKY, false);
                            events.ScheduleEvent(EVENT_CALL_OF_SKY, 20s);
                        }
                        break;
                    case EVENT_CLOUD_BURST:
                        {
                            events.ScheduleEvent(EVENT_CLOUD_BURST, 15s, 0, PHASE_DEFLECTING_WINDS);
                    ...
                    case EVENT_WAILING_WINDS:
                        {
                            ...
                            else
                            {
                                ...
                                events.ScheduleEvent(EVENT_ABSORB_STORMS, 15s, 0, PHASE_SIAMAT);
                                events.ScheduleEvent(EVENT_STORM_BOLT_S, 17s, 0, PHASE_SIAMAT);
                            }
                        }
                        break;
                    case EVENT_ABSORB_STORMS:
                        me->CastSpell(me, SPELL_ABSORB_STORMS, false);
                        events.ScheduleEvent(EVENT_ABSORB_STORMS, 33s, 0, PHASE_SIAMAT);
                        break;
...
public:
    ...
    struct npc_servant_of_siamatAI : public ScriptedAI
    {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            events.ScheduleEvent(EVENT_THUNDER_CRASH, 1s);
            events.ScheduleEvent(EVENT_LIGHTNING_NOVA, 5s);
        }
    ...
    void DamageTaken(Unit* /*who*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        {
            ...
            if (damage >= me->GetHealth())
            {
                ...
                {
                    if (instance)
                        me->CastSpell(me, SPELL_LIGHTNING_CHARGE, false);

                    LightningCharge = true;
                    me->SetReactState(REACT_PASSIVE);
                    me->AttackStop();
                    events.Reset();
                    events.ScheduleEvent(EVENT_SERVANT_DIED, 2s);
                }
            }
        }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_THUNDER_CRASH:
                        ...
                        events.ScheduleEvent(EVENT_THUNDER_CRASH,9s);
                        break;
                    case EVENT_LIGHTNING_NOVA:
                        me->CastSpell(me->GetVictim(), SPELL_LIGHTNING_NOVA, false);
                        events.ScheduleEvent(EVENT_LIGHTNING_NOVA, 9s);
                        break;
                }
            }
    ...
    void DamageTaken(Unit* /*who*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (events.ExecuteEvent() == EVENT_CHAIN_LIGHTNING)
            {
                ...
                events.ScheduleEvent(EVENT_CHAIN_LIGHTNING, 2s);
            }
...
public:
    ...
    struct npc_cloud_burstAI : public ScriptedAI
    {
        npc_cloud_burstAI(Creature* creature) : ScriptedAI(creature)
        {
            uiTickCount = 0;
            me->SetInCombatWithZone();
            events.ScheduleEvent(EVENT_PERIODIC_CAST, 1s);
        }
        ...
        class spell_wailing_winds_SpellScript : public SpellScript
        {
            void RandomJump(SpellEffIndex effIndex)
            {
                ...
                if (!roll)
                {
                    ...
                    target->KnockbackFrom(x, y, SpeedXY);
                }
                else
                {
