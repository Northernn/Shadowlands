...
struct boss_eregos : public BossAI
{
    boss_eregos(Creature* creature) : BossAI(creature, DATA_EREGOS)
    {
        Initialize();
    }
...
class spell_eregos_planar_shift : public AuraScript
{
    ...
    {
        AfterEffectRemove += AuraEffectRemoveFn(spell_eregos_planar_shift::OnRemove, EFFECT_0, SPELL_AURA_SCHOOL_IMMUNITY, AURA_EFFECT_HANDLE_REAL);
    }
};
...
class achievement_gen_eregos_void : public AchievementCriteriaScript
{
 ...
 {
    RegisterOculusCreatureAI(boss_eregos);
    RegisterSpellScript(spell_eregos_planar_shift);
