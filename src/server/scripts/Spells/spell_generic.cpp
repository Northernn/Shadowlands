...
/// Handles adding the memories to the collection after use item
/// </summary>
            ...
            if (auto player = caster->ToPlayer())
            {
                if (auto item = GetCastItem())
                {
                    if (auto runecarver = sDB2Manager.GetRuneforgeLegendaryAbilityEntryByItemID(item->GetEntry()))
                    {
                        ...
                        player->SendPlaySpellVisualKit(362, 1, 0); ///< Sniffed value
                    }
                }
            }
        }
        ...
        if (auto spell = GetSpell())
        {
            ...
            if (auto caster = GetCaster())
            {
                if (auto player = caster->ToPlayer())
                {
                    ...
                    /// Covenant system is not implemented in Dekkcore
...
void AddSC_generic_spell_scripts()
{
    ...
    //RegisterSpellScript(spell_gen_pony_mount_check); crash server in Argent Tournament zone
