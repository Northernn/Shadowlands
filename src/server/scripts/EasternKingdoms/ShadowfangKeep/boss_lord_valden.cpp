    ...
    public:
        ...
        struct boss_lord_valdenAI : public BossAI
        {
            ...
            void JustEngagedWith(Unit* pWho) override
            {
                if (IsHeroic())
                {
                    events.RescheduleEvent(EVENT_CONJURE_FROST_MIXTURE, 7s);
                    events.RescheduleEvent(EVENT_CONJURE_MYSTERY_TOXIN, 15s);
                }
                events.RescheduleEvent(EVENT_ICE_SHARDS, 30s);
                events.RescheduleEvent(EVENT_CONJURE_POISONOUS_MIXTURE, 6s);
...
class npc_valden_mystery_toxin : public CreatureScript
{
    public:
        ...
        {
            npc_valden_mystery_toxinAI(Creature* pCreature) : ScriptedAI(pCreature)
            {
            }
