...
class boss_lord_godfrey : public CreatureScript
{
    public:
        ...
        struct boss_lord_godfreyAI : public BossAI
        {
            ...
            {
                events.RescheduleEvent(EVENT_MORTAL_WOUND, 10s);
                events.RescheduleEvent(EVENT_CURSED_BULLET, 15s);
                events.RescheduleEvent(EVENT_SUMMON_BLOODTHIRSTY_GHOULS, 17s);
                events.RescheduleEvent(EVENT_PISTOL_BARRAGE, 22s);
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_PISTOL_BARRAGE:
                            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 1))
                            me->CastSpell(target, SPELL_PISTOL_BARRAGE_MISSILE_SUM);
                            ...
                            events.RescheduleEvent(EVENT_PISTOL_BARRAGE, 20s);
                            break;
                    }
                }
