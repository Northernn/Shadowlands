                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CHEST_BOMB:
                            ...
                            events.RescheduleEvent(EVENT_CHEST_BOMB, 17s);
                            break;
                        case EVENT_STICKY_BOMB:
                            ...
                            events.RescheduleEvent(EVENT_STICKY_BOMB, 9s);
                            break;
    ...
    public:
        ...
        struct npc_lumbering_oafAI : public ScriptedAI
        {
            ...
            void DoAction(const int32 action) override
            {
                ...
                {
                    case ACTION_CHARGE:
                        events.RescheduleEvent(EVENT_CHARGE_OAF1, 5s);
                        break;
                }
            }
            ...
            void JustEngagedWith(Unit* /*who*/) override
            {
                events.RescheduleEvent(EVENT_CHARGE_OAF0, 10s);
            }
            ...
            void MovementInform(uint32 type, uint32 id) override
            {
                if (type == POINT_MOTION_TYPE)
                {
                    ...
                    {
                        case POINT_START:
                            events.RescheduleEvent(EVENT_CHARGE_OAF2, 5s);
                            break;
                        case POINT_END:
                            events.RescheduleEvent(EVENT_CHARGE_OAF3, 5s);
                            break;
                    }
                }
            }
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CHARGE_OAF3:
                            ...
                            for (uint8 i = 0; i < 7; i++)
                                me->SummonCreature(NPC_MINE_RAT, RatPos[i], TEMPSUMMON_CORPSE_DESPAWN);
                            events.RescheduleEvent(EVENT_CHARGE_OAF0, 30s);
                            break;
                    }
                }
                DoMeleeAttackIfReady();
...
class npc_sticky_bomb : public CreatureScript
{
    public:
        ...
        struct npc_sticky_bombAI : public ScriptedAI
        {
            ...
            void JustEngagedWith(Unit* /*who*/) override
            {
                events.RescheduleEvent(EVENT_BOMB_READY, 6s);
                events.RescheduleEvent(EVENT_BOMB_EXPLODE, 18s);
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_STICKY_BOMB:
                            if (auto target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                                me->SummonCreature(NPC_STICKY_BOMB, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN);
                            events.RescheduleEvent(EVENT_STICKY_BOMB, 18s);
                            break;
                    }
                }
