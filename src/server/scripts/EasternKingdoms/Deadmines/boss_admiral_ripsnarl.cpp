...
public:
    ...
    {
        boss_admiral_ripsnarlAI(Creature* creature) : ScriptedAI(creature, DATA_ADMIRAL)
        {
        ...
        void JustEngagedWith(Unit* /*who*/) override
        {
            ...
            events.RescheduleEvent(EVENT_SWIPE, 10s);
            if (IsHeroic())
                events.RescheduleEvent(EVENT_GO_FOR_THE_THROAT,10s);
        ...
        void UpdateAI(uint32 uiDiff)
        {
            ...
            if (me->HealthBelowPct(75) && stage == 0)
            {
                ...
                events.RescheduleEvent(EVENT_SUMMON_VAPOR, 5s);
                SetFog(true);
                return;
            }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CONTINUE:
                    ...
                    if (IsHeroic())
                        events.RescheduleEvent(EVENT_GO_FOR_THE_THROAT, 10s);
                    if (auto pTarget = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                    {
                        AttackStart(pTarget);
                ...
                case EVENT_GO_FOR_THE_THROAT:
                    if (auto pTarget = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true))
                        DoCast(pTarget, SPELL_GO_FOR_THE_THROAT);
                    events.RescheduleEvent(EVENT_GO_FOR_THE_THROAT, 15s);
                    break;
    ...
    private:
        ...
        void EnterSpecialPhase()
        {
            ...
            events.RescheduleEvent(EVENT_CONTINUE, 20s);
        }
    };
    ...
    struct npc_admiral_ripsnarl_vaporAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* /*who*/, const SpellInfo* spellInfo) override
        {
            if (spellInfo->Id == SPELL_CONDENSE_1_1)
                events.RescheduleEvent(EVENT_SWIRLING_VAPOR, 5s);
            else if (spellInfo->Id == SPELL_CONDENSE_2_1)
            {
                events.CancelEvent(EVENT_SWIRLING_VAPOR);
                events.RescheduleEvent(EVENT_CONDENSING_VAPOR, 6s);
            }
            else if (spellInfo->Id == SPELL_CONDENSE_3_1)
            {
                events.CancelEvent(EVENT_CONDENSING_VAPOR);
                events.RescheduleEvent(EVENT_FREEZING_VAPOR, 5s);
                events.RescheduleEvent(EVENT_COALESCE, 8s);
            }
        }
